...
measurement concern remains binary-id-neutral. \emph{Evidence}: Section~\ref{sec:axis-triad} (structure),
...
answered, RQ1.b empirically open. \emph{Eighteen} organ main axes in fixed slot order T0--T17 (name array
...
Map, Container, Graph orders the genera beneath it (Section~\ref{sec:anatomy-levels}). RQ1.a is answered in
...
measurement of the fill behaviour is not available. \emph{Evidence}: Sections~\ref{sec:axis-triad}
and~\ref{sec:mess-chain} (structure), Section~\ref{sec:impl-contracts} (test). \emph{Consequence}: RQ1
...
(extensibility): a new candidate under test joins as a living being of its family into the genus --- family is the
...
(Section~\ref{sec:stamp-model}). Whether the candidate under test matches an existing family, founds a new one, or
...
available. On RQ2.c (extensibility): a new candidate under test joins as a living being of its family into
the genus (the genus build admission judges at compile time), docks at the probe dock, passes the
...
test matches a family, founds a new one, or is a hybrid form is decided by the family cut
...
not the scope of the check; full build-out and calibrated thresholds are missing. \emph{Evidence}:
...
Section~\ref{sec:eval-workloads} (structure). \emph{Consequence}: RQ2 structurally, RQ2.c conceptually,
...
parameters without recompilation (Section~\ref{sec:measurement-system}). \emph{Evidence}:
Sections~\ref{sec:prtart-demo},~\ref{sec:sota-instances}, and~\ref{sec:measurement-system} (structure),
Appendix~\ref{app:interfaces} (test). \emph{Consequence}: the number presupposes series~A\@; the
...
and~\ref{sec:mess-chain} (structure). \emph{Consequence}: RQ4 and RQ4.a structurally answered; heuristics
