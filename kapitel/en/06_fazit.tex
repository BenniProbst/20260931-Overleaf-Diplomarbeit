...
kept apart: the golden reference catalogue with $2^{17} = 131072$ binary identities per system permutation,
secured against regression by a CRC-64 checksum (Section~\ref{sec:impl-contracts}), and the cardinality of
a planned space computed by the planner (\texttt{simulate}). The 33 corpus works each possess a profile XML
...
\end{nurlang}
\begin{nurkurz}
\paragraph{Implementation status.} The declared build scope (per system permutation the 131072 organ
permutations of the golden reference catalogue; the catalogue comment estimates roughly 224~GB and
24~hours per build, not a measured value) has not been carried out; the end-to-end runs across the target
...
--- and the calibration run (250 of 250 tier binaries, 0.521~s and roughly 457~kB per binary; from this
roughly 240~GB for 524288 tier binaries over the four system permutations of the golden profile). The
...
\begin{enumerate}[label=(\arabic*)]
  \item the complete build according to the declared scope (Section~\ref{sec:eval-build-scope}) and the
