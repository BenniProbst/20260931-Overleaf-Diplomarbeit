...
\end{nurlang}
\begin{nurkurz}
Orthogonal to all five questions, the introduction posed the categorization question whether the axes of
the design space separate cleanly into organ, system, and measurement axes --- and how such a separation is
to be accomplished where a design does not provide for it. Both are answered
(Section~\ref{sec:axis-triad}): the axes separate into three \emph{realms} --- organ, system, measurement
--- with one class root, generated registry, and stamp line each at the binary
(Section~\ref{sec:stamp-model}); the three line carriers of the fingerprint are, as types, pairwise
unconstructible from one another. Only the organ main axes enter the \texttt{binary\_id}, system and
measurement axes are binary-id-neutral. Compile-time \emph{re-entry guards}
(Section~\ref{sec:impl-system-axes}) reject compiler, scheduling, and load framework as system main axes:
the scheduling is a sub-axis of the target ISA, the compiler a sub-axis group of the complex bracket, the
load framework the meta-meta axis of the measurement realm. This triad is the second shape of the
separability problem and supports the answers to the following sub-questions.
\end{nurkurz}
...
\begin{nurkurz}
\paragraph{RQ0 (Main question).} The question is by how much active, measurement-driven cache-aware
decisions outperform their passive, statically dimensioned counterparts on different CPU platforms and
whether a measured gain can be attributed to the organ, system, or measurement concern (RQ0.E). The
\emph{status}: RQ0 is open --- without end-to-end runs no number; the structure is implemented: the same
structure is built once actively (a runtime-dynamic cache-line policy, which the policy selector derives
from an evaluated profile aggregate of operation mix and working set, and measurement-driven binary
selection) and once passively (statically fixed organ choice in the \texttt{binary\_id}) and compared under
an identical compiler and flag basis (default \texttt{-O2}, \texttt{-O3} only as an opt-in). The
measurement machines are prod1, prod2, and an ODROID board; Sapphire Rapids is provided for, not measured
(n/a; Section~\ref{sec:eval-platforms}). RQ0.E is structurally answered by the triad: an organ effect
permutes the \texttt{binary\_id}, a platform concern stands at the binary as system stamp line, a
measurement concern remains binary-id-neutral.
\emph{Evidence}: Section~\ref{sec:axis-triad} (structure),
Section~\ref{sec:impl-system-axes} (test), Section~\ref{sec:eval-method} (measurement plan).
\emph{Consequence}: the quantification presupposes the full build and the run of series~A
(Section~\ref{sec:outlook}); the cache-line effect from the synthesized measurement curves follows as the
first evaluation step (Section~\ref{sec:heuristics-curves}).
\end{nurkurz}
...
one value --- $2^{17} = 131072$ binary identities per system permutation under the superseded assumption of
two configurations per axis, as a regression detector (not an upper bound)
...
platform (subcommand \texttt{simulate}) as an upper bound known only at execution time; all eighteen axes are planned for variation (not
...
paper-derived candidates under test holds this translation at compile time against duplicates and gaps. The variadic
...
Map, Container, Graph orders the families beneath it (Section~\ref{sec:anatomy-levels}). RQ1.a is
...
\end{nurlang}
\begin{nurkurz}
\paragraph{RQ1 (Composition).} The question is how a search-algorithm design can be canonically decomposed
into orthogonal axes and systematized in a permutation matrix, such that state-of-the-art methods become
reconstructible bias-free as profiles and permutable; RQ1.a asks about the category cut, RQ1.b about the
organ couplings and the solely varied ISA/SIMD axis. The \emph{status}: RQ1 is structurally answered, RQ1.a
answered, RQ1.b empirically open.
\emph{Eighteen} organ main axes in fixed slot order T0--T17 (name array
in the code) span the matrix (Section~\ref{sec:axis-triad}); \texttt{disk\_io} is the nineteenth registry
entry and the example of the meta-meta level M3: stamped in the bracket appendage of the organ line, the
bracket depth is the meta-meta level (Section~\ref{sec:stamp-model}). Two reference quantities are to be
kept apart: the golden reference catalogue with $2^{17} = 131072$ binary identities per system permutation (superseded
assumption of two configurations per axis), secured against regression by a CRC-64 checksum
(Section~\ref{sec:impl-contracts}), and the cardinality of a planned space computed by the planner
(\texttt{simulate}) as an upper bound known only at execution time. The 33 corpus works each possess a profile XML
(30 full profiles, three abstract ones; Section~\ref{sec:sota-instances}); the variadic mapping
(Appendix~\ref{app:interfaces}) carries algorithm- as well as container-oriented patterns, the family triad
Map, Container, Graph orders the families beneath it (Section~\ref{sec:anatomy-levels}). RQ1.a is answered in
the affirmative: the decomposition remains orthogonal because platform and measurement concerns are kept
out, and the re-entry guards hold the cut (Section~\ref{sec:impl-system-axes}). On RQ1.b: the coupling node
type $\times$ SIMD $\times$ path compression falls apart --- node type $\times$ path compression remains a
marked organ coupling, SIMD is a build control quantity of the system axis \texttt{external\_utils}; the
measurement of the fill behaviour is not available.
\emph{Evidence}: Sections~\ref{sec:axis-triad}
and~\ref{sec:mess-chain} (structure), Section~\ref{sec:impl-contracts} (test).
\emph{Consequence}: RQ1
structurally answered; RQ1.b presupposes the full build.
\end{nurkurz}
...
the macro level (\texttt{macro}; the concept and family interface operations of the tier binary), and
...
\end{nurlang}
\begin{nurkurz}
\paragraph{RQ2 (Measurement methodology).} The question is how a reproducible measurement pipeline with
hardware-counter collection measures every axis permutation fairly in three measurement series over three
granularities and when a platform counts as verified; RQ2.a asks about measurement instruments as axes of
their own, RQ2.b about contradictory literature findings, RQ2.c about extensibility, RQ2.d about the
workloads of the state of the art. The \emph{status}: RQ2 is structurally answered, RQ2.a and RQ2.d
answered. Production and experiment mode are switched by the master switch
\texttt{COMDARE\_\allowbreak{}MEASUREMENT\_\allowbreak{}ON}; the experiment mode always measures, in the
store only what is missing is re-measured (full build-out not implemented; Section~\ref{sec:lager}); the
\texttt{ExperimentDriver} controls the runner phases load, execute, measure, teardown. Every series is
collected on three \emph{measurement levels} --- \texttt{wallclock}, \texttt{macro}, \texttt{micro} ---;
w/ma/mi is the registered alias, the three levels are the measurement-tooling main axis
(Section~\ref{sec:mess-chain}). RQ2.a is answered in the affirmative: the measurement instruments are a
realm of their own in the planner --- the load framework \texttt{load\_framework} as meta-meta axis, the
measurement tooling as main axis, the sixteen measurement categories as sub-axis ---, always present and
binary-id-neutral; the separation is held by the double root cut (system realm at the CacheEngineBuilder,
measurement realm at the planner), and the legend chain measurement axis $\to$ CacheEngineBuilder type
$\to$ system axes $\to$ tier binary is code as a naming scheme (Section~\ref{sec:axis-triad}). For the
workload, the driver passes a default workload through per module, a profile brings its own; the planned
full permutation over all workloads counters the \emph{load-profile selection bias} --- the distortion of a
comparison by load profiles under which one design performs preferentially
well~\cite{blackburn2006dacapo,blackburn2016truth,mytkowicz2009wrongdata} ---; it is not implemented.
\end{nurkurz}
...
(extensibility): a new candidate under test joins as a living being of its concept into the family --- concept is the
first level (Map, Container, Graph), family the second, and the family build admission carries the
slot sets per concept as a compile-time verdict ---; it docks at the probe dock, passes the
three-stage evaluation (Section~\ref{sec:mess-chain}), and its merge with the cache engine bears
one of the registered names: \texttt{Verbund1\_CeOnly} (no candidate directive),
\texttt{Verbund2\_Replace} (the candidate axis replaces the cache-engine axis, with fallback),
\texttt{Verbund2\_Hybrid} (cache engine and candidate under test hybrid per candidate under test; declared, not materialized ---
...
hierarchically extended algorithm names and flags of the organ line --- a candidate organ carries its
...
(Section~\ref{sec:stamp-model}). Whether the candidate under test matches an existing concept, founds a new one, or
constitutes a hybrid form is decided by the concept cut (Section~\ref{sec:anatomy-levels}); a concept
...
binary, the seventeen \texttt{std::map} operations marked in Appendix~\ref{app:interfaces} (under
\texttt{--debug} additionally the standard test set of the tier interface) --- the
...
\end{nurlang}
\begin{nurkurz}
On RQ2.b (organ axes): the contradictory literature findings on the optimal node/cache-line size --- one
cache line per node~\cite{rao1999css,rao2000csb} versus sixteen and more~\cite{hankins2003nodesize} and
several per node~\cite{chen2001prefetch} --- are not adopted from their original setups but re-measured
axis-isolated (one organ axis varied at a time, system and measurement axes constant); the number is not
available. On RQ2.c (extensibility): a new candidate under test joins as a living being of its concept into
the family (the family build admission judges at compile time), docks at the probe dock, passes the
three-stage evaluation (Section~\ref{sec:mess-chain}), and bears in its merge with the cache engine one of
the names \texttt{Verbund1\_CeOnly}, \texttt{Verbund2\_Replace}, \texttt{Verbund2\_Hybrid} (declared, not
materialized), or \texttt{Verbund3\_Union} (prepared, Section~\ref{sec:eval-series}); at the binary it
remains distinguishable via the hierarchical algorithm names of the organ line, the flag \texttt{e} denotes
an efficiency-core algorithm, not a mixture (Section~\ref{sec:stamp-model}). Whether the candidate under
test matches a concept, founds a new one, or is a hybrid form is decided by the concept cut
(Section~\ref{sec:anatomy-levels}; a family of its own only with a fundamentally different base axis set);
the paper-isolation mode is not implemented (Section~\ref{sec:outlook}). On RQ2.d (workload state of the
art): it is formed by YCSB with the six load profiles A--F, the index benchmark SOSD, the TPC family, SuRF
in RocksDB, real string corpora, as well as SPEC~CPU and CloudSuite
(Section~\ref{sec:workload-basics-known}). YCSB is the registered load framework (six tokens, routed per
permutation from the profile) over six real string datasets with a reproducibility record (\texttt{xml}
without checksum, n/a; Section~\ref{sec:eval-workloads}). A platform counts as verified when the
conformance gate confirms, per tier binary, the seventeen \texttt{std::map} operations marked in
Appendix~\ref{app:interfaces} (twelve core check blocks, \texttt{std::map} as oracle; under
\texttt{--debug} additionally the standard test set of the tier interface) and the platform
probe covers the reported axis subset; the five drive operations of the module boundary are ABI primitives,
not the scope of the check; full build-out and calibrated thresholds are missing.
\emph{Evidence}:
Sections~\ref{sec:mess-chain},~\ref{sec:measurement-system}, and~\ref{sec:anatomy-levels} (structure),
Appendix~\ref{app:interfaces} (test); RQ2.d: Section~\ref{sec:workload-basics-known} (literature),
Section~\ref{sec:eval-workloads} (structure).
\emph{Consequence}: RQ2 structurally, RQ2.c conceptually,
RQ2.d answered; isolation mode, the number for RQ2.b, and the full permutation of the workloads are open.
\end{nurkurz}

\begin{nurlang}
\paragraph{RQ3 (Candidate comparison).} The question is whether the candidate under test PRT-ART wins against the
...
\texttt{std::map} as oracle and, under \texttt{--debug}, additionally the standard test set of the tier
interface (family-generic and genus-specific part); the full build-out to the complete interface is
planned but not
...
switching rules by occupancy density and access type are part of the PRT-ART profile; the candidate under test
...
\emph{evidence} is Section~\ref{sec:prtart-demo} (candidate under test and page representation; structure),
...
\end{nurlang}
\begin{nurkurz}
\paragraph{RQ3 (Candidate comparison).} The question is whether the candidate under test PRT-ART wins
against the eight Rank-1 designs --- ART~\cite{leis2013art}, HOT~\cite{binna2018hot}, Masstree, CoCo-Trie,
START, B\textsuperscript{2}-tree, Wormhole, SuRF --- and the Rank-2/3 SOTA not through asymptotic novelty
but through better microarchitectural properties in the mandatory measurement series A, B, and C\@; RQ3.a
asks about the cheapest page representation per branch point, RQ3.b about the compile-time force. The
\emph{status}: RQ3 is open, RQ3.b answered, RQ3.a structurally answered. The comparison presupposes the
first measurement runs; the apparatus is implemented: the PRT-ART profile, the three series, derived
mechanically from the merge stage (Table~\ref{tab:stage-series}), the \texttt{ExperimentDriver} with
auto-pickup of the 33 profile XMLs, and the ABI-conformant adapter with the \texttt{std::map} core contract
(Appendix~\ref{app:interfaces}). The conformance gate is implemented, its full build-out is not; the
measurement hook per axis is opt-in. On RQ3.a: array-indexed versus vector page per branch point and the
switching rules (fixed density thresholds 25, 50, and 75~percent, Section~\ref{sec:prtart-demo}) are part
of the PRT-ART profile but remain prescriptions without measurement evidence. RQ3.b is answered in the
affirmative and is the construction principle: every axis divides into a compile-time-fixed \emph{main
axis} (\texttt{binary\_id} segment or stamp) and runtime-variable \emph{sub-axes}, which vary their
parameters without recompilation (Section~\ref{sec:measurement-system}).
\emph{Evidence}:
Sections~\ref{sec:prtart-demo},~\ref{sec:sota-instances}, and~\ref{sec:measurement-system} (structure),
Appendix~\ref{app:interfaces} (test).
\emph{Consequence}: the number presupposes series~A\@; the
hypotheses H1--H3 supply the evaluation criteria.
\end{nurkurz}
...
telemetry metrics H1--H3 are measurement organs of the candidate under test, not these hypotheses
...
\begin{nurkurz}
\paragraph{Hypotheses and target path.} The evaluation hypotheses H1--H3
(Section~\ref{sec:eval-hypotheses}) were formulated before the apparatus; the target path ranks them as
touchstones of the evaluation: H1 (page-type cost) and H3 (distribution of the value placement) are checked
in series~A per load profile, H2 (code quality per source) via the tool-computed \texttt{cppcheck} record
with 33 entries, 22 of them as n/a without a vendored original source; PRT-ART possesses no original
repository (n/a). Criterion and threshold stand before the measurement~\cite{wohlin2012experimentation},
the number of repetitions per cell follows the required confidence~\cite{kalibera2013rigorous}. H2 counts
as accepted if, over the full profiles with a vendored original source, the Spearman rank correlation
between finding density and measured throughput per load profile and cell (platform, workload, regime) is
negative with $|\rho| \geq 0.5$ at $\alpha = 0.05$; as rejected at $|\rho| < 0.3$; in between as undecided;
below eight profiles with a score H2 is not testable. The manual audit over seven assessment axes remains
the qualitative classification. The PRT-ART telemetry metrics H1--H3 are measurement organs of the
candidate under test, not these hypotheses (Section~\ref{sec:eval-hypotheses}); the variant ChainRef of the
value placement is the subject of H3, no abort criterion for H2.
\end{nurkurz}
...
\end{nurlang}
\begin{nurkurz}
\paragraph{RQ4 (Compile time vs.\ run time).} The question is which axis properties must be fixed at
compile time to produce platform-optimized, provenance-verifiable binaries per permutation, and which
remain selectable at run time without violating the identical compiler and flag basis; RQ4.a asks about the
end-to-end XML chain from the build to the evaluation, RQ4.E whether the boundary runs along the axis
categories. The \emph{status}: RQ4 and RQ4.a are structurally answered, RQ4.E answered. The boundary runs
along the categories: the three system main axes together with the complex bracket
\texttt{build\_target\_complex} (Section~\ref{sec:complex-axis}) belong to the compile-time identity of the
binary and appear as system stamp line, never as a \texttt{binary\_id} segment. Provenance is secured by
the tier fingerprint (SHA-512, eleven members, format~6; the overlay member hashes the source files of the
overlay cut --- per axis the files of its own implementation plus the shared anatomy substance ---, the
toolchain member carries build-compiler identifier and optimization identity); the planner fingerprint is a
SHA-256, and \texttt{get\_compiler()} returns the original compiler of the publication --- a statement of
origin, not a build identifier. Measurement choices remain runtime-variable, layout and page selection
selectable via the static \texttt{binary\_id} and the dynamic cache-line policy. Two-way split (planner
runtime and builder compile time) are load framework, telemetry, and hardware detection
(Sections~\ref{sec:axis-triad} and~\ref{sec:hw-probe}). What is stamped is always the platform
\emph{class}, never a measured value; the \emph{fit stamp}, which checks the stamped class on every
measurement run against the runtime verdict of the hardware detection, is designed, not implemented
(Section~\ref{sec:outlook}); the forward-looking sub-question of whether the measurement itself can tailor
ISA- and operating-system-optimized binaries for a recombination is taken up by the outlook as heuristics
automation; the operating-system dimension is provided for with three values and two measurement regimes,
the second regime not implemented (Section~\ref{sec:eval-platforms}). On RQ4.a: the chain is declarative
end to end and built as two binaries: the planner checks the user XML read-only and emits the plan
deterministically, the measurement driver runs the seven-phase measurement pipeline from
\textsc{Enumerate} to \textsc{Persist}, the nine-stage toolchain leads to the manuscript
(Section~\ref{sec:impl-pipeline}, Figure~\ref{fig:eval-xml-kette}). \texttt{compare} (selection of the
best binary) and \texttt{release} are registered as work modes, their sequence is not implemented
(Section~\ref{sec:anatomy-levels}); implementation status: a first end-to-end run (43 permutations) is
available; headless service, XML creation mode, \texttt{<publish>} section, and CSV tree form are missing.
\emph{Evidence}: Sections~\ref{sec:complex-axis},~\ref{sec:hw-probe}, and~\ref{sec:stamp-model}
(structure), Section~\ref{sec:impl-system-axes} (test); RQ4.a: Sections~\ref{sec:impl-pipeline}
and~\ref{sec:mess-chain} (structure).
\emph{Consequence}: RQ4 and RQ4.a structurally answered; heuristics
automation, fit stamp, and the sequence of \texttt{compare} and \texttt{release} open.
\end{nurkurz}
...
RQ2.c & extensibility, integration of new candidates under test & answered (conceptually) & {\ref{sec:anatomy-levels}}, {\ref{sec:mess-chain}} (structure), {\ref{app:interfaces}} (test) & paper-isolation mode; materialization of \texttt{Verbund2\_Hybrid} \\
...
\begin{nurlang}
\paragraph{Implementation status.} The complete build has not been carried out; its scope is not a
declared fixed value: the upper bound of binary identities per system permutation is the product of the
configurations selected per organ axis in the XML experiment (several per axis, not on/off), determined by
the planner from the XML experiment and known only at execution time; the planning simulation (subcommand
\texttt{simulate}) today computes this product times system permutations arithmetically, the planner
extension that correctly captures the upper bound and the batch size of an experiment as the basis of the
load estimate is planned but outstanding (the 131072 organ permutations of the golden reference catalogue
with the catalogue comment of roughly 224~GB and roughly 24~hours per build are a historical estimate under
the superseded assumption of two configurations per axis, not a build scope); the end-to-end runs of the measurement driver across the target platforms (x86-64
...
follows, as a worked example of the historical golden profile (four system permutations times $2^{17}$, two
optimization levels times two SIMD options), a storage requirement of roughly 240~GB for 524288 tier
binaries --- not a build scope of the system. The smoke series --- 43 permutations in two sub-blocks: 27 Cartesian
...
\end{nurlang}
\begin{nurkurz}
\paragraph{Implementation status.} The complete build has not been carried out; its scope is not a fixed
value: the upper bound per system permutation is the product of the configurations selected per organ axis
in the XML experiment, determined by the planner only at execution time; the planning simulation
(\texttt{simulate}) today computes this product arithmetically, the planner extension for upper bound and
batch size as the basis of the load estimate is outstanding (the 131072 organ permutations of the golden
reference catalogue with roughly 224~GB and 24~hours per build are a historical estimate under the superseded
assumption of two configurations per axis); the end-to-end runs across the target
platforms (x86-64, aarch64) are not available, nor is the delivery measurement over the 320-identity
catalogue. Substantiated are two partial executions: the first end-to-end run of the chain
(Section~\ref{sec:eval-xml}) --- a measurement job of the smoke stage on the AMD Zen~5 machine (pipeline
15800, measurement job 377503, host prod1) with 10000 operations and 671.5~ns per operation (p50
\texttt{lookup} 750~ns; a preceding run, pipeline 15764, yielded 1199.047~ns on an undocumented machine)
--- and the calibration run (250 of 250 tier binaries, 0.521~s and roughly 457~kB per binary; from this, as a worked example of the historical golden profile, roughly 240~GB for 524288 tier binaries
over its four system permutations, not a build scope of the system). The
smoke series (43 = 27 + 16 permutations) is available in Appendix~\ref{app:measurements}; its data state
still counted SIMD and telemetry as organ axes. It approximates axis effects partly via wall-clock proxies,
individual effects are instrumentation artefacts (the change of the observed behaviour by the measuring
instrument itself~\cite{gait1986probe}); the caveats per series are listed in
Table~\ref{tab:le:limitierung} (Section~\ref{sec:measurements:limitations}), the acceptance per package
runs through the two-gate protocol (Section~\ref{sec:impl-contracts}). The \emph{consequence}: RQ0 and RQ3
remain open as long as series~A has not run; all eighteen axes are planned for variation (not measured).
\end{nurkurz}
...
together with the umbrella include, the legacy path with the candidate stub is separated from the
...
\begin{nurkurz}
\section{Limitations}\label{sec:limitations}
The following limits describe what a user cannot achieve with the system; caveats of individual
measurement series are listed in the measurement appendix (Table~\ref{tab:le:limitierung}), implementation
states in Chapter~\ref{ch:impl}, the implementation status of build and smoke series in
Section~\ref{sec:answers}. Machine-specific empirical optimization is the essential trait of cache-aware
methods (ATLAS~\cite{whaley2001atlas}, FFTW~\cite{frigo1998fftw}; cache-oblivious
methods~\cite{frigo1999cache} as the opposite pole), and an apparatus that makes it its principle inherits
its costs; the module bodies are not a limit (the code generator of the measurement path emits per
permutation the eighteen fully qualified axis types together with the umbrella include,
Section~\ref{sec:impl-pipeline}).
\begin{itemize}
  \item \textbf{Version change of an axis algorithm:} Every tier binary whose overlay cut contains the
  changed file becomes a new identity (version lock, Section~\ref{sec:impl-contracts}); its store stock
  has to be regenerated. What the user cannot do: exchange a single algorithm and keep the measurements of
  the remaining binaries.
  \item \textbf{Store per machine type and hardware configuration:} Measured values are keyed to the
  hardware identity of the measuring machine (stock~2, Section~\ref{sec:lager}) --- the price of
  machine-specific optimization. What the user cannot do: transfer measured values from one machine to
  another.
  \item \textbf{Coverage of the hardware counters per microarchitecture:} Collected are four generic
  counters (L1, last-level, dTLB, branch misses via the Linux system call \texttt{perf\_event\_open}) and
  two model-bound raw counters (L2 demand misses, fills from a foreign CCX as coherence observables) only
  with a cross-checked encoding (AMD Zen~5); otherwise the fields carry an explicitly coded
  non-observation (n/a with reason, never a zero: planned missing data introduce no
  bias~\cite{graham2006planned,graham2009missing}); the last-level event fails to open on Zen~5 (no
  generic mapping), RAPL is best-effort and empty without zone read permission, IPC is declared as a
  category, not collected; in the smoke series the hardware columns are n/a
  (Table~\ref{tab:le:limitierung}, row~1). What the user cannot do: obtain L2 and coherence counters on
  models other than Zen~5 (Figure~\ref{fig:measurement-triangle}).
  \item \textbf{Hardware fit without automatic check:} The hardware detection is available in three
  functions: the acquisition, the compile-time factory, and the runtime verdict; runner wiring,
  write-back, provenance round trip, and fit stamp are missing. What the user cannot do: have the fit
  between machine and binary checked automatically (Sections~\ref{sec:hw-probe}
  and~\ref{sec:impl-system-axes}).
  \item \textbf{Output without sheet fan-out:} xlsx is the default, the CSV form its counterpart
  generated from it (Section~\ref{sec:eval-identity}); sheet fan-out and CSV tree form are planned, not
  implemented. What the user cannot do: obtain results per sub-axis permutation as separate sheets or
  obtain the CSV storage as a tree.
  \item \textbf{Permutation space and budget:} A complete build of the golden reference catalogue costs,
  per system permutation, store and time in the extent that the implementation status quantifies
  (Section~\ref{sec:answers}); the planner computes the cardinality per profile and offers budget inputs
  in the subcommand \texttt{simulate}. What the user cannot do: materialize the full product space of all
  axis catalogues --- they choose catalogues or subsets, and the number of repetitions per measurement
  remains a trade-off between effort and confidence~\cite{kalibera2013rigorous}.
  \item \textbf{No runtime switch between tier binaries:} The hybrid stage is implemented as
  classification, dock layer, XML parser, and binary proxy (Section~\ref{sec:impl-hybrid-lager}); export,
  eviction, and router are missing. What the user cannot do: operate a hybrid tier that switches between
  preloaded binaries at run time; the state handover on switching --- the memento reconciliation with the
  two release artefacts single and hybrid in the store --- is likewise designed, not implemented.
  \item \textbf{Operating-system regime:} \texttt{operating\_system} is declared with three values;
  measurement takes place under root Linux, the second regime (immutable operating system) is not
  implemented (Section~\ref{sec:eval-platforms}). What the user cannot do: collect measurement series
  under Windows or macOS or without perf permissions (\texttt{perf\_event\_paranoid}, RAPL zones).
  \item \textbf{No isolated re-measurement of a single publication:} The operating mode that offers
  exactly one publication as a closed axis set is not implemented. What the user cannot do: measure a
  publication in isolation, faithfully to the original (Section~\ref{sec:outlook}).
  \item \textbf{No automatic selection of the best configuration:} The full automation is not
  implemented; present are the measurement-driven selection increment and the runtime-dynamic cache-line
  policy. What the user cannot do: have an axis configuration recommended automatically from load-profile
  and hardware features; the recommendation remains a manual derivation from the evaluation
...
\begin{enumerate}[label=(\arabic*)]
  \item the complete build according to the scope determined by the planner from the XML experiment
  (Section~\ref{sec:eval-build-scope}) and
...
--- cache engine, candidate under test, super-repository, manuscript --- allows further candidates under test to be brought in
analogously to PRT-ART without changing the building-block library: the candidate contract of the
...
\section{Outlook}\label{sec:outlook}
In the short term the following steps are due; implemented parts appear with their open remainder,
designed ones with their implementation status:
\begin{enumerate}[label=(\arabic*)]
  \item the complete build according to the scope determined by the planner from the XML experiment
  (Section~\ref{sec:eval-build-scope}) and the
  delivery measurement over the 320-identity catalogue;
  \item the end-to-end run of the three mandatory measurement series together with the diagrams;
  \item the full build-out of the store: the binaries with and without measurement sensors as two release
  artefacts, curve files, re-measure only what is missing (Section~\ref{sec:lager});
  \item the hardware detection: runner wiring, write-back, provenance round trip, fit stamp;
  \item the sheet fan-out of the xlsx workbook per sub-axis permutation plus info sheet;
  \item the cross-check of the Intel raw counters on prod2;
  \item the second operating-system regime and the Windows counter driver;
  \item the paper-isolation mode from RQ2.c;
  \item the profile connection of \texttt{disk\_io} and the permutation build over the scheduling
  sub-axis;
  \item the full permutation of the workloads against all living-being binaries (RQ2);
  \item the evaluation of the H2 criterion after the run of series~A\@.
\end{enumerate}

\paragraph{Heuristic automation and binary delivery.} The heuristics-driven \emph{full} automation of the
configuration choice demanded by sub-task~7 of the thesis assignment is not implemented; a first increment
is available as the tool \texttt{best\_binary\_selector}, which ranks and delivers the best permutation
per metric from the evaluation output (the xlsx workbook is the output, the CSV form its generated tree).
Besides the selection (commitment~1), the same strand carries the runtime-dynamic cache-line adaptation
(commitment~2). The full build-out needs (i) a \emph{heuristic/ML classifier} from load-profile and axis
features to the configuration, (ii) the implemented \emph{XML parser} of the heuristic policies, and (iii)
a \emph{binary packager} for the selected binary; the prerequisite is complete end-to-end measurement
series, until then the configuration recommendations from Section~\ref{sec:eval-results} are derived
manually.

\medskip
In the medium to long term: the measurement of the PIM allocator family on PIM hardware
(processing-in-memory: memory devices with computing units of their own; A16 is
integrated~\cite{pimmalloc2026}); the formal verification of selected permutation classes in the style of
StarMalloc~\cite{reitz2024starmalloc} (A13 is integrated); the transfer of the anatomy principle to
accelerators (GPU); and the hybrid components live ranking, state handover, and measurement-error
interpolation (Section~\ref{sec:heuristics-modes}). The four-repository architecture
(Section~\ref{sec:repos}) takes up further candidates under test analogously to PRT-ART without changing
the building-block library. Beyond tree structures, the anatomy principle is extensible to the hash-based,
spatial, and flat classes from Section~\ref{sec:overview} without rebuilding the apparatus --- an
essential lever for future experimental progress.
\end{nurkurz}
