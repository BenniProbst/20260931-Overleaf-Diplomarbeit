...
guiding question is cache-line awareness: Which design decision changes the cache-near measured
quantities --- L1, L2, and last-level misses, dTLB misses, branch misses, and the latency per
family-interface call --- on which platform, and to what degree? Three evaluation objects structure the
...
three evaluation objects structure the subsections; cache-line awareness is their common guiding axis.

The guiding axis of the evaluation is cache-line awareness: Compared are the cache-near measured
quantities --- L1, L2, and last-level misses, dTLB misses, branch misses, energy, and the latency
percentiles per family-interface function; they are sixteen registered measurement categories with one
regime ordinal per category --- per axis assignment, per concept- and family-interface call (the channel
address always reads axis $\to$ family $\to$ category), and per platform, that is, per registered machine
...
cache-line-related costs on which microarchitecture. The PMC categories (performance monitoring
...
\end{nurlang}
\begin{nurkurz}
Three evaluation objects structure the chapter: the \emph{design space} (hypotheses H1--H3, sensitivity
analysis), the \emph{apparatus} itself (heuristic counter-proof, measurement error of the measurement
instances), and the \emph{automation} from the experiment XML\@; the methodology
(Section~\ref{sec:eval-method}) unfolds them in eight, the results (Section~\ref{sec:eval-results}) in seven
subsections. The evaluation follows the empirical chain of research question, hypothesis, measured quantity,
experimental setup, execution, result, validity check, and
discussion~\cite{wohlin2012experimentation,shaw2003writing}: every hypothesis names measured quantity and
criterion before the measurement, every result its
uncertainty~\cite{georges2007rigorous,kalibera2013rigorous}; the constructed artefact is assessed by the
utility, rigour, and traceability of design-science
research~\cite{hevner2004designscience,peffers2007dsrm,wieringa2014designscience}, and reference quantity,
repetitions, dispersion, platform, and values not collected (n/a) stand at every result according to the
reporting rules for performance measurements~\cite{hoefler2015benchmarking,blackburn2016truth}. The guiding
axis is cache-line awareness: compared are the cache-near measured quantities (L1, L2, and last-level misses,
dTLB misses, branch misses, energy, latency percentiles; sixteen registered measurement categories with a
regime ordinal) per axis assignment and family-interface call (channel address axis $\to$ family $\to$
category) and per machine signature: prod1 (AMD Zen~5), prod2 (Intel Alder Lake), and the ODROID board with
Intel Gracemont cores. The PMC categories (performance monitoring counters, the hardware counters of the
processors) are the primary measured quantities, the time categories the cross-check; because counters are
trustworthy only when cross-checked~\cite{weaver2008counters}, Section~\ref{sec:eval-counters} discloses
their semantics per model.
\end{nurkurz}
...
\begin{description}
  ...
  documentation, maintainability, maintenance status, licence, build system; scale 1--5) for 22
...
space, the latter measurement organs of the candidate under test that, among other things, supply the data for this
test.
...
\begin{nurkurz}
\subsection{Fully Automatic Experiment Evaluation from the XML}\label{sec:eval-xml}
The evaluation is itself an object of the automation (Figure~\ref{fig:eval-xml-kette}): A user XML with the
root \texttt{<comdare\_experiment>} declares \emph{what} is measured, \emph{where}, and \emph{when}; decisive
are the sections \texttt{workloads}, \texttt{datasets}, \texttt{measurement\_tooling},
\texttt{measurement\_categories}, \texttt{run\_methodology}, \texttt{writeback\_methods}, and
\texttt{output}, which the profile validator checks against the registries. From the XML the plan arises
deterministically, from it the build jobs of the CacheEngineBuilder and the tier binaries, whose measurement
lies in the workbook (xlsx); the CSV form is the tree extract generated from the workbook
(Section~\ref{sec:eval-dataflow}), and the generators of the tool repository render tables and diagrams per
language into the \texttt{tabellen} directory of the manuscript. No step is executed by hand; every number in
the appendix carries its run (Appendix~\ref{app:measurements}). Implementation status: The chain is
demonstrated in a first end-to-end run (smoke measurement series over 43 permutations);
\texttt{<publish>}, the creation mode (Section~\ref{sec:impl-pipeline}), and the third output factory LaTeX
(Section~\ref{sec:eval-dataflow}) are planned but not implemented.
\begin{figure}[htbp]\centering
\begin{tikzpicture}[font=\footnotesize,>=Stealth,
  st/.style={draw,rounded corners,fill=blue!7,text width=2.55cm,minimum height=1.1cm,align=center,inner sep=2pt,font=\scriptsize},
...
\begin{nurkurz}
\subsection{Workloads and Datasets}\label{sec:eval-workloads}
Phase~5 of the measurement pipeline (Execute, Section~\ref{sec:impl-pipeline}) derives the YCSB workload per
permutation as a fallback heuristic from the profile's traversal tag (\texttt{RANGE} to YCSB-E,
\texttt{HOT\_PATH} to YCSB-A, otherwise YCSB-C), unless an \texttt{<expected\_workload>} tag overrides the
heuristic (Section~\ref{sec:sota-instances}); the global default is the permutation over all workloads of
the load framework YCSB~\cite{cooper2010ycsb}, implementation status: The driver falls back to exactly one
workload, the full permutation is planned but not implemented. The data basis is real string datasets
(Table~\ref{tab:datasets}), each with a reproducibility record of source, checksum, line count,
preprocessing, and seed rule: four records are computed against the preprocessed files, \texttt{dna}
carries the record of the Pizza\&Chili source, \texttt{xml} a specification without checksum (n/a, file not
local); a canon test checks the records against the six-element canon of the table.
\end{nurkurz}
...
\begin{nurkurz}
\subsection{Platforms, Vendor Lanes, and Regimes}\label{sec:eval-platforms}
The measurements target three registered measurement machines: prod1, the AMD machine with Zen~5 cores
(signature \texttt{prod1\_zen5}, with AVX-512), prod2, the Intel machine with Alder Lake (signature
\texttt{prod2\_alder\_lake}, without AVX-512, with P and E cores), and an ODROID board with Intel Gracemont
cores (\texttt{odroid\_gracemont}). The ZIH cluster (Barnard node: Sapphire
Rapids)~\cite{zih_hpc_compendium_hardware} is declared as sub-axis IS3 of the target ISA, registered in no
signature, and not measured (n/a); an HBM platform is registered nowhere. An \texttt{IPlatformProbe} reports
per platform the runnable subset of the axes; the exclusion of a missing extension is always decided
\emph{machine-side} with a warning, never as a silent profile filter, per platform a scalar and an AVX2
build are prepared, an AVX-512 build only with a matching signature, and the XML creation mode with platform
filter is not implemented (Section~\ref{sec:impl-pipeline}). On hybrid CPUs, P and E cores are measured
separately via the core PMU domains of the kernel; the core modes follow the six-mode model from
Section~\ref{sec:axis-triad} with \emph{HybridAware} as the sixth, mandatory mode, their attachment to the
run profile is not implemented (Section~\ref{sec:impl-system-axes}).
\end{nurkurz}
...
\end{nurlang}
\begin{nurkurz}
The productive measurement operation is a \emph{two-machine operation} of prod1 and prod2 with two
individually activatable vendor lanes of the counter collection, \texttt{pmc:amd} and \texttt{pmc:intel},
each in its own PMU resource group. Completeness is an evaluation rule, not a CI gate: A measurement series
counts as complete only once both lanes have delivered, a lane run on one side only is reported as a partial
measurement (Section~\ref{sec:eval-validity}). The measurement plan provides two operating-system regimes
per measurement: an \emph{immutable} operating system (Talos) with boot-/compile-static cache settings
(Section~\ref{sec:hardware-history}) and a \emph{root Linux with full hardware-counter access} for the
counter-based categories (Section~\ref{sec:sota-instances}). The Talos regime is designed, not carried out:
All reported measurements originate from the root-Linux regime of both machines.
\end{nurkurz}
...
To be separated from this is which counters that access actually populates
(Table~\ref{tab:eval-pmc-zaehler}): The measurement chain opens via
\texttt{perf\_event\_open}~\cite{linux_perf_event_open,demelo2010perf} four generic counters (L1D,
last-level, dTLB, and branch misses, portable on Intel and AMD); L2 misses and coherence invalidations are
collected model-bound via a RAW event catalogue whose Zen~5 encodings stem from the event table of the Linux
kernel~\cite{linux_perf_amdzen5} and are cross-checked; for Intel no entry exists (n/a), because the
cross-check on an Intel host is missing. The last-level event fails on Zen~5 at opening (\texttt{ENOENT}),
IPC/CPI is declared but not wired up, and the energy is read best-effort from the RAPL sysfs zone (Running
Average Power Limit, the energy counters of the processors~\cite{david2010rapl,khan2018rapl}). The principle
of explicitly coded non-observation is realized as a field flag: Per counter the counter bundle carries a
\texttt{*\_source\_available} flag --- a number means the source has delivered, n/a means the source is
missing ---, and the distinction travels end-to-end into workbook, tables, and diagrams. Hardware counters
count as trustworthy only after cross-checking~\cite{weaver2008counters}; the cross-check for Zen~5 is
substantiated by measurement, the one for Intel is not available.
\begin{table}[htbp]\centering\footnotesize
\begin{tabular}{@{}l>{\raggedright\arraybackslash}p{4.6cm}>{\raggedright\arraybackslash}p{3.2cm}>{\raggedright\arraybackslash}p{2.3cm}@{}}
\toprule
...
family-interface functions of the tier binary; \emph{micro} (mi) --- the time and the success
...
fan-out. The analysis keeps the three levels apart: axis interface, concept and family interface, and
...
\end{nurlang}
\begin{nurkurz}
Time and observer measurement are united in the measurement checkpoint (\texttt{checkpoint\_measure}); their
responsibilities are separated by the measurement-tooling main axis in exactly this order: \emph{wallclock},
\emph{macro}, \emph{micro} (Section~\ref{sec:measurement-system}, Figure~\ref{fig:traeger-flaechen}); every
other order is syntactically wrong, w/ma/mi is the registered alias. Three collectors (wall clock, observer
snapshot, PMC) collect as building blocks of the measurement axis \texttt{collector}, the sixteen
measurement categories are the sub-axis of the tooling axis, and the analysis keeps the three levels apart
(Section~\ref{sec:eval-sensitivity}); PMC measurements as a figure type of their own in the appendix
generator are planned but not implemented. Implementation status: Registry and level canon are implemented,
the order validation and the self-contained collector are not.
\end{nurkurz}
...
\end{nurlang}
\begin{nurkurz}
The time- and observer-based categories run identically under both regimes, the PMC categories only under
the privileged regime; the registry carries the regime per category as an ordinal (nine time/observer, seven
counter categories). Operating system, kernel, and counter domain are logged per platform; in the WIDE
schema only the columns \texttt{platform} and \texttt{build\_version} carry platform tag and build version,
the log column per domain is not implemented.
\end{nurkurz}
...
fixed (Section~\ref{sec:mess-chain}) --- a simplifying, superseded assumption, the catalogue is the
regression detector, not the build scope; the system axes multiply the build matrix, never the
\texttt{binary\_id}. The upper bound of binary identities per system permutation is the product of the
configurations selected per organ axis in the XML experiment; the planner determines it from the XML
experiment, it is known only at execution time, and the cardinality of the real permutation space the
planner computes dynamically from its inputs (subcommand \texttt{simulate}); it is not a fixed number. Results of the runs of the
full build are not available (Section~\ref{sec:eval-available}).
\end{nurlang}
\begin{nurkurz}
\subsection{Build Scope and Optimization Levels}\label{sec:eval-build-scope}
The scope of the full build is fixed in the profile: two optimization levels (O2, O3) times two SIMD levels
(no extension, AVX2), that is, four build variants per permutation (Section~\ref{sec:axis-triad}). Excluded
is Ofast alone, because it would break the IEEE-754 and the run-to-run determinism as well as the CRC-64
checksum of the golden reference catalogue; the default is O2, O3 is selected via the XML and is
identity-effective via the toolchain member of the fingerprint, O0 to O3 as a user choice and the
pass-through of further compiler flags are planned but not implemented. Per build variant the golden
reference catalogue is built ($2^{17} = 131072$ binary identities under the superseded assumption of two
configurations per axis, regression detector, not build scope; Section~\ref{sec:mess-chain}); the upper
bound per system permutation is the product of the configurations selected per axis in the XML experiment,
determined by the planner only at execution time, and the cardinality of the real permutation space the
planner computes dynamically (subcommand \texttt{simulate}).
Results of the runs of the full build are not available (Section~\ref{sec:eval-available}).
\end{nurkurz}
...
definition of the minimal mode is data-bound, and two series of the same candidate under test with different modes
...
\subsection{Fairness and Reproducibility}\label{sec:fairness}
Fair comparability is secured by fixed rules. Each comparison separates a common minimal mode (common
denominator) from the PRT-ART native mode, declared per SOTA series as \texttt{fairness\_mode}
(\texttt{common\_denominator} or \texttt{native}); implementation status: The mode is a series metadatum
without influence on the measurement and never part of the \texttt{binary\_id}, which is why two series with
different modes produce the same \texttt{binary\_id}, which the validator reports as a warning. All methods
receive identical, normalized byte keys and identical miss, update, and delete sets; the warm-up is the
two-phase driver, and each configuration is measured in several separate runs (repetition dimension
\texttt{repetition}). Thread pinning: The actuator is implemented but not attached to the run profile, which
is why the measurement emission reports the actual state as \emph{unpinned} (on prod1 with two L3 complexes
of unequal size an unpinned run is not reproducible). Missing capabilities of a baseline are reported as n/a
rather than emulated (sample statuses \emph{NotApplicable}, \emph{SourceUnavailable}, \emph{Failed}).
Provenance: toolchain, flags, ISA summary, and the commit states of the four repositories enter the
provenance manifest at configuration time, a provenance per individual foreign library is not implemented.
The latency percentiles are determined by the evaluation tree nearest-rank (Hyndman and Fan, type~1, without
interpolation~\cite{hyndman1996quantiles}); the presentation tools possess their own copies of the
conversion (a declared gap), and the HDR histogram~\cite{tene_hdrhistogram} (high dynamic range: bins of
fixed relative resolution over several orders of magnitude) is a method of its own of the distributional
view. Layout and environment effects that produce wrong conclusions without a recognizable
error~\cite{mytkowicz2009wrongdata} remain a limit: A randomized layout
stabilization~\cite{curtsinger2013stabilizer} is missing (Section~\ref{sec:eval-validity}).
\end{nurkurz}
...
Appendix~\ref{app:blocks}. The H2 dossier marks reconstructions without a vendored original source as
...
\end{nurlang}
\begin{nurkurz}
For foreign libraries the original-compiler rule applies: Where feasible, the foreign source is built with
its original compiler. The proof of originality is implemented --- the \texttt{is\_original} marking per
function, the validator, and the relock gate of the CI (Appendix~\ref{app:blocks}) ---, the automatic intake
of the vendored sources with their original compiler is planned but not implemented.
\end{nurkurz}
...
deterministic result order ---, while the threading of the concept stays in the tier binary.
...
\end{nurlang}
\begin{nurkurz}
The measurement \emph{operation}, too, is part of these fairness rules, and it runs through the store
(Section~\ref{sec:lager}, Figure~\ref{fig:lager-bestaende}): stock~1 keeps the binary under its SHA-512
fingerprint, stock~2 the measured values under the pair of fingerprint and hardware identity of the
measuring machine; the binary with and the one without measurement sensors lie side by side as two entries,
because the measurement-gate state is a fingerprint member. Measurement is always switched on, re-measured
is only what is missing in the store: A binary with a known fingerprint and an inventoried test attestation
is skipped, otherwise it is built or measured (Figure~\ref{fig:eval-lager-kreislauf}); the CEB produces the
parallel requests to the tier binaries. Implementation status: Stock~1 and~2 as well as the key schema of
all four stocks are implemented, the full store build-out is not.
\begin{figure}[htbp]\centering
\begin{tikzpicture}[font=\footnotesize,>=Stealth,thick,
  k/.style={draw,rounded corners,fill=blue!7,text width=2.7cm,minimum height=1.15cm,align=center,inner sep=2pt,font=\scriptsize},
  lg/.style={k,fill=green!10},
...
switch of the planner shell for the wiring check, which stays locked for users, not a work mode (under
\texttt{--debug} the conformance gate additionally runs the standard test set of the tier interface; this indirect debug mode
coincides with the release build and is not kept as a separate mode); the
threading of the concept itself is thereby left to the tier binary rather than overridden by the
...
conformance check at the probe dock (Section~\ref{sec:impl-contracts}): the conformance gate of the
family contract (under \texttt{--debug} extended by the standard test set of the tier interface) before the post-compile
attestations of the measurement interfaces, beside it the
...
\end{nurlang}
\begin{nurkurz}
Measurement remains strictly separated from building: measurement runs are triggered explicitly. The run
methodology knows four work modes --- \texttt{build}, \texttt{measure}, \texttt{compare}, and
\texttt{release} (Section~\ref{sec:impl-pipeline}) ---; the measure mode measures with exactly \emph{one}
measurement thread, the threading of the concept itself is left to the tier binary, and \texttt{--debug} is
a switch of the planner shell, not a work mode (under \texttt{--debug} the conformance gate additionally
runs the standard test set of the tier interface; this indirect debug mode coincides with the release build). A binary found in
the store is skipped only with an
inventoried test attestation (Figure~\ref{fig:eval-lager-kreislauf}), every measurement is preceded by the
conformance check at the probe dock (Section~\ref{sec:impl-contracts}), and build as well as measurement runs
are processed per machine in milestone batches of 4096 binaries each with inventoried resumption after
interruption.
\end{nurkurz}
...
carrier stage of its own that is optionally inserted between the CEB probe dock and the tier binary:
it docks at the probe dock, carries dock slots for tier binaries, and itself produces no code. The
measurement-curve type system supplies the estimation basis
...
\begin{tikzpicture}[font=\footnotesize,>=Stealth,
  ...
  \node[hy,font=\tiny] (hb) at (12.7,1.6){\textbf{Hybrid tier binary}\\optional mux insertion between CEB probe dock and tier binaries; docks at the probe dock; produces no code};
...
hybrid tier binary is the mux stage optionally inserted between the CEB probe dock and the tier binary,
docks at the probe dock, carries dock slots for tier binaries, and itself produces no code; the
comparison is made on the wall clock, primarily against the paper reconstructions,
...
\begin{nurkurz}
\subsection{The Heuristic Counter-Proof as Fourth Dimension}\label{sec:eval-counterproof}
Beyond the three hypotheses, the analysis receives a fourth, self-contained \emph{evaluation dimension}: the
heuristic counter-proof, whose concept Chapter~\ref{ch:measurement} develops
(Sections~\ref{sec:heuristics-modes} and~\ref{sec:heuristics-curves}). The CacheEngineBuilder stage compiles
the heuristically estimated recombination of the tier binaries into a hybrid tier binary, a carrier stage of
its own that is optionally inserted between the CEB probe dock and the tier binary (mux at the probe dock
with dock slots for tier binaries); the measurement-curve type system supplies the estimation basis
(Figure~\ref{fig:eval-messkurven}): per axis one curve over its parameters and per measurement level, from
these the break-even points and the estimated recombination. The assessment question reads: \enquote{Is the
heuristically estimated recombination of the tier binaries faster than the existing paper algorithms?}
Unlike H1--H3, the counter-proof tests not the design space but the apparatus itself, and therefore stands
\emph{beside} the ranking of the sensitivity analysis (Section~\ref{sec:eval-sensitivity}).
Implementation status: Classification layer, dock layer, XML parser, and binary proxy of the hybrid stage
are implemented (six of the nine envisaged files, Section~\ref{sec:impl-hybrid-lager}); router and eviction
presuppose measurement curves and are not implemented.
\begin{figure}[htbp]\centering
\begin{tikzpicture}[font=\footnotesize,>=Stealth,
  k/.style={draw,rounded corners,fill=blue!7,text width=2.55cm,minimum height=1.1cm,align=center,inner sep=2pt,font=\scriptsize},
  hy/.style={k,fill=orange!14},
...
\begin{nurkurz}
The test plan for this is fixed in two stages; the corresponding measurement runs are not available
(Section~\ref{sec:eval-available}). The first stage quantifies the measurement error that the measurement
instances themselves introduce: The same experiment is set up in three build variants --- observers in
\emph{all} tier binaries and in the hybrid tier, \emph{only} in the heuristic tier binary, in \emph{none} of
the compiled artefacts ---, kept apart via the measurement-gate member of the fingerprint
(Section~\ref{sec:stamp-model}) with the fields \texttt{tm}/\texttt{tmi} (tier level) and
\texttt{hm}/\texttt{hmi} (hybrid level), off state \texttt{mg=m0;s0;st0;x0;tw0;tm0;tmi0;hm0;hmi0}. The
differences between the variants quantify the observer overhead by origin and yield, per workload and
platform, an \emph{uncertainty band} from repetitions with a confidence interval~\cite{georges2007rigorous},
below which two configurations count as indistinguishable; the validity criterion is $t_1 \ge t_2 \ge t_3$
for the wall-clock times of the three variants (with measurement sensors it must be slower than without), a
violation is a measurement defect of the cell, not a result. Implementation status: The check is not
implemented; it connects to the measurement-error interpolation as the fifth evaluation component of the
compare stage (Section~\ref{sec:heuristics-curves}), whose constellations the planner computes dynamically,
...
\end{nurlang}
\begin{nurkurz}
The second stage is the complementary \emph{fourth step} of the heuristic assembly: The tier binaries
subordinate to the heuristic tier are rebuilt without micro- and macro-benchmarks and without observers and
compared purely on the wall clock --- primarily against the 33 reconstructed paper algorithms
(Section~\ref{sec:sota-instances}), secondarily against all known tier-binary permutations, in particular
the best single binary.
\end{nurkurz}
...
\end{nurlang}
\begin{nurkurz}
From this follows the decision rule: The counter-proof counts as \emph{delivered} per workload class if the
observer-free rebuilt recombination beats the fastest paper reconstruction on the wall clock and the margin
exceeds the uncertainty band, and as \emph{missed} if a paper algorithm or the best single binary comes out
ahead --- this, too, would be a result, because it would quantify the estimation error of the curve
prognosis. Cells that violate the validity criterion of the first stage do not enter the decision; the
measurement data of this dimension are not available (Section~\ref{sec:eval-available}).
\end{nurkurz}
...
binary, its axis versions, and its machine. The mixing strategy of a candidate under test is expressed not via a
...
under the namespace prefix of the candidate under test; the flag \texttt{e} of the entry grammar denotes an
...
individual sources and is therefore anchored as a fixed schema), the pipeline view with 16 columns
...
\midrule
WIDE (E4) & semicolon & 189 & measurement output; header at runtime from four individual sources, anchored as a fixed schema & iterator of the CacheEngineBuilder (\texttt{lazy\_csv\_header}); fixed schema \\
...
\begin{nurkurz}
\subsection{Data Flow from the Measurement to the Manuscript}\label{sec:eval-dataflow}
Per series run the measurement stage writes the one official \texttt{measurements.csv} as a projection of
the workbook (implemented is a flat file, planned the CSV tree with one folder per sheet); the
\texttt{<output>} paths of the user XML the run entry resolves as run provenance. Three column lists are to
be distinguished: the WIDE schema with 189 columns (separator semicolon; the schema authority of the
measurement output, anchored as a fixed schema), the pipeline view with 16 plus 16 additional columns, that
is 32, for the LaTeX stages (separator comma; per permutation for instance \texttt{permutation\_id},
\texttt{fingerprint}, and \texttt{total\_cycles}), and the thirteen-column legacy form of the result
aggregator. \texttt{total\_cycles} carries under its legacy name the representative p50 latency in
nanoseconds, not cycles; the renaming or an actual cycle channel is a documented design decision. Values not
collected stand as n/a, never as zero.
\end{nurkurz}
...
from which the manuscript includes the files --- each language version includes 38 generated files of
the appendix.
\end{nurlang}
\begin{nurkurz}
The workbook (xlsx) always arises; it is the output and the standard, and the CSV form is not a single child
document but an ordered tree of CSV files --- one overall folder per workbook, beneath it one folder per
sheet, therein the CSV files of the individual sub-axis sweeps ---, which is always generated from the
workbook and is its mirrored counterpart. What goes to disk is decided by \texttt{<writeback\_methods>}
alone --- xlsx, csv, or both, with xlsx as default (Section~\ref{sec:impl-system-axes}); the xlsx writer
writes via libxlsxwriter. Implementation status: The seam writes the CSV projection as one flat file, the
tree form is planned but not implemented. The analysis enriches the projection via binary-to-CSV with
profile ID, paper reference, and axis assignment, renders CSV-to-LaTeX fact sheets per permutation ID and,
via the diagram generator, bars, scatter plots, heatmaps, and the families of the measurement appendix
(ECDF, sweep curves, ratio matrices, Pareto scatter, and more); the appendix generator writes per language
into a \texttt{tabellen} directory, from which each language version includes 38 generated files.
\end{nurkurz}
...
factory, the warnings section of the info sheet, and the catch-up of the fixed WIDE schema.
\end{nurlang}
\begin{nurkurz}
The workbook of a run is one file with one sheet per selected sub-axis permutation and an info sheet; the
sheets are numbered in the binding order measurement $\to$ system $\to$ organ (\texttt{S001} to
\texttt{Snnn}), the columns stem exclusively from the WIDE header, and columns constant within a run move
into the metadata of the info sheet. The contract tests of the persistence check at the CSV output
(Section~\ref{sec:impl-contracts}); besides xlsx and csv, a third factory LaTeX is planned. Implementation
status: Writer and projection are implemented (one data sheet under a constant sheet key, flat projection);
not implemented are the fan-out into one sheet per sub-axis permutation plus info sheet
(Section~\ref{sec:limitations}), the CSV tree, the check at the xlsx, the export element, the chart
functions, the LaTeX factory, the warnings section of the info sheet, and the catch-up of the fixed WIDE
schema.
\end{nurkurz}
...
assignment, the user XML declares per axis the \texttt{merge} mode of the candidate under test
(Table~\ref{tab:eval-verbund-modi}): \texttt{replace} (the default, also with an empty attribute; the
candidate axis replaces the cache-engine axis with fallback, merge strategy
\texttt{Verbund2\_Replace}), \texttt{merge} (cache engine and candidate under test hybrid per candidate under test,
...
absence of any candidate directive, and \enquote{stage~1 to~3} remains the alias of the merge-strategy
...
stamp line of their own they do not possess, for the candidate under test mixes against the organ axes and thus
...
empty / \texttt{replace} & \texttt{Verbund2\_Replace} & stage~2 (replace) & the candidate axis replaces the cache-engine axis, with fallback & A \\
\texttt{merge} & \texttt{Verbund2\_Hybrid} & stage~2 (hybrid) & cache engine and candidate under test hybrid per candidate under test; materialization open, no production caller & A \\
...
\begin{nurkurz}
\subsection{Measurement Series and Merge Strategies}\label{sec:eval-series}
Three measurement series are evaluated (Table~\ref{tab:stage-series}): series~A --- PRT-ART against the
state of the art with the evaluation hypotheses H1--H3 (Section~\ref{sec:eval-hypotheses}); series~B --- the
systematic variation of the cache-engine permutations from the union of stage~3; series~C ---
merge/regression of old against new. Orthogonal to this, the user XML declares per axis the \texttt{merge}
mode of the candidate under test: \texttt{replace} (default; the candidate axis replaces the cache-engine
axis with fallback, \texttt{Verbund2\_Replace}), \texttt{merge} (cache engine and candidate under test
hybrid, \texttt{Verbund2\_Hybrid}), and \texttt{union} (union of all axis variants without discarding,
\texttt{Verbund3\_Union}); stage~1 is \texttt{Verbund1\_CeOnly}, the absence of any candidate directive,
\enquote{stage~1 to~3} remains the alias of the merge-strategy names and the full join the alias of
\texttt{union}. Series~A works with \texttt{replace} and \texttt{merge}, series~B with \texttt{union}. The
states remain distinguishable at the binary via the hierarchically extended algorithm names of the organ
line (Section~\ref{sec:eval-identity}); a stamp line of their own they do not possess. Implementation
status: Materialized are \texttt{Verbund1\_CeOnly}, \texttt{Verbund2\_Replace}, and
\texttt{Verbund3\_Union}, while \texttt{Verbund2\_Hybrid} remains without a production caller.
\end{nurkurz}
...
family-interface operations of the tier binary --- and \emph{micro} --- the axis interface, the set of
...
(Section~\ref{sec:eval-counters}). The guiding axis is cache-line awareness: which axis moves the
...
system). Construct validity: Do the categories measure cache-line awareness? The PMC semantics are
...
\begin{nurkurz}
\subsection{Validity and Limits}\label{sec:eval-validity}
The validity of the evaluation is checked according to the four classes of threats to a conclusion, the
threats to validity of empirical software research (construct, internal, external, and conclusion
validity~\cite{wohlin2012experimentation}; Table~\ref{tab:eval-gueltigkeit};
Section~\ref{sec:measurement-system} carries the same classes as quality criteria). Construct: The PMC
semantics are disclosed per model (Section~\ref{sec:eval-counters}), and hardware counters count as
trustworthy only when cross-checked~\cite{weaver2008counters}. Internal: The observer overhead is bounded
via the uncertainty band of the three build variants (Section~\ref{sec:eval-counterproof}); layout and
environment bias~\cite{mytkowicz2009wrongdata,curtsinger2013stabilizer} are countered by identical keys,
identical build matrix, and repetitions, a layout randomization is missing; warm-up and repetitions follow
Section~\ref{sec:fairness}~\cite{kalibera2013rigorous}. External: The measurement machines comprise two
x86-64 microarchitectures and one Gracemont node; ZIH and HBM are declared, not measured
(Section~\ref{sec:eval-platforms}), the Intel RAW assignment is missing. Conclusion: percentiles
nearest-rank without interpolation, geometric aggregation, n/a instead of zero. The limitations of the
measurement appendix (Table~\ref{tab:le:limitierung}, Section~\ref{sec:measurements:limitations}) and of the
conclusion (Section~\ref{sec:limitations}) are the substantiated list of these limits; no number is named
without a run, and the reporting rules for performance
measurements~\cite{hoefler2015benchmarking,blackburn2016truth} apply to every number.
\begin{table}[htbp]\centering\footnotesize
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{1.6cm}>{\raggedright\arraybackslash}p{4.0cm}>{\raggedright\arraybackslash}p{4.9cm}>{\raggedright\arraybackslash}p{3.7cm}@{}}
\toprule
Class & Threat & Countermeasure in the apparatus & Implementation status \\
...
\begin{nurkurz}
\subsection{Summary of the Evaluation}\label{sec:eval-summary}
The evaluation records per evaluation object. Design space: The hypotheses H1--H3 carry measured quantity
and criterion, the sensitivity analysis is fixed as a procedure over the three measurement levels; evidence
is available for the smoke series (43 permutations, Appendix~\ref{app:measurements}), for the mandatory
measurement series not (n/a). Apparatus: The counter-proof is fixed as a fourth dimension with test plan,
validity criterion, and decision rule, its measurement data are not available; the measurement-error
foundation (gate fields in the fingerprint, field flags per counter, n/a rule) is implemented. Automation:
The chain from the experiment XML to the included appendix is demonstrated in a first end-to-end run. The
results are discussed against the leitmotif of the thesis, the transition from heuristic to informed cache
management; the cache-line effect from the synthesis curves is data-dependent and is not anticipated here.
By the standards for research contributions~\cite{shaw2003writing} and empirical
assessments~\cite{blackburn2016truth}, the contribution of this chapter is a validated experimental setup
together with its automation; Chapter~\ref{ch:conclusion} answers the research questions on this basis.
\end{nurkurz}
