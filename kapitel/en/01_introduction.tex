...
\begin{itemize}
  ...
  (\texttt{comdare\_create\_anatomy()} $\to$ \texttt{IAnatomyBase}) exports exactly one living being
  per permutation; behind it lies a three-layer abstraction chain (CacheEngine $\to$ ExecutionEngine
  $\to$ SearchEngine) with variadically templated hybrid-API specializations (1~parameter
  $\Rightarrow$ \texttt{std::vector} API; 2~parameters $\Rightarrow$ \texttt{std::map} API; $N>2
