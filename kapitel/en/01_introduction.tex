...
configurations, and (iii)~measures both individual axes and whole algorithms on a single, fair
basis.
...
\begin{description}
  ...
  hardware-counter collection be designed so that every axis permutation is measured fairly (ceteris
  paribus, identical compiler and flag basis) across three measurement series (A--C) over three
  granularities (micro-, macro-, and overall benchmarking), and under which platform-calibrated
  criteria does a platform count as successfully verified? \emph{Sub-question:} How can
  contradictory findings in the literature on the optimal node/cache-line size (one cache line for
  CSS/CSB\textsuperscript{+} versus up to sixteen for Hankins/Patel) be re-measured bias-free
  through axis-isolated, ceteris-paribus permutation rather than adopted from incompatible original
  setups?
...
CacheEngineBuilder, CacheEngine, probe). This \emph{anatomy} view (genus $\to$ living being $\to$
axes) runs \emph{parallel} to the ABI/runtime layering CacheEngine $\to$ ExecutionEngine $\to$
SearchEngine --- it is not nested with it: the ABI layering provides the ABI-stable module
interfaces, while the nineteen axes live in the anatomy; both views share the same root (the
execution engine) and are joined only at run time. The concept of the axis architecture itself is
adopted from prior work by the author (Comdare\,/\,BEP~Venture~UG, originally for optimizing
deduplication procedures in the UltiHash database); the contribution of this thesis is its transfer
to cache-aware search structures and its measurement-oriented realization. The central contributions
are:
\begin{itemize}
  \item A three-layer ABI/runtime layering (CacheEngine $\to$ ExecutionEngine $\to$ SearchEngine)
  --- parallel to the axis anatomy --- with ABI-stable C++23 module interfaces and variadically
  templated hybrid-API specializations (1~parameter $\Rightarrow$ \texttt{std::vector} API;
