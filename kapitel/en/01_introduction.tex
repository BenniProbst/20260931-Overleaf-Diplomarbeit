...
% (4 occurrences); the active text uses eighteen organ axes (T0--T17).
% SUPERSEDED (order 506, 2026-09-26): the taxonomy terms of this legacy file are outdated
% (old family -> concept, old genus -> family, new genus = subspecies/implementation; levels 0..5a/5b).
% Governing: kapitel/<lang>/03_messsystem_prtart.tex (sec:anatomy-levels) and anhang/<lang>/C_glossary.tex.
...
architecture. In doing so, the focus when finding heuristics rests mainly on the assumption of
empirically discovered heuristic improvements and measurements. Because code whose behaviour rests
on cache-line-optimized execution is so fundamental, it is sensible to segment the constituent parts
of the search-algorithm shell into axes. This amounts to the backward abstraction of known
search-algorithm components into reusable modules across the most important state-of-the-art papers.
...
influence of cache-line settings, the following research questions arise: one main question (RQ0)
and four measurable sub-questions (RQ1--RQ4); complementary aspects are attached as
\emph{sub-questions}.
\begin{description}
...
configurations, measure and evaluate cache-line-aware behaviour both per constituent and for the
...
(SearchAlgorithm, Container, Graph), beneath it genus subclasses with a fixed axis set,
...
composition of its nineteen axis-organs. From it a standalone binary with an ABI-stable
search-algorithm interface is compiled --- optionally with or without the measurement observer for
debug, measurement, or release. The root \texttt{ExecutionEngine} carries the anatomy-bearing genera (the \emph{living being} \texttt{SearchAlgorithm}, the keyless \texttt{Container}, and the \texttt{Graph}) and
axis-less \emph{viruses} as siblings; the layering
...
concept of the axis architecture itself is adopted from prior work by the author (on its origin see
Section~\ref{ssec:sota-design-contribution}); the contribution of this thesis is its transfer to
cache-aware search structures and its measurement-oriented realization. The central contributions
are:
\begin{itemize}
  ...
  root-Linux regime is available, the Talos mirror is pending.
...
measurement technologies, and the architectural design --- closing with the research gap, with a
focus on cache-line-aware optimization as a single-configuration setting in the cache engine.
