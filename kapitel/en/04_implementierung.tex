...
\begin{tikzpicture}[font=\footnotesize,>=Stealth,
  ...
  \node[hy] (s4) at (7.9,0){\textbf{Insertion: hybrid}\\{\tiny optional, between stage 2 and 3; hybrid \texttt{.so}}\\[2pt]{\tiny mux switch at the CEB probe dock; dock slots 1 to $N$ for tier binaries; the same mandatory symbols; \texttt{genus()} reports the target genus}};
  ...
  \node[fl] (f3) at (11.85,-2.95){Surfaces 1--3: genus interface, stamp (three lines), measurement cut-through \texttt{IMessVisitor}};
...
vendor is missing instead of breaking the build, with a fallback chosen per family
...
reports). Beneath it lie the four families and the six genera (Section~\ref{sec:anatomy-levels}); in
code the family is the derivation of the genus (\texttt{gattung\_of}), not a second label. The living
...
\texttt{comdare\_\allowbreak{}anatomy\_\allowbreak{}genus}, spelled identically across all families, as
well as the version lines of the stamp. The identical spelling is a condition: the family-independent
loader loads every binary via the same loading route (POSIX \texttt{dlopen}~\cite{posix_dlopen}) and
determines the family from the genus \emph{before} it constructs an instance. The ABI carries major
version~9, whose seam is the genus interface. The variadic 1/2/N hybrid API belongs to the candidate
...
\end{lstlisting}
  ...
  and reports its target genus via \texttt{genus}) and the variadic 1/2/N API as the view of the candidate under test.
...
The contracts between the carrier stages are partitioned into surfaces at the binaries. The
three-surface model (Section~\ref{sec:anatomy-levels}) is present in code by name: surface~1 is the
genus interface of the tier binary, surface~2 the stamp and surface~3 the measurement cut-through
\texttt{IMessVisitor} --- a visitor at the \texttt{dlopen} transition, which exists \emph{so that}
family and genus interfaces remain unchanged. The stamp contract is executed as a matrix of seven
...
addresses axis $\to$ genus $\to$ category to hundreds of measurement categories (implementation status:
...
behind the genus interface through a control surface of its own, the CEB takes over the inner state of
...
\begin{tikzpicture}[font=\footnotesize,>=Stealth,
  ...
  \node[font=\bfseries\scriptsize,anchor=north west] at (-4.5,-3.15){Hybrid \texttt{.so} (family HeuristikAdapter, reroute genus) --- inserted mux stage};
...
reports the \emph{target} genus, never the reroute genus, and the genus build admission carries as its
sixth entry the reroute genus with the dock cap (default 32) as arity. The hybrid live ranking as the
