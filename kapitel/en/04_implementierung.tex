...
\begin{tikzpicture}[font=\footnotesize,>=Stealth,
  ...
  \node[fl] (f3) at (7.9,-2.95){Surfaces 1--3: genus interface, stamp (three lines), measurement pass-through \texttt{IMessVisitor}};
...
analyzer (\texttt{p27\_bundle\_finder}, a C++23 port of the original tool \texttt{hp\_soft} that
...
\begin{tikzpicture}[font=\footnotesize,>=Stealth,align=center,
  ...
  \node[iface,fill=orange!10] (mess) at (-4.9,-2.35){IMessVisitor\\{\normalfont\scriptsize surface 3: measurement pass-through}};
...
---; two runs are byte-identical, and the plan header annotates the registry trio of the three offer
...
ABI-stable) and surface~3 the measurement pass-through \texttt{IMessVisitor} --- a visitor at the
...
carriers --- 28~cells with tri-valued admission (mandatory, permitted, forbidden), of which the existing
...
line is carried only by tier and hybrid --- the absence at the CEB is the static bar \enquote{the CEB
...
\bottomrule
\end{tabular}
\caption{The stamp contract as a matrix of seven interfaces and four carriers (28 cells, tri-valued
admission M = mandatory, P = permitted, F = forbidden; P is unoccupied in the existing code). The
forbidden cell of the organ line at the CEB is the static bar \enquote{the CEB carries no organ axes};
...
\texttt{kSuffixSegmentOrder} of the build steering quantities --- compiler identifier, optimization,
...
its first bearer is the load framework \texttt{load\_framework} (stamp token
...
measurement always coincide. The first switch-off stage switches off the parameter collection in macro
and micro, so that the wallclock remains in place for \texttt{measure} and \texttt{compare}; the
second, permutative switch-off stage lets the wallclock fall away as well for \texttt{release}, and with
...
mode without instrumentation (\texttt{measurement\_on} off); the two-stage switch-off as a permutation
...
\end{tikzpicture}
\caption[The two switch-off stages of the measurement]{The two switch-off stages of the measurement over the three measurement levels:
...
\begin{tikzpicture}[font=\footnotesize,
  ...
  \node[mu] (m4) at (11.55,0){\textbf{Visitor}\\{\scriptsize measurement pass-through (surface 3)}\\[2pt]{\tiny \texttt{IMessVisitor} at the \texttt{dlopen} transition: the host passes the sink, the tier calls \texttt{visit}; double dispatch}};
