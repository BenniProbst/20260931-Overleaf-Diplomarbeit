...
\end{nurlang}
\begin{nurkurz}
This chapter descends into detail: from the repository architecture with the carrier-stage chain
(Section~\ref{sec:anatomy-levels}) through the key points of PRT-ART, the axis implementation, the build
chain and the contract tests to the realm roots, the hybrid stage and the store. At its core, the
\emph{one} architecture is a self-optimizing compiler-compiler structure: the planner generates the
build orders from the user XML, the builder generates the compiled artefacts from them, and the
evaluation of the measured values feeds the next planning (Section~\ref{sec:mess-chain}) --- the line of
the empirical autotuning
systems~\cite{whaley2001atlas,frigo1998fftw,ansel2014opentuner,balaprakash2018autotuning}, except that
here the organ assignment of a living being is permuted and a separate binary is generated per
permutation.
\end{nurkurz}
...
\begin{itemize}
  ...
  \item \texttt{comdare-prt-art} --- the \textbf{candidate under test}: the hybrid \texttt{PrtArtSearchEngine} with the
  ...
  candidate-side comparison re-implementations of the rank-2/3 works (\texttt{legacy\_\allowbreak{}reimpl},
  ...
  without a memory backend of their own; which candidate building blocks reach the measurement apparatus is
...
planner $\to$ CEB $\to$ tier with the hybrid optionally inserted between the CEB probe dock and the tier
(Section~\ref{sec:anatomy-levels}): the stamp contract knows exactly four carriers, and the source tree
maps them under \texttt{libs/traeger} as four subprojects in stage form --- \texttt{comdare\_planner},
\texttt{comdare\_ceb}, \texttt{comdare\_tier\_emission} and \texttt{comdare\_hybrid} ---, where in the
build order stage $N+1$ depends only on stage $N$ (no hybrid without a tier), never the reverse, and each
stage closes onto its predecessor stage with a strict
...
its partition along the stages is planned but not implemented. The candidate under test is not a stage but the
...
\begin{tikzpicture}[font=\footnotesize,>=Stealth,
  ...
  hy/.style={st,dashed,fill=blue!3},
  ...
  \node[st] (s2) at (3.95,0){\textbf{Stage 2: CEB}\\{\tiny\texttt{comdare-messung-driver}}\\[2pt]{\tiny carries the measurement choice compiled in, permutes system $\times$ organ; one probe dock as attachment point for tier or hybrid; legend per host \texttt{tier:build-batch:\allowbreak<host>}, attestations \texttt{zelle=\allowbreak[d,e,f]\allowbreak[g,h,i]}}};
  \node[st] (s3) at (11.85,0){\textbf{Stage 3: tier}\\{\tiny\texttt{comdare\_perm\_<fp>.so}}\\[2pt]{\tiny one organ permutation per binary; seven mandatory symbols; measurement batch per host \texttt{measure:\allowbreak[a,b,c]:\allowbreak batch:\allowbreak<host>}, measurement attestations \texttt{zelle=\allowbreak[a,b,c]\allowbreak[d,e,f]\allowbreak[g,h,i]}}};
  \node[hy] (s4) at (7.9,0){\textbf{Insertion: hybrid}\\{\tiny optional, between stage 2 and 3; hybrid \texttt{.so}}\\[2pt]{\tiny mux switch at the CEB probe dock; dock slots 1 to $N$ for tier binaries; the same mandatory symbols; \texttt{genus()} reports the target family}};
  \draw[->,thick] (s1)--(s2); \draw[->,thick] (s2)--(s4); \draw[->,thick] (s4)--(s3);
  \draw[->,thick] (s2.north) -- ++(0,0.5) coordinate (bp) -| (s3.north);
  \node[font=\tiny,anchor=south,text width=11.5cm,align=center] at ([xshift=3.95cm,yshift=1pt]bp){standard route without hybrid: CEB probe dock $\to$ tier binary directly; with hybrid: probe dock $\to$ mux $\to$ dock slot $\to$ tier binary};
  ...
  \node[fl] (f3) at (11.85,-2.95){Surfaces 1--3: family interface, stamp (three lines), measurement cut-through \texttt{IMessVisitor}};
  \node[fl] (f4) at (7.9,-2.95){Surfaces 1--3 as for the tier, plus the attached stamp lines (runtime hook)};
  \draw[dashed,gray] (s1)--(f1); \draw[dashed,gray] (s2)--(f2); \draw[dashed,gray] (s3)--(f3); \draw[dashed,gray] (s4)--(f4);
\end{tikzpicture}
\caption[The carrier-stage chain in code]{The carrier-stage chain in code: four carriers, of which the hybrid is a stage
optionally inserted between the CEB probe dock and the tier binary (stamp chain in fixed order planner $\to$ CEB
$\to$ [hybrid] $\to$ tier), per stage the binary, its legend form in the emitted pipeline and the contract surfaces
expressed at it. In the stamp chain, stage $N+1$ depends only on stage $N$; the standard route leads from the CEB
probe dock directly to the tier binary, the hybrid is a mux switch at the probe dock with dock slots for tier
binaries (build dependency: no hybrid without a tier); only the measurement attestations contain all three axis
brackets.
...
instrumentation. All four build as CMake Release; alongside, the build variants debug, measurement, and release remain in force, and the debug variant is being extended further
--- a measuring debug regime, however, no longer exists, because
...
under \texttt{--debug} the conformance gate additionally runs the standard test set of the tier interface, and this indirect debug
mode coincides with the release build and is not kept separately.
Implementation status: \texttt{compare} is a selectable label with build semantics identical to
\texttt{release}; its mode-specific execution (replay comparison instead of measuring) is planned for later main waves but
...
The implementation is distributed across four Git repositories with clearly separated responsibilities:
\begin{itemize}
  \item \texttt{comdare-cache-engine} --- \textbf{how} measurement is done: the building-block library
  and the builder (code generation, permutation loop, module loader, the seven-phase
  \texttt{ExperimentDriver} and the planner \texttt{ExperimentPlanDirector},
  Section~\ref{sec:impl-pipeline}), the ABI headers under \texttt{abi/} together with the test cluster
  isolated from the build (Section~\ref{sec:axes-impl}), the 33~SOTA profiles as executable XML
  (Section~\ref{sec:sota-instances}) and the test suites; the source tree follows a Pitchfork layout,
  added to these are \texttt{libs/traeger} with the four carrier stages (below) and \texttt{heuristik/}
  with six headers (axis spline, break-even, measurement-curve loader, offline clustering, feature
  vector, optimization catalogue).
  \item \texttt{comdare-prt-art} --- the \textbf{candidate under test}: the hybrid
  \texttt{PrtArtSearchEngine} with the 1/2/N parameter rule (one parameter demands the function
  interfaces of a vector, two those of a map, more than two those of a map onto a tuple), its ABI
  adapters and building blocks as well as the re-implementations of the rank-2/3 works
  (\texttt{legacy\_\allowbreak{}reimpl}, fourteen directories); rank~1 (centrality rank,
  Section~\ref{sec:rqs}) comprises the eight independent search designs with original adapter as well as
  the works P08 and P09 that enter as organs. Pool, prefetch and layout components are descriptor
  skeletons without a memory backend of their own; which candidate building blocks reach the measurement
  apparatus is decided by the slot assignment (Section~\ref{sec:prtart-impl}).
  \item \texttt{probst-Diplomarbeit-cache-engine} --- \textbf{what} is tested by the user: the
  experiment configurations (XML together with the XSD schema), the measurement driver
  \texttt{messung\_driver} (the CEB role) and the evaluation toolchain of nine numbered programs from
  \texttt{01\_\allowbreak{}sample\_\allowbreak{}data\_\allowbreak{}generator} to
  \texttt{09\_\allowbreak{}tex\_\allowbreak{}formatter}.
  \item the \textbf{manuscript repository} --- the versioned experiment execution directly in the
  document: the generator output of the toolchain is committed in the appendix, the repository has its
  own CI, and the project repository triggers the manuscript build after a measurement run
  (\texttt{trigger:thesis}); a measured value in the document is a versioned artefact state with
  provenance.
\end{itemize}
The fourfold order of the code is not the repository boundary but the \emph{carrier-stage chain}
planner $\to$ CEB $\to$ tier with the hybrid optionally inserted between the CEB probe dock and the tier
(Section~\ref{sec:anatomy-levels}); the source tree maps it under \texttt{libs/traeger} as four
subprojects in stage form, where in the build order stage $N+1$ depends only on stage $N$ (no hybrid
without a tier). Implementation status: the subprojects form a scaffold, the implementation lies as a
monolith under \texttt{libs/\allowbreak{}cache\_\allowbreak{}engine}. The candidate under test is not a
stage but the transversal supplier of organ building blocks; the stage chain is at the same time the
layer at whose binaries the contracts are partitioned into surfaces (Section~\ref{sec:impl-pipeline});
Figure~\ref{fig:traeger-stufen} shows it with its binaries, legend forms and surfaces.
\begin{figure}[htbp]\centering
\begin{tikzpicture}[font=\footnotesize,>=Stealth,
  st/.style={draw,rounded corners,fill=blue!7,minimum width=3.75cm,minimum height=3.9cm,text width=3.55cm,align=center,inner sep=3pt},
...
\texttt{libs/cache\_engine}.}\label{fig:traeger-stufen}
\end{figure}
Across this, the terminology separates two engine terms: the \texttt{CacheEngine} is the \emph{library
of axis building blocks} (per axis the interchangeable design constituents as concept/CRTP components,
without measurement logic), the \texttt{ExecutionEngine} the \emph{execution and measurement
infrastructure} lying across all axes with the measurable root \texttt{IExecutionEngine}
(Section~\ref{sec:measurement-system}); the measurement couplings of the runtime adapter are switched
via \texttt{COMDARE\_\allowbreak{}MEASUREMENT\_\allowbreak{}ON}, and the CMake option
\texttt{COMDARE\_\allowbreak{}EXPERIMENT\_\allowbreak{}MODE} encodes the invariant that an experiment
build is always a measurement build; the separation of building blocks and tools keeps the production
path measurement-free.

The four \emph{work modes} \texttt{build}, \texttt{measure}, \texttt{compare} and \texttt{release} are
registered as a sub-axis of the measurement realm (Section~\ref{sec:anatomy-levels}): \texttt{build}
builds without measuring and may run in parallel, \texttt{measure} is the deterministic single-thread
measurement run and the only mode with measurement switched on, \texttt{compare} reads the measurement
store (implementation status: a selectable label, the replay comparison is not implemented),
\texttt{release} produces the reference binary without instrumentation; all four build as CMake
Release, and the wiring check of the former debug regime is realized as the switch \texttt{--debug} of
the planner shell --- a flag, not a state; under \texttt{--debug} the conformance gate additionally runs
the standard test set of the tier interface, an indirect debug mode that coincides with the release build.
\end{nurkurz}
...
The candidate under test contributes six key points of its own, two of them on the telemetry axis, which in the
...
retained only as a labelled anti-pattern. The candidate under test adds the DensityTracker
...
These key points are \emph{building blocks of the candidate repo}; which of them reach the measurement
apparatus is decided by the slot assignment, not by the contribution list
(Section~\ref{sec:prtart-demo}). The candidate under test provides four slot hulls: the page type of the build axis
...
measurement path is planned but not implemented (update of the candidate integration).
...
\begin{tikzpicture}[font=\footnotesize,
  ...
  \node[pa] (p6) at (10.2,-2.85){\textbf{DensityTracker $+$ H1/H2/H3}\\{\scriptsize telemetry axis (own contributions)}\\[2pt]{\tiny own measurement organs: occupancy-density tracker and the candidate's three hypothesis metrics}};
\end{tikzpicture}
\caption{Gallery of the six own key points of PRT-ART, each at its axis: a 4+1+2 pool allocator (T6), optimistic lock coupling with reserved value blocks (T8), path-oriented bundle prefetch (T7), a MultiLevel layout (T5) and cache-coherence-aware telemetry together with the DensityTracker and the hypothesis metrics H1/H2/H3 --- the telemetry axis is split in two, a sweep sub-axis of the measurement tooling and a compile-time quantity of the builder (Section~\ref{sec:axis-triad}). The tiles show the \emph{building blocks of the candidate repo}; overridden in the apparatus are the path-compression redirect (T3, engine-side reference composition), the prefetch (T7) and the ChainRef value handle (T10, slot composition of the candidate under test) as well as the build axis page type; the building blocks for T5/T6 run in the measurement path via the cache-engine default, OLC is inherited from ART, and the remaining organ axes the slot composition draws from the ART reference of the cache engine (sixteen of eighteen).}\label{fig:prtart-gallery}
\end{figure}
\end{nurlang}
\begin{nurkurz}
\section{PRT-ART in Detail}\label{sec:prtart-impl}
Building on the state of the art, this section names each key point of PRT-ART together with the
related technique from which it emerges: pool allocation with role-pure pools (Hoard, jemalloc,
mimalloc~\cite{berger2000hoard,evans2006jemalloc,leijen2019mimalloc}), optimistic lock coupling
(ART~\cite{leis2016olc}), software prefetch along the tree
paths~\cite{chen2001prefetch,zhang2025hierarchical}, cache-line-aware node layouts (ART and
FAST~\cite{leis2013art,kim2010fast}) and coherence-aware telemetry against the false-sharing
problem~\cite{torrellas1994falsesharing,kuehn2023bplustree}; the DensityTracker and the hypothesis
metrics H1/H2/H3 are own contributions without an external template.

The candidate under test contributes six key points of its own, two of them on the two-way-split
telemetry axis (sweep sub-axis of the measurement tooling in the planner, compile-time quantity in the
builder, Section~\ref{sec:axis-triad}): a \emph{4+1+2 pool allocator} (four page pools with the roles
trie hull, dense pages, MultiLevel and decision span, one rest pool, two value pools) as a
cache-line-aligned, low-fragmentation free list; \emph{optimistic lock coupling (OLC) with reserved
value blocks} via a concurrency manager (OLC, HTM, STM, lock-free, RCU, hazard
pointers~\cite{michael2004hazard}); a \emph{path-oriented bundle prefetch} of the hot search paths; a
\emph{MultiLevel layout}; coherence-aware telemetry --- per-node counters cause cache-line ping-pong
and falsify the measurement they collect --- as \emph{leaf-only counters} (main variant),
\emph{sampling} and \emph{offline recompute}, the per-node counter is retained only as a labelled
anti-pattern; and the DensityTracker together with the metrics H1/H2/H3 as telemetry organs of its own.
In the engine source tree, the telemetry building blocks (TM1--TM3) and the ISA sub-axes (IS1--IS3)
lie under the organ axes, while telemetry regime and target ISA are carried in the system registry
(Section~\ref{sec:impl-pipeline}) --- a double location in the code, not an assignment question of the
model.

These key points are \emph{building blocks of the candidate repo}; which of them reach the measurement
apparatus is decided by the slot assignment (Section~\ref{sec:prtart-demo}). The candidate under test
provides four slot hulls: the page type of the build axis (\texttt{PrtArtBPlusPageType}), the prefetch
(\texttt{PrtArtRedirectPrefetch}, T7), the ChainRef value handle (\texttt{PrtArtChainRefHandle}, T10)
and a telemetry wrapper with the per-node counter as comparison baseline. Its slot composition
\texttt{PrtArtCompositionDemo} takes over sixteen of the eighteen organs from the ART reference
composition of the cache engine and replaces prefetch and value handle; the engine-side
\texttt{PrtArtComposition} differs from ART only in the path-compression redirect (T3, the \emph{R} in
PRT-ART); pool allocator (T6) and MultiLevel layout (T5) run in the measurement path via the
cache-engine default, their binding is envisioned but not implemented. The remaining two of the eight
building-block layers from the contribution list (Section~\ref{sec:contributions}), value handles and
signalling-bit serialization, provide the value-handle slot (T10) or are catalogued as a building block
of the serialization axis (T9) in Appendix~\ref{app:blocks}.
\end{nurkurz}
...
safe fallback chosen per concept: allocator adapters replace the missing vendor in \texttt{if constexpr}
...
build remains independently runnable. The organ axes not occupied by the candidate under test draw their building
...
\section{Axis Implementation and Measurement-System Levels}\label{sec:axes-impl}
Each axis is implemented as a generalized interface; alongside lie the levels of the measurement system
up to the seven-phase pipeline, the level picture E4--E0 in code (Figure~\ref{fig:impl-layers}) and the
contract surfaces; the pattern register follows at the end of the chapter (Figure~\ref{fig:muster}).
Each axis is a topic directory with a two-layer \emph{concept}~\cite{sutton2012concepts} and a
\texttt{requires}-secured CRTP base class. The live path of building-block selection is monomorphic:
from the type list of all building blocks (\texttt{All*}) a compile-time filter over the enable flag
(\texttt{mp\_filter}, Boost.Mp11~\cite{dimov_mp11}) forms the \texttt{Enabled*} list, and a composition
assigns exactly one type per slot; the hot path contains neither \texttt{virtual} calls nor runtime
switches --- an unused abstraction costs nothing~\cite{stroustrup2012foundations}. SOTA designs and
allocators are integrated via thin \emph{adapters}: vendor shims switch the building block off when the
vendor is missing instead of breaking the build, with a fallback chosen per concept
(\texttt{portable\_aligned\_alloc}; \texttt{StdMalloc} is a building block of its own, not a makeshift),
and a stub that throws a \texttt{runtime\_error} at runtime does not occur in the organ axes. The organ
axes not occupied by the candidate under test draw their building block from the cache engine
(Section~\ref{sec:prtart-impl}); hierarchical prefetching~\cite{zhang2025hierarchical} brings along the
call-graph analyser \texttt{p27\_bundle\_finder} at compile time.
\end{nurkurz}
...
\begin{nurkurz}
A design decision delimits this live path from a rejected alternative: a variant-based building-block
resolution (\texttt{resolve\_baustein} over \texttt{baustein\_variants}) is expressly forbidden for
static organ axes and exists only as a test cluster isolated from the build. The reason is the parsimony
of the compiled artefact: a tier binary maps exactly the one permutation being run;
\texttt{std::variant}, \texttt{virtual} and runtime switches are forbidden in tier and hybrid binaries
--- whoever needs another permutation needs another compiled artefact. The one permitted variant site
of the house is the dock array of the hybrid stage (\texttt{HybridDockVariant},
Section~\ref{sec:impl-hybrid-lager}); besides this, the code tolerates \texttt{std::variant} in two
axis-foreign building blocks (\texttt{axis\_error}, \texttt{pressure\_state}) and in the three
expected/result seams of the hardware probes (NUMA pages, NUMA core binding, operating system). The ban
is thus stricter than the mere avoidance of virtual calls~\cite{alexandrescu2001modern}: it forbids
every form of runtime branching between building blocks.
\end{nurkurz}
...
\begin{nurkurz}
Within each axis category there are main axes, sub-axes and meta-meta axes. In code, the category is
the trichotomy \texttt{AxisCategory} with the fixed order mess $<$ system $<$ organ (a
\texttt{static\_assert} holds it against silent re-sorting), the family the discriminator
\texttt{AxisKind} with six values; a sub-axis is itself a full axis and can at the same time be a
permutation axis. The inventory per category: the organ realm comprises eighteen main axes, twenty
sub-axis headers and the organ meta-meta \texttt{disk\_io}; the system realm three main axes, the
complex axis \texttt{build\_target\_complex} with the sub-axis group \texttt{build\_toolchain}
(compiler, optimization level, atomics), \texttt{target\_isa\_complex} (three members), the scheduling
with five dimensions and the system meta-metas under the hub of the extension hardware; the measurement
realm the tooling main axis (wallclock/macro/micro), four registered sub-axes (sixteen categories, work
mode, measurement framework, write-back method) and the measurement meta-meta \texttt{load\_framework}.
The designation meta-meta follows the four-level meta-level architecture of the Object Management Group
(MOF)~\cite{omg2016mof}; the reading M0--M3 for this apparatus is developed in
Section~\ref{sec:impl-system-axes}.
\end{nurkurz}
...
composition chooses exactly one per slot; their number is, as \texttt{kOrganAxisCount} $=$ 18, the
single source of the organ line, and their names and slot order are carried by the array
\texttt{kCompositionAxisNames}; the complete building-block matrix with all origin sources is given in
Appendix~\ref{app:blocks}, the SOTA contributions per axis in Table~\ref{tab:axes-overview}.
Next to the eighteen canonical main axes stands the organ meta-meta \texttt{disk\_io}, which never
touches the \texttt{binary\_id} (Section~\ref{sec:axis-triad}); a nineteenth, optional organ axis is
planned but not implemented; only its implementation introduces it as slot T18. That no composition
omits an organ is enforced by the \texttt{IsComposition} concept: it demands per algorithm the
assignment of exactly these eighteen organs, so that no composition compiles that leaves an axis open.
\end{nurkurz}
...
\texttt{lifecycle\_state} as reports. Beneath it lie, on level~1, the four concepts Map, Container,
Graph and HeuristikAdapter (Section~\ref{sec:anatomy-levels}) --- Graph as a declared stub without a
family --- and, on level~2, the six families SearchAlgorithm, Set, Sequence, Adapter, View and
FunctionInterfaceReroute; in code the concept is the derivation of the family (\texttt{gattung\_of}), not
...
\begin{nurkurz}
\subsection{The One Architecture in Code}\label{sec:impl-architecture}
The \emph{one} architecture from Section~\ref{sec:anatomy-model} is, in code, a single hierarchy. Its
root is \texttt{IExecutionEngine} with three engine kinds --- Anatomy, Virus, Hybrid --- and a contract
of seven methods (\texttt{warm\_up}, \texttt{run}, \texttt{reset} and \texttt{shutdown} as measurement
lifecycle as well as \texttt{engine\_name}, \texttt{engine\_kind} and \texttt{lifecycle\_state} as
reports). Beneath it lie the four concepts and the six families (Section~\ref{sec:anatomy-levels}); in
code the concept is the derivation of the family (\texttt{gattung\_of}), not a second label. The living
being \texttt{SearchAlgorithm} and its ABI runtime view --- the \enquote{SearchEngine} --- are two
views of the same entity across the module boundary: the body is \texttt{SearchAlgorithmAnatomy<C>},
the composition and wiring of the eighteen organs, and the view is
\texttt{SearchAlgorithmAbiAdapter<A>}. The layering
\texttt{CacheEngine}\,$\to$\,\texttt{ExecutionEngine}\,$\to$\,\texttt{SearchEngine}
(Section~\ref{sec:repos}) separates roles, not class trees; the library skeleton of the same name,
\texttt{libs/search\_engine}, contains no code. Axis-less \emph{viruses} stand as siblings beneath the
same root but possess no anatomy (Figure~\ref{fig:one-architecture}). In this hierarchy the
compiler-compiler character of the apparatus is anchored: every composition is a type from which the
builder generates a separate compiled artefact per permutation.
\end{nurkurz}
...
\begin{figure}[!htbp]
  ...
  \begin{tikzpicture}[box/.style={draw, rounded corners, align=center, font=\footnotesize, inner sep=3pt, text width=3.0cm},
    ...
    \node[box] (ana) at (-4.4,-2.0) {\texttt{IAnatomyBase}\\{\scriptsize living being (four concepts, level~1)}};
    \node[box] (vir) at (3.4,-2.0) {\texttt{IVirus\-Execution\-Engine}\\{\scriptsize virus (axis-less, yet measurable; e.g.\ graph BFS)}};
    \node[box,text width=3.2cm] (g1) at (-7.5,-3.9) {Map concept\\{\tiny family \texttt{SearchAlgorithm} (mammal)}};
    \node[box,text width=3.2cm] (g2) at (-3.95,-3.9) {Container concept\\{\tiny families Set, Sequence, Adapter, View}};
    \node[box,text width=3.2cm,dashed] (g3) at (-0.4,-3.9) {Graph concept\\{\tiny declared stub, no family}};
    \node[box,text width=3.2cm] (g4) at (3.15,-3.9) {HeuristikAdapter concept\\{\tiny family \texttt{FunctionInterfaceReroute}}};
    ...
    \node[gen,text width=3.1cm] (n6) at (4.1,-5.7) {\texttt{FunctionInterfaceReroute}\\reroute family};
    ...
    \node[box,fill=orange!8,text width=3.4cm] (comp) at (-6.6,-9.7) {concrete composition (e.g.\ \texttt{ArtComposition})\\{\scriptsize level 3: genus (e.g.\ ART) $=$ assignment of \emph{all} 18 organ axes (level 4)}};
    \node[box,fill=orange!16,text width=3.4cm] (bin) at (-6.6,-12.0) {tier binary \texttt{comdare\_perm\_\-<fp>.so}\\{\scriptsize level 5a: compiled per permutation $+$ system axes}};
    \draw[->] (root) -- (ana);
    \draw[->] (root) -- (vir);
  ...
  Anatomy, Virus and Hybrid; beneath it branch the anatomy-bearing \emph{concepts} (level~1: Map,
  Container, Graph, HeuristikAdapter) with their \emph{families} (level~2) and the axis-less
  \emph{viruses}. \texttt{SearchAlgorithm} \emph{is} the focal living being (elephant); its anatomy is its
  \emph{body} --- the fixed composition \emph{and wiring} of the 18 organs ---, and
  \enquote{SearchEngine} only its ABI runtime view. The elephant branch (family \texttt{SearchAlgorithm}) is drawn out down to the
  individual: a genus (level~3) with its concrete axis composition (level~4) is compiled per permutation
  as a standalone tier binary (level~5a, the individual). Container possesses an organ set of its own with the families Set, Sequence, Adapter
  and View, whose arities are carried by the family build admission; Graph is declared and possesses no
  family; the concept HeuristikAdapter carries the reroute family of the hybrid stage.}\label{fig:one-architecture}
\end{figure}
...
\texttt{comdare\_\allowbreak{}anatomy\_\allowbreak{}genus}, spelled identically across all concepts, as
well as the version lines of the stamp. The identical spelling is a condition, not a convenience: the
concept-independent loader loads every binary via the same loading route (POSIX
\texttt{dlopen}~\cite{posix_dlopen}) and can determine the family \emph{before} it constructs an
instance --- concept-independent names with concept-specific values; the concept is here derived from the
family. The ABI carries major version~9, whose seam is the family interface. The variadic 1/2/N hybrid
API belongs to the candidate under test: its \texttt{PrtArtSearchEngine<Ts\ldots>} specializes by parameter count ---
...
the families of the cache engine (eighteen for the family SearchAlgorithm, each their own for Set,
Sequence, Adapter and View from the family build admission) are to be read separately from it. Complex
...
of fixed length, which the candidate under test uses as the key type of its memory model (\texttt{BinaryKey}); this key
...
\end{nurlang}
\begin{nurkurz}
Concretely, the module boundary is a C-stable interface --- the ABI of the apparatus --- that exports
exactly one living being per permutation: every tier binary contains seven mandatory symbols with C
linkage --- the four primal symbols ABI version, magic number, factory
\texttt{comdare\_\allowbreak{}create\_\allowbreak{}anatomy} and destroy, plus the two reports
\texttt{comdare\_\allowbreak{}anatomy\_\allowbreak{}gattung} and
\texttt{comdare\_\allowbreak{}anatomy\_\allowbreak{}genus}, spelled identically across all concepts, as
well as the version lines of the stamp. The identical spelling is a condition: the concept-independent
loader loads every binary via the same loading route (POSIX \texttt{dlopen}~\cite{posix_dlopen}) and
determines the concept from the family \emph{before} it constructs an instance. The ABI carries major
version~9, whose seam is the family interface. The variadic 1/2/N hybrid API belongs to the candidate
under test: \texttt{PrtArtSearchEngine<Ts\ldots>} specializes by parameter count (\texttt{std::vector},
\texttt{std::map}, \texttt{std::map} onto a tuple). Complex keys are reduced by a fingerprint component
(\texttt{fixed\_\allowbreak{}length\_\allowbreak{}fingerprint}, FNV-1a base) to 16-byte binary strings,
which the candidate under test uses as its key type (\texttt{BinaryKey}); this key fingerprint is to be
distinguished from the three identity fingerprints of the apparatus ---
\texttt{permutation\_\allowbreak{}fingerprint} in the ABI POD, SHA-512 tier fingerprint, SHA-256
planner fingerprint (Section~\ref{sec:stamp-model}).
\end{nurkurz}
...
// Candidate view: variadic 1/2/N API (key K, values V)
...
\end{lstlisting}
  ...
  and reports its target family via \texttt{genus}) and the variadic 1/2/N API as the view of the candidate under test.
...
\begin{tikzpicture}[font=\footnotesize,>=Stealth,align=center,
  ...
  \node[comp] (gatt) at (-4.9,2.75){Concept (4, level 1)};
  \node[comp] (family) at (-4.9,1.75){Family (6, level 2)};
  ...
  \draw[->] (anat)--(gatt); \draw[->] (gatt)--(family); \draw[->] (family)--(body); \draw[->] (body)--(abi); \draw[->] (abi)--(drive); \draw[->] (drive)--(mess);
...
family interface of the tier binary (runtime, abstract factory), surface~2 the stamp (compile time,
ABI-stable) and surface~3 the measurement cut-through \texttt{IMessVisitor} --- a visitor at the
\texttt{dlopen} transition with double dispatch, which exists \emph{so that} concept and family interfaces
...
\toprule
Interface & Content & Planner & CEB & Hybrid & Tier \\
\midrule
...
\texttt{Angeschlossene} & attached stamp lines (runtime hook)               & F & F & M & F \\
\bottomrule
\end{tabular}
...
the cells are checked per heir via \texttt{static\_assert}. The column order follows, like
Figure~\ref{fig:traeger-flaechen}, the processing topology with the hybrid tier binary as the optional
insertion between CEB and tier binary.}\label{tab:stempel-matrix}
\end{table}
\end{nurlang}
\begin{nurkurz}
\subsection{From Profile to Tier Binary: Planner, Builder and Measurement Pipeline}\label{sec:impl-pipeline}
The chain from the user XML to the tier binary (Section~\ref{sec:mess-chain}) has, in code, the shape
of two binaries. Stage~1, the planner role, is \texttt{comdare-experiment-planner} with the subcommands
\texttt{validate}, \texttt{plan dump|ci|cmake}, \texttt{cache-key}, \texttt{fingerprint},
\texttt{status}, \texttt{check-size}, \texttt{simulate}, \texttt{version} and \texttt{help}:
\texttt{validate} checks, purely read-only, against the compiled-in registry catalogues, \texttt{plan}
drives the deterministic, byte-identically repeatable walk of the \texttt{ExperimentPlanDirector}
(\emph{Director} and \emph{Builder}~\cite{gamma1994patterns}: one walk, one builder per output syntax),
\texttt{check-size} and \texttt{simulate} compute the measurement set before the run
(Section~\ref{sec:impl-contracts}). Stage~2, the CEB role, is the measurement driver
\texttt{comdare-messung-driver} (\texttt{tier ci|cmake}, \texttt{run}, \texttt{version},
\texttt{help}): it emits the tier build orders of its build space and drives the measurement run; the
planner steers the CEB orders, the CEB the tier orders --- two modules, not merely surface. The legends
of the emitted attestations have three forms: \texttt{ceb:build:[a,b,c]} on stage~1, per host a build
and check batch \texttt{tier:build-batch:<host>} with the attestations \texttt{zelle=[d,e,f][g,h,i]}
(the milestone batches in Section~\ref{sec:fairness}) and per host a measurement batch
\texttt{measure:[a,b,c]:batch:<host>}, whose measurement attestations alone contain all three brackets
(Figure~\ref{fig:traeger-stufen}).

The installation has two control paths, the planner directly via shell and an API command line with a
headless service (a cell on Kubernetes or on bare metal), plus a command-line-guided creation mode for
the user XML with hardware filter; the shell path is implemented, service and creation mode are planned
but not implemented. Two properties secure the \emph{one} main channel: the text plan reports per build
step the actually generated file stem (\texttt{built\_stem}), produced by the same function that the
build orchestrator calls; and there is \emph{one} XML source and \emph{one} channel that builds it ---
variant formation lies in the C++23 metaprogramming, CMake is confined to building.

Inside the CEB, the builder transforms each profile into a precompiled C++23 module. Each permutation
is identified by a bit-packed \emph{permutation flag} structure with ten banks of 64~bits each, a
constraint filter (\texttt{is\_valid\_combination}) excludes incoherent combinations; the measurement
path identifies the eighteen organ axes via the eighteen-segment path (the \texttt{binary\_id},
Section~\ref{sec:stamp-model}) together with the three fingerprints, and the permutation count follows
a mixed-radix encoding --- the product space of the variability models of software product
lines~\cite{pohl2005spl}. Over this runs the \emph{seven-phase measurement pipeline}
(Figure~\ref{fig:pipeline}) of the \texttt{ExperimentDriver}: (1)~\textsc{Enumerate},
(2)~\textsc{Generate}, (3)~\textsc{Compile}, (4)~\textsc{Link+Load}, (5)~\textsc{Execute},
(6)~\textsc{Measure} and (7)~\textsc{Persist}; two opt-in phases compile missing modules and run a
functional test per module with ABI version check, and the measurement-series modes \texttt{Defined},
\texttt{Full} and \texttt{FullSampled} (Section~\ref{sec:mess-chain}) choose which permutations a
series comprises. The downstream evaluation toolchain is a sequence of nine numbered programs from
\texttt{sample\_data\_generator} to \texttt{tex\_formatter}; the xlsx workbook is the output, the CSV
form its tree extract generated from the workbook (Section~\ref{sec:eval-dataflow}); implemented is one
flat file. The measurement couplings are separated from the production path by the
\texttt{COMDARE\_\allowbreak{}MEASUREMENT\_\allowbreak{}ON} gating (Section~\ref{sec:repos}), and
continuous integration~\cite{fowler2006ci} runs across all four repositories.
\begin{figure}[htbp]\centering
% No \resizebox: the drawing is only ~13.5cm wide; blowing it up to \linewidth (x1.19) would inflate
% the \footnotesize font beyond the body text. Rule: resizebox only to SHRINK (cf. fig:m-model).
\begin{tikzpicture}[font=\footnotesize,>=Stealth,align=center,
...
The levels E4--E0 from Section~\ref{sec:anatomy-layers} each have a place in code, and the boundaries
between them each have a contract test (Section~\ref{sec:impl-contracts});
Figure~\ref{fig:impl-layers} shows this code shape: E4 is the XML profile with the planner and the
evaluation programs 04 (\texttt{csv\_to\_latex}) and 08 (\texttt{appendix\_generator}), E3 the
experiment tree (\texttt{experiment\_tree}) with the \texttt{binary\_id} as path, E2 the ABI adapter
with the module ABI declaration, E1 the interface \texttt{IResourceControllableTier} with the dynamic
dimensions of the measurement registry XML, E0 are CI, runners and store, and the cross-cutting
concern~M is the system-axis root \texttt{SystemAxis} (Section~\ref{sec:impl-system-axes}).
\begin{figure}[htbp]\centering
\begin{tikzpicture}[font=\footnotesize,>=Stealth,
  eb/.style={draw,rounded corners,fill=blue!7,minimum width=11.4cm,minimum height=0.95cm,text width=11.1cm,align=left,inner sep=3pt},
  ct/.style={font=\tiny,fill=white,inner sep=1pt}]
...
The contracts between the carrier stages are partitioned into surfaces at the binaries. The
three-surface model (Section~\ref{sec:anatomy-levels}) is present in code by name: surface~1 is the
family interface of the tier binary, surface~2 the stamp and surface~3 the measurement cut-through
\texttt{IMessVisitor} --- a visitor at the \texttt{dlopen} transition, which exists \emph{so that}
concept and family interfaces remain unchanged. The stamp contract is executed as a matrix of seven
interfaces and four carriers --- 28~cells with three-valued admission (Table~\ref{tab:stempel-matrix});
the absence of the organ line at the CEB is the static barrier \enquote{the CEB carries no organ
axes}, and a \texttt{static\_assert(StempelVertrag<Erbin, Traeger>)} beneath every heir turns a
missing mandatory or a forbidden interface into a compilation error --- a form of consumer-driven
contract~\cite{robinson2006cdc}.
\begin{table}[htbp]\centering\footnotesize
\begin{tabular}{@{}llcccc@{}}
\toprule
Interface & Content & Planner & CEB & Hybrid & Tier \\
...
which checks every imported binary against the fixed contract before the measurement (under
\texttt{--debug} additionally against the standard test set of the tier interface). For the tier
...
seventeen axes with two representatives each, the persistence target pinned to one; a simplifying,
superseded assumption, since the axes carry several configurations, the catalogue is the regression
detector, not an upper bound --- and is confirmed
...
\begin{nurkurz}
\section{Quality Assurance: Contract Tests per Layer}\label{sec:impl-contracts}
The layer model E4--E0 with its interface contracts was developed as a concept in
Section~\ref{sec:anatomy-layers}; this section describes its implementation as test infrastructure.
Changes that alter several layers at once produce errors from interactions that neither local tests nor
review make visible (ADR-5, Appendix~\ref{app:adr}); because the product space rules out any complete
materialization (Section~\ref{sec:mess-chain}), each layer is completed individually --- with a fixed
interface contract to the neighbouring level, its own test harness and its own definition of
acceptance.

The carrying test principle reads: \emph{each layer is tested against a compile-time substitute of its
neighbouring level.} The neighbouring level is replaced in each case by a \emph{fake} (a template- and
concept-based stand-in in the test scope, no runtime switch in production code), and the test question
of every boundary, whether the layer passes its definition on \emph{without loss}, becomes an
executable acceptance test.

Per layer (the harness names are shown in Figure~\ref{fig:impl-layers}): for E4, golden fixtures
compare the axis levels emitted from a profile against a reference, a fake E3 replaces the permutation
tree; for E3 the bijection of the binary identity (mixed-radix back and forth) is run against a fake
E2, and in addition a conformance gate checks every imported binary before the measurement (under
\texttt{--debug} also against the standard test set of the tier interface); for E2 the
contract is the ABI itself: the Wormhole reference DLL must satisfy \texttt{IAnatomyBase},
\texttt{IObservableTier} and \texttt{IResourceControllableTier}, and the response to a control-quantity
request (\texttt{apply}) is the \emph{number of axes that actually accepted the value} --- nothing is
accepted silently; for E1, edge tests check in-process the corner cases of the dynamic regime (reset of
a reused tier, pool budget below the live data set as a named state, every control quantity as a named
column of the result file, preservation of the two-phase operation loop). Persistence is checked at the
result file; the lead is the standard xlsx, the CSV form its tree extract. Implementation status: the
E1 check reads the CSV output, which the seam writes as one flat file; the check at the xlsx and the
tree form are planned but not implemented.

Above the layer harnesses, a \emph{two-gate protocol} secures every implementation package (continuous
integration and delivery as safeguarding stages~\cite{fowler2006ci,humble2010cd}).
\emph{Gate~1} is
the strict gate of the engine repository: the full test suite runs in four cells, gcc and clang each as
release and debug build, without change skip and without an exception list; the double run forces every
static check through two translators and makes the run-to-run stability of the number itself the
object of testing. Part of gate~1 is the golden anchoring of the permutation space
(Section~\ref{sec:mess-chain}): the \emph{reference catalogue} holds $2^{17} = 131072$ binary
identities (assumption of two configurations per axis, superseded; regression detector, not an upper bound)
and is confirmed against a fixed CRC-64 checksum (\texttt{0x56F1B721C72DC10E}), a % chktex 29
small catalogue of $320$ identities serves as a byte-exact check of the serialization form --- a
characterization test in the sense of Feathers~\cite{feathers2004legacy} ---, and the
\emph{cardinality} of the space planned in a measurement series is different from this and never
static: the planner counts it in the director walk from the XML (\texttt{check-size},
\texttt{simulate}), never as a constant.
\emph{Gate~2} is the integration gate of the project
repository: its pipeline builds without error and without an exception set; the two declared failure
tolerances (an arm64 smoke build without a runner, a manual relock maintenance job) are a platform
boundary, not a test set.
...
Completion follows the layering top-down --- E4, E3, E2, then the measurement cross-cutting concern~M
before E1, the infrastructure~E0 as the last step (Section~\ref{sec:anatomy-layers}). All four
boundaries possess their harness, and the cross-cutting concern~M possesses one of its own without any
tier binary: a synthetic measurement source is driven through the system-axis root and traced into the
result columns. Implementation status: the hardware acquisition chain runs at the release seam of the
builder (Section~\ref{sec:impl-system-axes}); the CEB-driven release path of the measurement source is
planned but not implemented. The code shape of the cross-cutting concern is described in the following
section.
\end{nurkurz}
...
\section{Realm Roots in Code: System and Measurement Axes}\label{sec:impl-system-axes}
Four abstraction levels order the notions of this section (Figure~\ref{fig:m0-m3}), following the
four-level meta-level architecture of the Object Management Group (MOF)~\cite{omg2016mof}: M3 are the
\emph{meta-meta axes} in the sense of the indirection of a description --- axes that describe how other
axes are described (\texttt{load\_framework}, \texttt{disk\_io}, the system meta-metas) ---, M2 the
registry line with category, family and building-block list, M1 the CRTP axis with its concept and M0
the binary code of the tier binary with exactly one compiled-in assignment. In the stamp, the meta-meta
level of an entry is mechanically its bracket depth (bits~3--5; a main axis is bracket-less, a
meta-meta hangs as a bracket appendage on its line, a depth above seven aborts), and the short form
\texttt{c\{p.e\}} of a version flag denotes \texttt{c} with the sub-members \texttt{p} and \texttt{e},
never a flat abbreviation. Per realm there is a dedicated meta-meta discriminator; PMC acquisition
(performance monitoring counters, the hardware counters of the CPU) is a meta-meta measurement axis
because it describes with which counter instrument other measurements are collected.
\begin{figure}[htbp]\centering
\begin{tikzpicture}[font=\footnotesize,>=Stealth,
  mb/.style={draw,rounded corners,minimum width=13.6cm,minimum height=1.05cm,text width=13.3cm,align=left,inner sep=3pt}]
  \node[mb,fill=orange!14] (m3) at (0,0){\textbf{M3 meta-meta axes} --- indirection of a description\\{\scriptsize examples: \texttt{load\_framework} (measurement realm, token \texttt{ycsb@1.0.0.c}), \texttt{disk\_io} (organ realm, bracket appendage), system meta-metas under \texttt{external\_utils}}};
...
\begin{nurkurz}
The tripartition of the axis realms (Section~\ref{sec:axis-triad}) is executed in code as a type
separation. All axes share the family roof \texttt{topics::Axis<Derived>}, an empty, non-polymorphic
CRTP base whose concept check rejects any smuggled-in vtable already at compile time, and declare
category and family via \texttt{AxisCategory} and \texttt{AxisKind} (Section~\ref{sec:axes-impl}). Per
family there is a dedicated CRTP root: \texttt{OrganAxis} and \texttt{OrganMetaMetaAxis} for the organ
realm, \texttt{SystemAxis} for the host-side cross-cutting concern, \texttt{CebSystemAxis} for the
build and steering axes of the CEB, \texttt{SystemMetaMetaAxis} for the system meta axes and
\texttt{MeasurementMetaMetaAxis} for the measurement-realm axes of the planner. An axis \emph{is} its
family through its base class, not through a re-attachable label: the system realm roots at the CEB,
the measurement realm at the planner (Section~\ref{sec:axis-triad}).
\end{nurkurz}
...
\begin{nurkurz}
On the system side, the canonical order itself is code: the system main axes \texttt{target\_isa},
\texttt{operating\_system} and \texttt{external\_utils} are held in their binding sequence by the constant
three-element array \texttt{kSystemAxisOrder}, and every consumer that needs an order reads it there
instead of re-deriving it (order single source with drift checks). Three compile-time
\emph{re-entry guards} fix the cut of the main axes: \texttt{static\_assert} checks reject compiler,
scheduling and load framework as system main axes --- the compiler is a sub-axis group
\texttt{build\_toolchain} of the complex wrapper \texttt{build\_target\_complex}, the scheduling a
sub-axis at \texttt{target\_isa} with five dimensions, the load framework a meta-meta main axis of the
measurement realm ---, and whoever re-enters one of the three breaks the build and must revoke the
assignment explicitly. The build version identifier of the system line (Section~\ref{sec:stamp-model})
likewise draws from a single declarative source, the ordered eight-segment sequence
\texttt{kSuffixSegmentOrder} (compiler identifier, optimization, SIMD extension, CEB, target ISA,
telemetry, build type, gate contribution), in which an empty member produces no segment and the gate
segment stands at the end (prefix stability).
\end{nurkurz}
...
\begin{nurkurz}
On the measurement side, the meta-meta root \texttt{MeasurementMetaMetaAxis} is the planner's own;
its first carrier is the load framework \texttt{load\_framework} (stamp token \texttt{ycsb@1.0.0.c}),
and like all measurement-realm axes it is strictly binary-id-neutral (\texttt{binary\_id} $=$
\texttt{never}). The measurement-tooling \emph{main} axis holds exactly three versioned entries in the
only permitted order wallclock/macro/micro (alias w/ma/mi, Section~\ref{sec:measurement-system}); a
fourth tooling is a compilation error. It fans out four sub-axis registries: the work mode
(\texttt{WorkMode}: build, measure, compare, release, Section~\ref{sec:repos}), the measurement
framework (\texttt{MeasurementFramework}: exactly one entry, YCSB), the write-back method
(\texttt{WritebackMethod}: CSV, LaTeX table, comparison metrics and xlsx as default, expressly no PDF)
and the sixteen measurement categories (below); each registry is a \texttt{constexpr} catalogue with
the check index $=$ enum value, a further entry breaks the compilation. Two gates mirror the tooling
ladder: the \emph{statistics gate} switches the observer statistics of the macro regime, the
\emph{segment-timing gate} the eighteen per-axis segment timers; an unset segment-timing gate
\emph{inherits} the state of the statistics gate, a set one without the statistics gate breaks the
compilation. The actually effective gate state of every translation unit travels as a measurement-gate
member with nine \texttt{;}-separated fields into the fingerprint preimage of the stamp
(Section~\ref{sec:stamp-model}) --- in the off state \texttt{mg=m0;s0;st0;x0;tw0;tm0;tmi0;hm0;hmi0}:
measurement, statistics, segment timing, experiment mode, then wallclock, macro and micro of the tier
stage and macro and micro of the hybrid stage ---; the stamp never claims an outfit that the build does
not carry. The target picture next to this registry state is the axis-interface model from
Section~\ref{sec:mess-chain}.
\end{nurkurz}
...
\begin{tikzpicture}[font=\footnotesize,>=Stealth,
  ...
  \node[ze,fill=orange!14] at (4.3,0){time $+$ parameters per family-interface function};
...
Switching off the measurement is a staged model over the three measurement levels
(Figure~\ref{fig:abschaltungsstufen}). The measuring instrument \texttt{checkpoint\_measure} always
collects the input parameters at a \emph{point in time}; the wallclock is the minimum of a
time-measuring, otherwise parameter-empty checkpoint and thus the minimal measuring instrument of the
CEB\@. The first shut-off level switches off the parameter collection in macro and micro, so that the
wallclock remains in place for \texttt{measure} and \texttt{compare}; the second, permutative shut-off
level lets the wallclock together with the surfaces and interfaces of the measurement fall away as
well for \texttt{release}. Implementation status: the code contains the gates G2 and G3 and the release
mode without instrumentation (\texttt{measurement\_on} off); the two-level shut-off as a permutation
quantity is planned but not implemented.
\begin{figure}[htbp]\centering
\begin{tikzpicture}[font=\footnotesize,>=Stealth,
...
\begin{nurkurz}
The PMC tooling is multi-layered because the hardware counters differ per core type and vendor (access
via \texttt{perf\_event\_open}~\cite{linux_perf_event_open}). A generic event list
(\texttt{pmc\_event\_set}) decides whether PMC is built in: the planner's host probe checks exactly the
events that the real source opens later. A model-bound RAW catalogue fills the channels for L2 misses
and coherence per microarchitecture: for AMD Zen~5 (prod1) the assignment is cross-checked against the
named \texttt{perf} events with identical counter readings (17\,314\,023 against 17\,314\,023) ---
\texttt{cache\_misses\_l2} captures there the demand misses without L2 prefetch,
\texttt{coherence\_invalidations} the cache-to-cache fills from a foreign CCX (core complex, a core
cluster with its own L3) ---; for Intel no entry exists, because a RAW assignment without
cross-checking would be a guess~\cite{intel_sdm} --- a declared gap. A vendor registry carries the PMC
as \emph{one} feature with two values, AMD and Intel, with a version identifier each, because the
built-in counter form co-determines the fingerprint of the CEB\@; the CI runs two counter paths,
\texttt{pmc:amd} on prod1 and \texttt{pmc:intel} on prod2 (Intel Alder Lake with P- and E-cores). The
separate measurement of the P- and E-cores is laid out as a scheduling dimension (six-mode model,
Section~\ref{sec:axis-triad}); the runtime pinning per core type is planned but not implemented.
\end{nurkurz}
...
which permeates all layers without ever touching the binary identity (Section~\ref{sec:axis-triad})
--- roots in the CRTP base \texttt{SystemAxis} (file \texttt{system\_axis.hpp}), which docks only
reading onto the host-side result structures: an empty, non-polymorphic zero-cost abstraction whose
conformance a dedicated concept enforces. Two \texttt{consteval} gates translate the concept
invariants: each of the sixteen measurement categories is assigned to exactly one of the two regimes
\texttt{TimeObserver} and \texttt{PmcCounter} (nine time- and observer-based, seven counter-based PMC
categories), and every axis that mixes categories of both regimes is rejected --- the regime-mixing
ban thus breaks the compilation rather than only the evaluation.
\end{nurkurz}
...
\begin{nurkurz}
The sixteen measurement categories are carried by a single \texttt{constexpr} registry
(\texttt{measurement\_\allowbreak{}axis\_\allowbreak{}registry.hpp}) with a static gate over index,
names and regime --- a seventeenth category or a silent index drift thereby becomes a compilation
error. Conceptually the categories are a \emph{sub}-axis of the measurement tooling and manifest as
columns of the result tables (Section~\ref{sec:axis-triad}); the registry names are at the same time
the reporting vocabulary of the evaluation layer (the column names of the E4 reports).
\end{nurkurz}
...
instructions, L1/L2/L3 cache, dTLB and branch misses, memory stalls, coherence invalidations, energy)
it can express at all --- and delivers per measurement interval a \texttt{MeasuredDelta} with
\emph{per-event validity}: zero with the validity flag set is an actual zero measurement, an unset flag
means \emph{not measured}. As a guaranteed, expressly \emph{degraded} fallback there is the wall-clock
source (\texttt{WallClockSource}); the existing PMC family is attached by the adapter
\texttt{PmcSourceAdapter}, which advertises only the seven structurally expressible channels, while
cycles, instructions and memory stalls remain invalid in the counter POD\@.
\end{nurkurz}
...
axis $\to$ family $\to$ category (hierarchy depth three) to hundreds of measurement categories, one per
...
function behind the family interface through a control surface of its own, so that afterwards all axes
...
\begin{nurkurz}
In the axis-interface model, the measurement-source interface for micro-benchmarking is collected per
axis interface specifically for its parameter set; the category set thereby grows via the channel
addresses axis $\to$ family $\to$ category to hundreds of measurement categories (implementation status:
sixteen, statically fixed by the registry). Added to this is the \emph{unthrottled re-measurement}: the
actions of every axis are rolled back via the checkpoint of the first-called axis-interface function
behind the family interface through a control surface of its own, the CEB takes over the inner state of
a tier binary with measurement sensors into the analogous tier binary without measurement sensors and
measures anew from the same point, recursively per macro-benchmarking interface --- planned but not
implemented. The test criterion of the re-measurement is the plausibility invariant that the binary
with measurement sensors is slower than the one without (Chapter~\ref{ch:eval}).
\end{nurkurz}
...
\begin{nurkurz}
At the evaluation end of the cross-cutting concern, the first approach to the curve fitting from
Section~\ref{sec:heuristics-curves} is present as a skeleton (\texttt{curve\_fit.hpp}): a
measurement-axis curve over the working-set size is fitted by least squares to the model
$y = a\log_2 x + b$ --- the form expected for cache-hierarchy curves~\cite{hill1989associativity}. No
fit without valid data: the fit status distinguishes \emph{no data}, \emph{underdetermined} and
\emph{invalid data} from the success case --- never an \emph{Ok} with NaN coefficients ---, and the
result reader accepts only strictly numeric cells. Per measurement curve stands a cubic spline over
$\log_2 x$, natural or monotonicity-preserving according to Fritsch and
Carlson~\cite{fritsch1980monotone}, which on monotone data produces no spurious extrema. The
heuristics family (\texttt{heuristik/}) contains the axis spline, the break-even finder over the
intersections of two curves of the same axis, the measurement-curve loader, the offline clustering of
the workloads and the optimization catalogue; break-even curves are formed only over the
w/ma/mi-measured organ-axis parameters. The curve formation itself is developed in
Section~\ref{sec:heuristics-curves}; here stands only its code shape.
\end{nurkurz}
...
\begin{nurkurz}
Implemented with this are the realm roots including order and re-entry guards, the registries, the
source abstraction with degraded fallback, the two-stage hardware detection
(Section~\ref{sec:hw-probe}: provenance vocabulary, SPD parser, error domain, acquisition chain as
chain of responsibility, compile-time factory per platform cell and runtime verdict declaration against
acquisition) and the first fit approach with spline; the binding of these cross-cutting building blocks
to runner and probe dock (the collector of the success parameters) and the mapping of the registry
names onto the existing columns of the result files are not implemented. The three concept invariants
of the system axes read: they \emph{permute} the build and measurement space --- the complex axis
\texttt{build\_target\_complex} is the command wrapper of the recombination of the three main axes
with the sub-axis group \texttt{build\_toolchain} and the scheduling with five dimensions, and this
system multiplier multiplies the organ space per platform ---; they do not stand in the organ
permutation tree of the \texttt{binary\_id}; and they never touch the binary identity (registry
XML\@: \texttt{binary\_id} $=$ \texttt{never}; only the eighteen organ main axes enter the
eighteen-segment path). The construction itself preserves this: the root docks only reading onto the
host-side result structures and remains invisible to the ABI boundary.
\end{nurkurz}
...
The hybrid stage is the fourth carrier, inserted as an optional mux stage between the CEB probe dock and
the tier binary (Section~\ref{sec:heuristics-modes}); its code state lies
...
the classification layer (classification, concept gate, strategy per family, synthesis matrix) and three
further headers for docking, the module ABI and the stamp chain, thirteen headers in total.
The Hybrid Module ABI v1 exports the same mandatory symbols as the plain tiers including concept and
family, and \texttt{comdare\_anatomy\_genus} reports the \emph{target} family (what the module is for the
caller), never the reroute family of the classification --- the same concept-independent loader loads
the hybrid \texttt{.so} via the one loading route; the stamp chain of the hybrid stage and the hybrid positions
of the measurement-gate member (\texttt{hm}/\texttt{hmi}) carry its identity, and the family build
admission carries six entries, the sixth for the reroute family with the dock cap (default 32) as arity.
...
\begin{tikzpicture}[font=\footnotesize,>=Stealth,
  ...
  \node[draw,rounded corners,dashed,minimum width=9.0cm,minimum height=4.8cm] (frame) at (0,-0.7){};
  \node[font=\bfseries\scriptsize,anchor=north west] at (-4.5,-3.15){Hybrid \texttt{.so} (concept HeuristikAdapter, reroute family) --- inserted mux stage};
  ...
  \node[hb,fill=blue!16] (ceb) at (-2.4,2.75){CEB probe dock (stage 2)\\attachment point (mux) of the optional hybrid stage};
  \node[hb,fill=gray!10] (tier) at (2.3,2.75){tier binary (stage 3)\\standard route without hybrid: directly at the CEB probe dock};
  \draw[->,thick] (ceb)--node[right,font=\tiny]{drive}(proxy);
  \draw[->,thick,dashed] (ceb)--node[above,font=\tiny]{standard route}(tier);
  ...
  \draw[->,dashed] (xml)--(d1);
  \node[ex] (sym) at (6.3,0.9){exports: seven mandatory symbols as for plain tiers; \texttt{genus()} reports the target family (route C)};
  \draw[->,dashed] (frame.east)--(sym);
  \node[ex] (st) at (6.3,-1.4){stamp chain of the hybrid stage; measurement-gate fields \texttt{hm}/\texttt{hmi}; family build admission: 6th entry};
\end{tikzpicture}
\caption[The hybrid stage in code]{The hybrid stage in code: the mux stage optionally inserted between the CEB probe
dock and the tier binary, attached at the CEB probe dock and carrying dock slots for tier binaries (standard route
without hybrid: probe dock directly to the tier binary). The binary proxy makes the hybrid \texttt{.so} an ordinary tier binary to the
...
plain tiers, \texttt{genus()} reports the target family. Not implemented: \texttt{.so} export,
...
\begin{nurkurz}
\subsection{Pattern Register, Hybrid Stage and Store}\label{sec:impl-hybrid-lager}
The design patterns of the apparatus are a register (Figure~\ref{fig:muster}); the naming follows Gamma
et al.~\cite{gamma1994patterns} (\enquote{Gang of Four}, GoF), and every pattern is executed at compile
time instead of runtime-polymorphically, because the hot path may contain no runtime branching
(Section~\ref{sec:axes-impl}): \emph{Strategy} per axis (building blocks behind a concept, bound via
CRTP~\cite{coplien1995crtp}), \emph{Director} and \emph{Builder} for the plan emission, an
\emph{Abstract Factory} for the hybrid docks and the result workbooks of the store (xlsx workbook as
standard, CSV tree as a selectable strategy of the same factory; implemented is one flat file), the
\emph{Visitor} \texttt{IMessVisitor} at the \texttt{dlopen} transition, a \emph{Chain of
Responsibility} with type list for the hardware acquisition chain (boot cache --- the filtered
SMBIOS/DMI table of the system~\cite{dmtf_smbios} --- $\to$ SPD $\to$ declaration) and the selection
filter chain of the CEB\@; a \emph{Memento} for the state of stateful axes per tier
(\texttt{memento\_all} after the warm-up), \emph{Command} as the command base of the axes and command
wrapper of the complex axis \texttt{build\_target\_complex} as well as a \emph{Facade} for the appendix
generation --- program~08 as a facade over the programs 04 (\texttt{csv\_to\_latex}) and 05
(\texttt{diagram\_generator}) --- and the purely reading plan emission of the planner.
\begin{figure}[htbp]\centering
\begin{tikzpicture}[font=\footnotesize,
  mu/.style={draw,rounded corners,fill=blue!7,minimum width=3.6cm,minimum height=2.55cm,text width=3.4cm,align=center,inner sep=2pt}]
  \node[mu] (m1) at (0,0){\textbf{Strategy $+$ CRTP}\\{\scriptsize per axis}\\[2pt]{\tiny interchangeable building blocks behind a concept; \texttt{topics/axis}, \texttt{AxisBase<Derived>}; bound at compile time}};
...
The hybrid stage is the fourth carrier, inserted as an optional mux stage between the CEB probe dock and
the tier binary (Section~\ref{sec:heuristics-modes}); its code state lies
under \texttt{hybrid/}. Implemented are the dock contracts (four contract forms \texttt{standard},
\texttt{rollback}, \texttt{scan} and \texttt{resource\_control} with sixteen status constants), the
probe dock, the dock factory as the single construction site, the dock array with
\texttt{HybridDockVariant} (the one permitted variant site, Section~\ref{sec:axes-impl}) and the
runtime cap \texttt{max\_docks}, the parser of the \texttt{<hybrid\_tier>} section and the binary
proxy, which makes a hybrid \texttt{.so} an ordinary tier binary to the outside; not implemented are the
\texttt{.so} export, the eviction and the router --- six of the nine files of the layer. The Hybrid
Module ABI exports the same mandatory symbols as the plain tiers, \texttt{comdare\_anatomy\_genus}
reports the \emph{target} family, never the reroute family, and the family build admission carries as its
sixth entry the reroute family with the dock cap (default 32) as arity. The hybrid live ranking as the
fourth evaluation component of the compare stage and the state reconciliation via memento with two
release artefacts, single and hybrid, in the store are planned (Section~\ref{sec:heuristics-modes})
but not implemented, nor is the fix of the schema drift (the XSD does not carry the
\texttt{<hybrid\_tier>} section); the hybrid mode compiles only with the help of the CEB\@.

The store (Section~\ref{sec:lager}) has in code a compile-time-static key schema
(\texttt{bestand\_\allowbreak{}schluessel\_\allowbreak{}schema}) that declares the four stocks:
stock~1, the binaries, via the one SHA-512 key that is identical to the tier fingerprint; stock~2, the
measured values, via the 128-hex fingerprint of the measured binary and the hardware identity of the
measuring machine; stock~3, the synthesis batches, and stock~4, the solutions, as a work step of the
full expansion of the store; drift between schema and mechanics breaks at build time. The version lock
(\texttt{axis\_version.lock}, format v3, 719 records) binds every source file of the axes --- the
heuristics headers and the overlay cut (per axis the file set of its own implementation plus the shared
anatomy substance), over which the overlay member of the tier fingerprint hashes --- to content digest
and stamped version, the CI gate \texttt{contract:axis-version-lock} enforces this. From the schema
follow the store properties: the measurement is always switched on, in the store only what is missing
is re-measured; a version change of an axis rebuilds the affected store stack; and every machine type
has its own stock of measured values. Implementation status: schema, lock and the stocks~1 and~2 are
implemented, the full expansion is planned but not implemented.
\end{nurkurz}

With this, the apparatus is fully described --- from the four repositories through the candidate under test, the
