...
\end{itemize}
The fourfold order of the code is not the repository boundary but the \emph{carrier-stage chain}
planner $\to$ CEB $\to$ tier $\to$ hybrid (Section~\ref{sec:anatomy-levels}); the source tree maps it
under \texttt{libs/traeger} as four subprojects in stage form, where stage $N+1$ depends only on
stage $N$. Implementation status: the subprojects form a scaffold, the implementation lies as a
...
integration and delivery as safeguarding stages~\cite{fowler2006ci,humble2010cd}). \emph{Gate~1} is
...
\texttt{simulate}), never as a constant. \emph{Gate~2} is the integration gate of the project
...
proxy, which makes a hybrid .so an ordinary tier binary to the outside; not implemented are the
