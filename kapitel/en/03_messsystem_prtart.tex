...
the optional, disabled organ meta-meta axis \texttt{disk\_io} (Section~\ref{sec:axis-triad}) ---; in the
golden reference catalogue it is pinned to its in-memory representative, so that this fixed regression
detector counts $2^{17} = 131072$ identities, while the cardinality of the real permutation space is
computed dynamically by the planner from the activated catalogues (Section~\ref{sec:mess-chain}). This
...
full eighteen-segment path (Section~\ref{sec:stamp-model}), and serves as the regression detector with a
fixed CRC-64 checksum; the cardinality of the real permutation space, by contrast, is computed
dynamically by the planner from its inputs (subcommand \texttt{simulate}) --- it lies orders of
magnitude above and is never a fixed count. Three mechanisms dampen the explosion: the
