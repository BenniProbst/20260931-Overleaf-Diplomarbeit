...
\begin{tikzpicture}[font=\footnotesize,>=Stealth,align=center,
  ...
  \node[band,minimum height=1.5cm] (l3) at (1.3,-5.0){\textbf{Level 3 --- genus (subspecies):} implementation of the family interface, possibly with extended sub-interfaces --- mammoth $=$ ART, African elephant $=$ HOT, Indian elephant $=$ START, \dots; beneath the genus lie one or more search strategies (simple $=$ assignment of a single axis, complex $=$ composition of several axes). Today the code carries the genus only as an axis assignment; the genus implementation class is an acknowledged architecture error (fix after the defence).};
...
extended sub-interfaces of the genus); and a
...
for the extended sub-interfaces); and a plausibility criterion
...
meta-meta level, zero the main axis. The flag alphabet at depth~0 names the target hardware class:
\texttt{c} $=$ CPU, \texttt{g} $=$ GPU, \texttt{f} $=$ FPGA, \texttt{n} $=$ NPU\@. The bases \texttt{g}, \texttt{f} and
\texttt{n} are reserved in the catalogue and are not produced today, because the cache engine generates CPU code
exclusively --- every version it generates carries the CPU base \texttt{c}, an entry without \texttt{c} fails at
the catalogue guard. Beneath \texttt{c} stand as sub-members the core classes \texttt{p} $=$ performance core and
\texttt{e} $=$ efficiency core; \texttt{c} without a bracket means \texttt{c\{p\}} (default), \texttt{c\{e\}}
denotes an efficiency-core algorithm, \texttt{c\{p.e\}} both core classes. The identity of a flag is the pair of
token and parent: \texttt{f} at depth~0 is FPGA, \texttt{f} inside the bracket of \texttt{x512} the AVX-512
foundation subset.
...
and is never read flat as \texttt{cpe}; the bracket depth of an entry is its meta-meta level. The flag alphabet at depth~0 names
the target hardware class (\texttt{c} $=$ CPU, \texttt{g} $=$ GPU, \texttt{f} $=$ FPGA and \texttt{n} $=$ NPU as classes; only
\texttt{c} is produced and mandatory, \texttt{g}/\texttt{f}/\texttt{n} are reserved), beneath \texttt{c} the core
classes \texttt{p} $=$ performance core (default: \texttt{c} $=$ \texttt{c\{p\}}) and \texttt{e} $=$ efficiency
core; \texttt{c\{e\}} denotes an efficiency-core algorithm, and the identity of a flag is the pair of token and
parent (\texttt{f} beneath \texttt{x512} is the AVX-512 foundation subset, not FPGA). A fourth realm line for the mixing strategy of a
