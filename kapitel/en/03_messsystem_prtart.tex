...
\begin{tikzpicture}[font=\scriptsize,>=Stealth,align=center,
  ...
  \node[tr] (t2) at (5.0,0){tier binary\\{\tiny\mdseries stage 3}};
  \node[tr,dashed] (t3) at (7.5,0){hybrid tier\\{\tiny\mdseries insertion, optional}};
  \draw[->,thick] (t0)--(t1); \draw[->,thick] (t1)--(t2); \draw[->,thick] (t3)--(t2);
  \draw[->,thick,dashed] (t1.north) -- ++(0,0.32) coordinate (bp) -| (t3.north);
  \node[font=\tiny,anchor=south] at ([xshift=1.25cm,yshift=1pt]bp){configuration travels: XML $\to$ planner $\to$ CEB $\to$ (hybrid) $\to$ tier (no stage passes through layers)\\dashed: optional insertion --- hybrid as mux switch at the CEB probe dock with dock slots for tier binaries};
...
\end{tikzpicture}
\caption{Carrier stages and interface surfaces: the three-valued admission matrix of the seven stamp interfaces over the four carrier stages planner, CacheEngineBuilder (CEB), tier binary, and hybrid tier binary (28 cells; green $=$ mandatory, grey $=$ forbidden; the cell organ line/CEB is the static barrier \enquote{the CEB carries no organ axes}) and beneath it the interface surfaces per stage --- surface~1 genus interface (runtime), surface~2 stamp (compile time, ABI-stable), surface~3 measurement cut-through. The configuration travels from the XML via planner and CEB to the tier binary; the hybrid tier binary is a stage optionally inserted between the CEB probe dock and the tier binary (dashed: mux switch at the probe dock with dock slots for tier binaries; the column order of the matrix follows the build order, not the insertion position); no stage passes through layers, each contains its layer compiled in.}\label{fig:traeger-flaechen}
\end{figure}
\end{nurlang}
\begin{nurkurz}
