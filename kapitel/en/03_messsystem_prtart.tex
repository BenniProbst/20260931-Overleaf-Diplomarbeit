...
shown by a pure hash table as a counter-check within the Map family: an open-addressing hash after Knuth
...
counter-check requires no family of its own.
\end{nurkurz}
...
the \emph{hybrid and B\textsuperscript{+} family} with the B\textsuperscript{2}-tree
...
\begin{nurkurz}
Non-tree-like search methods --- such as the hash counter-check of Section~\ref{sec:sota-instances} ---
remain the same family with a restricted profile: a hash table, too, is a tree that searches from the root
of its interface over a fan-out of hashes; every method that translates a key into a value belongs to the
family Map and its genus \texttt{SearchAlgorithm} (Section~\ref{sec:anatomy-levels}). A vivid metaphor
...
collected in a shared axis matrix. In software-engineering terms an axis is a feature, the \emph{family}
the feature \emph{core} composition (the shared interface core of all genera) and the \emph{genus} the
feature \emph{special} composition (the real tier interface beyond the core,
...
three levels: the \emph{family} as the outward-visible algorithm interface (the \emph{probe dock}), the
\emph{genus} (alias: \emph{tier subclass}) with a fixed axis set, and the \emph{organs} themselves
(Figure~\ref{fig:three-levels}). The separation of levels can be read off at the ABI boundary: every tier
binary exports two letter-identical C symbols that answer family and genus without an instance, whereby
the family is derived from the genus\@. \enquote{Living being} and \texttt{SearchAlgorithm} denote the
...
Non-tree-like methods thus require no family of their own but occupy a restricted axis subset of the same
family.
\begin{figure}[htbp]\centering
% No \resizebox: the drawing is only ~14cm wide; blowing it up to \linewidth would inflate
% the \scriptsize font beyond the body text. Rule: resizebox only to SHRINK (cf. fig:m-model).
...
\begin{tikzpicture}[font=\scriptsize,>=Stealth,align=center,
  ...
  \node[fl] at (7.5,-5.25){1 genus interface\\2 stamp\\3 measurement cut-through \texttt{IMessVisitor}};
  \node[fl] at (5.0,-5.25){1 inherited target genus\\2 stamp $+$ attached\\3 measurement cut-through};
...
\end{tikzpicture}
\caption{Carrier stages and interface surfaces: the three-valued admission matrix of the seven stamp interfaces over the four carrier stages planner, CacheEngineBuilder (CEB), hybrid tier binary (optional insertion), and tier binary (28 cells; green $=$ mandatory, grey $=$ forbidden; the cell organ line/CEB is the static barrier \enquote{the CEB carries no organ axes}) and beneath it the interface surfaces per stage --- surface~1 genus interface (runtime), surface~2 stamp (compile time, ABI-stable), surface~3 measurement cut-through. The configuration travels from the XML via planner and CEB to the tier binary; the hybrid tier binary is a stage optionally inserted between the CEB probe dock and the tier binary (dashed: mux switch at the probe dock with dock slots for tier binaries; the column order of the matrix follows the processing topology with the hybrid at its insertion position, the arc is the standard route without hybrid); no stage passes through layers, each contains its layer compiled in.}\label{fig:traeger-flaechen}
\end{figure}
\end{nurlang}
\begin{nurkurz}
...
three-surface model: surface~1 is the genus interface of a tier binary (runtime), surface~2 the stamp
...
latter is the \emph{anatomy view} and orders the structure of the measured (family, genus, organs), the
...
\end{nurlang}
\begin{nurkurz}
The container genera are formed from the same axis pool as the search algorithms and are no standard
...
out completely, and the genus iterator library for the order-based operations is not implemented.
\end{nurkurz}
...
measurement takes place, belonging to no family. From this follow three invariants. First, system axes
...
lies under stamp keys (Figure~\ref{fig:lager-bestaende}); the store tree is organized by family, genus,
...
three wallclock levels (axis interfaces, family interface functions, test load from the load
...
are clustered according to similar sequences of equal access patterns on the family and genus
...
preloaded binaries at the ABI boundary. The probe dock of the family
...
Its centrepiece is that the heuristic adapter is a \emph{family} of its own with its own genus, the
fourth family \texttt{HeuristikAdapter} with the reroute genus \texttt{FunctionInterfaceReroute}
(Section~\ref{sec:anatomy-levels}): it adopts by metaprogramming the family and genus interfaces of its
...
binary exports the same mandatory symbols and reports as genus the inherited target genus. The
...
themselves introduce: the three measurement facilities --- wall clock in the CacheEngineBuilder, genus
