...
stamp, and the repository in which what is built and what is measured lies under stamp keys.
...
artifact of binary, version sidecar, and manifest (Section~\ref{sec:heuristics-loop}). The cache-aware
...
thesis, not a CE baseline); the PRT-ART family is brought in by the probe via its build axis page type.
...
23-work corpus (individual works contribute several variants); the 23 allocator profiles with license
...
\begin{table}[!htbp]
  \caption{Allocator profiles (23, all with \texttt{expected\_workload} tag) with license, workload affinity, and attachment state}\label{tab:allocator-profiles}
  \centering\small
  \begin{tabular}{lllll}
    \toprule
    \textbf{Profile} & \textbf{Family} & \textbf{License} & \textbf{expected\_workload} & \textbf{Code} \\
    \midrule
  ...
  \smallskip
  {\footnotesize \texttt{expected\_workload} = YCSB workload class (A/B/C/E); license verbatim from the profile
...
maps them to the using publications. Most widespread is the Yahoo!\ Cloud Serving Benchmark
...
a pure hash table as a counter-check within the Map genus --- as two selectable
...
thus requires no genus of its own.
...
remain the same genus with a restricted profile, for a separate genus would arise only if the
...
genus Map and its genus subclass \texttt{SearchAlgorithm} (Section~\ref{sec:anatomy-levels}). Second,
...
\begin{tikzpicture}[font=\footnotesize, align=center, >=Stealth,
  ...
  \node[habitat,minimum width=8.2cm] (hab) at (2.7,-2.45){\textbf{habitat} (system axes): the planet with its resources --- always present, belonging to no genus};
...
genera --- the search genus accesses axes in its interface functions and uses
container genus subclasses of its own, and parts of the axis pool are reused for the containers
...
An axis corresponds to a feature, a living being to a feature composition. The \emph{genus} is the
feature \emph{core} composition: the shared interface core that all genus subclasses of a genus share and
...
container genus this core consists of identity, axis observation, and size. The \emph{genus subclass}
...
optionally) or restricts its admissible assignments; sub-question FF1.b (Section~\ref{sec:rqs})
...
\end{figure}
\subsection{The Three-Level Model and the Four Genera}\label{sec:anatomy-levels}
...
beneath that, three levels can be distinguished: the \emph{genus} as the outward-visible
algorithm interface (the \emph{probe dock}), beneath it the \emph{genus subclass} (alias: \emph{tier subclass})
...
C symbols that answer genus and genus subclass without an instance, whereby the genus is not maintained as a second datum
but derived from the genus subclass --- the loader thus determines the genus via a
genus-independent symbol, without knowing it in advance\@. \enquote{Living being} and \texttt{SearchAlgorithm} denote the same
...
\begin{tikzpicture}[font=\footnotesize,>=Stealth,align=center,
  ...
  \node[lvl] at (-5.1,-1.6){Level 1: genus\\{\tiny feature core composition}};
  \node[box] (sa) at (-3.0,-1.6){Map};
  \node[box] (co) at (0,-1.6){Container};
  \node[box] (gr) at (2.6,-1.6){Graph};
  \node[box] (ha) at (5.3,-1.6){HeuristikAdapter};
  \node[lvl] at (-5.1,-3.3){Level 2: genus subclass\\{\tiny feature special composition}};
  ...
  \node[box,dashed,fill=gray!8] (gr2) at (2.6,-3.3){no genus\\subclass\\{\scriptsize implemented}};
...
\end{tikzpicture}
\caption{The three-level model: genus (level~1, outer interface, feature core composition) $\to$ genus subclass (level~2, fixed axis set, feature special composition) $\to$ organs/axes (level~3). Four genera: \texttt{Map} possesses as its sole genus subclass \texttt{SearchAlgorithm}, the \enquote{mammal} with the eighteen organ axes; \texttt{Container} the genus subclasses Set/Sequence/Adapter/View, which are formed from the same axis pool; \texttt{Graph} is laid out as an interface, a genus subclass is not implemented; \texttt{HeuristikAdapter} possesses the reroute genus subclass \texttt{FunctionInterfaceReroute} of the hybrid tier.}\label{fig:three-levels}
\end{figure}

At the topmost level stand four genera. The \texttt{Map} genus --- the
key-value-oriented probe dock and the focus of this thesis --- possesses as its sole genus subclass
\texttt{SearchAlgorithm}, the full eighteen-axis \enquote{mammal}; the \texttt{Container} genus
(keyless storage) fans out into the genus subclasses Set, Sequence, Adapter, and View, which are likewise formed from
axes of the shared pool and whose slot count per genus subclass is fixed
compile-statically; the \texttt{Graph} genus is laid out as an interface, a genus subclass is not implemented; and
the \texttt{HeuristikAdapter} genus possesses the reroute genus subclass \texttt{FunctionInterfaceReroute} --- the
genus of the hybrid tier (Section~\ref{sec:heuristics-modes}), which by metaprogramming inherits the
interfaces of a genus together with a genus subclass and passes requests through to the actual tier binaries;
classification (of which kind the reroute is) and pass-through (the inherited target genus subclass that the binary
reports outwardly) lie on separate levels (Figure~\ref{fig:genera}). Mnemonic:
\enquote{Adapter} without prefix always means the container genus subclass; the hybrid genus is always called
...
Non-tree-like methods thus require no genus of their own but occupy a restricted
...
\begin{tikzpicture}[font=\footnotesize,>=Stealth,align=center,
  ...
  % Level 2 --- genus subclasses (tier subclasses)
  ...
  \node[tbd,text width=2.4cm,anchor=north] (gr1) at (4.3,-0.7){interface laid out,\\{\scriptsize no genus subclass}\\{\scriptsize implemented}};
  \node[sub,minimum width=2.9cm,text width=2.9cm,anchor=north] (h1) at (7.4,-0.7){\texttt{FunctionInterface}\\\texttt{Reroute}\\{\scriptsize hybrid tier:}\\{\scriptsize inherited target genus subclass}};
  ...
  \node[band] (l3) at (1.3,-4.75){\textbf{Level 3 --- organs:} the 18 organ axes (node type, layout, allocator, prefetch, \dots) \emph{permute}; the container genus subclasses are formed from the same axis pool.};
...
\end{tikzpicture}
\caption[The four genera and the tier hierarchy down to the compiled binary]{The genus/tier hierarchy across five levels. \texttt{Map}, \texttt{Container}, \texttt{Graph}, and \texttt{HeuristikAdapter} are the four \emph{genera} (level~1, outer interface). Each genus hosts its \emph{own} genus subclasses (level~2, alias tier subclasses): \texttt{Map} exclusively the \enquote{mammal} (genus subclass \texttt{SearchAlgorithm}, full eighteen-axis anatomy), \texttt{Container} every \emph{non}-mammal genus subclass (bird/reptile/invertebrate/plant $=$ Set/Sequence/Adapter/View), \texttt{Graph} a laid-out interface without an implemented genus subclass, \texttt{HeuristikAdapter} the reroute genus subclass of the hybrid tier. The mammal branch is drawn out down to the individual: the eighteen axis organs (level~3) permute into a concrete axis composition $=$ one concrete animal (level~4, e.g.\ ART, HOT, Masstree), which is compiled per permutation into a standalone tier binary (level~5, the carrier stage of the tier binary, additionally refined by the probe build axis page type and platform-stamped by the three system axes, Section~\ref{sec:axis-triad})\@. \emph{Every tier binary is thus an animal of its own with an actual binary genus-subclass interface and three interface surfaces}; the hierarchy reaches from the genus down to the single compiled permutation. Cf.\ the condensed model in Figure~\ref{fig:three-levels}.}\label{fig:genera}
\end{figure}
...
offer catalogues and chooses the measurement axes at its runtime; the CEB contains this measurement choice
...
genus-subclass interface of a tier binary (runtime, Abstract Factory), surface~2 the stamp (compile time,
ABI-stable, Section~\ref{sec:stamp-model}), surface~3 the measurement cut-through (\texttt{IMessVisitor}), which
crosses the measurement as a seam of its own, so that genus and genus-subclass interfaces remain unchanged. On tier
...
\begin{tikzpicture}[font=\scriptsize,>=Stealth,align=center,
  ...
  \node[fl] at (5.0,-5.25){1 genus-subclass interface\\2 stamp\\3 measurement cut-through \texttt{IMessVisitor}};
  \node[fl] at (7.5,-5.25){1 inherited target genus subclass\\2 stamp $+$ attached\\3 measurement cut-through};
...
\end{tikzpicture}
\caption{Carrier stages and interface surfaces: the three-valued admission matrix of the seven stamp interfaces over the four carrier stages planner, CacheEngineBuilder (CEB), tier binary, and hybrid tier binary (28 cells; green $=$ mandatory, grey $=$ forbidden; the cell organ line/CEB is the static barrier \enquote{the CEB carries no organ axes}) and beneath it the interface surfaces per stage --- surface~1 genus-subclass interface (runtime), surface~2 stamp (compile time, ABI-stable), surface~3 measurement cut-through. The configuration travels from the XML via planner and CEB to the tier binary; no stage passes through layers, each contains its layer compiled in.}\label{fig:traeger-flaechen}
\end{figure}
...
single-thread measurement run whose result fills the measurement-value repository; \emph{compare} reads the measurement-value repository
...
the planner shell for the wiring check --- a flag, not a work mode. Implementation state: the
...
\begin{tikzpicture}[font=\footnotesize,>=Stealth,thick,align=center,
  ...
  \node[built] (m) at (3.1,0){\textbf{measure}\\1 thread, deterministic, fills the repository};
  \node[plan] (c) at (6.2,0){\textbf{compare}\\reads the repository, computes the best binary};
...
\end{tikzpicture}
\caption{The run chain of the four work modes build $\to$ measure $\to$ compare $\to$ release as a registered sub-axis of the measurement realm, cumulative in this order and repeated synchronously on the hybrid level; compare reads the measurement-value repository and computes the best binary or recombination, release generates the fully optimized binary without measurement sensors and re-measures its wall clock. A debug state does not exist. The assignment to the operating target pictures measurement, evaluation, working, and hybrid mode is given by Section~\ref{sec:heuristics-modes}.}\label{fig:modi-kette}
\end{figure}
...
in comments on ABI adapters of the genus subclasses, platform probe, code generation, permutation commands,
...
standalone binary loaded across the ABI-stable interface (level~5 of the genus hierarchy,
...
Each layer is completed individually, with a frozen interface contract to its
...
with an explicitly \emph{frozen interface contract} to its respective neighbouring level
--- the E4$\to$E3 boundary, say, freezes how an XML definition is translated into axis levels, the
...
too large for simultaneous changes across several levels; only the frozen contracts make each level individually
...
engine, the apparatus itself is developed in interface layers against frozen contracts;
...
\emph{anatomy view} --- it orders \emph{one} living being from the genus through the genus subclass to the
...
the same living being that anatomically decomposes into genus, genus subclass, and organs is \emph{configured}
...
\texttt{std::map<Key,\,Value>} with point query (\texttt{find}), insertion/update (\texttt{insert},
...
compile time versus run time; the genus subclass, which occupies only a subset of the axes; the axis algorithms
...
that sub-question FF3.b (Section~\ref{sec:rqs}) addresses: the \emph{main axis} is the
...
The container genus subclasses are formed from the same axis pool as the search algorithms and are no
standard containers --- out of necessity, not convenience: only thus do the axes permitted in the compilate of a
...
C++ library this control would be lost. The shared core of the container genus consists of
identity, axis observation, and size (Section~\ref{sec:anatomy-metaphor}); the genus subclasses Set, Sequence,
...
are bundled by a library of its own over the iterators of the genus-subclass interfaces, which covers all Cartesian products
of the axes equally at compile time; this genus-subclass iterator library is not implemented.
...
genus-subclass interface functions of the tier binary; \emph{micro} (mi) --- the time and the success parameters per
...
$\to$ genus subclass $\to$ category, never via a free name. Three points are not
...
with one class root each, one offer catalogue each, and one stamp line each
...
fingerprint --- and is stored under this fingerprint in the \emph{repository}, the stock of binaries
...
measurement takes place, belonging to no genus --- and an animal is the better, the more precisely it is adapted to
...
this is the release of a target platform for building, which sub-question FF1.b (Section~\ref{sec:rqs})
presupposes ---, and
...
organ permutation are different artifacts and must differ in the fingerprint: build control quantities
...
is bracketless, a meta-meta hangs as a bracket appendix on its realm line
(Section~\ref{sec:stamp-model}) ---, and the bit encoding of this level in the entry is described by
Section~\ref{sec:impl-system-axes} ---, the disk I/O \texttt{disk\_io}: it stamps in the bracket appendix of the organ line,
...
class root and its own offer catalogue (three registry XML); the code shape of these roots
...
measurement-gate state of its own translation unit as a fingerprint member, so that two compilates
...
is therefore a mandatory constituent of the measurement model, not an option. Implementation state: the
...
but the \emph{automated resubmission}: a tool result collected once and then frozen
...
the meta-meta main axis of the measurement realm --- as a bracket appendix at its \emph{end} (pattern
...
this name level that the hybrid and full-join states remain distinguishable at the compilate; byte-different
...
identity, the roles of cache, repository, and recognition key as well as classification. The tier binary carries a
...
recognition of an already built permutation in the repository (Section~\ref{sec:lager}), and the
...
\subsection{The Repository: Stocks under Stamp Keys}\label{sec:lager}
The \emph{repository} is the persistent stock of the apparatus, in which what is built and what is measured lies
under stamp keys (Figure~\ref{fig:lager-bestaende}). The repository tree is organized by genus,
genus subclass, and realm --- \texttt{gattung=} $\to$ \texttt{genus=} $\to$ \texttt{realm=}
(\texttt{binaries} or \texttt{messdaten}) ---, and the hybrid genus is sorted into it like every other
genus. Four stocks are fixed compile-statically in the key schema:
...
references to the participating stocks. Measurement is always switched on; in the repository only what is missing there
...
Two consequences are properties of the repository, not of its implementation: an
algorithm version change alters the organ line and thus the fingerprint, hence triggers the rebuild of the
affected repository stack together with re-measurement; and because the hardware identity is part of the
measured-value key, every machine configuration has its own measured-value stock.
Implementation state: stock~1 and~2 as well as the key schema of all four stocks are implemented; the
mechanics of stock~3 and~4, the curve facet, the storage path of the hybrid tier, and the target selection
via the XML form the full repository build-out and are not implemented.
\begin{figure}[htbp]\centering
...
experiment \emph{planner} resolves the XML against the three offer catalogues and builds, per
...
search algorithm; its genus interface functions \emph{use} the axis interfaces ---
...
wallclock of the genus interface functions, and the wallclock of a test load from the
...
frozen CRC-64 checksum over all identities; the cardinality of the real permutation space over all
...
resolves the \texttt{binary\_id} to the binary in the repository, and delivers binary, version sidecar, and manifest
as a standalone artifact. The heuristic curves arise as natural cubic splines over the
...
operating modes is the \emph{paper isolation mode} asked for in sub-question FF2.c (Section~\ref{sec:rqs}):
...
as similar as possible, of equal access patterns on the genus and genus-subclass interfaces of the tier binaries: every
...
main memory and switches internally the tier binary with its specifically optimal
axis permutations at the ABI-stable interface --- the same binary-switching mechanism that the
measurement mode uses, only load-driven instead of measurement-driven. Level-correctly the
compile-time principle remains preserved: there is no runtime switch \emph{inside} the tier --- the binaries
themselves remain compile-time-permuted; switching happens \emph{between} preloaded binaries at
the deliberate ABI boundary. The probe dock of the genus (Section~\ref{sec:anatomy-levels}) is called in this
...
release artifacts are stored, a release for single operation without sidecar and a release for the
...
centrepiece is that the heuristic adapter is a \emph{genus} of its own with its own genus subclass --- the fourth
genus \texttt{HeuristikAdapter} with the reroute genus subclass \texttt{FunctionInterfaceReroute}
(Section~\ref{sec:anatomy-levels}): it adopts by metaprogramming the genus and
genus-subclass interfaces of its tier binaries, is itself compiled into a tier binary, and docks at the
...
mandatory symbols as every other and reports as genus subclass the inherited target genus subclass --- classification
...
through its ABI-stable genus interface --- to the outside a \emph{virtual whole
...
(Section~\ref{sec:heuristics-loop}); in the stage compare it is computed from the measurement-value repository which
...
repository (Section~\ref{sec:lager}). The cardinality of every dimension of this
...
CacheEngineBuilder, the genus-subclass interface time of the tier binary (macro), and the axis interface time
...
foundation: the measurement gates per compilate as a fingerprint member, whose coverage CEB off, tier on is proven by a test,
...
measurement gates per compilate.
