...
(Figure~\ref{fig:modi-kette}). There is no debug work mode; the build variants debug, measurement, and release remain in force, and the debug variant is being extended further: \texttt{--debug} is a switch of
the planner shell for the wiring check --- a flag, not a work mode; under \texttt{--debug} the conformance
gate additionally runs the genus test set (as planned: tests only with the debug binaries, debug and
...
(Figure~\ref{fig:modi-kette}). There is no debug work mode; the build variants debug, measurement, and release remain in force, and the debug variant is being extended further: \texttt{--debug} is a flag of the planner shell,
not a work mode; under \texttt{--debug} the conformance gate additionally runs the genus test set, and this
indirect debug mode coincides with the release build (locked for users, rc~8). Implementation status:
...
flag of the planner shell and not a mode (under \texttt{--debug} the conformance gate additionally runs the
genus test set; this indirect debug mode coincides with the release build). None of these sub-axes ever
