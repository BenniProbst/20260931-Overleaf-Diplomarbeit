...
and the measurement system built upon it --- demonstrated on the candidate under test PRT-ART\@. In doing so, the
...
\end{nurlang}
\begin{nurkurz}
Building on the foundations from Chapter~\ref{ch:foundations}, this chapter develops the problem statement,
the anatomy model as its solution, and the measurement system built upon it, demonstrated on the candidate
under test PRT-ART\@. It newly introduces three orderings that Chapters~\ref{ch:impl} to~\ref{ch:conclusion}
presuppose: the triad of the axes into measurement, system, and organ axes, the four carrier stages of the
apparatus with their identity stamp, and the store in which what is built and what is measured lies under
stamp keys.
\end{nurkurz}
...
\end{nurlang}
\begin{nurkurz}
The state of the art sharpens this problem in two directions: in substance, in the studied corpus \emph{not a
single method occupies all design dimensions together} --- some dimensions are filled only with textbook
variants, others appear only as a measurement target, and I/O, migration, filtering, and buffering are never
occupied together with the search decisions; methodologically, results are reported in incomparable
settings, and favourably tailored load profiles hamper a fair cross-comparison. From this follow four
requirements: \emph{decompose} cache-aware search algorithms into interchangeable design constituents,
\emph{reconstruct} the state-of-the-art designs as explicit configurations, \emph{measure} the
cache-line-aware behaviour per constituent as well as for the whole algorithm on a common basis --- all load
profiles against all methods, bias-reduced --- and \emph{deliver} the best overall algorithm
system-optimized: as a ranking per interface function and metric, as a Pareto front of non-dominated
candidates, and as a delivery artefact of binary, version sidecar, and manifest
(Section~\ref{sec:heuristics-loop}). The guiding quantity is the cache-line behaviour itself, which shall
become measurable in isolation per design decision.
\end{nurkurz}
...
\begin{nurkurz}
The measurement requirement sharpens, at the juxtaposition of cache-aware and cache-oblivious designs
(Section~\ref{sec:hardware-history}), into a third path: the actual cache properties of a target platform
are \emph{determined} and enter a system-optimized binary as fixed quantities, while the \emph{configurable}
cache-near settings --- prefetch distance, pool budget, batch size, inline threshold, thread count, and
workload as well as, as far as operating system and architecture permit, the hardware prefetcher --- are
permuted as sub-axis parameters against the loaded binary; which grade a platform offers is determined by
the planner's platform probe.
\end{nurkurz}
...
\end{nurlang}
\begin{nurkurz}
Once the design space is spanned this way, the separability problem becomes solvable: varying a decision
means moving along exactly one axis while all others stay fixed. The contribution of this thesis is the
orthogonal axis decomposition of the known designs together with a \emph{permuted measurement
architecture} that reconstructs them as explicit configurations and measures them on a common basis; the
processing chain from definition to evaluation remains manageable only as layers with fixed contracts
(Section~\ref{sec:anatomy-layers}). Section~\ref{sec:sota-instances} develops the adoption of the
instances dialectically; the solution concept (Section~\ref{sec:anatomy-model}) and the measurement system
(Section~\ref{sec:measurement-system}) close the double gap.
\end{nurkurz}
...
\begin{nurkurz}
\section{State of the Art as Axis Instances}\label{sec:sota-instances}
The methods known from Chapter~\ref{ch:foundations} now appear as axis compositions; the adoption proceeds
\emph{dialectically}: per axis a foreign organ is \emph{adopted} (thesis) or an obsolete assumption is
\emph{replaced} (antithesis), from which the own contribution emerges as synthesis --- guided by the
\enquote{third way} from Section~\ref{sec:task-concerns}. Often the antithesis is merely the implicit
assumption of not regarding the decision as selectable at all; the synthesis then makes it an explicit axis
in the first place (replaced column \enquote{---}). Two cluster assignments are deliberate approximations
(T1 draws from layout theory and prefetching, the reclamation sub-axis of T6 shares its sources with T8).
The comparison is carried by the living-being metaphor --- every method a living being whose organs are
adopted or replaced; the metaphor itself is defined in Section~\ref{sec:anatomy-metaphor}.
Table~\ref{tab:dialectic} summarizes this adoption per composition axis.
\end{nurkurz}
...
is ordered by Section~\ref{sec:axis-triad}) --- and the candidate build axis page type. Besides the eighteen
...
\begin{nurkurz}
The systematic core is the axis-wise \emph{dissection}: every overall algorithm is decomposed into its
organs, per axis the contributing organ algorithms are listed. The framework carries \emph{eighteen organ
composition axes} in fixed slot order (T0--T17), alongside them the quantities of the system sphere (target
ISA, SIMD and extension hardware, two-way-split telemetry; Section~\ref{sec:axis-triad}) and the candidate
build axis page type. A \emph{main axis} is an axis whose chosen assignment is compiled compile-statically
into the binary; to these is added an optional \emph{organ meta-meta axis} for disk I/O (\emph{meta-meta}
means: the axis does not describe an assignment but describes how other axes are described;
Section~\ref{sec:axis-triad}). Many axes are refined into \emph{sub-axes} --- statically as a compile-time
choice among partial algorithms, dynamically as runtime parameters without recompilation --- and provide
other axes with interfaces for their detail operations; exactly these \emph{axis interfaces} are captured
by the micro-benchmarking (Section~\ref{sec:mess-chain}) per call by runtime and success parameters
(Section~\ref{sec:measurement-system}). Table~\ref{tab:axes-overview} gives the overview of which
publications contribute an organ per axis.
\end{nurkurz}
...
\begin{table}[!htbp]
  ...
  candidate build axis, and the optional organ meta-meta axis; per axis the sources contributing an organ.}\label{tab:axes-overview}
  ...
  \begin{tabular}{@{}l l p{6.8cm}@{}}
    ...
    Build (candidate) page\_type & PG1--PG3 & P01,P02,P04,P05,P06,P10,P11,P12,P13,P20; PRT-ART (2026) \\
...
\begin{table}[!htbp]
  ...
  \begin{tabular}{@{}l l p{6.8cm}@{}}
    ...
    Sys.\ target\_isa & IS1--IS3 & x86-64/AArch64/Power/RISC-V (vendor-spec) \\
    Sys.\ external\_utils: SIMD & SE1--SE3 & SSE2/AVX2/AVX-512/NEON/SVE2/RVV\@; GPU\@: CUDA \\
    Sys.\ external\_utils: ext.\ HW & HW1--HW4 & platform profiles as code/baseline (Appendix~\ref{app:blocks}, Section \enquote{Build-HW}) \\
    Sys./meas.\ telemetry & TM1--TM3 & P01,P02,P16,P17,P26,P28; HdrHistogram (Gil Tene) \\
    Build (candidate) page\_type & PG1--PG3 & P01,P02,P04,P05,P06,P10,P11,P12,P13,P20; PRT-ART (2026) \\
    Organ meta-meta disk\_io & staging (runtime) & CE baseline; optional, disabled (Section~\ref{sec:axis-triad}) \\
    \bottomrule
...
thesis, not a CE baseline); the PRT-ART profile family is brought in by the candidate under test via its build axis page type.
\end{nurlang}
\begin{nurkurz}
The \emph{search axis} (T0) comprises the most organs: ART~\cite{leis2013art}, HOT~\cite{binna2018hot},
START~\cite{fent2020start}, SuRF~\cite{zhang2018surf}, and Wormhole~\cite{wu2019wormhole} as well as generic
search methods such as k-ary search~\cite{schlegel2009kary}, interpolation search~\cite{perl1978interpolation},
Eytzinger-layout search~\cite{khuong2017eytzinger}, skip list~\cite{pugh1990skiplist},
B-tree~\cite{bayer1972btree}, and open-addressing hashing~\cite{knuth1998taocp3}. The traversal axes separate
the algorithm and cache views: T1 contains the three traversal building blocks
\texttt{Linear\allowbreak{}Fanout}, \texttt{Hash\allowbreak{}Lookup}, and
\texttt{Binary\allowbreak{}Search\allowbreak{}Fanout} (literature in Appendix~\ref{app:blocks}), T2 (mapping)
the linear 1:1 mapping (\texttt{Direct\allowbreak{}Placement}) and the pool-relative offset mapping
(\texttt{Pool\allowbreak{}Relative}); hash redirect~\cite{wu2019wormhole} (MP04) and permutation
index~\cite{mao2012masstree} (MP03) are roadmap values.
The \emph{path compression} (T3) instantiates the byte-wise variant~\cite{leis2013art}, the Patricia
split~\cite{morrison1968patricia,binna2018hot}, and the pass-through variant \texttt{None}; the
\emph{node-type axis} (T4) bundles the four adaptive ART node formats Node4/16/48/256~\cite{leis2013art},
whereas HOT compound~\cite{binna2018hot}, Internal/Border~\cite{mao2012masstree},
Decision/Span~\cite{schmeisser2022b2tree}, LOUDS-Dense/Sparse~\cite{zhang2018surf}, and the
CSS/CSB\textsuperscript{+}/Hankins nodes are reconstruction targets of this thesis; the PRT-ART family is
brought in by the candidate under test via its build axis page type.
\end{nurkurz}
...
The \emph{memory-layout axis} (T5) ranges in the code from the cache-line-aligned AoS
(\texttt{AoS\allowbreak{}Strict\allowbreak{}Memory\allowbreak{}Layout},
\texttt{Cache\allowbreak{}Line\allowbreak{}Aligned\allowbreak{}Memory\allowbreak{}Layout}) through SoA and AoSoA
to the bit-packed LOUDS layout
(\texttt{Packed\allowbreak{}Bitmap\allowbreak{}Memory\allowbreak{}Layout})~\cite{jacobson1989louds}; pointer-free
arrangement~\cite{rao1999css}, sibling cluster~\cite{rao2000csb}, and embedded
tree~\cite{schmeisser2022b2tree} are related literature (Appendix~\ref{app:blocks}). The \emph{allocator
axis} (T6) is the most densely occupied with the corpus A01--A23 and is refined into seven sub-axes, from
free-list topology through thread locality/NUMA affinity~\cite{ungethuem2017survey} to reclamation (epoch,
RCU~\cite{mckenney2001rcu}, hazard pointers~\cite{michael2004hazard}, QSBR); the code provides 26 allocator
wrappers, the 23 profiles with licence and workload affinity are listed by
Table~\ref{tab:allocator-profiles}. The \emph{value axis} (T10) distinguishes inline
value~\cite{leis2013art}, external pool~\cite{wu2019wormhole}, versioned pointer~\cite{mao2012masstree},
immutable shared ref~\cite{mckenney2001rcu}, and the chained external reference (ChainRef), the
\emph{serialization axis} (T9) raw binary, succinct LOUDS/FST, and varint~\cite{leis2013art}.
\end{nurkurz}
...
the candidate's path-oriented bundle prefetch (\texttt{Path\allowbreak{}Oriented\allowbreak{}Prefetch},
...
concepts: the CRTP organ registry \texttt{All\allowbreak{}Telemetries} with four strategies (leaf-only counter,
...
\end{nurlang}
\begin{nurkurz}
The \emph{prefetch axis} (T7) contains four building blocks: no prefetch (\texttt{None\allowbreak{}Prefetch},
measurement baseline), the distance-estimated software prefetch after ART~\cite{leis2013art}, the hardware
prefetch instruction from Wormhole~\cite{wu2019wormhole}, and the candidate's path-oriented bundle prefetch
(\texttt{Path\allowbreak{}Oriented\allowbreak{}Prefetch}, this thesis); the related literature from
software-fixed~\cite{chen2001prefetch} to hierarchical bundle prefetch~\cite{zhang2025hierarchical} is given
in Appendix~\ref{app:blocks}. The system axis \emph{target ISA} and the SIMD extension of the
extension-hardware hub (Section~\ref{sec:axis-triad}) carry the vendor specifications; two strategy axes
record what an algorithm actively \emph{uses} (\emph{hardware strategy}: SIMD family, cache-level targeting,
NUMA, prefetch hardware, atomic family) and how it \emph{distributes} work (\emph{scheduling strategy}:
worker-pool layout, SIMD worker limit, P-/E-core dispatch, co-routines, batch granularity). The
\emph{telemetry} --- not an organ axis but split in two, into a measurement-tooling sub-axis and a build
quantity of the system sphere (Section~\ref{sec:axis-triad}) --- instantiates density
tracker~\cite{leis2013art}, insert counter~\cite{binna2018hot}, HdrHistogram, and path-read
counter~\cite{zhang2024pathprefix} (registry \texttt{All\allowbreak{}Telemetries} and vtable family
\texttt{ITelemetry\allowbreak{}Strategy}, Appendix~\ref{app:blocks}); the coherence-friendly leaf-only
variants go back to Kuehn (P28~\cite{kuehn2023bplustree}), the counter in all nodes counts as an
...
identities (under the superseded assumption of exactly two configurations per axis), while the upper bound of
binary identities per system permutation is the product of the configurations selected per axis in the XML
experiment, which the planner determines from the properties of the XML experiment and which is known only at
execution time (Section~\ref{sec:mess-chain}). This recombination is not reached in
reality, since not all axis organs are compatible with one another.
\end{nurlang}
\begin{nurkurz}
The \emph{concurrency axis} (T8) splits into the sub-axes pattern and reclamation (pattern $\perp$
reclamation, Table~\ref{tab:dialectic}): from the serial zero point through coarse
locking~\cite{dijkstra1965concurrent} and optimistic lock coupling~\cite{leis2016olc} to
lock-free~\cite{michael1996queue}, wait-free~\cite{herlihy1991waitfree}, as well as
RCU~\cite{mckenney2001rcu} and hazard-pointer~\cite{michael2004hazard} methods (sub-axes CC1/CC2,
Appendix~\ref{app:blocks}). The \emph{I/O axis} (T12) instantiates the RAM-only zero point as well as
buffered~\cite{stonebraker1981os}, direct, and memory-mapped I/O~\cite{crotty2022mmap}; the
\emph{migration axis} (T13) hot/cold separation and multi-stage tier migration (RAM, SSD, HDD); the
\emph{buffering axes} (T15/T16) range from the Disruptor ring buffer~\cite{thompson2011disruptor} through
delta chain~\cite{levandoski2013bwtree}, copy-on-write~\cite{driscoll1989persistent}, and tombstone
buffer~\cite{oneil1996lsm} to lock-free SPSC~\cite{lamport_spsc} and MPMC queues~\cite{michael1996queue};
the \emph{index-organization axis} (T11) distinguishes clustered arrangement~\cite{bayer1972btree}, heap
storage, secondary index~\cite{comer1979btree}, and index-organized table, the \emph{filter axis} (T14)
Bloom~\cite{bloom1970}, Cuckoo~\cite{fan2014cuckoo}, SuRF~\cite{zhang2018surf}, and Xor
filter~\cite{graf2020xor}. The \emph{persistence-target axis} (T17), finally, makes the storage target
explicit --- in-memory as the standard, a disk-writeback target as a disabled option in combination with
the optional, disabled organ meta-meta axis \texttt{disk\_io} (Section~\ref{sec:axis-triad}) ---; in the golden reference catalogue
it is pinned to its in-memory representative, so that this fixed regression detector counts $2^{17} = 131072$
identities (under the superseded assumption of exactly two configurations per axis), while the upper bound of
binary identities per system permutation is the product of the configurations selected per axis in the XML
experiment, which the planner determines from the properties of the XML experiment and which is known only at
execution time (Section~\ref{sec:mess-chain}). This
recombination is not reached in reality, since not all axis organs are compatible with one another.
\end{nurkurz}
...
The intent is to unite all available benchmark systems for the measurement system under the umbrella of this
axis; in code and plan alike, the benchmark systems thereby count, through their load profiles, as a
sub-axis of the measurement system (the runtime parameter \texttt{workload} of the load framework), because
they manage runtime-specific changes to the chain from the XML to the tier binary
(Section~\ref{sec:mess-chain}): load profiles and datasets change without the tier binary being recompiled.
\end{nurlang}
\begin{nurkurz}
The \emph{loads}, too, appear as instances. The workloads used in the literature stem from \emph{several},
mutually incomparable benchmark frameworks. Most widespread is the Yahoo!\ Cloud Serving Benchmark
(YCSB)~\cite{cooper2010ycsb}; besides it appear the index benchmark SOSD~\cite{kipf2019sosd}, the
TPC family~\cite{tpc_benchmarks}, the CPU suite SPEC~CPU~\cite{spec_cpu2017}, the scale-out suite
CloudSuite~\cite{ferdman2012cloudsuite}, and --- for the allocator corpus ---
mimalloc-bench~\cite{mimalloc_bench} and the classics Larson, threadtest, and shbench. Since these
frameworks produce incomparable loads, the thesis abstracts them into a common set of load profiles; in the
measurement realm the load framework is registered as a meta-meta axis and carries YCSB as the only enabled
building block, while TPC, SOSD, SPEC~CPU, CloudSuite, and the allocator suites are attached via a
\emph{dataset loader} (Section~\ref{sec:axis-triad}).
The intent is to unite all available benchmark systems for the measurement system under the umbrella of this
axis; in code and plan alike, the benchmark systems thereby count, through their load profiles, as a
sub-axis of the measurement system (the runtime parameter \texttt{workload} of the load framework), because
they manage runtime-specific changes to the chain from the XML to the tier binary
...
\end{nurlang}
\begin{nurkurz}
From this analysis the thesis distils \emph{fourteen} distinct load profiles (LP01--LP14): they give the
heterogeneous loads a common, uint64-based operation model --- the op-mix names the shares of insert,
lookup, delete, scan, and read-modify-write over defined key distributions --- and are each derived from
...
a pure hash table as a counter-check within the Map concept --- as two selectable
...
thus requires no concept of its own.
\end{nurlang}
\begin{nurkurz}
The state of the art dissected here rests on a fixed corpus: 33~deeply analysed search-algorithm
publications (P01--P33) and a parallel allocator corpus of 23~works (A01--A23), whose axis assignments are
persisted as XML profiles in the \texttt{comdare-cache-engine} (three of them as abstract profiles, see
below); the complete building-block and allocator matrices are given in the appendix. The dissection
follows a provenance table (sources checked against the original publications) over roughly 110~organ
algorithms; every state-of-the-art algorithm thereby appears as a point in the Cartesian product of the
axes (Figure~\ref{fig:design-space}). That the axis framework also accommodates non-tree-like methods is
shown by a pure hash table as a counter-check within the Map concept: an open-addressing hash after Knuth
(S14, implemented) and the scalar SwissTable replica~\cite{abseil_swisstable} (S22, disabled by default;
legend of the S numbers in Appendix~\ref{app:blocks}). Tree- and trie-specific axes do not drop out: the
path compression (T3) carries the pass-through variant \texttt{None}, the node type (T4) remains regularly
assigned an ART node format that the hash building block does not use; the ordered range iteration is not
an organ axis but the scan capability of the module boundary (Appendix~\ref{app:interfaces}), and the
counter-check requires no concept of its own.
\end{nurkurz}
...
\emph{hybrid and B\textsuperscript{+} concept} unites the B\textsuperscript{2}-tree
...
\end{nurlang}
\begin{nurkurz}
The publications stem from six \emph{origin clusters} that serve the placement of each living being before
it is decomposed into its organs: the \emph{trie family} with ART (P01)~\cite{leis2013art}, HOT
(P02)~\cite{binna2018hot}, Masstree (P03)~\cite{mao2012masstree}, CoCo-Trie (P04)~\cite{boffa2024coco},
START (P05)~\cite{fent2020start}, LOUDS (P09)~\cite{jacobson1989louds}, and SuRF (P10)~\cite{zhang2018surf};
the \emph{hybrid and B\textsuperscript{+} concept} with the B\textsuperscript{2}-tree
(P06)~\cite{schmeisser2022b2tree}, Wormhole (P07)~\cite{wu2019wormhole}, and the CSS, CSB\textsuperscript{+},
and Hankins trees (P11--P13)~\cite{rao1999css,rao2000csb,hankins2003nodesize}; the \emph{layout theory} with
Samuel (P14)~\cite{samuel2005procaware}, Graefe/Larson (P15)~\cite{graefe2001btree}, Bender et
al.\ (P16/P17)~\cite{bender2000cobtree,bender2005coboblivious,bender2002treelayout,bender2002scanning,bender2002orderlist},
and Saikkonen/Soisalon-Soininen (P18/P19)~\cite{saikkonen2008multilevel,saikkonen2016layout}; the
\emph{prefetching} with \emph{B-Trees Are Back} (P20)~\cite{mueller2025btreesback}, Chen et
al.\ (P21/P22)~\cite{chen2001prefetch,chen2002fractal}, Khan (P23)~\cite{khan2010adaptive}, Naderan-Tahan
(P24)~\cite{naderan2016adaptivefilter}, Mahling (P25)~\cite{mahling2025hotpath}, Zhang
(P26/P27)~\cite{zhang2024pathprefix,zhang2025hierarchical}, and Kuehn (P28)~\cite{kuehn2023bplustree}; the
\emph{synchronization} with OLC-ART (P08)~\cite{leis2016olc}, RCU (P29)~\cite{mckenney2001rcu}, and hazard
pointers (P30)~\cite{michael2004hazard}; the \emph{TU-Dresden line} with Ungeth\"{u}m
(P31)~\cite{ungethuem2017survey}, Schmidt (P32)~\cite{schmidt2025hbm}, and VAMPIR
(P33)~\cite{berthold2023vampir}. The \emph{allocator corpus} (A01--A23) spans from
dlmalloc~\cite{lea1996dlmalloc}, Hoard~\cite{berger2000hoard},
Slab/Magazines/Vmem~\cite{bonwick1994slab,bonwick2001vmem}, and Michael's lock-free
allocation~\cite{michael2004lockfree} through mimalloc~\cite{leijen2019mimalloc},
jemalloc~\cite{evans2006jemalloc}, TCMalloc~\cite{ghemawat2005tcmalloc}, snmalloc~\cite{lietar2019snmalloc},
scalloc~\cite{aigner2015scalloc}, rpmalloc~\cite{jansson2017rpmalloc}, and lrmalloc~\cite{leite2018lrmalloc}
to CAMA~\cite{herter2011cama}, NUMAlloc~\cite{yang2023numalloc}, StarMalloc~\cite{reitz2024starmalloc}, and
...
\texttt{allocator}, \texttt{prefetch}, \texttt{layout}, \texttt{reclamation}), the candidate build axis
...
profile XML and anchored compile-statically in the candidate registry of the cache engine: 30
...
and leaves the rest to the default toolkit; it is the same type that the candidate under test PRT-ART
corresponds to (Section~\ref{sec:prtart-demo}), and full profiles of foreign candidates under test can be integrated
...
4$\times$~YCSB\_B (95/5 read/update); PRT-ART itself is tagged in its candidate profile with YCSB\_F
...
\end{nurlang}
\begin{nurkurz}
Finally, the quality criteria of scientific measurement (cf.\ the definitional classes in
Section~\ref{sec:measurement-system}), too, appear as concrete technologies: \emph{hardware performance
counters} (PMC, via \texttt{perf})~\cite{demelo2010perf} deliver IPC/CPI, cache-miss rates, dTLB misses, and
branch misses; \emph{profilers} such as OTF2/VAMPIR~\cite{berthold2023vampir} and
VTune~\cite{reinders2005vtune} collect trace-based runtime profiles, \emph{simulators} such as
gem5~\cite{binkert2011gem5} provide hardware-accurate estimates where real counters are absent, and the
\emph{HdrHistogram}~\cite{tene_hdrhistogram} collects the latency percentiles. Median and percentiles are
reported instead of arithmetic means, differences are tested with the distribution-free \emph{Mann-Whitney U
test}~\cite{mann1947utest} (Holm correction~\cite{holm1979sequential}) and quantified via
\emph{Cliff's~$\delta$}~\cite{cliff1993dominance}, following~\cite{hoefler2015benchmarking}; a line of its
own is the hardware-near measurement methodology of TU
Dresden~\cite{ungethuem2017survey,schmidt2025hbm,berthold2023vampir}. From all these technologies follows
the set of measurement categories --- cache-line utilization, cache misses (L1/L2/L3), dTLB misses, memory
footprint, branch misses, IPC/CPI, latency (mean and percentiles p50--p999), throughput, energy, and
fill-buffer occupancy ---, whose time- and observer-based subset the present apparatus already resolves,
while the counter-based PMC categories are mandatorily built into the measurement path and bound per
microarchitecture to an event catalogue (Section~\ref{sec:axis-triad}); the complete measurement
architecture is developed by the measurement system (Section~\ref{sec:measurement-system}) and
Chapter~\ref{ch:eval}.
\end{nurkurz}
...
\subsection{Axis Composition and the Living-Being Metaphor}\label{sec:anatomy-metaphor}
The central idea is to treat an algorithm not as a monolith but as a \emph{composition of orthogonal
axes} --- each axis a partial decision of a cache-aware or system-aware index. A concrete algorithm is then
a point in the Cartesian product of all axes, a state-of-the-art design an explicit configuration. The
decisive property is \emph{modular exchangeability}: a new axis variant can be studied in isolation
without reimplementing the others --- this is precisely what solves the separability problem, since the
contribution of each constituent becomes measurable in isolation while the other axes are held fixed; in
the code the reference compositions of ART and PRT-ART differ in exactly one axis, the path compression
(Section~\ref{sec:prtart-demo}). The decomposition is carried by the fact that search algorithms share, in
the large, the same \emph{tree anatomy} --- even a sorted key--value list is a single leaf of a tree ---;
the tree structure is the universal canvas on which the axes are placed.
\end{nurkurz}

\begin{nurlang}
Non-tree-like search methods --- such as the hash counter-check of Section~\ref{sec:sota-instances} ---
remain the same concept with a restricted profile, for a separate concept would arise only if the
...
concept Map and its family \texttt{SearchAlgorithm} (Section~\ref{sec:anatomy-levels}). Second,
...
\begin{nurkurz}
Non-tree-like search methods --- such as the hash counter-check of Section~\ref{sec:sota-instances} ---
remain the same concept with a restricted profile: a hash table, too, is a tree that searches from the root
of its interface over a fan-out of hashes; every method that translates a key into a value belongs to the
concept Map and its family \texttt{SearchAlgorithm} (Section~\ref{sec:anatomy-levels}). A vivid metaphor
treats an algorithm as a \emph{living being} whose \emph{organs} are the axes; it is canonically cast: the
\emph{blood} is the liquid organ that connects all organ axes and is condensed in the planner into a
compile-time configuration; the system axes are the \emph{habitat} of the animal with its resources; the
measurement axes are the equipment of the \emph{laboratory} in which the animal is examined. The
permutation of the axes is, in this reading, an organ exchange that produces new composite algorithms
with unknown cache-line behaviour; the organs themselves are won from the state of the art
(Section~\ref{sec:overview}), whose design building blocks are decomposed into orthogonal axes and
collected in a shared axis matrix. In software-engineering terms an axis is a feature, the \emph{concept}
the feature \emph{core} composition (the shared interface core of all families), the \emph{family} the
feature \emph{special} composition (the real tier interface beyond the core) and the \emph{genus} a
concrete product of that special composition (the implementation with its axis assignment,
Section~\ref{sec:anatomy-levels})~\cite{apel2013feature,batory1992hierarchical}; from here on the thesis
works consistently with this model.
\end{nurkurz}
...
\begin{tikzpicture}[font=\footnotesize, align=center, >=Stealth,
  ...
  \node[habitat,minimum width=8.2cm] (hab) at (2.7,-2.45){\textbf{habitat} (system axes): the planet with its resources --- always present, belonging to no concept};
...
container families of its own, and parts of the axis pool are reused for the containers
...
\end{nurlang}
\begin{nurkurz}
The \emph{anatomy} thus won is more than the enumeration of the organs: it is their \emph{wiring}. Each
organ exports a generalized interface through which the others use its services without knowing its
assignment; the hub is in the code the storage organ \texttt{LayoutAwareChunkedStore}, which composes node
type, layout, and allocator. The usage rules are broadly drawn --- an axis may use foreign axis
interfaces, call itself recursively, and use other families, parts of the axis pool are reused for the
containers (Section~\ref{sec:measurement-system}) ---; in software-engineering terms this is the
\emph{feature interaction} of feature-oriented product lines~\cite{apel2013feature,batory1992hierarchical}.
\end{nurkurz}

\begin{nurlang}
An axis corresponds to a feature, a living being to a feature composition. The \emph{concept} is the
feature \emph{core} composition: the shared interface core that all families of a concept share and
...
container concept this core consists of identity, axis observation, and size. The \emph{family}
(Section~\ref{sec:anatomy-levels}) is the feature \emph{special} composition:
...
a static feature-set definition; the \emph{genus} is a concrete product of that feature set, the
implementation of the family interface with its axis assignment. The anatomy is the wiring between the features. Which
...
beneath that, three interface levels can be distinguished: the \emph{concept} as the outward-visible
measurement surface (the animal class with its \emph{probe dock}), beneath it the \emph{family} as the
tier interface, beneath that the \emph{genus} as the implementation of that interface; the \emph{organs}
form a classification of their own beneath the genus (Figure~\ref{fig:three-levels})\@. The
separation of levels can be read off at the ABI boundary: every tier binary exports two letter-identical
C symbols that answer concept and family without an instance, whereby the concept is not maintained as a second datum
but determined at run time from the family --- the loader thus determines the concept via a
concept-independent symbol, without knowing it in advance\@. \enquote{Living being} and \texttt{SearchAlgorithm} denote the same
...
\begin{tikzpicture}[font=\footnotesize,>=Stealth,align=center,
  ...
  \node[lvl] at (-5.1,0){Level 0: kingdom\\{\tiny Animalia}};
  \node[box] (root) at (1.1,0){\texttt{IExecutionEngine} (measurable root) $\to$ \texttt{IAnatomyBase} (anatomy-bearing)};
  \node[lvl] at (-5.1,-1.6){Level 1: concept\\{\tiny animal class, measurement surface}};
  \node[box] (sa) at (-3.0,-1.6){Map\\{\scriptsize mammal}};
  \node[box] (co) at (0,-1.6){Container\\{\scriptsize bird}};
  \node[box] (gr) at (2.6,-1.6){Graph\\{\scriptsize plant}};
  \node[box] (ha) at (5.3,-1.6){HeuristikAdapter\\{\scriptsize insect}};
  \node[lvl] at (-5.1,-3.3){Level 2: family\\{\tiny tier interface, template}};
  \node[box] (tier) at (-3.0,-3.3){\texttt{SearchAlgorithm}\\{\scriptsize elephant}};
  \node[box] (cont2) at (0,-3.3){\texttt{Set}, \texttt{Sequence},\\\texttt{Adapter}, \texttt{View}};
  \node[box,dashed,fill=gray!8] (gr2) at (2.6,-3.3){no family\\{\scriptsize implemented}};
  \node[box] (ha2) at (5.3,-3.3){\texttt{FunctionInterface}-\\\texttt{Reroute} {\scriptsize(ant)}};
  \node[lvl] at (-5.1,-5.0){Level 3: genus\\{\tiny subspecies, implementation}};
  \node[box,fill=blue!6] (gen) at (-3.0,-5.0){mammoth $=$ ART,\\{\scriptsize African elephant $=$ HOT, \dots}};
  \node[box,dashed,fill=gray!8] (gen2) at (0.3,-5.0){genera per family\\{\scriptsize (design, not in code)}};
  \node[lvl] at (-5.1,-6.6){Level 4: organs\\{\tiny separate classification}};
  \node[box,fill=green!8] (org) at (-3.0,-6.6){axes T0--T17};
  \node[box,fill=green!8] (org2) at (0.3,-6.6){axes of the pool};
  \draw[->] (root)--(sa);\draw[->] (root)--(co);\draw[->] (root)--(gr);\draw[->] (root)--(ha);
  \draw[->] (sa)--(tier);\draw[->] (co)--(cont2);\draw[->,dashed] (gr)--(gr2);\draw[->] (ha)--(ha2);
  \draw[->] (tier)--(gen);\draw[->,dashed] (cont2)--(gen2);
  \draw[->] (gen)--(org);\draw[->,dashed] (gen2)--(org2);
\end{tikzpicture}
\caption{The three-level model: the three interface levels concept (level~1, animal class, measurement surface: probe dock) $\to$ family (level~2, tier interface, template outward) $\to$ genus (level~3, subspecies: implementation of the family interface, possibly with extended special interfaces); beneath them the organs/axes (level~4, a classification of their own), above them the kingdom (level~0). Four concepts: \texttt{Map} (mammal) possesses as its sole family \texttt{SearchAlgorithm} (elephant), whose genera are the concrete implementations (mammoth $=$ ART, African elephant $=$ HOT, Indian elephant $=$ START); \texttt{Container} (bird) the families Set/Sequence/Adapter/View (woodpecker/stork/cockatoo/falcon), which are formed from the same axis pool; \texttt{Graph} (plant) is laid out as an interface, a family is not implemented; \texttt{HeuristikAdapter} (insect) possesses the reroute family \texttt{FunctionInterfaceReroute} (ant) of the hybrid tier. Today the code carries the genus only as an axis assignment; the genus implementation class is an acknowledged architecture error (fix after the defence). Tier binary (level~5a) and hybrid tier (level~5b) are the post-phase of the compilation (Figure~\ref{fig:genera}).}\label{fig:three-levels}
\end{figure}
\end{nurlang}
\begin{nurkurz}
Composing the pairwise \emph{organ couplings} --- an organ coupling is the mutual dependency of two axes
in which the one uses the interface of the other (mandatorily or optionally) or restricts its admissible
assignments; sub-question RQ1.b (Section~\ref{sec:rqs}) addresses it for node type, SIMD, and path
compression --- across all organs together yields a directed inter-organ usage graph with the storage
organ as the only canonical hub.
\subsection{The Three-Level Model and the Four Families}\label{sec:anatomy-levels}
These organs are embedded in \emph{one} architecture --- a single hierarchy without parallel trees. At its
root stands the \texttt{ExecutionEngine} as the common, measurable abstraction (\texttt{IExecutionEngine}
as the measurable root, \texttt{IAnatomyBase} as the anatomy-bearing view beneath it); beneath that lie
three interface levels: the \emph{concept} as the outward-visible measurement surface (the animal class
with its \emph{probe dock}), the \emph{family} as the tier interface, and the \emph{genus} as its
implementation; the \emph{organs} form a classification of their own beneath the genus
(Figure~\ref{fig:three-levels}). The separation of levels can be read off at the ABI boundary: every tier
binary exports two letter-identical C symbols that answer concept and family without an instance, whereby
the concept is determined at run time from the family\@. \enquote{Living being} and \texttt{SearchAlgorithm} denote the
same entity; the anatomy is the \emph{body} of this living being, the fixed composition of its eighteen
axis-organs.
\begin{figure}[htbp]\centering
\begin{tikzpicture}[font=\footnotesize,>=Stealth,align=center,
  box/.style={draw,rounded corners,inner sep=3pt,minimum height=0.7cm},
...
\begin{nurlang}
At the topmost level stand four concepts. The \texttt{Map} concept --- the
key-value-oriented probe dock and the focus of this thesis --- possesses as its sole family
\texttt{SearchAlgorithm}, the full eighteen-axis \enquote{elephant}; the \texttt{Container} concept
(keyless storage) fans out into the families Set, Sequence, Adapter, and View, which are likewise formed from
axes of the shared pool and whose slot count per family is fixed
compile-statically; the \texttt{Graph} concept is laid out as an interface, a family is not implemented; and
the \texttt{HeuristikAdapter} concept possesses the reroute family \texttt{FunctionInterfaceReroute} --- the
family of the hybrid tier (Section~\ref{sec:heuristics-modes}), which by metaprogramming inherits the
interfaces of a concept together with a family and passes requests through to the actual tier binaries;
classification (of which kind the reroute is) and pass-through (the inherited target family that the binary
reports outwardly) lie on separate levels (Figure~\ref{fig:genera}). Mnemonic:
\enquote{Adapter} without prefix always means the container family; the hybrid concept is always called
...
Non-tree-like methods thus require no concept of their own but occupy a restricted
...
\begin{tikzpicture}[font=\footnotesize,>=Stealth,align=center,
  ...
  \node[draw,rounded corners,fill=red!6,font=\footnotesize] (root) at (1.3,1.6){level 0: kingdom \enquote{Animalia} ($\equiv$ \texttt{IAnatomyBase}, generic measurement methods)};
  % Level 1 --- concepts (animal classes, measurement surface)
  \node[gen] (g1) at (-4.0,0){Map\\{\scriptsize\mdseries mammal}};
  \node[gen] (g2) at (0.6,0){Container\\{\scriptsize\mdseries bird}};
  \node[gen] (g3) at (4.3,0){Graph\\{\scriptsize\mdseries plant}};
  \node[gen,minimum width=2.9cm] (g4) at (7.4,0){HeuristikAdapter\\{\scriptsize\mdseries insect}};
  \draw[->] (root)--(g1); \draw[->] (root)--(g2); \draw[->] (root)--(g3); \draw[->] (root)--(g4);
  % Level 2 --- families (tier interfaces)
  \node[sub,fill=green!18] (s1) at (-4.0,-1.4){\textbf{elephant} $=$ \texttt{SearchAlgorithm}\\{\scriptsize \texttt{map} hull $\cdot$ 18 organ axes}};
  \node[sub,anchor=west] (c1) at (-1.2,-1.25){woodpecker $=$ Set {\scriptsize(\texttt{set})}};
  \node[sub,anchor=west] (c2) at (-1.2,-2.05){stork $=$ Sequence {\scriptsize(\texttt{vector})}};
  \node[sub,anchor=west] (c3) at (-1.2,-2.85){cockatoo $=$ Adapter {\scriptsize(\texttt{stack})}};
  \node[sub,anchor=west] (c4) at (-1.2,-3.65){falcon $=$ View {\scriptsize(\texttt{span})}};
  \node[tbd,text width=2.4cm,anchor=north] (gr1) at (4.3,-0.8){interface laid out,\\{\scriptsize no family}\\{\scriptsize implemented}};
  \node[sub,minimum width=2.9cm,text width=2.9cm,anchor=north] (h1) at (7.4,-0.8){\texttt{FunctionInterface}\\\texttt{Reroute} $=$ ant\\{\scriptsize hybrid tier:}\\{\scriptsize inherited target family}};
  \draw[->] (g1)--(s1); \draw[->,dashed] (g3)--(gr1); \draw[->] (g4)--(h1);
  \draw (g2.south) -- ++(0,-0.3) -| (-1.5,-3.65);
  \draw[->] (-1.5,0 |- c1.west) -- (c1.west); \draw[->] (-1.5,0 |- c2.west) -- (c2.west);
  \draw[->] (-1.5,0 |- c3.west) -- (c3.west); \draw[->] (-1.5,0 |- c4.west) -- (c4.west);
  % Levels 3--5 --- elephant branch down to the compiled individual
  \node[band,minimum height=1.5cm] (l3) at (1.3,-5.0){\textbf{Level 3 --- genus (subspecies):} implementation of the family interface, possibly with extended special interfaces --- mammoth $=$ ART, African elephant $=$ HOT, Indian elephant $=$ START, \dots; beneath the genus lie one or more search strategies (simple $=$ assignment of a single axis, complex $=$ composition of several axes). Today the code carries the genus only as an axis assignment; the genus implementation class is an acknowledged architecture error (fix after the defence).};
  \node[band,minimum height=1.5cm] (l4) at (1.3,-6.75){\textbf{Level 4 --- organs:} the 18 organ axes (node type, layout, allocator, prefetch, \dots) form a classification of their own with axis interfaces and \emph{permute}; the genus implementation binds them: a full profile assigns \emph{all} axes (ART, HOT, Masstree, SuRF, START, Wormhole, \dots; 30 full profiles), an abstract profile (PRT-ART, OLC, LOUDS, VAMPIR) only some; the container families are formed from the same axis pool.};
  \node[band,fill=orange!20,very thick,minimum height=1.4cm] (l5) at (1.3,-8.45){\textbf{Level 5a --- tier binary (post-phase of the compilation, the individual):} compiled per permutation to \texttt{comdare\_perm\_\-<fp>.dll}\,/\,\texttt{.so}, refined by the candidate build axis page type; the platform is stamped by the three system axes, the measurement choice by the measurement line; every tier binary offers the three interface surfaces (Figure~\ref{fig:traeger-flaechen}).};
  \node[band,fill=orange!14,minimum height=0.95cm] (l5b) at (1.3,-9.85){\textbf{Level 5b --- hybrid tier:} inserted between the CEB probe dock and the tier binaries as a multiplexer with slots; it inherits the target family and passes requests through to the tier binaries in its slots.};
  \draw[->,thick] (s1.south) -- (s1.south |- l3.north);
  \draw[->,thick] (l3.south) -- (l4.north); \draw[->,thick] (l4.south) -- (l5.north);
  \draw[<->,thick,dashed] (l5.south) -- (l5b.north);
\end{tikzpicture}
\caption[The four concepts and the tier hierarchy down to the compiled binary]{The concept/tier hierarchy across levels~0 to~5. Level~0 is the kingdom \enquote{Animalia} (\texttt{IAnatomyBase}, the generic measurement methods). \texttt{Map}, \texttt{Container}, \texttt{Graph}, and \texttt{HeuristikAdapter} are the four \emph{concepts} (level~1, the animal classes mammal/bird/plant/insect: measurement surface and probe dock). Each concept hosts its \emph{own} families (level~2, the tier interfaces): \texttt{Map} exclusively the \enquote{elephant} (family \texttt{SearchAlgorithm}, full eighteen-axis anatomy), \texttt{Container} the bird families woodpecker/stork/cockatoo/falcon $=$ Set/Sequence/Adapter/View, \texttt{Graph} a laid-out interface without an implemented family, \texttt{HeuristikAdapter} the reroute family \enquote{ant} of the hybrid tier. The elephant branch is drawn out down to the individual: the genera (level~3, subspecies $=$ implementations of the family interface, e.g.\ mammoth $=$ ART, African elephant $=$ HOT, Indian elephant $=$ START) bind the eighteen axis organs (level~4, a classification of their own), which permute into a concrete axis composition; each permutation is compiled into a standalone tier binary (level~5a, the post-phase of the compilation and the individual, additionally refined by the candidate build axis page type and platform-stamped by the three system axes, Section~\ref{sec:axis-triad}); the hybrid tier (level~5b) is inserted between the CEB probe dock and the tier binaries as a multiplexer with slots\@. \emph{Every tier binary is thus an animal of its own with an actual binary family interface and three interface surfaces}; the hierarchy reaches from the kingdom down to the single compiled permutation. Cf.\ the condensed model in Figure~\ref{fig:three-levels}.}\label{fig:genera}
\end{figure}
\end{nurlang}
\begin{nurkurz}
At the topmost level stand four concepts (Figure~\ref{fig:genera}): \texttt{Map} --- the key-value-oriented
probe dock and the focus of this thesis --- possesses as its sole family \texttt{SearchAlgorithm}, the full
eighteen-axis \enquote{elephant}; \texttt{Container} fans out into the families Set, Sequence, Adapter, and
View, which are formed from the same axis pool; \texttt{Graph} is laid out as an interface, a family is not
implemented; \texttt{HeuristikAdapter} possesses the reroute family \texttt{FunctionInterfaceReroute} ---
the family of the hybrid tier (Section~\ref{sec:heuristics-modes}), which by metaprogramming inherits the
interfaces of a concept together with a family and passes requests through to the actual tier binaries.
Mnemonic: \enquote{Adapter} without prefix always means the container family, the hybrid concept is always
called HeuristikAdapter; the kingdom \enquote{Animalia} ($\equiv$ \texttt{IAnatomyBase}) is the
anatomy-bearing view beneath the measurable root \texttt{IExecutionEngine} (Chapter~\ref{ch:impl}).
Non-tree-like methods thus require no concept of their own but occupy a restricted axis subset of the same
family.
\begin{figure}[htbp]\centering
% No \resizebox: the drawing is only ~14cm wide; blowing it up to \linewidth would inflate
% the \scriptsize font beyond the body text. Rule: resizebox only to SHRINK (cf. fig:m-model).
...
measurement task ---, the tier binary, and the hybrid tier binary optionally inserted between the CEB probe
dock and the tier binary. The planner resolves the user XML against the three
...
family interface of a tier binary (runtime, Abstract Factory), surface~2 the stamp (compile time,
ABI-stable, Section~\ref{sec:stamp-model}), surface~3 the measurement cut-through (\texttt{IMessVisitor}), which
crosses the measurement as a seam of its own, so that concept and family interfaces remain unchanged. On tier
...
\begin{tikzpicture}[font=\scriptsize,>=Stealth,align=center,
  ...
  \node[tr] (t0) at (0,0){planner\\{\tiny\mdseries stage 1}};
  \node[tr] (t1) at (2.5,0){CEB\\{\tiny\mdseries stage 2 (probe dock)}};
  \node[tr] (t2) at (7.5,0){tier binary\\{\tiny\mdseries stage 3}};
  \node[tr,dashed] (t3) at (5.0,0){hybrid tier\\{\tiny\mdseries insertion, optional}};
  \draw[->,thick] (t0)--(t1); \draw[->,thick,dashed] (t1)--(t3); \draw[->,thick,dashed] (t3)--(t2);
  \draw[->,thick] (t1.north) -- ++(0,0.32) coordinate (bp) -| (t2.north);
  \node[font=\tiny,anchor=south,text width=10cm] at ([xshift=1.25cm,yshift=1pt]bp){configuration travels: XML $\to$ planner $\to$ CEB $\to$ (hybrid) $\to$ tier (no stage passes through layers)\\dashed: optional insertion --- hybrid as mux switch at the CEB probe dock with dock slots for tier binaries; arc: standard route CEB $\to$ tier without hybrid};
  ...
  \node[v] at (0,-4.17){forbidden}; \node[v] at (2.5,-4.17){forbidden}; \node[v] at (7.5,-4.17){forbidden}; \node[p] at (5.0,-4.17){mandatory (RT hook)};
  ...
  \node[fl] at (7.5,-5.25){1 family interface\\2 stamp\\3 measurement cut-through \texttt{IMessVisitor}};
  \node[fl] at (5.0,-5.25){1 inherited target family\\2 stamp $+$ attached\\3 measurement cut-through};
...
\end{tikzpicture}
\caption{Carrier stages and interface surfaces: the three-valued admission matrix of the seven stamp interfaces over the four carrier stages planner, CacheEngineBuilder (CEB), hybrid tier binary (optional insertion), and tier binary (28 cells; green $=$ mandatory, grey $=$ forbidden; the cell organ line/CEB is the static barrier \enquote{the CEB carries no organ axes}) and beneath it the interface surfaces per stage --- surface~1 family interface (runtime), surface~2 stamp (compile time, ABI-stable), surface~3 measurement cut-through. The configuration travels from the XML via planner and CEB to the tier binary; the hybrid tier binary is a stage optionally inserted between the CEB probe dock and the tier binary (dashed: mux switch at the probe dock with dock slots for tier binaries; the column order of the matrix follows the processing topology with the hybrid at its insertion position, the arc is the standard route without hybrid); no stage passes through layers, each contains its layer compiled in.}\label{fig:traeger-flaechen}
\end{figure}
\end{nurlang}
\begin{nurkurz}
The anatomy is carried by four \emph{carrier stages}: the planner, the \emph{CacheEngineBuilder} (CEB) ---
the measurement tool that the planner generates per target platform and measurement task ---, the tier
binary, and the hybrid tier binary optionally inserted between the CEB probe dock and the tier binary. The
planner resolves the user XML against the three offering catalogues and chooses the measurement axes at its
runtime; the CEB contains this measurement choice compiled in,
knows the platform, and permutes system and organ axes into tier binaries; the tier binary contains the
organ permutation; the hybrid tier binary additionally carries its attached tiers. The chain of releases is
directed, and no stage passes through layers --- each contains its layer compiled in fixedly, only the
configuration travels from the XML to the finished tier binary. The interfaces of the carriers follow a
three-surface model: surface~1 is the family interface of a tier binary (runtime), surface~2 the stamp
(compile time, ABI-stable, Section~\ref{sec:stamp-model}), surface~3 the measurement cut-through
(\texttt{IMessVisitor}), which crosses the measurement as a seam of its own. The contracts are distributed
per stage locally over seven stamp interfaces; the absence of the organ line at the CEB is the static
barrier \enquote{the CEB carries no organ axes} (Figure~\ref{fig:traeger-flaechen}).
\begin{figure}[htbp]\centering
\begin{tikzpicture}[font=\scriptsize,>=Stealth,align=center,
  tr/.style={draw,rounded corners,fill=blue!10,minimum width=2.1cm,minimum height=0.6cm,text width=1.9cm,font=\bfseries\scriptsize},
  p/.style={draw,fill=green!18,minimum width=2.1cm,minimum height=0.48cm,text width=1.95cm,font=\tiny},
...
(Figure~\ref{fig:modi-kette}). There is no debug work mode; the build variants debug, measurement, and release remain in force, and the debug variant is being extended further: \texttt{--debug} is a switch of the planner shell for the wiring check --- a flag and not a work mode of its own in
the chain build/measure/compare/release; it enables the indirect debug mode of the debug variant (the unofficial
fifth maintenance mode): under \texttt{--debug} the conformance gate additionally runs the standard test set of the
tier interface (as planned: tests only with the debug binaries, debug and
release build side by side), and this indirect debug mode coincides with the release build and is not
kept as a separate mode; it stays locked for users (rc~8). Implementation status: the
...
measured values) is envisioned for later main waves but not implemented --- compare and release are
...
\begin{tikzpicture}[font=\footnotesize,>=Stealth,thick,align=center,
  ...
  \node[font=\tiny,align=left,anchor=north west] at (-3.0,-2.95){green: mode selectable, validatable, and effective in the build.\\dashed/orange: mode selectable; the mode-specific procedure (replay comparison,\\generation of the optimal binary) is envisioned but not implemented.\\\texttt{--debug}: not a work mode of its own (flag; debug variant extended).};
\end{tikzpicture}
\caption{The run chain of the four work modes build $\to$ measure $\to$ compare $\to$ release as a registered sub-axis of the measurement realm, cumulative in this order and repeated synchronously on the hybrid level; compare reads the measurement store and computes the best binary or recombination, release generates the fully optimized binary without measurement sensors and re-measures its wall clock. A debug state does not exist; under \texttt{--debug} the conformance gate additionally runs the standard test set of the tier interface, an indirect debug mode that coincides with the release build. The assignment to the operating target pictures measurement, evaluation, working, and hybrid mode is given by Section~\ref{sec:heuristics-modes}.}\label{fig:modi-kette}
\end{figure}
\end{nurlang}
\begin{nurkurz}
Across the carrier stages runs a \emph{run chain} of four work modes, registered in the measurement realm
as a sub-axis and traversed cumulatively in fixed order: \emph{build} builds the binaries and does not
measure; \emph{measure} is the deterministic single-thread measurement run that fills the measurement
store; \emph{compare} computes from the store the tier binary or hybrid recombination that must, according
to the measurement, be the best; \emph{release} generates from that the fully optimized binary without
measurement instrumentation and re-measures its wall clock; on the hybrid level the same chain repeats
(Figure~\ref{fig:modi-kette}). There is no debug work mode; the build variants debug, measurement, and release remain in force, and the debug variant is being extended further: \texttt{--debug} is a flag of the planner shell and not a work mode of its own in the chain build/measure/compare/release; it enables the indirect debug mode of the debug variant (the unofficial fifth maintenance mode): under
\texttt{--debug} the conformance gate additionally runs the standard test set of the tier interface, and this indirect debug mode coincides with the release build (locked for users, rc~8). Implementation status:
the four modes are selectable, validatable, and effective in the
build; the mode-specific procedure of compare and release is envisioned for later main waves but not implemented,
both are labels with release-equivalent build semantics.
\begin{figure}[htbp]\centering
\begin{tikzpicture}[font=\footnotesize,>=Stealth,thick,align=center,
  mode/.style={draw,rounded corners,minimum width=2.7cm,minimum height=1.15cm,text width=2.55cm,font=\scriptsize},
  built/.style={mode,fill=green!12},
...
in comments on ABI adapters of the families, platform probe, code generation, permutation commands,
...
the choice between candidate under test and cache-engine default per axis is made by the merge strategy of the
...
standalone module across the ABI boundary --- thus the candidate under test PRT-ART is compiled, by metaprogramming, in
...
\begin{nurkurz}
\subsection{Design Patterns and Compile-Time Patterns}\label{sec:anatomy-patterns}
The actual contribution lies less in a single structure than in a \emph{tool}: the permutation and
measurement apparatus with which arbitrary axis combinations can be generated and fairly measured.
Scientifically it situates itself in the software engineering of \emph{composition} --- invasive software
composition~\cite{assmann2003invasive}, generative programming~\cite{czarnecki2000generative}, and
feature-oriented product lines~\cite{apel2013feature,batory1992hierarchical}. The axis concept itself
stems from prior work by the author under the Comdare brand, originally for the deduplication
optimization of the UltiHash database; ComdareDB is its successor product as a concept without code, not
a further entity. The contribution of this thesis is the transfer of the principle to cache-aware search
structures as compiled, machine-optimized binaries; in software-engineering terms the apparatus is a
composition of named design patterns and two new compile-time patterns (Figure~\ref{fig:patterns}).
\begin{figure}[htbp]\centering
\begin{tikzpicture}[font=\footnotesize,>=Stealth,align=center]
  \node[draw,rounded corners,fill=green!5,minimum width=12.8cm,minimum height=2.4cm] (classic) at (0,0){};
  \node[font=\bfseries\footnotesize] at (0,0.85){Classical design patterns (GoF) --- used throughout the code};
...
\caption{Design patterns as building blocks of the architecture: above the classical GoF patterns used throughout the code with their respective role, in the middle the two new compile-time patterns named in this thesis, below the code-dissected \emph{three-stage concept chain} per axis (topic concept $\to$ axis concept $\to$ cache-engine permutation concept $\to$ CRTP base $\to$ \texttt{mp\_list} registry) as a refinement of \emph{Strategy}/\emph{CRTP}; a literal \texttt{std::variant} occurs in no organ axis (forbidden) but only in a test stock isolated from the build. Their composition forms the design-pattern-driven, measurement-oriented architecture (Chapter~\ref{ch:impl}).}\label{fig:patterns}
\end{figure}
The classical patterns are the wiring of the apparatus: \emph{Strategy} encapsulates each organ axis as
an interchangeable algorithm, \emph{Director} and \emph{Builder} emit the plan, an \emph{Abstract
Factory} constructs docks and result workbooks at exactly one place, an \emph{Iterator} traverses the
lazily materialized experiment tree, \emph{Adapter} binds foreign code, \emph{Observer} collects the
measurement snapshots, a \emph{Visitor} is the measurement seam at the \texttt{dlopen} transition,
\emph{Chain of Responsibility} forms the hardware collection and selection filter chains, and a
\emph{Memento} secures the state between warm-up and measurement~\cite{gamma1994patterns};
\emph{Singleton}, \emph{Flyweight}, and \emph{Bridge} are not used in the hot path, the measurement hulls
are no \emph{Decorator}. The \emph{Command} pattern fulfils several roles (command base of the axes,
system complex axis, command channel of the hybrid mode, Section~\ref{sec:heuristics-modes}), a
\emph{Facade} bundles appendix generation and reading plan emission (pattern register:
Section~\ref{sec:impl-hybrid-lager}, Appendix~\ref{app:adr}, ADR-11). Static polymorphism is provided by
the \emph{Curiously Recurring Template Pattern} (CRTP) together with \emph{Template-Method} concept
checks; the choice between candidate under test and cache-engine default per axis is made by the merge
strategy of the merge plan (\texttt{Verbund1\_CeOnly}, \texttt{Verbund2\_Replace},
\texttt{Verbund2\_Hybrid}, \texttt{Verbund3\_Union}; Section~\ref{sec:mess-chain}), \emph{policy-based
design} captures the composition as a tuple of axis policies~\cite{alexandrescu2001modern}, and C++23
modules by themselves guarantee no portable ABI stability~\cite{wg21_modules_abi}. Beyond that the thesis
names two constructions without a known scientific designation: the \emph{lazy-sparse Cartesian
materialization}, which counts the combinatorially exploding variant product space of all axis catalogues
only arithmetically and generates each path individually on demand in mixed radix, rather than blowing
up the compiler with the full type tree; and the \emph{type-driven source emission}, which extracts fully
qualified type names from a compile-time axis composition, renders them to C++ source, and loads each
permutation as a standalone module across the ABI boundary --- thus the candidate under test PRT-ART is
compiled in matrix layers against the cache engine. How this \emph{one} architecture becomes visible in
the code is deepened in Chapter~\ref{ch:impl} (Section~\ref{sec:impl-architecture}); before that, the
following subsection orders the apparatus into layers.
\end{nurkurz}
...
standalone binary loaded across the ABI-stable interface (level~5a of the concept hierarchy,
...
\begin{nurkurz}
\subsection{Five Interface Layers: E4 down to E0}\label{sec:anatomy-layers}
The anatomy model orders the \emph{measured}; orthogonal to that, the apparatus needs a second ordering
principle --- the view of the \emph{experiment pipeline} that transforms a declared axis assignment into
measured results. It is organized into five layers E4 down to E0, complemented by the cross-cutting
concern~M (Figure~\ref{fig:e-layers}), counted from the most abstract to the most concrete level:
\emph{E4} is the definition and evaluation layer, the user view --- experiments are described
declaratively as XML profiles, and the same layer receives the measurement results as a return channel;
\emph{E3} is the permutation B\textsuperscript{+}-tree, which unfolds the assignment into the experiment
tree (compile-static axes determine the binary identity, dynamic ones the runtime dimension); \emph{E2}
are the tier binaries, per permutation one binary loaded across the ABI-stable interface
(Figure~\ref{fig:genera}); \emph{E1} is the runtime configuration of the dynamic sub-axis parameters on
the loaded binary; \emph{E0} the infrastructure, whose completion forms the last step. The E identifiers
are a concept ordering, not a code constant. The measurement system permeates as the cross-cutting
concern~\emph{M} all layers without being one itself (Section~\ref{sec:measurement-system}).
\end{nurkurz}
...
(Section~\ref{sec:anatomy-patterns}): just as the candidate under test is compiled in matrix layers against the cache
...
\end{nurlang}
\begin{nurkurz}
Fundamental is the completion principle: each layer is completed \emph{individually}, with a \emph{fixed
interface contract} to its neighbouring level, and the test question per boundary is always whether the
layer passes its definition on \emph{losslessly}; a system whose product space rules out any complete
materialization (Section~\ref{sec:mess-chain}) is too large for simultaneous changes across several
levels. The principle corresponds to the matrix programming style of the apparatus
(Section~\ref{sec:anatomy-patterns}); how each layer is tested against a substitute of its neighbouring
level is shown in Section~\ref{sec:impl-contracts}.
\end{nurkurz}
...
\emph{anatomy view} --- it orders \emph{one} living being from the concept via family and genus to the
...
the same living being that anatomically decomposes into concept, family, genus, and organs is \emph{configured}
...
\end{nurlang}
\begin{nurkurz}
The E-layers are to be distinguished from the three-level model (Section~\ref{sec:anatomy-levels}): the
latter is the \emph{anatomy view} and orders the structure of the measured (concept, family, genus, organs), the
former are the \emph{pipeline view} and order the process of measuring from the definition back to the
evaluation; both are orthogonal to each other, since the same living being is \emph{configured} through
the layers. The following section turns to the question of how the living beings thus composed can be
measured fairly.
\end{nurkurz}
...
compile time versus run time; the family, which occupies only a subset of the axes; the axis algorithms
...
interface: the C++ contract of the ordered map hull \texttt{std::map<Key,\,Value>} with exact lookup,
insertion/update, deletion, and order-based navigation~\cite{iso_cpp}; the axes describe the \emph{inner}
behaviour behind this fixed interface, in parallel \texttt{std::vector} serves as the container comparison
case. Behind that the system distinguishes \emph{levels of dynamics} that determine \emph{when} an axis
decision is made. Therein lies the structure of every axis that sub-question RQ3.b (Section~\ref{sec:rqs})
addresses: the \emph{main axis} is the compile-time-fixed choice --- for organ axes a segment of the
\texttt{binary\_id}, for system axes a stamp entry together with build version tag ---, the
\emph{sub-axes} are its refinement, dynamically as runtime-variable parameters (dynamic dimensions of the
stage \texttt{runtime} in the measurement registry) or statically as compile-time tuples of their main
axis. The idea of regarding data structures as a design space shapes Idreos' \emph{Periodic Table of Data
Structures}~\cite{idreos2018periodic} and the \emph{Data Calculator}~\cite{idreos2018datacalculator} and
coincides with the axis dissection of this thesis (Section~\ref{sec:sota-instances}); against this static
enumeration and classical \emph{policy-based design}~\cite{alexandrescu2001modern} the taxonomy expressly
carries compile- and runtime-variable axis levels together (cf.\ Section~\ref{sec:anatomy-patterns}).
\end{nurkurz}

\begin{nurlang}
The container families are formed from the same axis pool as the search algorithms and are no
...
C++ library this control would be lost. The shared core of the container concept consists of
identity, axis observation, and size (Section~\ref{sec:anatomy-metaphor}); the families Set, Sequence,
...
are bundled by a library of its own over the iterators of the family interfaces, which covers all Cartesian products
of the axes equally at compile time; this family iterator library is not implemented.
\end{nurlang}
\begin{nurkurz}
The container families are formed from the same axis pool as the search algorithms and are no standard
containers --- out of necessity: only thus do the axes permitted in the compiled artefact remain
controllable, does the micro measurement reproduce the same runtime, and do \texttt{SearchAlgorithm} and
container remain comparable over a uniform interface. The shared core of the container concept consists of
identity, axis observation, and size (Section~\ref{sec:anatomy-metaphor}); the families Set, Sequence,
Adapter, and View are implemented; the hulls \texttt{std::map} and \texttt{std::vector} are to be carried
out completely, and the family iterator library for the order-based operations is not implemented.
\end{nurkurz}
...
checks the \texttt{std::map} contract before every measurement, so that only comparable things are compared,
and under \texttt{--debug} additionally runs the standard test set of the respective tier interface: the
family-generic part (valid for all genera of the family) and the genus-specific part (the possibly
extended special interfaces of the genus); and a
...
model assumption ---; implementation status: the platform-calibrated thresholds are not available; their
...
These building blocks are only as meaningful as their measurement. Scientific measurement is subject to
established quality criteria~\cite{hoefler2015benchmarking}: \emph{reproducibility} (fixed seeds,
deterministic order, documented configuration), \emph{validity} (internal, construct, external),
\emph{fairness} (identical keys, workloads, and warm-up; missing capabilities as \emph{N/A} rather than
covertly emulated), and \emph{dispersion} as part of the statement (percentiles instead of means, with
significance testing). Reported percentiles follow the nearest-rank method: the rank
$R(q,n)=\lceil q\cdot n\rceil$ is one-based, the median the special case $q=0.5$, which yields, for even
$n$, the lower middle value. On these the measurement patterns build: a \emph{two-phase operation loop}
separates a discarded warm-up from the measurement in a definitely warm cache, where the parameter
difference between the two phases is itself a source of insight and only rows with a valid second phase
enter the evaluation; the collection is \emph{hybrid} (isolated axis algorithms directly against one
another, a composed living being centrally over an ABI-stable observer interface); a \emph{conformance
gate} checks the \texttt{std::map} contract before every measurement and under \texttt{--debug} additionally
runs the standard test set of the tier interface (family-generic part for all genera, genus-specific part
for the extended special interfaces); and a plausibility criterion
discards every run in which a measurement with measurement sensors is not slower than the same without.
Thresholds are \emph{platform-calibrated} (determined per machine signature from measurement runs); the
values are not available (Section~\ref{sec:answers}). These concepts form the \emph{definitional classes}
of measurement; their technical \emph{instances} have been introduced in Section~\ref{sec:sota-instances}.
\end{nurkurz}
...
family interface functions of the tier binary; \emph{micro} (mi) --- the time and the success parameters per
...
$\to$ family $\to$ category, never via a free name. Three points are not
...
\begin{nurkurz}
\subsection{The Triad: Measurement, System, and Organ Axes}\label{sec:axis-triad}
Within these definitional classes the measurement system answers the categorization question of the
introduction (Section~\ref{sec:rqs}): the axes separate along the three concerns --- the measuring itself,
the build and target platform, the measured --- into \emph{three} categories, which are developed as
standalone \emph{realms} with one class root each, one offering catalogue each, and one stamp line each
(Section~\ref{sec:stamp-model}); binding is the canonical overall order first measurement tooling, then
the three system axes, then the organ slots T0--T17. Two terms are anticipated: every tier binary
possesses an identity \emph{stamp} (one line per realm plus a fingerprint) and lies under this fingerprint
in the \emph{store}, the stock of binaries and measured values from which the apparatus recognizes what
exists instead of building or measuring it anew (Sections~\ref{sec:stamp-model} and~\ref{sec:lager}). The
triad is the second shape of the separability problem: the measured, the platform, and the measurement
instrument, too, must stay separated, so that an effect can be attributed unambiguously; the measurement
axes settle whether a measurement apparatus is to be built into the cache engine and the tier binaries at
all.
\end{nurkurz}
...
build, measure, compare, and release (Section~\ref{sec:anatomy-levels}), where \texttt{--debug} is a flag of the planner shell and not a work mode of its own in the chain
build/measure/compare/release; it enables the indirect debug mode of the debug variant (the unofficial fifth
maintenance mode), which is being extended further: under \texttt{--debug} the conformance gate additionally runs
the standard test set of the tier interface, and this indirect debug mode coincides with the release build; the
build variants debug, measurement, and release remain in force. None of these sub-axes ever
becomes part of a binary identity:
...
\begin{nurkurz}
The \emph{measurement axes} are the concerns of measuring itself --- in the metaphor the equipment of the
laboratory. The measurement realm comprises (a) the load framework (\texttt{load\_framework}) as a
meta-meta axis, which brings in the workload conventions of Section~\ref{sec:workload-basics-known} ---
YCSB as the only enabled building block, the remaining frameworks via the dataset loader
(Section~\ref{sec:sota-instances}); (b) the measurement-tooling main axis with the three measurement
levels wallclock, macro, and micro (Section~\ref{sec:measurement-system}), which alone occupies the
measurement stamp line; (c) the measurement categories as a \emph{sub}-axis of this tooling, which
manifest as columns of the result tables --- the registry carries sixteen, yet with the collection per
axis interface their number grows dynamically and is never a fixed count; and (d) the run methodology as
sub-axis \texttt{work\_mode} with the four modes build, measure, compare, and release
(Section~\ref{sec:anatomy-levels}). Measurement axes are strictly binary-id-neutral. Within the categories
there is a \emph{regime bipartition}: the time- and observer-based regime (nine categories, among them
latency, throughput, memory footprint) and the counter-based PMC regime (seven categories, among them
cache, dTLB, and branch misses, IPC/CPI, energy); the regime is a property of the measurement axis, not of
the measured organ. The observer-based subset is resolved by the apparatus, the PMC categories are
mandatorily built in; not implemented are the collection source per axis-interface parameter set and the
attachment of the P-/E-core measurement to the run profile.
\end{nurkurz}
...
measurement takes place, belonging to no concept --- and an animal is the better, the more precisely it is adapted to
...
\end{nurlang}
\begin{nurkurz}
The \emph{system axes} are the concerns of the build and target platform: canonically \emph{exactly three}
main axes in fixed order --- the target ISA (\texttt{target\_isa}), the operating system
(\texttt{operating\_system}), and the extension hardware (\texttt{external\_utils}), the latter exclusively
the collecting hub of the hardware meta axes (SIMD/vector extensions, GPU/FPGA/NPU, external accelerators)
---, bundled by the command wrapper \texttt{build\_target\_complex} (Section~\ref{sec:complex-axis}). In
the metaphor they are not an organ but the \emph{habitat} of the animal: always present as soon as
measurement takes place, belonging to no concept. From this follow three invariants. First, system axes
never touch the binary identity (\texttt{binary\_id} $=$ \texttt{never}); the effects of the organ axes
must never be blended with those of platform and measurement instruments. Second, they are nevertheless
permutable: the planner releases which system-axis assignments the CacheEngineBuilder builds --- the
release of a target platform for building, which sub-question RQ1.b (Section~\ref{sec:rqs}) presupposes
---, the build space of the CEB permutes system and organ axes together, and the system sub-axes permute
as well, say the scheduling with its five dimensions on the wrapper of the target ISA\@. Third, different
build worlds of the same organ permutation are different artefacts: build control quantities such as
optimization level and SIMD extension appear in the build version tag and in the toolchain member of the
fingerprint (Section~\ref{sec:stamp-model}), never in the organ permutation space.
\end{nurkurz}

\begin{nurlang}
The \emph{organ axes} are the \emph{measured}: organs of the living being, part of the concepts,
...
The \emph{organ axes} are the \emph{measured}: organs of the living being, part of the concepts,
permutable --- every assignment changes the identity of the tier binary. Canonically there are
\emph{eighteen} organ main axes in fixed slot order T0--T17 (Table~\ref{tab:axes-overview}); the
persistence target is the eighteenth, telemetry and target ISA do not belong to the organs. To these is
added the optional \emph{organ meta-meta axis} \texttt{disk\_io} --- \emph{meta-meta} in the sense of the
indirection of a description: an axis that describes how other axes are described. In the four-stage
ordering of levels of this thesis (M0 the binary code, M1 conception and runtime implementation of the
axes, M2 the registry line, M3 the meta-meta axes; Section~\ref{sec:impl-system-axes}) meta-meta is the
topmost stage M3, and in the stamp the meta-meta level of an entry is its bracket depth: a main axis is
bracketless, a meta-meta hangs as a bracket appendage on its realm line
(Section~\ref{sec:stamp-model})\@. \texttt{disk\_io} stamps in the bracket appendage of the organ line,
does not touch the \texttt{binary\_id}, takes effect only with a disk-writeback target, and is disabled;
in the golden reference catalogue its appendix remains empty.
\end{nurkurz}
...
\end{nurlang}
\begin{nurkurz}
Conceptually the triad has two realm roots: the system realm is rooted at the \texttt{CacheEngineBuilder},
which knows the platform and builds the tier binaries, the measurement realm at the experiment
\emph{planner}, which resolves the experiments from the XML (Section~\ref{sec:mess-chain}); each category
has its own class root and its own offering catalogue (Section~\ref{sec:impl-system-axes}). The assignment
of the further quantities is unambiguous: the compiler is a sub-axis group of the build toolchain
\texttt{build\_toolchain} (Section~\ref{sec:complex-axis}), the scheduling a sub-axis on the wrapper of
the target ISA with five dimensions, the load framework belongs to the measurement realm, and the
telemetry is \emph{split in two}, since counter instrumentation must be built in for measuring. This
pattern is called the \emph{two-way-split main axis}: an axis that runs across the runtime of the
preceding stage \emph{and} the compile time of the following stage (planner $\to$ builder $\to$ tier
binary); instances are the load framework, the telemetry, and the hardware detection
(Section~\ref{sec:hw-probe}). System and organ axes are \emph{two-phase} (the planner releases, the
following stage contains compiled in), measurement axes \emph{three-phase}: the planner chooses them, the
CacheEngineBuilder contains the measurement choice compiled in as its provenance, and the tier or hybrid
binary carries the measurement-gate state of its translation unit as a fingerprint member, so that two
compiled artefacts with and without measurement sensors possess different identities; the contract is
local per carrier stage (Figure~\ref{fig:traeger-flaechen}), the coverage CEB off, tier on proven by a
test.
\end{nurkurz}
...
So that the declared platform members do not become silently ageing constants, the
...
meta-meta level, zero the main axis. At depth~0 stand the hardware base flags \texttt{c} $=$ CPU, \texttt{g} $=$ GPU, \texttt{f} $=$ FPGA and
\texttt{n} $=$ NPU, which name the target hardware class, beside the SIMD-width bases \texttt{x128},
\texttt{x256} and \texttt{x512}, the media base \texttt{m64} and companion flags such as \texttt{gfni}. The
bases \texttt{g}, \texttt{f} and \texttt{n} are reserved in the catalogue and are not produced today, because
the cache engine generates CPU code exclusively; every version it generates carries the CPU base \texttt{c}:
an entry with flags but without \texttt{c} (such as \texttt{1.0.0.g}) is not well-formed, the flagless form
\texttt{1.0.0} is well-formed and fails only at the separate CPU-mandatory guard. Beneath \texttt{c} stand as
sub-members the core classes \texttt{p} $=$ performance core and \texttt{e} $=$ efficiency core; \texttt{c}
without a bracket semantically means \texttt{c\{p\}}, yet as stamp strings \texttt{c} and \texttt{c\{p\}}
are not merged, because the stamp is identity and the parser does not normalize; \texttt{c\{e\}} denotes an
efficiency-core algorithm, \texttt{c\{p.e\}} both core classes. The identity of a flag is the pair of token
and parent: \texttt{f} at depth~0 is FPGA, \texttt{f} inside the bracket of \texttt{x512} the AVX-512
foundation subset.
A fourth \emph{realm} line for the mixing strategy of a candidate under test, by contrast, cannot exist: a
candidate under test mixes against the organ axes and is thus already described in the organ line. The mixing becomes
recognizable there in the axis name, which the candidate under test stamps under its own namespace prefix
...
under stamp keys (Figure~\ref{fig:lager-bestaende}). The store tree is organized by concept,
family, and realm --- \texttt{gattung=} $\to$ \texttt{genus=} $\to$ \texttt{realm=}
(\texttt{binaries} or \texttt{messdaten}) ---, and the hybrid concept is sorted into it like every other
...
\end{tikzpicture}
\caption{The store: the store tree is organized by concept, family, and realm; beneath lie four stocks under stamp keys --- stock~1 the tier binaries (SHA-512 fingerprint; the binary with and the one without measurement sensors are two entries, because the measurement-gate state is a fingerprint member), stock~2 the measured values (fingerprint of the binary plus hardware identity of the machine), stock~3 the synthesis batches with the synthesized measurement curves as files, and stock~4 the solutions per user XML\@. The apparatus recognizes what exists via the fingerprint and re-measures only what is missing.}\label{fig:lager-bestaende}
\end{figure}
\end{nurlang}
\begin{nurkurz}
What these measurement patterns collect are three opposing components --- \emph{latency},
\emph{throughput}, and \emph{memory consumption} ---, against which every axis permutation spans as a
trade-off profile (Figure~\ref{fig:measurement-triangle}); with competing target quantities the
\enquote{best} permutation is no single winner but an element of the Pareto
front~\cite{ehrgott2005multicriteria} (Section~\ref{sec:heuristics-loop}). The fourth, central
component --- the cache-aware behaviour at the node, data, and index level --- is collected by
mandatorily built-in hardware performance counters: for AMD Zen~5 (prod1) the event catalogue is
cross-checked, for Intel no entry exists, the last-level counter fails on Zen~5; each gap appears as a
missing value, never as a zero. The separate measurement of the P- and E-cores carries the scheduling
dimension \texttt{hetero\_core\_dispatch} of the target ISA as a six-mode model: four pinning modes per
core kind, the unpinned reference run, and \emph{HybridAware}, which pins its threads at runtime to
cores of both kinds and measures with the counter set of the respective kind; the cache-line-awareness
measurement presupposes HybridAware, the mode is therefore mandatory. Implemented is the pinning
actuator, the attachment of the core modes to the run profile (prod2) is missing.
\begin{figure}[htbp]\centering
\begin{tikzpicture}[font=\footnotesize,>=Stealth,align=center]
  \coordinate (lat) at (0,3.0);
  \coordinate (thr) at (-3.2,-0.9);
...
\end{figure}
\subsection{Complex Axes: the Target ISA as a Fixed Recombination}\label{sec:complex-axis}
To the outside, the three system main axes behave like \emph{one}: a \emph{complex axis} is a bracket
over a fixed recombination of several main axes that appear together as one (in the sense of the
\emph{Command} pattern). The \emph{outer} complex bracket spans
$\texttt{target\_isa} \times \texttt{operating\_system} \times \texttt{external\_utils}$ and contains
as its sub-axis group the build toolchain \texttt{build\_toolchain}; the target ISA is \emph{in itself}
an inner complex axis of the identity members RAM frequency, CAS latency, and CPU fabrication, on whose
wrapper hang the sub-system axes scheduling, NUMA node, and memory page size. The members are
\emph{declared} per machine in the same user XML that also displays the experiment
(Section~\ref{sec:mess-chain}). In the identity stamp (Section~\ref{sec:stamp-model}) a complex axis
occupies exactly \emph{one} field of its type line; the CAS latency is a purely declared member
(provenance level \enquote{declared, not measured}, Section~\ref{sec:hw-probe}).
\subsection{Hardware Detection as a Two-Way-Split Measurement Axis}\label{sec:hw-probe}
So that the declared platform members do not become silently ageing constants, the concept introduces
hardware detection as a further two-way-split main axis --- as a \emph{measurement} axis that measures
properties of the hardware but changes neither system nor organ axes.\ \emph{Level~1} lies with the
planner: at its runtime it detects the cell of ISA vendor family and operating system and selects by
metaprogramming the detection software that is compiled into the \texttt{CacheEngineBuilder}; the
selection matrix is \emph{total}, a catalogue entry without a cell breaks the compilation.\
\emph{Level~2} lies with the builder, which collects the actual properties of the machine and thereby
sets the compile-time stamps of the tier binaries; the stamp of this axis carries only the device
segment of the measurement stamp line (Section~\ref{sec:stamp-model}), never a measured value. The core
topology of hybrid CPUs is carried by the sub-axis \texttt{hetero\_core\_dispatch} of the target ISA
(six-mode model with HybridAware as the mandatory mode, Section~\ref{sec:axis-triad}). Every collection
result carries a \emph{provenance} level together with the collection time, in descending order of
trust: \emph{configured-measured} (privileged system query), \emph{SPD JEDEC base value}
(unprivileged from the module memory, deliberately only the JEDEC nominal rate), and
\emph{declared-not-measured} (user XML); a lower level displaces a higher one only with a marker that
travels along, what is not collected receives n/a instead of a zero (principle of declared
non-observation, Section~\ref{sec:axis-triad}), and four error classes separate missing, unreadable,
corrupt, and unknown-format sources. The gain is the \emph{automated resubmission}: a tool result fixed
once --- thus the RAM nominal rate of 4800\,MT/s recorded for prod2, with \texttt{dmidecode} as the
declaration source --- is replaced by a collection that takes place anew on every run and carries its
trust level along. Provenance vocabulary, SPD parser, error domain, collection chain, and platform
selection matrix of this axis are implemented (Chapter~\ref{ch:impl}).
\subsection{Identity Stamps: One Line per Realm}\label{sec:stamp-model}
The triad becomes readable on the tier binary itself. Its \emph{permutation} identity is the
\texttt{binary\_id}: the path of eighteen \texttt{axis=value} segments in canonical slot order --- one
segment per organ axis, and \emph{only} organ axes. In addition, every tier binary contains compiled-in
\emph{stamp lines}, exactly one per realm: the \emph{organ line} records per main axis the chosen
algorithm together with its algorithm version; the \emph{system line} the three system main axes with
their \emph{build} version (suffix tag of toolchain, optimization level, extension, and further build
control quantities); the \emph{measurement line} the compiled-in measurement toolings as bracketless
segments with the load framework as a bracket appendage at its end (pattern
\texttt{measurement\_tooling=wallclock@1.0.0.c;\allowbreak{}[load\_framework=ycsb@1.0.0.c]}); an empty
measurement line is an explicitly declared statement (no measurement tooling compiled in). The entry
grammar is defined once for all lines: \texttt{axis=algo@X.Y.Z[.flag]*} with recursive composite
brackets --- \texttt{c\{p.e\}} is the base \texttt{c} with the sub-members \texttt{p} and \texttt{e}
and is never read flat as \texttt{cpe}; the bracket depth of an entry is its meta-meta level. At depth~0 stand
the hardware base flags \texttt{c} $=$ CPU, \texttt{g} $=$ GPU, \texttt{f} $=$ FPGA and \texttt{n} $=$ NPU
beside the SIMD-width bases \texttt{x128}/\texttt{x256}/\texttt{x512}, the media base \texttt{m64} and
companion flags such as \texttt{gfni}; only \texttt{c} is produced (\texttt{g}/\texttt{f}/\texttt{n} are
reserved), an entry with flags but without \texttt{c} is not well-formed, the flagless form fails at the
separate CPU-mandatory guard. Beneath \texttt{c} stand the core classes \texttt{p} $=$ performance core and
\texttt{e} $=$ efficiency core (\texttt{c} semantically means \texttt{c\{p\}}, as stamp strings \texttt{c}
and \texttt{c\{p\}} stay distinct); \texttt{c\{e\}} denotes an efficiency-core algorithm, and the identity
of a flag is the pair of token and parent (\texttt{f} beneath \texttt{x512} is the AVX-512 foundation
subset, not FPGA). A fourth realm line for the mixing strategy of a
candidate under test does not exist: the candidate under test stamps under its own namespace prefix in
the organ line (pattern \texttt{prt-art.memory.abc=prt\_patricia@2.3.4.c\{p.e\}}), so that mixing
constellations remain distinguishable by name. The stamp is closed per carrier stage by a
\emph{fingerprint}; the stamp thereby carries, besides the identity, the roles of cache, store, and
recognition key as well as classification. The tier binary carries a SHA-512 over a preimage of eleven
members in fixed order --- format identifier (\texttt{fingerprint\_format=6}), measurement, system, and
organ line, sub-axis value set, toolchain member, enabled sets of the build axes, overlay source hash,
effective measurement-gate state, hybrid composite line, and base of the build version ---, on which
the recognition in the store hangs (Section~\ref{sec:lager}); the planner carries a SHA-256 over
version, target ISA, and operating system, the CacheEngineBuilder a SHA-512 as the provenance of its
measurement choice, which is no reuse criterion. Measured \emph{values} never appear in stamp or
\texttt{binary\_id}: what is stamped is the platform \emph{class}, never a measured value
(Section~\ref{sec:hw-probe}).
\subsection{The Store: Stocks under Stamp Keys}\label{sec:lager}
The \emph{store} is the persistent stock of the apparatus, in which what is built and what is measured
lies under stamp keys (Figure~\ref{fig:lager-bestaende}); the store tree is organized by concept, family,
and realm (\texttt{gattung=} $\to$ \texttt{genus=} $\to$ \texttt{realm=}). Four stocks are fixed
compile-statically in the key schema: stock~1 the tier binaries, keyed by their SHA-512 fingerprint;
stock~2 the measured values, keyed by the fingerprint of the measured binary and the hardware identity
of the measuring machine (a class property of CPU fabrication and RAM pair, never the host name) --- a
measured value never enters its own key; stock~3 the synthesis batches with the synthesized measurement
curves as files; stock~4 the solutions per user XML\@. Measurement is always switched on; in the store
only what is missing there is re-measured, and a permutation is recognized via its fingerprint instead
of being built anew. Because the measurement-gate state is a fingerprint member, per permutation the
binary with and the one without measurement sensors lie side by side as two entries --- the
precondition for re-measuring a release without measurement sensors against the measured version
(Section~\ref{sec:heuristics-modes}). Two consequences: an algorithm version change alters the organ
line and thus the fingerprint and triggers the rebuild of the affected stack together with
re-measurement; and because the hardware identity is part of the measured-value key, every machine
configuration has its own measured-value stock. Implemented are stock~1 and~2 as well as the key schema
of all four stocks; the mechanics of stock~3 and~4, the curve facet, the storage path of the hybrid
tier, and the target selection via the XML form the unimplemented full store build-out.
\begin{figure}[htbp]\centering
\begin{tikzpicture}[font=\scriptsize,>=Stealth,thick,align=center,
  lvl/.style={draw,rounded corners,fill=blue!8,minimum width=2.6cm,minimum height=0.62cm},
  best/.style={draw,rounded corners,minimum width=3.3cm,minimum height=1.6cm,text width=3.1cm,font=\tiny},
...
with the default implementations per axis), and the \texttt{candidate under test} PRT-ART (which supplies some axis-organs
itself and consumes cache-engine services for the rest); cache engine, candidate under test, and this
...
which these four are connected: beneath it, the candidate under test and every engine register as
...
cache-engine\,$\leftrightarrow$\,candidate relationship: the cache engine instantiates the candidate under test per
permutation configuration, and the candidate under test calls cache-engine services (telemetry, prefetch,
heuristics) at runtime during its own lookups. This symmetry founds the \emph{multi-candidate} capability,
with which measurement series~A compares the candidate under test against many SOTA adapters in a single pipeline
...
\begin{figure}[!htbp]
  ...
  \begin{tikzpicture}[comp/.style={draw, rounded corners, align=center, font=\footnotesize, inner sep=4pt, minimum width=3.3cm},
    ...
    \node[comp] (probe) at (2.9,-5.3) {candidate under test (PRT-ART)\\{\scriptsize own axis-organs}};
  ...
  \texttt{CacheEngine} supplies the default axis building blocks, the candidate under test PRT-ART its own organs;
  their bidirectional relationship ((a)~instantiates, (b)~calls services) founds the multi-candidate capability. The
...
default variant per axis, \textbf{Stage~2} (candidate only) replaces the overridden axes by the
candidate variants and otherwise keeps the defaults, and \textbf{Stage~3} (full join, non-redundant)
combines default and all candidate variants; in the code these are the merge strategies of the merge plan
...
\begin{figure}[!htbp]
  ...
  \begin{tikzpicture}[stage/.style={draw, rounded corners, align=center, font=\footnotesize, inner sep=4pt}]
    ...
    \node[stage] (s2) at (0,-1.8) {\textbf{Stage~2}\\\scriptsize candidate only:\\overridden axes\\(Replace) or mixing (Hybrid)};
    \node[stage] (s3) at (4.3,-1.8) {\textbf{Stage~3}\\\scriptsize full join:\\default $\times$ candidate};
...
default binaries of Stage~1 (including the reconstructed SOTA profiles) and the candidate binaries of
...
\begin{table}[!htbp]
  ...
  \begin{tabular}{@{}lll@{}}
    ...
    Stage~2 (candidate only) & overridden axes = candidate variants & A \\
    Stage~3 (full join) & default $\times$ candidate, non-redundant & B \\
...
Section~\ref{sec:axis-triad}), each with its full eighteen-segment path (Section~\ref{sec:stamp-model}),
under the simplifying assumption of exactly two configurations per axis (on/off) over seventeen axes; this
assumption is superseded, the axes carry several configurations, and the catalogue remains the regression
detector of the whole system with a fixed CRC-64 checksum over all identities, not an upper bound (the small
320-identity catalogue, too, is superseded as the measurement canon; the canon is the XML profile via the
planner and Director chain). The upper bound of binary identities per system permutation is not a constant
but the product over the organ axes of the number of configurations selected in the XML experiment (several
per axis, not on/off); the planner determines it from the properties of the XML experiment, it is known only
at execution time. The cardinality of the real permutation space over all axis catalogues including their
sub-axes, by contrast, is computed dynamically by the planner from its inputs (subcommand \texttt{simulate})
as an upper bound known only at execution time --- it lies orders of magnitude above and is never a fixed
count. The planning simulation today computes the product of the XML releases per organ axis times system
permutations arithmetically; the planner extension that correctly captures this upper bound and the batch
size of an experiment as the basis of the load estimate at execution time is planned but outstanding. Three mechanisms dampen the explosion: the \texttt{<expected\_workload>} profile filter routes
...
\begin{nurkurz}
\subsection{Subsystems and the Chain from the XML to the Tier Binary}\label{sec:mess-chain}
The apparatus is organized as \emph{one} main channel: \emph{one} user XML is the only source of the
entire process. It \emph{displays} which experiments, axes, settings, and procedures are to be carried
out, while per realm a \emph{registry} catalogue holds the \emph{offer} of what exists --- the
declarative configuration model of Section~\ref{sec:software-means} modelled on the Java POM\@; an
unregistered reference is a hard validation error. The experiment \emph{planner} resolves the XML
against the three offering catalogues and builds, per target platform and measurement task, \emph{one}
\texttt{CacheEngineBuilder} (CEB), into which the chosen measurement axes are compiled; this one
orchestrates the build of the tier binaries and stamps them with its platform
(Section~\ref{sec:stamp-model}); the tier binary contains the organ permutation. Each stage feeds from
its kind-specific offer (measurement catalogue $\to$ planner, system catalogue $\to$ builder, organ
catalogue $\to$ tier binary), and the tier binary stays readable in two strictly separated layers: the
hardware and environment layer of the system axes and the application-logic layer of the organ axes.
The installation is controlled via the planner shell (implemented) or via API command line with a
headless server; an optional creation mode declares the available XML settings from the detected
hardware; headless service and creation mode are not implemented.

Building on this chain, the measurement system separates four formally separated subsystems: the
\texttt{messung\_driver} (the evaluation orchestrator that runs the pipeline per mandatory measurement
series A/B/C), the \texttt{CacheEngineBuilder} (an autonomous platform-measurement system with the
seven-phase pipeline of Section~\ref{sec:impl-pipeline}), the \texttt{CacheEngine} (the axis
building-block library), and the \texttt{candidate under test} PRT-ART (which supplies some organs
itself and consumes cache-engine services for the rest); cache engine, candidate under test, and this
thesis reside in source repositories of their own, the planner role is realized as a binary of its own
(\texttt{comdare-experiment-planner}), the builder role as the measurement driver
(\texttt{comdare-messung-driver}). The \texttt{ExecutionEngine} is not a fifth subsystem but the shared
measurable root (\texttt{IExecutionEngine}), beneath which candidate under test and engines register.
Defining is the \emph{bidirectional} cache-engine\,$\leftrightarrow$\,candidate relationship: the cache
engine instantiates the candidate under test per permutation configuration, and the candidate under
test calls cache-engine services at runtime; this symmetry founds the \emph{multi-candidate}
capability, with which measurement series~A compares the candidate under test against many SOTA
adapters in one pipeline.

The builder orchestrates a \emph{three-stage evaluation}: \textbf{Stage~1} (cache-engine only) uses the
default variant per axis, \textbf{Stage~2} (candidate only) replaces the overridden axes by the
candidate variants, and \textbf{Stage~3} (full join, non-redundant) combines default and all candidate
variants; in the code these are the merge strategies of the merge plan --- Stage~1 is
\texttt{Verbund1\_CeOnly}, Stage~2 splits into replacing (\texttt{Verbund2\_Replace}) and hybrid mixing
(\texttt{Verbund2\_Hybrid}), Stage~3 is \texttt{Verbund3\_Union}. Each stage can produce thousands of
non-redundant permutation binaries that are loaded at runtime across the C-stable module boundary as
C++23 modules; what is materialized is always a capped subset, the full product space only counted.
...
The three measurement series are thus: \emph{series~A} (PRT-ART vs.\ state of the art) with the modes
A\_defined (fast verification against the 8 Rank-1 full profiles) and A\_full (all 33 profiles; the
standard, since only the full run delivers the bias-reduced measurement of all load profiles against
all methods, Section~\ref{sec:task-concerns}); \emph{series~B} (variation of the axis decisions under
otherwise equal conditions, to isolate the contribution of each axis); and \emph{series~C}
(merge/regression old vs.\ new over concrete merge points, run build-spanning). Each series is
collected in three granularities that correspond to the three measurement levels
(Section~\ref{sec:measurement-system}): \emph{micro-benchmarking} (micro) measures each axis over its
\emph{axis interface} --- the set of functions that \emph{all} algorithms of this axis must supply ---,
\emph{macro-benchmarking} (macro) the individual \texttt{std::map} interface operations against example
loads, and \emph{overall benchmarking} (wallclock, over the load frameworks) the YCSB load profiles over
every recombination --- w/ma/mi, in this order; as a counter-check the apparatus additionally measures
three wallclock levels (axis interfaces, concept interface functions, test load from the load
frameworks). The architectural obligation of this cut: all axis algorithms use exclusively axis
interfaces instead of generic operating-system calls --- compiled in inline, without
\texttt{std::variant} ---, so that the micro measurement of an axis actually measures the axis.
Precisely this methodology is the answer to the separability problem: each constituent is first
measured in isolation over its axis interface before it appears in the operation and overall context;
the contributions remain separately measurable instead of remaining blended in the overall throughput.

The system reduces the space thus spanned deliberately. The golden reference catalogue --- per organ
axis two enabled representatives, the persistence axis pinned to its in-memory representative --- counts
$2^{17} = 131072$ binary identities per system permutation (Section~\ref{sec:axis-triad}), each with its
full eighteen-segment path (Section~\ref{sec:stamp-model}), under the simplifying assumption of exactly two
configurations per axis (on/off) over seventeen axes; this assumption is superseded, the axes carry several
configurations, and the catalogue remains the regression detector with a fixed CRC-64 checksum, not an upper
bound (the 320-identity catalogue is superseded as the measurement canon; the canon is the XML profile via the
planner chain). The upper bound of binary identities per system permutation is the product over the organ
axes of the number of configurations selected in the XML experiment (several per axis); the planner
determines it from the XML experiment, it is known only at execution time. The cardinality of the real
permutation space, by contrast, is computed dynamically by the planner from its inputs (subcommand
\texttt{simulate}) as an upper bound known only at execution time --- it lies orders of magnitude above and
is never a fixed count; the planner extension that correctly captures this upper bound and the batch size of
an experiment as the basis of the load estimate is planned but outstanding. Three mechanisms dampen the explosion: the
\texttt{<expected\_workload>} profile filter (only compatible workloads), the \texttt{Defined} mode
(only the declared tuples), and the \texttt{Full} or \texttt{Full-Sampled} mode (full space or
seed-stable 1:$n$ subset). The default workload is the full permutation over all available workloads;
the routing of this default is not implemented (Section~\ref{sec:eval-workloads}). Full and sampled
runs run on the registered machine signatures prod1 (AMD Zen~5), prod2 (Intel Core i9-12900K, Alder
Lake), and an ODROID board with Intel Gracemont cores; a run on the ZIH cluster of TU Dresden is not
measured (n/a).
\end{nurkurz}

\begin{nurlang}
\section{PRT-ART as Demonstrator}\label{sec:prtart-demo}
PRT-ART is the \emph{candidate under test} on which the load-bearing capacity of the system shows: an abstract living being that supplies some
...
candidate repo is loaded via the plugin controller of the cache engine
...
\texttt{pruefling\_\allowbreak{}slots\_\allowbreak{}v1}, the candidate slots ---, because
presence may only refute, never permit: the deposited evidence headers can only
break the assurance, never open a gate, and a requested but non-declaring candidate under test
breaks the configuration with an error message instead of being skipped silently; a candidate-own
adapter path is deactivated and not part of this assurance.
What PRT-ART contributes lies on three levels. First, the \emph{candidate slots} in its own repository:
...
ART, not a PRT-ART contribution. Third, the codegen-capable candidate composition, which takes sixteen of
eighteen axes from the ART composition and replaces prefetch and value handle by the
candidate slots. Its further own building blocks, the 4+1+2 pool allocator
(free-list sub-axis) and the MultiLevel layout, reside in the candidate repo and run in the measurement path via
...
SOTA building blocks via the permutation configuration. Thus the candidate under test demonstrates that the measurement system
...
\begin{tikzpicture}[font=\footnotesize,align=center,
  ...
  \node[font=\bfseries\footnotesize] at (2.9,2.55){PRT-ART (candidate under test): 18 axis-organs $+$ build axis page type on three levels};
  \node[row] at (-3.0,1.6){candidate slots\\{\tiny (prt-art repo)}};
  ...
  \node[font=\scriptsize,align=center,text width=13.2cm] at (2.0,-1.85){\textcolor{orange}{\textbf{orange}} $=$ candidate slot in its own repository; \textcolor{red!70!black}{\textbf{red}} $=$ sole difference of the ce composition to ART; \textcolor{green!50!black}{\textbf{green}} $=$ inherited from ART or cache-engine default (merge strategy of the merge plan). The codegen-capable candidate composition takes 16 of 18 axes from ArtComposition and replaces prefetch and value handle.};
\end{tikzpicture}
\caption{PRT-ART as a demonstrator in the M-model on three levels: the four candidate slots in its own repository (page type as build axis, prefetch, value handle, telemetry as measurement-tooling sub-axis, Section~\ref{sec:axis-triad}); the reference composition of the cache engine, which differs from ART in exactly one axis --- the path compression as redirect organ ---, while the optimistic lock coupling is inherited from ART\@; and the codegen-capable candidate composition, which takes sixteen of eighteen axes from the ART composition and replaces prefetch and value handle. The further own building blocks (4+1+2 pool allocator, MultiLevel layout) run in the measurement path via the cache-engine default (attachment: subsequent implementation step, not implemented). Thus a complete, measurable living being arises from a few own plus many default organs.}\label{fig:prtart-demo}
\end{figure}
\end{nurlang}
\begin{nurkurz}
\section{PRT-ART as Demonstrator}\label{sec:prtart-demo}
PRT-ART is the \emph{candidate under test} on which the load-bearing capacity of the system shows: an
abstract living being that supplies some axis-organs itself and obtains the rest via the merge strategy
of the merge plan from the default toolkit of the cache engine (Section~\ref{sec:mess-chain}) --- as
monomorphic CRTP concept axes, without runtime fallback; the runtime binding occurs via the
execution-engine adapter. The candidate repo is loaded via the plugin controller of the cache engine:
its entry snippet must expressly give its \emph{assurance} via
\texttt{comdare\_\allowbreak{}pruefling\_\allowbreak{}deklarieren} (the single registered capability:
the candidate slots), because presence may only refute, never permit; a requested but non-declaring
candidate under test breaks the configuration with an error message. What PRT-ART contributes lies on
three levels. First, the \emph{candidate slots} in its own repository: the build axis page type, the
prefetch, the value handle (ChainRef), and the telemetry tooling with the hypothesis metrics H1/H2/H3
(a measurement-tooling sub-axis, Section~\ref{sec:axis-triad}); their assessment is contained in
Chapters~\ref{ch:eval} and~\ref{ch:conclusion}. Second, the reference composition of the cache engine,
\texttt{PrtArtComposition}, which differs from the ART composition in exactly one axis --- the path
compression as redirect organ, the \emph{R} in PRT-ART\@; the optimistic lock coupling is inherited
from ART\@. Third, the codegen-capable candidate composition, which takes sixteen of eighteen axes from
the ART composition and replaces prefetch and value handle by the candidate slots. Its further own
building blocks, the 4+1+2 pool allocator and the MultiLevel layout, run in the measurement path via
the cache-engine default (attachment envisioned, not implemented). Thus the candidate under test
demonstrates that the measurement system holds: a novel living being arises from a few own organs plus
the default toolkit and is measurable over the same probe dock as every reconstructed SOTA method.
\end{nurkurz}
...
\emph{The full loop is an outlook mechanism; realized of it are the automatic selection and the binary
delivery (Section~\ref{sec:contributions}).} The selector reads the measured-value workbook --- the
xlsx workbook is the output of the apparatus, the CSV form its tree extract generated from it
(Section~\ref{sec:eval-dataflow}) ---, ranks per interface function and metric over the valid
two-phase rows (median, nearest rank), determines with competing target quantities the Pareto front,
resolves the \texttt{binary\_id} to the binary in the store, and delivers binary, version sidecar, and
manifest. The heuristic curves arise as natural cubic splines over the sampling points per axis and
$x$-dimension; a decision lambda tree reads the break-even thresholds from the spline intersections and
chooses at a query coordinate the leading binary. Beyond mere ranking, a \emph{heuristic extraction} is
envisioned as a follow-up phase after \textbf{Persist}, the last phase of the seven-phase pipeline
(Section~\ref{sec:impl-pipeline}), as an ML classification in three steps: from the measured-value
workbook, load-profile, dataset, and axis features are extracted per permutation with the measured best
performance as the target quantity; over these an \emph{ML classifier} learns the mapping to the best
configuration; and it delivers per workload/dataset class the recommended axis composition without
re-running the permutation space. The result is persisted as a reusable \emph{XML load profile} and fed
back into the \texttt{<expected\_workload>} filter; this \emph{measure $\to$ profile $\to$ config $\to$
filter} loop is measurement-driven autotuning in the line of OpenTuner~\cite{ansel2014opentuner},
FFTW~\cite{frigo1998fftw}, and ATLAS~\cite{whaley2001atlas}, with the database counterpart in
OtterTune~\cite{vanaken2017ottertune} and the Database Tuning Advisor~\cite{agrawal2005autoadmin}, the
online runtime-near variant in Active Harmony~\cite{tapus2002activeharmony}, and the term
\enquote{software-in-the-loop} from the X-in-the-loop taxonomy~\cite{tibba2016xil}. The ML
classification is not implemented (Chapter~\ref{ch:conclusion}); the loop itself is only the first
stage of the operating target picture, which the following subsection develops.
\end{nurkurz}
...
\subsection{The Four Operating Modes of the Builder}\label{sec:heuristics-modes}
The heuristic loop is the core of an operating regime of \emph{four} operating modes of the builder
interface that build on one another and are to be distinguished from the run chain build $\to$ measure
$\to$ compare $\to$ release (Section~\ref{sec:anatomy-levels}, Figure~\ref{fig:modi-kette}); the
assignment is unambiguous: the \emph{measurement mode} comprises build and measure, the
\emph{evaluation mode} is compare, the \emph{working mode} the release of a single binary, and the
\emph{hybrid mode} the release of a hybrid recombination, whose chain repeats on the hybrid level. The
first mode is the operation of the apparatus; modes two through four are the declared goal of this
thesis and are recorded as a concept --- the core is the measuring through and generating of the tier
binaries together with break-even analysis before every application of the hybrid, and the thesis does
not go in the chain beyond \texttt{hybrid-compare} after \texttt{single-release}. To be distinguished
from these is the \emph{paper isolation mode} asked for in sub-question RQ2.c (Section~\ref{sec:rqs}),
which offers the design constituents of exactly one publication as a closed \emph{axis set} in
isolation, in order to measure them faithfully; it is not implemented and remains outlook
...
as similar as possible, of equal access patterns on the concept and family interfaces of the tier binaries: every
...
the builder runs the measurement series over the admitted permutations; afterwards the framework knows
the properties of all admitted axis algorithms against all load-profile frameworks and workloads. The
\emph{evaluation mode} condenses these measurements into minimal strands per \emph{workload cluster}:
for groups of similar loads, the small set of axis compositions that dominates them remains. Workloads
are clustered according to similar sequences of equal access patterns on the concept and family
interfaces (per profile a directed transition graph of the interface accesses with successor
probabilities, whose clustering at the same time fixes the measurement order). This procedure is not
implemented; it is at the same time the fourth evaluation component of the hybrid tier
...
the deliberate ABI boundary. The probe dock of the concept (Section~\ref{sec:anatomy-levels}) is called in this
...
\end{nurlang}
\begin{nurkurz}
The \emph{working mode} is the production goal: the builder interface keeps the tier binaries relevant
for the currently frequent workload load preloaded in main memory and switches the tier binary with the
specifically optimal axis permutations at the ABI-stable interface --- the same binary-switching
mechanism that the measurement mode uses, only load-driven. The compile-time principle remains
preserved: there is no runtime switch \emph{inside} the tier, switching happens \emph{between}
preloaded binaries at the ABI boundary. The probe dock of the concept
(Section~\ref{sec:anatomy-levels}) is called in this mode the \emph{working dock}: the same interface
now delivers the productive answers. The dock layer of this switch is implemented (dock contracts,
probe dock, dock factory, dock field, parser of the hybrid section of the XML, binary proxy); router,
eviction, and the \texttt{.so} export are not implemented. The hybrid always compiles with the help of
the CacheEngineBuilder, never by itself.
\end{nurkurz}
...
\end{nurlang}
\begin{nurkurz}
On switching from one tier binary to another, every participating tier binary must contain a
compiled-in \emph{state surface}, which reads and re-sets the state in the hybrid mode --- a checkpoint
procedure after the \emph{Memento} pattern~\cite{gamma1994patterns}, which pulls in only the versioned
changed blocks. The state surface is a metaprogrammed sidecar library that occurs only in the hybrid
pattern; therefore two release artefacts are stored, without and with sidecar
(Section~\ref{sec:lager}). State surface and sidecar are not implemented.
\end{nurkurz}

\begin{nurlang}
The \emph{hybrid mode}, finally --- the goal of this thesis --- combines working mode and measurement. Its
centrepiece is that the heuristic adapter is a \emph{concept} of its own with its own family --- the fourth
concept \texttt{HeuristikAdapter} with the reroute family \texttt{FunctionInterfaceReroute}
(Section~\ref{sec:anatomy-levels}): it adopts by metaprogramming the family and
family interfaces of its tier binaries, is itself compiled into a tier binary, and docks at the
...
mandatory symbols as every other and reports as family the inherited target family --- classification
...
Its centrepiece is that the heuristic adapter is a \emph{concept} of its own with its own family, the
fourth concept \texttt{HeuristikAdapter} with the reroute family \texttt{FunctionInterfaceReroute}
(Section~\ref{sec:anatomy-levels}): it adopts by metaprogramming the concept and family interfaces of its
tier binaries, is itself compiled into a tier binary, docks at the probe dock of the builder, and
passes commands via the \emph{Command} pattern~\cite{gamma1994patterns} on to the tier binaries
statically assigned to it; to the outside it is indistinguishable from a single binary, because its
binary exports the same mandatory symbols and reports as family the inherited target family. The
recombination of the heuristic tier binaries is itself permutable against the existing single tier
binaries; the adapter can therefore use the measurement function curves for estimates and compile an
optimal multidimensional recombination --- to the outside a \emph{virtual whole tier binary}. At the
same time the hybrid mode measures along: wall-clock time as well as macro- and micro-benchmarks are
logged per measurement command across all tier binaries.
\end{nurkurz}
...
CacheEngineBuilder, the family interface time of the tier binary (macro), and the axis interface time
...
\end{nurlang}
\begin{nurkurz}
Thereby the hybrid mode delivers the \emph{heuristic counter-proof}: after entry into the working mode
the heuristic tier binary is measured anew --- its wall-clock behaviour against the best single
binaries and against the reconstructed paper algorithms decides whether the heuristic recombination
reaches the performance predicted by the spline curves (Section~\ref{sec:heuristics-loop}); the stage
compare computes from the measurement store the tier binary or recombination that must, according to
the measurement, be the best, the stage release measures the maximally optimized compiled binary
without measurement sensors through once more. The comparison against the paper algorithms presupposes
per publication a wall-clock reference, which is missing as a data source. The goal of the thesis is
thus formulated twice over: to provide the best single binary \emph{and} the heuristic tier binary of
the best mixed cache-aware binaries in production-ready form. All four modes shall be documented
automatically via the evaluation chain of the definition layer (Section~\ref{sec:anatomy-layers}); the
assessment of the counter-proof is defined by Chapter~\ref{ch:eval}.
\subsection{Measurement-Curve Type System, Filter Chain, and Measurement-Error Detection}\label{sec:heuristics-curves}
For the evaluation and the hybrid mode to hold, this thesis formulates as a target picture a
\emph{measurement-curve type system}; implemented of it is a first approach --- a curve fit over one
axis and one $x$-dimension as a natural cubic spline (Section~\ref{sec:heuristics-loop}) ---, while the
type-system key framework $\times$ workload $\times$ operation $\times$ size is not implemented. Every
tier-binary total permutation receives per measurement axis and per measurement level wallclock, macro,
and micro (Section~\ref{sec:measurement-system}) a wall-clock measurement curve over the working set,
separated by load-profile framework, workload type, operation type, and observer type; the synthesized
curves lie as files in the store (Section~\ref{sec:lager}). The cardinality of every dimension is to be
organized into classes --- design-fixed constants, registered, dynamically growing quantities (the
measurement categories, Section~\ref{sec:axis-triad}), profile-bound products, and runtime-free
iteration spaces.

The compare stage consists of five evaluation components that are gradations of a chain: first, the
ranking per metric with Pareto front and delivery (implemented, Section~\ref{sec:heuristics-loop});
second, the spline curves with the break-even decision (first approach implemented); third, the filter
chain over the permutation families (mechanism implemented, evaluation links not); fourth, the live
ranking of the preloaded tier binaries from the recorded interface accesses (not implemented); and
fifth, the measurement-error interpolation over the measurement constellations (not implemented). The
\emph{filter chain} follows the design pattern \emph{Chain of Responsibility}~\cite{gamma1994patterns}:
every chain link checks, based on the curves so far, whether a permutation family is pursued further,
or passes the decision on; it is located in the CacheEngineBuilder, because it controls the next build
round, and filters for the release the binary that is statistically the fastest according to the XML
requirements. Implemented are the mechanism (verdicts Pass, Reject, and pass-on) and a core link for
the resumption of interrupted runs; the evaluation links paper comparison and Pareto dominance follow,
with the precedence of the paper comparison.

Finally, the hybrid mode needs a control of the measurement error that the measurement instances
themselves introduce: the three measurement facilities --- wall clock in the CacheEngineBuilder, family
interface time of the tier binary (macro), and axis interface time (micro) --- can be built in all
arrangements; the number of constellations is dynamic and is never presented statically. From the
differences of all measured values with measurement sensors against the respectively most accurate
remaining total time, a procedure interpolates the measurement error over the constellation matrix and
computes it out --- the fifth evaluation component; three build variants of the same experiment
(observers everywhere, only in the heuristic tier, nowhere) are three of these constellations, and a
fourth step rebuilds the subordinate tier binaries without observers and compares them purely on the
wall clock against all known permutations and the paper algorithms, with precedence of the paper
comparison. The validity criterion of every constellation: the measurement with measurement sensors is
slower than the same without; what is laid is the foundation (measurement gates per compiled artefact
as a fingerprint member, declared non-observation per measured value, Section~\ref{sec:axis-triad}),
interpolation and constellation matrix are not implemented.

Implemented are thus the measurement apparatus with its merge stages, the evaluation chain up to tables
and diagrams, the curve fit together with break-even tree, the Pareto selection with delivery, the
filter-chain mechanism with resumption link, the dock and classification layer of the hybrid tier, and
the measurement gates per compiled artefact; the measurement runs of the complete measurement series
are not available. Not implemented are the evaluation links of the filter chain, router, eviction, and
\texttt{.so} export of the hybrid switch, the state synchronization with sidecar, the workload
clustering, the measurement-error interpolation, the type-system key of the measurement curves, the
collection per axis-interface parameter set, and the attachment of the P-/E-core measurement to the run
profile. The assessment of the heuristic counter-proof is taken up by Chapter~\ref{ch:eval}, the
placement of the outlook by Chapter~\ref{ch:conclusion}; first, Chapter~\ref{ch:impl} deepens the
implementation of the building blocks introduced here.
\end{nurkurz}
