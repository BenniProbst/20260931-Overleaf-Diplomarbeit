...
stands the \emph{concept} as the externally visible measurement surface --- Map, Container, and
Graph (plus HeuristikAdapter for hybrids); beneath it lie \emph{families} as tier interfaces with a
fixed axis set (SearchAlgorithm with its nineteen axes under Map, as well as Set, Sequence, Adapter,
and View under Container) and beneath them the \emph{genera} as their implementations. An \emph{axis} is an orthogonal partial decision --- an ``organ'' --- of an overall
algorithm; the classical families of Sections~\ref{ssec:comparison-trees}--\ref{ssec:flat-exotic}
turn out, against this backdrop, to be mere reading groupings of orthogonal axis assignments, whose
classification as a design space is developed in Section~\ref{ssec:sota-design-space}. As its own
probe this thesis contributes PRT-ART, whose axis contributions are likewise dissected only in
Chapters~\ref{ch:concept} and~\ref{ch:impl}. Thus the external classification
(Sections~\ref{ssec:comparison-trees}--\ref{ssec:container-taxonomy}) and the axis-oriented
dissection view used in this thesis remain cleanly separated.
...
the load profile under which its own algorithm performs best. Only when \emph{all} load profiles run
against \emph{all} living-being binaries (and additionally the container shells) can bias-reduced
comparability be gained; the concrete corpus finding of this bias is presented in
Section~\ref{ssec:sota-wl-bias}, while the ``all-against-all'' matrix itself belongs in the
evaluation methodology (Chapter~\ref{ch:methodology}).
...
\texttt{std::map} contract, so that only comparable things are compared, and under \texttt{--debug}
additionally runs the standard test set of the tier interface (family-generic and genus-specific part);
each configuration is collected in several separate runs with reported
dispersion~\cite{hoefler2015benchmarking}.
