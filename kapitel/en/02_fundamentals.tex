...
owns the page~\cite{drepper2007memory}.
...
and the \texttt{riscv\_hwprobe} interface on RISC-V~\cite{arm_arm,intel_sdm} --- but cannot change
it. The depth of the hierarchy likewise varies: not every platform has an L3 cache (simple RISC-V
cores such as the SiFive~U74 stop at a shared L2), while others bring a large or even asymmetrically
...
the hardware-near works --- \emph{SPEC~CPU}~\cite{spec_cpu2017},
