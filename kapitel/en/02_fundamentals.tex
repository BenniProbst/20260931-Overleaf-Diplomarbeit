...
to 128~bytes~\cite{hennessy2019architecture}. Programs can \emph{read} the actual size at run time
--- via \texttt{CPUID} on x86, the \texttt{CTR\_EL0} register on ARM, and the
...
\subsubsection{B- and B\textsuperscript{+} Trees}
...
hardware-near works --- \emph{SPEC~CPU}~\cite{spec_cpu2017},
\emph{CloudSuite}~\cite{ferdman2012cloudsuite}, and the architecture simulator
\emph{gem5}~\cite{binkert2011gem5}; the allocator corpus is measured with allocator-specific suites
(\texttt{mimalloc-bench}~\cite{mimalloc_bench}, Larson, threadtest, shbench, and LADDIS). Because
these frameworks produce different, mutually incomparable loads, the thesis abstracts them into a
common set of load profiles and runs these as a \emph{dynamic workload axis} over all living-being
binaries (Chapter~\ref{ch:methodology}); the connection of the non-YCSB frameworks occurs via a
dataset loader.
