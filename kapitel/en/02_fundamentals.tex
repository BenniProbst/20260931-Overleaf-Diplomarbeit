...
shell, the different measurement dimensions and methods, the YCSB workload types, and the C++23
...
word first; DDR5 splits the memory channel into two independent 32-bit sub-channels; and
non-temporal or write-combining stores bypass the regular line granularity~\cite{intel_sdm}. An
access that finds its datum in a cache level is cheap; a miss is forwarded to the next, slower
level, and a miss in the last-level cache reaches main memory at a cost of several hundred
...
and the system-level cache~\cite{apple_optimization}. Programs can \emph{read} the actual size at
run time --- via \texttt{CPUID} on x86, the \texttt{CTR\_EL0} register on ARM, and the
\texttt{riscv\_hwprobe} interface on RISC-V~\cite{arm_arm,intel_sdm} --- but cannot change it. The
depth of the hierarchy likewise varies: not every platform has an L3 cache (simple RISC-V cores such
as the SiFive~U74 stop at a shared L2), while others bring a large or even asymmetrically
distributed L3 (e.g.\ the stacked 3D V-Cache of some AMD processors). For a cache-line-aware layout,
the line size and cache depth determined per target platform are therefore an
\emph{input parameter of compilation} rather than a fixed assumption;
Section~\ref{ssec:aware-oblivious} takes this up again for the delimitation of the thesis.
...
run-time-configurable (mutable Linux with MSR access) through boot- or compile-time static
(immutable operating systems) to fixed (read-only cache registers on ARM and RISC-V). Thus
cache-oblivious behaviour is complemented by \emph{measured} locality knowledge that enables a
heuristic, measurement- oriented selection of the settings per load pattern; the \emph{why} of this
construction is developed in Chapter~\ref{ch:concept}.
...
arise the self-balancing \emph{AVL trees}~\cite{adelsonvelsky1962avl} and
\emph{red-black trees}~\cite{bayer1972symmetric,guibas1978redblack}; for block- and
external-memory-oriented indexes, \emph{B-} and
\emph{B\textsuperscript{+} trees}~\cite{bayer1972btree,comer1979btree} became established. Directly
pertinent to main memory is the \emph{T-tree}~\cite{lehman1986ttree}.
...
representation to the actual branching degree and serves this thesis throughout as a reference
design; \emph{fanout} and \emph{node representation} thereby determine both space requirement and
cache behaviour.
...
families of Section~\ref{ssec:genera} turn out to be mere reading groupings of orthogonal axis
...
is internally wired --- an aspect that a dedicated cache-engine developer document deepens.
...
container shells) can absolute, bias-free comparability be gained. The concrete ``all-against-all''
...
TPC-DS)~\cite{tpc_benchmarks}, integration into real systems (e.g.\ SuRF as a Bloom-filter
replacement in RocksDB), real string corpora (URLs, DNA, dictionary terms), and --- for the
hardware-near works --- \emph{SPEC~CPU}~\cite{spec_cpu2017} and
...
shbench). Because these frameworks produce different, mutually incomparable loads, the thesis
abstracts them into a common set of load profiles and runs these as a \emph{dynamic workload axis}
over all living-being binaries (Chapter~\ref{ch:methodology}); the connection of the non-YCSB
frameworks occurs via a dataset loader.
...
documented measurement configurations, so that a run is bit-exactly repeatable. \emph{Validity}
breaks down into internal (causally cleanly isolated effects), construct (the metric actually
measures the intended concept), and external validity (transferability to other platforms and
loads). \emph{Fairness} demands identical inputs --- the same keys, workloads, and warmup --- for
all comparison candidates; missing capabilities of a baseline are reported as \emph{N/A} rather than
...
implementation and a cache-engine default; and C++23 \emph{modules} form an ABI-stable boundary
between the permutation binaries and the builder. Together these facilities realize the
