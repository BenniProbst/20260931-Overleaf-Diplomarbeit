...
path-compressed radix/PATRICIA trees~\cite{morrison1968patricia}\footnote{PATRICIA
(\emph{Practical Algorithm To Retrieve Information Coded in Alphanumeric}): Morrison's path-compressed
binary trie whose inner nodes record only the bit position at which the keys of the respective subtree
first differ~\cite{morrison1968patricia}.}, which collapse sparsely populated
one-child chains,
and the \emph{Adaptive Radix Tree}
(ART)~\cite{leis2013art}, which adapts the node representation to the actual fanout.
ART serves this thesis as the reference design: its design decisions --- the node classes Node4, Node16,
Node48, and Node256 graded by fanout, as well as the linear search within small nodes --- recur in
Chapter~\ref{ch:measurement} as individually measurable partial decisions, whereas the ART tree as a
whole appears as a candidate under test and not as a structure measured alongside throughout.
\end{nurlang}
\begin{nurkurz}
\section{Concept and Overview of the State of the Art}\label{sec:overview}
We first map the landscape of search structures --- comparison-based trees, digital and prefix-based methods,
hashing, spatial and flat structures. The literature refers to the entirety of the best published methods at
any given time as the \emph{state of the art} (SOTA): the published search-structure designs against which a
new design must measure itself. Here it already becomes apparent that every algorithm can be located in a
shared design space; how this design space is systematically spanned is developed in
...
\end{figure}
Search structures can be classified by the mechanism with which they map a key to a
record~\cite{knuth1998taocp3}\@. \emph{Comparison-based} structures presuppose a total order: the self-balancing
AVL trees~\cite{adelsonvelsky1962avl} and red-black trees~\cite{bayer1972symmetric,guibas1978redblack}, the
B/B\textsuperscript{+} trees~\cite{bayer1972btree,comer1979btree} with nodes of cache or memory block
size~\cite{graefe2001btree,hankins2003nodesize}, and the T-tree~\cite{lehman1986ttree}\@. \emph{Digital}
structures branch on the bytes of the key, so that the search-path length depends on the key length rather
than on the number of keys: the \emph{trie}~\cite{delabriandais1959trie,fredkin1960trie}, the path-compressed
radix/PATRICIA trees~\cite{morrison1968patricia}, and the \emph{Adaptive Radix Tree} (ART)~\cite{leis2013art}.
ART serves this thesis as the reference design: its node classes Node4, Node16, Node48, and Node256 graded by
fanout, as well as the linear search within small nodes, recur in Chapter~\ref{ch:measurement} as individually
measurable partial decisions, whereas the ART tree as a whole appears as a candidate under test and not as a
structure measured alongside throughout.
\end{nurkurz}
...
form the non-tree-like comparison class of the
literature\@. \emph{Spatial} structures (k-d tree~\cite{bentley1975kdtree},
...
universe of size~$u$, or the Fusion Tree~\cite{fredman1993fusion} that undercuts the comparison
bound\@. The flat class comprises the randomized skip list~\cite{pugh1990skiplist}, the flat, sorted
array with binary or interpolation search~\cite{knuth1998taocp3} --- strictly speaking the single leaf
...
algorithm engineering~\cite{manzini2002suffixarray}.
\end{nurlang}
\begin{nurkurz}
Beyond the tree-like methods, \emph{hashing} maps keys directly via a hash function and has no inherent order,
which makes range queries expensive~\cite{knuth1998taocp3}; cuckoo hashing~\cite{pagh2004cuckoo} and tables
with SIMD group probing such as SwissTable~\cite{abseil_swisstable} form the non-tree-like comparison class of
the literature\@. \emph{Spatial} structures (k-d tree~\cite{bentley1975kdtree}, quadtree~\cite{finkel1974quadtree},
R-tree~\cite{guttman1984rtree}) and the theoretical \enquote{exotics} van Emde Boas tree~\cite{vanemdeboas1975},
y-fast trie~\cite{willard1983yfast}, and Fusion Tree~\cite{fredman1993fusion} lie outside the scope of this
thesis; the flat class comprises the randomized skip list~\cite{pugh1990skiplist}, the sorted array with binary
or interpolation search~\cite{knuth1998taocp3} --- strictly speaking the single leaf of a tree --- and the
suffix array~\cite{manzini2002suffixarray}.
\end{nurkurz}

\begin{nurlang}
The scope of this thesis is structured along this landscape in three stages. The objects of investigation
are, first, the tree-like structures of the comparison- and digital-based classes
(Figure~\ref{fig:search-map}) and with them the elementary search methods on sorted arrays, of which
tree nodes consist: binary and k-ary search~\cite{knuth1998taocp3,schlegel2009kary}, interpolation
search~\cite{perl1978interpolation}, the Eytzinger
layout~\cite{khuong2017eytzinger}\footnote{Eytzinger layout: the implicit, pointer-free arrangement of a
sorted array in the level order of a complete binary tree --- the children of the element at
position~$i$ lie at positions $2i$ and $2i+1$ ---, so that the search path is computable without pointers
and the memory accesses of the search become predictable for
prefetching~\cite{khuong2017eytzinger}.} and the linear search within small nodes~\cite{leis2013art};
the flat, sorted array thus does not lie outside the scope but is, as the node interior, its smallest
building block. Second, approximate membership filters belong to the scope as upstream building
blocks --- the Bloom filter~\cite{bloom1970}, the cuckoo filter~\cite{fan2014cuckoo}, the xor
filter~\cite{graf2020xor}, and SuRF~\cite{zhang2018surf} as a range-capable filter; cuckoo hashing thus
appears in this thesis not as a hash table but in its filter form. Third, further representatives ---
the skip list, hash search, the binary search tree, the B-tree, the whole trees
ART~\cite{leis2013art}, HOT~\cite{binna2018hot}, START~\cite{fent2020start},
Wormhole~\cite{wu2019wormhole}, and SuRF~\cite{zhang2018surf}, as well as a scalar replica of the
SwissTable --- are available as switchable building blocks that are not activated in the reference run;
the SwissTable replica permits no measurement statement about the SIMD-based original. Outside the
scope remain the spatial structures, the theoretical exotics, and the suffix array. As outwardly
different as the classes appear, they share recurring, mutually separable partial decisions; which
building blocks are actually measured in the reference run is stated in Chapter~\ref{ch:measurement}
together with implementation status and source.
\end{nurlang}
\begin{nurkurz}
The scope of this thesis is structured along this landscape in three stages. The objects of investigation are,
first, the tree-like structures of the comparison- and digital-based classes (Figure~\ref{fig:search-map})
together with the elementary search methods on sorted arrays, of which tree nodes consist: binary and k-ary
search~\cite{knuth1998taocp3,schlegel2009kary}, interpolation search~\cite{perl1978interpolation}, the Eytzinger
layout~\cite{khuong2017eytzinger} as the implicit, pointer-free tree arrangement of a sorted array, and the
linear search within small nodes~\cite{leis2013art}; the flat, sorted array is thus, as the node interior, the
smallest building block. Second, approximate membership filters belong to the scope as upstream building
blocks: the Bloom filter~\cite{bloom1970}, the cuckoo filter~\cite{fan2014cuckoo}, the xor
filter~\cite{graf2020xor}, and the range-capable SuRF~\cite{zhang2018surf}. Third, further representatives ---
skip list, hash search, binary search tree, B-tree, the whole trees ART~\cite{leis2013art},
HOT~\cite{binna2018hot}, START~\cite{fent2020start}, Wormhole~\cite{wu2019wormhole}, and
SuRF~\cite{zhang2018surf}, as well as a scalar SwissTable replica without measurement statement about the
SIMD-based original --- are available as switchable building blocks that are not activated in the reference
run; spatial structures, theoretical exotics, and the suffix array remain outside. As outwardly different as
the classes appear, they share recurring, mutually separable partial decisions; which building blocks are
actually measured in the reference run is stated in Chapter~\ref{ch:measurement}.
\end{nurkurz}
...
common basis against which structurally different methods can be measured uniformly. This thesis rests
on two of them as contract hulls: on the ordered map as the common denominator of comparison- and
digital-based search structures alike and on the \texttt{std::vector} as the sequence view; the hash
table remains, as the unordered counter-check, the conceptual counterpole of the literature, but it is no
contract hull of its own in this thesis --- its building blocks, where present, are subordinate to the
ordered map hull and merely switchable (preceding paragraph). The placement of both hulls in the
conceptual framework of this thesis is developed in Chapter~\ref{ch:measurement}; their complete
elaboration in the implementation is shown in Chapter~\ref{ch:impl}.
...
\end{nurlang}
\begin{nurkurz}
All methods named so far are \emph{statically designed} structures; an established line of research, by
contrast, moves the design into run time: \emph{database cracking}~\cite{idreos2007cracking} builds the index
adaptively as a by-product of query processing, \emph{stochastic cracking}~\cite{halim2012stochastic} makes
this robust against unfavourable query sequences, \emph{learned indexes}~\cite{kraska2018learned} replace the
search structure with a model of the key distribution, and VEGA~\cite{li2025vega} readjusts a learned index at
run time, AirIndex~\cite{chockchowwat2023airindex} designs the index hierarchy from the data and storage-I/O
profile of the target system. For the present thesis these active or adaptive approaches are the closest
related known approaches: they demonstrate that design decisions can be kept variable; how they can be
decomposed systematically and measured individually is developed in Chapter~\ref{ch:measurement}.
\end{nurkurz}
...
blocks through clustering, colouring, and compression~\cite{chilimbi1999layout}.
...
\emph{static and compile-time-relevant} properties that later support the abstraction in
...
(RISC-V\@; restricted, among other things CBO block
sizes)~\cite{arm_arm,intel_sdm,linux_riscv_hwprobe}\footnote{\texttt{CTR\_EL0} is the \emph{Cache Type
Register} of the ARM architecture, a system register that encodes, among other things, the smallest line
size of the instruction and the data caches~\cite{arm_arm}; \texttt{riscv\_hwprobe} is the Linux system
call through which a RISC-V program queries properties of the running core, among them the block sizes
of the cache-block operations (CBO) for zeroing, managing, and prefetching cache
blocks~\cite{linux_riscv_hwprobe}.} ---,
but not change it; the depth of the hierarchy also varies (simple
...
the line size determined per target platform is therefore an \emph{input parameter of compilation}, not
a fixed assumption; how this input parameter is bound in this thesis is shown in
Chapter~\ref{ch:measurement}. Moreover, the transfer of a line does not proceed strictly block-at-once: \emph{critical-word-first} delivers
...
\begin{nurkurz}
\section{Hardware: Historical Adaptation to Search Performance}\label{sec:hardware-history}
The performance of search structures has always been driven by hardware; we trace the \emph{static and
compile-time-relevant} properties of cache-near hardware that later support the abstraction in
Section~\ref{sec:software-means}.
\begin{figure}[htbp]\centering
\begin{tikzpicture}[font=\footnotesize, align=center]
  \foreach \i in {0,...,7} { \draw (\i*0.55,0) rectangle (\i*0.55+0.55,0.55); }
...
\caption{Cache-line utilization: a cache line is always loaded in full; if only a few of its bytes are used, effective bandwidth drops --- cache-aware layout raises utilization.}\label{fig:cache-line}
\end{figure}
Modern processors interpose several cache levels (L1, L2, mostly L3) between the registers and main
memory~\cite{hennessy2019architecture}; a miss down to main memory costs several hundred
cycles~\cite{drepper2007memory}. The transfer unit between the levels is the \emph{cache line}
(Figure~\ref{fig:cache-line}); its size is hard-wired per microarchitecture --- 64\,bytes on x86-64 and common
ARM cores, 128\,bytes on Apple Silicon, for instance~\cite{apple_optimization}, 32\,bytes on older or embedded
cores~\cite{hennessy2019architecture,intel_optimization,amd_sog} ---, \emph{readable} at runtime
(\texttt{CPUID}, \texttt{CTR\_EL0}, \texttt{riscv\_hwprobe})~\cite{arm_arm,intel_sdm,linux_riscv_hwprobe}, but
not changeable. For a cache-line-aware layout, the line size determined per target platform is therefore an
\emph{input parameter of compilation}, not a fixed assumption; how it is bound in this thesis is shown in
Chapter~\ref{ch:measurement}. Moreover, the transfer of a line does not proceed strictly at once
(\emph{critical-word-first}, the DDR5 split of the channel into two independent 32-bit
sub-channels~\cite{micron_ddr5}, write-combining~\cite{intel_sdm}).
\end{nurkurz}
...
systems~\cite{hennessy2019architecture}. The classification guides action, since it
...
\begin{nurkurz}
\subsection{Cache Organization: Associativity, Miss Classes, and Parallelism}\label{subsec:cache-organisation}
Where a line may reside in the cache is determined by \emph{associativity}: the cache is organized into
\emph{sets}, each holding $n$ \emph{ways} (line slots), and each address may reside only in the $n$ slots of
\emph{its} set; common data caches are 4- to 16-way
associative~\cite{hennessy2019architecture,intel_optimization,amd_sog}. Hill and Smith classify misses in the
\emph{3-C model} as \emph{compulsory} (first access), \emph{capacity} (working set exceeds the capacity), and
\emph{conflict} (more hot lines per set than ways)~\cite{hill1989associativity}; multi-core systems know
\emph{coherence} misses as a fourth class~\cite{hennessy2019architecture}. The classification separates the
countermeasures: prefetching against compulsory misses (Section~\ref{subsec:prefetching}), more compact
layouts against capacity misses; conflict misses arise when hot elements at power-of-two distances compete for
the same sets~\cite{hennessy2019architecture,drepper2007memory}.
\end{nurkurz}
...
\end{nurlang}
\begin{nurkurz}
A miss does not block a modern cache: since Kroft's \emph{lockup-free} organization, the cache keeps one
\emph{miss status holding register} per outstanding miss (MSHR\@; realized at Intel as \emph{line fill
buffers} between L1 and L2)~\cite{kroft1981lockupfree}; the fixed number of these registers limits the memory
accesses in flight at once --- the \emph{memory-level parallelism}
(MLP)~\cite{hennessy2019architecture,intel_optimization}. Whether a search structure can offer it is a design
question, since the search path of a tree is a chain of mutually dependent accesses
(Section~\ref{subsec:prefetching}).
\end{nurkurz}
...
the column-oriented struct-of-arrays, the bit-packed, succinct layout as used by LOUDS-based
tries~\cite{jacobson1989louds} and SuRF~\cite{zhang2018surf}, and the SIMD-tiled
AoSoA\footnote{AoSoA (\emph{Array of Structures of Arrays}): a hybrid layout that groups the records into
tiles of fixed width and stores them field-wise within each tile, so that a SIMD register loads exactly
one tile column; it combines the access locality of the array-of-structs with the vectorizability of the
struct-of-arrays.}. The multi-level hierarchy itself is moreover the subject of software/hardware-cooperative
...
\begin{nurkurz}
\subsection{Cache-Line-Oriented Layout Research}\label{subsec:layout-research}
The cache-line-oriented layout is the guiding idea of the relevant research. As layout options, the literature
knows the dense and the cache-line-aligned array-of-structs (AoS), the column-oriented struct-of-arrays (SoA),
the bit-packed, succinct LOUDS layout~\cite{jacobson1989louds,zhang2018surf}, and the SIMD-tiled AoSoA
(\emph{Array of Structures of Arrays}: records in tiles of fixed width, field-wise within each tile, so that a
SIMD register loads exactly one tile column --- the access locality of the AoS with the vectorizability of the
SoA); the reference designs CSS/CSB\textsuperscript{+}~\cite{rao1999css,rao2000csb} (cache-line node, sibling
cluster) and ART~\cite{leis2013art} (nodes graded by fanout) lay out the node cache-line-orientedly
(Section~\ref{subsec:cc-structures}). Schmidt et al.~\cite{schmidt2025hbm} measure stride versus sequential
accesses on high-bandwidth memory (HBM).
\end{nurkurz}
...
\end{nurlang}
\begin{nurkurz}
In addition, the architectures provide \emph{software} prefetch instructions (x86: the \texttt{PREFETCHh}
family; ARM\@: \texttt{PRFM}) with which the program itself announces an address together with a target
level~\cite{intel_sdm,arm_arm}; their tuning knob, the \emph{prefetch distance}, must hide the memory latency
without losing the fetched line before use and is thus to be calibrated per
platform~\cite{mittal2016prefetching}. Chen et al.\ justify wider B\textsuperscript{+} nodes precisely for this
reason, whose lines are announced in parallel~\cite{chen2001prefetch}; fractal prefetching transfers the
principle to disk pages~\cite{chen2002fractal}.
\end{nurkurz}
...
Behind the last cache lies the DRAM main memory, whose characteristics for the DDR5 generation are
fixed by the JEDEC standard JESD79-5~\cite{jedec_jesd79_5}. Two quantities are frequently
...
a page is placed on the node of the thread that first writes it~\cite{lameter2013numa}. For
...
which enlarge the TLB reach, are supported by several allocators --- mimalloc~\cite{leijen2019mimalloc},
jemalloc~\cite{evans2006jemalloc}, TCMalloc~\cite{hunter2021temeraire}, and
snmalloc~\cite{lietar2019snmalloc} --- and which Schmidt et al.~\cite{schmidt2025hbm} use empirically
with 2-MiB pages; these four allocators are available in this thesis as interchangeable allocator
building blocks, and which allocator building blocks are activated in the reference run is stated in
Chapter~\ref{ch:measurement}. A standalone dTLB optimization nevertheless remains underrepresented in
the search-algorithm publications, an observation to which the research gap in
Chapter~\ref{ch:measurement} (Section~\ref{sec:task-concerns}) returns.
\end{nurlang}
\begin{nurkurz}
With \emph{NUMA} (non-uniform memory access), each node owns local memory, remote accesses cost noticeably
more, and the \emph{first-touch} policy places a page on the node of the thread that first writes
it~\cite{lameter2013numa}. The \emph{translation lookaside buffer} (TLB) caches virtual-to-physical mappings,
a TLB miss triggers an expensive page walk, and large pages enlarge its \emph{reach} (entry count times page
size)~\cite{hennessy2019architecture}; for search structures the data TLB (dTLB) is the second locality
boundary alongside the cache line.
\end{nurkurz}
...
\subsection{Write Path, Replacement, and Coherence}\label{subsec:write-coherence}
The write and update behaviour of the caches, too, is largely microarchitecturally fixed: writes follow
\emph{write-through} (writing through) or \emph{write-back} (writing back the line marked \emph{dirty} only
upon eviction), and the allocation policy (write-allocate or no-write-allocate) governs the write
miss~\cite{arm_arm,intel_sdm}. Which line is evicted is decided by a heuristic replacement strategy from
LRU/pseudo-LRU to the scan- and thrash-resistant \emph{re-reference interval prediction} (RRIP, dynamic
variant DRRIP)~\cite{jaleel2010rrip} on the basis of the adaptive insertion policy~\cite{qureshi2007adaptive};
the inclusion policy governs whether a line resides in several levels, and in multi-core systems an
invalidation-based coherence protocol secures consistency: MESI (states \emph{Modified}, \emph{Exclusive},
\emph{Shared}, \emph{Invalid}), extended by vendors to MOESI (AMD~\cite{amd_sog}) or MESIF
(Intel~\cite{intel_optimization})~\cite{hennessy2019architecture}. Their effect --- in particular the
\emph{cache-line ping-pong} under competing writes to the same line --- must be made visible by a fair
measurement procedure (Chapter~\ref{ch:measurement}).
\end{nurkurz}
...
Two observations on the table lead further. First, every design binds only \emph{few}
...
\begin{nurkurz}
\subsection{Cache-Aware and Cache-Oblivious Data Structures}\label{subsec:cc-structures}
With the terminological taxonomy from Section~\ref{sec:overview} and the parameters now introduced, the state
of the art of cache-near data structures can be ordered. The empirical impulse of the cache-aware line was
given by Ailamaki et al., whose time breakdown of commercial database systems found around half of the
execution time in memory stall cycles~\cite{ailamaki1999dbms}. The answers of the following years each bind
part of the parameters from the previous sections explicitly into the design: the cache-line size
(CSS/CSB\textsuperscript{+} tree~\cite{rao1999css,rao2000csb}), the prefetch distance together with multi-line
nodes (pB\textsuperscript{+} tree~\cite{chen2001prefetch}, fractal prefetching~\cite{chen2002fractal}), page
size and SIMD width at once (FAST~\cite{kim2010fast}), or the fanout per cache line (ART~\cite{leis2013art});
the layout techniques of clustering and compression go back to Chilimbi et al.~\cite{chilimbi1999layout}. The
cache-oblivious counter-position is instantiated as well: Bender et al.\ ground the cache-oblivious B-tree on
the van Emde Boas layout~\cite{bender2005coboblivious} and a layout of a tree of fixed topology that minimizes
the block transfers nearly optimally even under unknown block
size~\cite{bender2002treelayout,bender2000cobtree}; Saikkonen and Soisalon-Soininen maintain a
cache-sensitive layout invariant even under updates~\cite{saikkonen2008multilevel,saikkonen2016layout}, and
Graefe/Larson~\cite{graefe2001btree} compare both approaches across the CPU cache levels.

Two observations on these designs lead further: first, every design binds only \emph{few} parameters firmly,
none treats the parameter set as an open input space; second, both philosophies leave open how
\emph{measured} locality knowledge of the target platform --- whose cache properties are predominantly
hard-wired (Section~\ref{subsec:write-coherence}) --- can be combined systematically with the cache-near
settings that are actually selectable. This question is taken up in Chapter~\ref{ch:measurement}
(Section~\ref{sec:task-concerns}).
\end{nurkurz}
...
interface --- an exact lookup (also called a point query: the search for exactly one key), an
insertion, a deletion, or a range scan. A \emph{workload} (in this
...
extrapolation, not a count. A reliable comparison therefore needs repetitions and an explicitly
reported dispersion figure~\cite{hoefler2015benchmarking,weaver2008counters}. Which of these metrics
...
Pattern} (CRTP)~\cite{coplien1995crtp,vandevoorde2017templates}\footnote{CRTP\@: a class derives from a
class template to which it passes itself as the template argument (\texttt{class Derived: Base<Derived>});
the base can thereby access members of the derived class already at compile time and replaces virtual
calls with statically resolved ones --- without run-time indirection and without a table of virtual
functions. The pattern was named and described by Coplien in 1995 in the
\emph{C++ Report}~\cite{coplien1995crtp}.} together with \emph{policy-based
design}~\cite{alexandrescu2001modern}, and the composition from
...
exchanged; how this thesis instead keeps the binary interface of its pre-compiled building blocks stable
is shown in Chapter~\ref{ch:impl}.
...
The established answer is \emph{declarative configuration models}: feature models have, since the
FODA study\footnote{FODA (\emph{Feature-Oriented Domain Analysis}): the domain-analysis method of the
Software Engineering Institute, which captures the common and the variable characteristics
(\emph{features}) of a product concept in a feature tree with mandatory, optional, and alternative nodes
together with selection constraints~\cite{kang1990foda}.}, described configurable variant spaces as
feature hierarchies with selection constraints~\cite{kang1990foda}, generative programming produces the system variant in the solution
...
functions. The pattern was named and described by Coplien in 1995 in the
\emph{C++ Report}~\cite{coplien1995crtp}.} together with \emph{policy-based
design}~\cite{alexandrescu2001modern}, and the composition from interchangeable building blocks (invasive
software composition from gray-box components~\cite{assmann2003invasive}, generative
programming~\cite{czarnecki2000generative}, feature-oriented software product
lines~\cite{apel2013feature,batory1992hierarchical}); C++23 modules structure such compositions but guarantee
no portable ABI stability~\cite{wg21_modules_abi}; how this thesis instead keeps the binary interface of
pre-compiled building blocks stable is shown in Chapter~\ref{ch:impl}.

For interchangeable building blocks to become a concrete system, a \emph{form of description} of the chosen
configuration is needed --- \emph{declarative configuration models}: feature models~\cite{kang1990foda}
describe variant spaces as feature hierarchies with selection constraints, generative programming produces
the system variant from them~\cite{czarnecki2000generative}, and the product-line literature systematizes the
binding times from compile time to run time~\cite{apel2013feature}. Tooling follows the same principle:
Maven's \emph{Project Object Model} (POM) declaratively describes \emph{what} makes up a modular Java
system~\cite{maven_pom}, CMake produces a native buildsystem from one declarative description via
interchangeable \emph{generators}~\cite{cmake_manual}, and Mokhov et al.\ capture build systems as a
combination of a few scheduler and rebuilder building blocks over declarative
dependencies~\cite{mokhov2018build}; that declarative models also accompany a system in operation is shown by
models@run.time~\cite{blair2009modelsruntime} and G\"{o}tz's MQuAT dissertation on model-driven
self-optimization of component-based systems~\cite{goetz2013mqat}.

For this thesis, both strands form the toolbox: composition techniques turn measured hardware properties into
a concrete layout at compile time, declarative configuration models describe the chosen configuration
reproducibly; their combination is developed in Chapter~\ref{ch:measurement}.
\end{nurkurz}
...
that derives the respective best configuration from the measurement results --- is the objective and
...
\emph{measurement-driven} (equivalently \emph{measured-value-driven}) emphasizes the data source --- the decision rests on collected measurements
