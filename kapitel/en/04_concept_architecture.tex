...
% (8 occurrences); the active text uses eighteen organ axes (T0--T17).
...
appropriation per composition axis, ordered by axis (T0--T18); the tag (A--F) in the axis column
names the origin cluster (Section~\ref{ssec:sota-corpus}). The per-axis contributing sources are listed in
...
\caption{Dialectical appropriation of the state-of-the-art instances per composition axis (ordered by axis T0--T18; cluster tags: A~trie/node, B~hybrid/layout, C~allocation, D~prefetching, E~synchronization, F~hardware/measurement/engineering)}\label{tab:dialectic}\\
\toprule
...
\bottomrule
\endlastfoot%
T0 search-algorithm (A) & ART, HOT, START, SuRF, Wormhole; k-ary/interpolation search & monolith \enquote{structure is method} & search paradigm as permutable axis \\
...
T2 mapping (B) & linear 1:1 mapping; hash redirect, permutation index & --- & direct vs.\ pool-relative mapping explicit \\
...
T6 allocator (C) & A01--A23 (Hoard, mimalloc, jemalloc, TCMalloc, \ldots) + RCU/hazard & algorithm-hard-wired allocator & allocation as shared interface organ (seven sub-families) \\
...
T8 concurrency (E) & OLC (ART), RCU, hazard pointer; lock-/wait-free & coarse locking as the only assumption & pattern $\perp$ reclamation as two sub-axes \\
T9 serialization (F) & raw-binary, varint (ART), succinct (LOUDS/FST), LZ77 & --- & encoding/compression selectable \\
T10 telemetry (F) & density tracker, insert counter, HdrHistogram, path counter & all-node counters (cache-line ping-pong) & coherence-friendly leaf-only telemetry \\
T11 value-handle (F) & inline, external pool, immutable-shared (RCU), versioned & implicitly hard-embedded value & placement, ownership, versioning (+ chain-ref) \\
T12 ISA (F) & vendor spec x86-64/AArch64/Power/RISC-V (+ SIMD) & --- & target ISA as compile-time axis \\
T13 index-organization (B) & clustered, IOT, non-clustered, heap & --- & storage order as its own axis \\
T14 I/O-dispatch (F) & buffered, mmap, direct-IO & mmap as anti-pattern & persistence decoupled from search structure \\
T15 migration (F) & hot/cold, multi-tier NVM, LeCaR-ML & tiering never combined with search axes & migration logic as measurable axis \\
T16 filter (F) & Bloom, Cuckoo, SuRF range, Xor & filter never integrated with search axes & filter as orthogonal organ \\
T17 queuing-q1 (F) & disruptor ring, delta-chain, CoW, epoch-RCU, SPSC/MPMC & operation buffer never as an axis & buffer with access-pattern classes \\
...
one entity. The organism is addressed from outside through its genus interface (the probe dock, a
...
There is thus a single chain --- root (\texttt{ExecutionEngine}) through the genus probe dock down
...
externally visible algorithm interface (the probe dock): \texttt{SearchAlgorithm} (the key--value
