...
Before the one architecture (Section~\ref{sec:three-layer}) is developed, this thesis appropriates
the inventory catalogued in Chapter~\ref{ch:sota} \emph{dialectically}: each instance measured there
(\emph{thesis}) meets its limits in the bias-reduced cross-comparison (\emph{antithesis}); from this
follows, per axis, the decision to \emph{adopt} an organ or to \emph{replace} an obsolete
assumption, from which the thesis's own contribution emerges as \emph{synthesis}. The common thread
of this synthesis is a \enquote{third way} between hard-wired cache-awareness and cache-oblivious
ignorance: \emph{measured} locality knowledge that makes each axis explicit as an orthogonal,
permutable, and individually measurable organ. Table~\ref{tab:dialectic} summarizes this
...
\begin{figure}[!htbp]
  ...
  \begin{tikzpicture}[box/.style={draw, rounded corners, align=center, font=\footnotesize, inner sep=4pt}]
    ...
    \node[box] (ana) at (-3.4,-1.9) {\texttt{IAnatomyBase}\\{\scriptsize living organism}};
    \node[box] (vir) at (3.4,-1.9) {\texttt{IVirusExecutionEngine}\\{\scriptsize virus (axis-less, graph)}};
    \node[box] (sa) at (-3.4,-3.8) {SearchAlgorithm subclass\\{\scriptsize $\equiv$ \enquote{organism}}};
    \node[box] (body) at (-5.9,-5.7) {\texttt{SearchAlgorithmAnatomy<C>}\\{\scriptsize \emph{body}: 19 organs}};
    \node[box] (abi) at (-0.3,-5.7) {\texttt{SearchAlgorithmAbiAdapter<A>}\\{\scriptsize ABI view (\enquote{SearchEngine})}};
    \draw[->] (root) -- (ana);
    \draw[->] (root) -- (vir);
    \draw[->] (ana) -- (sa);
    \draw[->] (sa) -- (body);
    \draw[->] (sa) -- (abi);
  \end{tikzpicture}}
  \caption{The \emph{one} architecture as a single hierarchy with no parallel trees:
  \texttt{SearchAlgorithm} \emph{is} the organism, its anatomy is its \emph{body}, and
  \enquote{SearchEngine} is only its ABI runtime view.}\label{fig:one-architecture}
\end{figure}
...
dozen); the Cartesian product of the main axes spans around $10^{14}$ configurations
...
non-redundant permutation binaries, each loaded at run time as an ABI-stable C++23 module (Figure~\ref{fig:three-stage}).
...
\begin{figure}[!htbp]
  ...
  \begin{tikzpicture}[stage/.style={draw, rounded corners, align=center, font=\footnotesize, inner sep=4pt}]
    ...
    \node[draw, rounded corners, align=center, font=\footnotesize, inner sep=4pt] (out) at (0,-3.7) {thousands of non-redundant permutation binaries (ABI modules)};
    \draw[->] (b) -- (s1);
    \draw[->] (b) -- (s2);
    \draw[->] (b) -- (s3);
    \draw[->] (s1) -- (out);
...
\begin{table}[!htbp]
  ...
  mandatory measurement series~A/B/C; series~C is cross-stage.\par}
\end{table}
