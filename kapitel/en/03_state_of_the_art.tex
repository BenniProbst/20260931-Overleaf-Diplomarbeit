...
the SearchAlgorithm genus: it satisfies the same \texttt{std::map} interface over a
\emph{limited axis subset} --- the trie-specific axes (path compression, node type, range index)
collapse to pass-through variants, while allocation, prefetch, concurrency, memory layout, and ISA
SIMD (for the SwissTable group lookup) remain shared --- and thus requires no genus of its own.
...
Hoard~\cite{berger2000hoard} and Slab~\cite{bonwick1994slab} through
mimalloc~\cite{leijen2019mimalloc}, jemalloc~\cite{evans2006jemalloc},
TCMalloc~\cite{ghemawat2005tcmalloc}, and snmalloc~\cite{lietar2019snmalloc} to
NUMAlloc~\cite{yang2023numalloc}, StarMalloc~\cite{starmalloc2024}, and
...
an unknown block size~\cite{bender2002treelayout}, and Zhang (P27, ASPLOS 2025) bundles prefetches
hierarchically across L1/L2/L3.
...
The cache-update behaviour introduced in Section~\ref{ssec:coherence} is concretely fixed per ISA
and per cache level. \emph{Write protocols:} ARMv8 chooses write-through, write-back, or
non-cacheable, as well as read- or read-write-allocate, through page attributes~\cite{arm_arm},
...
policy~\cite{qureshi2007adaptive}. \emph{Inclusion policy:} Intel client processors maintain an
...
determine which levels a write updates; since they are microarchitecturally fixed and not selectable
at run time, they enter the system-optimized binary as \emph{measured} platform properties
...
(\texttt{axis\_<nn>\_<name>/}). Tables~\ref{tab:axes-overview-1} and~\ref{tab:axes-overview-2} give
the overview of which publications contribute an organ per axis; the following subsections elaborate
them grouped thematically. The thematic origin clusters of the prior works (trie family P01--P10,
...
\emph{methods}: k-ary search (Schlegel/Gemulla/Lehner, DaMoN 2009 --- TU Dresden), interpolation
search (Perl/Itai/Avni 1978), Eytzinger-layout search (Khuong/Morin 2017), skip list (Pugh 1990),
binary search tree and B-tree (Bayer/McCreight~\cite{bayer1972btree}), and open-addressing hashing
...
proximity (P32 Schmidt) to cache-oblivious recursion (P17 Bender~\cite{frigo1999cache}) and the
L1/L2/L3 bundle (P27 Zhang); T2 (mapping) connects the two via linear 1:1 mapping, hash redirect
(P07), or permutation index (P03 Masstree) --- an axis sparsely populated in the state of the art,
predominantly by a baseline. \emph{Path compression} (T3) instantiates the byte-wise variant (P01),
the single-bit PATRICIA split (Morrison 1968~\cite{morrison1968patricia}; P02), the collapsed macro
node (P04 CoCo), and multi-byte rewiring (P05). The \emph{node-type axis} (T4, NT1--NT3, 13 building
blocks) and the build axis \emph{page type} (PG1--PG3, 26 building blocks) bundle the adaptive
...
path-/jump-pointer-oriented (P26 Zhang), and hierarchical bundle prefetch (P27 Zhang); the explicit
hardware prefetch instruction stems from Wormhole (P07). The \emph{ISA axis} (T12) and the build
axis \emph{SIMD extension} (09b) carry the vendor specifications: x86-64, AArch64, Power, and RISC-V
with SSE2/AVX2/AVX-512, NEON/SVE2, RVV, and CUDA. Two overarching strategy axes record what an
...
from the serial zero point through coarse locking (Dijkstra 1965) and reader/writer locks (Courtois
1971) to optimistic lock coupling (P08), lock-free (Michael/Scott 1996) and wait-free methods
(Herlihy 1991), as well as RCU (P29) and hazard pointers (P30); the lock mode distinguishes
read-only, read-write, upgradeable, and optimistic validation. The \emph{I/O axis} (T14, IO1--IO3)
instantiates buffered (Stonebraker 1981), direct, and memory-mapped I/O (Crotty, CIDR 2022)
...
Disruptor ring buffer (2011), the delta chain (Bw-Tree 2013), the copy-on-write buffer (Driscoll
1989), the epoch buffer (P29), the skip list (Pugh 1990), and the tombstone buffer (LSM, O'Neil
1996) to lock-free SPSC (Lamport 1983) and MPMC queues (Michael/Scott 1996); as well as write-back
(flush) from eager through lazy, threshold- and time-window-based (Kafka) to adaptive (RocksDB LSM).
...
(Comer~\cite{comer1979btree}). The \emph{filter axis} (T16, FT1--FT3) carries the Bloom filter
(1970), the cuckoo filter (CoNEXT 2014), the succinct range filter SuRF (P10), and the Xor filter
(2020).
...
SPEC~CPU~\cite{spec_cpu2017}, the scale-out suite CloudSuite~\cite{ferdman2012cloudsuite}, the
architecture simulator gem5~\cite{binkert2011gem5}, and --- for the allocator corpus ---
mimalloc-bench~\cite{mimalloc_bench} and the classics Larson, threadtest, shbench, and LADDIS.
Because these frameworks produce incomparable loads, the thesis abstracts them into a common set of
load profiles (Section~\ref{ssec:sota-wl-profiles}); the frameworks not yet realized in the proposed
measurement apparatus (TPC, SOSD, SPEC, CloudSuite, gem5, allocator suites) are connected via a
dataset loader.
...
in its self-chosen home profile'' be unmasked and bias-free comparability be gained. This
...
in Chapter~\ref{ch:concept}: each prior work (thesis) meets its limits in the bias-free
...
\emph{not a single method populates all nineteen axes jointly}. Some axes are populated in the state
of the art exclusively with baseline or textbook variants (the mapping between algorithm and cache
traversal), others appear only as a \emph{measurement target} (the dTLB), and the spatial search
class is missing entirely; axes such as I/O, migration, filter, and buffering are each populated
individually, but never together with the search axes in one design.

Added to this is the methodological gap: the results are reported in incomparable settings, and the
paper bias --- each publication chooses its home-field load profile
(Section~\ref{ssec:sota-wl-bias}) --- prevents a fair cross-comparison. What is missing is a
bias-free measurement of \emph{all} load profiles against \emph{all} methods on a common basis.
