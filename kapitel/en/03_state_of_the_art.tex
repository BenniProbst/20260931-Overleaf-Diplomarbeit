% DEPRECATED (lead 2026-09-21, order-272 final landing A-41): legacy file, not included; the governing text
% lives in the six-chapter include block of diplomarbeit.tex (kapitel/<lang>/01_einleitung..06_fazit).
% Deletion only with owner GO; no PDF effect.
% NOT included (as of 2026-08-06): this file is not referenced from diplomarbeit.tex.
% Before reactivation, reconcile against the current state of the included chapters.
% CAUTION, outdated axis count: this text still uses the nineteen-axis counting
% (3 occurrences); the active text uses eighteen organ axes (T0--T17).
...
the SearchAlgorithm family: it satisfies the \emph{point-query subset} of the \texttt{std::map}
...
--- and thus requires no family of its own.
...
\emph{hybrid and B\textsuperscript{+} concept} (cluster~B) unites the B\textsuperscript{2}-tree
...
measurements: Graefe and Larson (P15, 2001)~\cite{graefe2001btree} study B-tree indexes explicitly
against the CPU cache levels, Bender, Demaine, and Farach-Colton (P16, 2002; P17, 2005) model a
multi-level memory model with a block-transfer parameter and solve the tree-layout problem within it
near-optimally even for an unknown block size~\cite{bender2002treelayout}, and Zhang et al.\ (P27,
ASPLOS 2025)~\cite{zhang2025hierarchical} study, with \emph{Hierarchical Prefetching}, a
software-/hardware-cooperative \emph{instruction} prefetcher for server applications; it is relevant
as a prefetching instance, not as a data-structure-specific L1/L2/L3 data prefetch for tree layouts.
...
layout theory: the CSS-tree (P11, Rao/Ross 1999)~\cite{rao1999css} introduces the cache-line node,
the CSB\textsuperscript{+}-tree (P12, Rao/Ross 2000)~\cite{rao2000csb} the contiguous sibling
cluster; Hankins and Patel (P13, 2003)~\cite{hankins2003nodesize} measure the effect of node size,
and Samuel et al.\ (P14, 2005)~\cite{samuel2005procaware} make the node processor-aware. The
Adaptive Radix Tree (P01~\cite{leis2013art}) graduates the node size cache-line-orientedly with
Node4/16/48/256, and Schmidt et al.\ (P32, 2025)~\cite{schmidt2025hbm} examine strided versus
...
(P09, Jacobson 1989)~\cite{jacobson1989louds}, and the SIMD-tiled AoSoA.
...
al.\ (P32, 2025)~\cite{schmidt2025hbm} with 2-MiB pages. A \emph{dedicated} dTLB-optimization axis,
however, remains underpopulated in the search-algorithm papers --- an observation to which the
research gap (Section~\ref{sec:gap}) returns.

\subsection{Write Protocols, Update Heuristics, and Inclusion Policy}\label{ssec:sota-update}
The cache-update behaviour introduced in Section~\ref{ssec:coherence} --- write protocols
(write-through/-back, write-/no-write-allocate), heuristic replacement (LRU through RRIP/DRRIP), and
inclusion policy --- is only partly visible through the ISA and predominantly fixed per
microarchitecture and cache level. As a concrete \emph{instance} of these fixed properties, the L3
inclusion policy, for example, varies across generations: early Intel client generations
...
victim-like~\cite{intel_optimization,amd_sog}. Since these aspects are microarchitecturally fixed
and not selectable at run time, they enter the system-optimized binary as \emph{measured} platform
...
Bender et al.\ (P17, 2005)~\cite{bender2005coboblivious} found the cache-oblivious B-tree on the van
Emde Boas layout --- building on the order-maintenance / packed-memory
primitive~\cite{bender2002orderlist} and the cache-oblivious traversal of ordered
sets~\cite{bender2002scanning} ---, P16 (2002)~\cite{bender2000cobtree} extends the multi-level
layout model, and Saikkonen/Soisalon-Soininen (P18, 2008; P19, 2016) maintain a cache-sensitive
layout invariant even under updates; Graefe/Larson (P15, 2001)~\cite{graefe2001btree} compare both
...
extension, hardware profile); many axes are refined into sub-axes and provide other axes with
distributed interfaces for carrying out and optimizing their detail operations. The sub-axis labels
(e.g.\ SA1--SA4 or AA1--AA7) denote the structuring level of this thesis's building-block matrix
...
hybrid/B\textsuperscript{+} concept P03/P06/P07/P11--P13, layout theory P14--P19, prefetching
...
The \emph{search axis} (T0, sub-axes SA1--SA4) carries the most organs: as original bindings ART
...
(cache-memory traversal) ranges from cache-line walking through stride prefetch (P21
Chen)~\cite{chen2001prefetch} and HBM proximity (P32 Schmidt)~\cite{schmidt2025hbm} to
cache-oblivious recursion (P17 Bender~\cite{frigo1999cache}); T2 (mapping) connects the two via
linear 1:1 mapping, hash redirect (P07), or permutation index (P03 Masstree)~\cite{mao2012masstree}
--- an axis sparsely populated in the state of the art, predominantly by a baseline.
\emph{Path compression} (T3) instantiates the byte-wise variant (P01)~\cite{leis2013art}, the
single-bit PATRICIA split (Morrison 1968~\cite{morrison1968patricia}; P02)~\cite{binna2018hot}, the
collapsed macro node (P04 CoCo)~\cite{boffa2024coco}, and multi-byte rewiring
(P05)~\cite{fent2020start}. The \emph{node-type axis} (T4, NT1--NT3, 13 building blocks) and the
build axis \emph{page type} (PG1--PG3, 26 building blocks) bundle the adaptive layouts:
Node4/16/48/256 (P01)~\cite{leis2013art}, HOT compound (P02)~\cite{binna2018hot}, internal/border
(P03)~\cite{mao2012masstree}, decision/span (P06
B\textsuperscript{2}-tree~\cite{schmeisser2022b2tree}), LOUDS dense/sparse
(P10)~\cite{zhang2018surf}, CSS node/CSB\textsuperscript{+} node group/Hankins wide (P11/P12/P13),
as well as the PRT-ART family.
...
through pointer-free contiguous arrangement (P11 CSS)~\cite{rao1999css} and sibling cluster (P12
CSB\textsuperscript{+})~\cite{rao2000csb} to the embedded tree (P06)~\cite{schmeisser2022b2tree},
bit-packed LOUDS (P09 Jacobson)~\cite{jacobson1989louds}, and the huge-page variants
(P32)~\cite{schmidt2025hbm}. The \emph{allocator axis} (T6) is the most densely populated, with the
corpus A01--A23, and is refined into seven sub-axes: AA1 free-list topology (size-class,
best-/first-fit, segregated), AA2 size-class schema, AA3 thread locality/NUMA affinity (P31
Ungethuem)~\cite{ungethuem2017survey}, AA4 synchronization, AA5 allocation strategy (slab, buddy,
region, pool, object cache), AA6 reclamation (epoch, RCU P29, hazard pointers P30, QSBR), and AA7
fragmentation strategy; the code provides 25 allocator implementations. Table~\ref{tab:allocator-profiles} lists the ten runnable
allocator profiles with license and workload affinity. The \emph{value axis} (T11, 5 building
blocks) distinguishes inline value (P01)~\cite{leis2013art}, pointer, redirect-to-node or external
pool (P07 Wormhole)~\cite{wu2019wormhole}, versioned pointer (P03 Masstree)~\cite{mao2012masstree},
and immutable shared-ref (P29 RCU)~\cite{mckenney2001rcu}. The \emph{serialization axis} (T9)
carries raw binary, block compression (LZ77, Ziv/Lempel 1977; LZ4/Snappy), succinct LOUDS/FST
(P09/P10), and varint (P01)~\cite{leis2013art}.
...
software-fixed (P21 Chen)~\cite{chen2001prefetch} and adaptively distanced (P23
Khan)~\cite{khan2010adaptive} to fill-buffer-aware (P25 Mahling)~\cite{mahling2025hotpath},
path-/jump-pointer-oriented (P26 Zhang)~\cite{zhang2024pathprefix}, and hierarchical
instruction-bundle prefetch (P27 Zhang)~\cite{zhang2025hierarchical}; the explicit hardware prefetch
instruction stems from Wormhole (P07)~\cite{wu2019wormhole}. The \emph{ISA axis} (T12) and the build
axis \emph{SIMD extension} (09b) carry the vendor specifications: x86-64, AArch64, Power, and RISC-V
with SSE2/AVX2/AVX-512, NEON/SVE2, and RVV, as well as --- separately, as a GPU/accelerator profile
(SIMT, not CPU SIMD) --- CUDA\@. Two overarching strategy axes record what an algorithm actively
\emph{uses} and how it \emph{distributes} work: the \emph{hardware strategy} (SIMD
family, cache-level targeting, NUMA, prefetch hardware, atomic family) and the
\emph{scheduling strategy} (worker-pool layout, SIMD-worker limit ---
typically two of $N$ cores, due to hardware ---, heterogeneous P-/E-core dispatch,
co-routine, batch granularity). Table~\ref{tab:hw-sched} assigns both per paper. The
\emph{telemetry axis} (T10, sub TM1--TM3, 6 building blocks) instantiates the density tracker
(P01)~\cite{leis2013art}, the insert counter (P02)~\cite{binna2018hot}, the HdrHistogram (Gil Tene)
for p50/p95/p99, the path-read counter (P26)~\cite{zhang2024pathprefix}, and the probability hints
(P16/P17 Bender). The coherence-friendly counter variants --- leaf-only, sampling, and offline
recompute --- go back to Kuehn (P28, personal communication 2023~\cite{kuehn2023bplustree}, beyond
the published DaMoN-2023 state on the cache-/ distribution-aware B\textsuperscript{+} layout), while
the counter in all (including inner) nodes counts as an \emph{anti-pattern} (cache-line ping-pong).
...
reader/writer locks (Courtois~\cite{courtois1971readers}) to optimistic lock coupling
(P08)~\cite{leis2016olc}, lock-free (Michael/Scott~\cite{michael1996queue}) and wait-free methods
(Herlihy~\cite{herlihy1991waitfree}), as well as RCU (P29)~\cite{mckenney2001rcu} and hazard
pointers (P30)~\cite{michael2004hazard}; the lock mode distinguishes read-only, read-write,
upgradeable, and optimistic validation. The \emph{I/O axis} (T14, IO1--IO3) instantiates buffered
(Stonebraker~\cite{stonebraker1981os}), direct, and memory-mapped I/O (Crotty~\cite{crotty2022mmap})
alongside the in-memory zero point. The \emph{migration axis} (T15, MG1--MG3) carries static
placement, hot/cold separation (anti-caching 2013), multi-level RAM$\to$NVM$\to$SSD migration
(2018), and ML-driven adaptive migration (LeCaR 2018). The \emph{buffering axes} (T17/T18), finally,
comprise about a dozen building blocks each: operation buffers from the FIFO/LIFO queue through the
Disruptor ring buffer~\cite{thompson2011disruptor}, the delta chain
(Bw-Tree~\cite{levandoski2013bwtree}), the copy-on-write buffer
(Driscoll~\cite{driscoll1989persistent}), the epoch buffer (P29)~\cite{mckenney2001rcu}, the skip
list (Pugh~\cite{pugh1990skiplist}), and the tombstone buffer (LSM, O'Neil~\cite{oneil1996lsm}) to
...
optimistic lock coupling of ART (P08)~\cite{leis2016olc} as a concurrency organ of the \texttt{art}
profile, the succinct LOUDS representation (P09, Jacobson 1989)~\cite{jacobson1989louds} as a
memory-layout organ (e.g.\ in SuRF, P10~\cite{zhang2018surf}), and the profiling tool VAMPIR
(P33)~\cite{berthold2023vampir} as a measurement technology (Section~\ref{ssec:sota-meas-tech}); the
remaining 30 algorithms each carry their own profile XML\@.
...
\emph{Hardware performance counters} (PMC, via \texttt{perf})~\cite{demelo2010perf} are nearly
universal in the state of the art and deliver cache-miss rates (L1/L2/L3), dTLB misses, branch
misses, and IPC/CPI --- the majority of the works P01--P28 report such counters.\ \emph{Profilers}
such as OTF2/VAMPIR (P33, TU Dresden)~\cite{berthold2023vampir} and VTune~\cite{reinders2005vtune}
collect trace-based runtime profiles, \emph{simulators} such as gem5~\cite{binkert2011gem5} provide
hardware-accurate estimates where real counters are missing (P25 Mahling~\cite{mahling2025hotpath},
P27 Zhang~\cite{zhang2025hierarchical}), and the \emph{HdrHistogram} (Gil
Tene)~\cite{tene_hdrhistogram} collects the latency percentiles (p50/p95/p99), thereby instantiating
...
significance with the distribution-free \emph{Mann-Whitney U test}~\cite{mann1947utest} (with Holm
correction of the family-wise error rate~\cite{holm1979sequential}), and their strength is
quantified via the effect measure \emph{Cliff's~$\delta$}~\cite{cliff1993dominance}. This
methodology follows the recommendations for scientific benchmarking by Hoefler and
...
group): Ungethuem et al.\ (P31)~\cite{ungethuem2017survey} systematize the hardware optimizations
for database machines, Schmidt et al.\ (P32)~\cite{schmidt2025hbm} measure stride versus sequential
accesses on HBM with dTLB and L2/L3 counters, and the VAMPIR tool (P33)~\cite{berthold2023vampir}
profiles non-functional properties of heterogeneous memory. This line connects the present thesis
directly to the database and software-engineering environment of its university.
...
adopts and which it replaces as obsolete is developed in Chapter~\ref{ch:concept}; thus the state of
the art connects directly to the research gap (Section~\ref{sec:gap}).
...
living being to a feature composition, a living-being type to a static feature-set definition, and a
family to a (search-)algorithm interface category; the \emph{anatomy}, finally, corresponds to the
wiring between the organs --- the feature interaction --- by which an axis algorithm uses the
interfaces of other axes (the allocation axis, for instance, provides a shared allocation interface
for the remaining organs).
...
its design-pattern-driven, measurement-oriented realization.
...
frequent choice of scenario-specific, often favourably tailored load profiles
...
developed dialectically (thesis--antithesis--synthesis) by the following chapter: each prior work
(thesis) meets its limits in the bias-reduced cross-comparison (antithesis), from which the deeper
results of the proposed apparatus emerge as synthesis.
