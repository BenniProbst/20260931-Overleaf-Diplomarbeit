...
given in Appendix~\ref{app:blocks}. As a non-tree-like counterprobe within the same
search-algorithm genus, a plain hash table (\texttt{absl::flat\_hash\_map}/SwissTable) is also
included: it realizes the same \texttt{std::map} interface over a \emph{limited axis subset}
--- the tree-/trie-specific axes (path compression, node type, range index) collapse to
pass-through variants, while allocation, prefetch, concurrency, memory layout, and ISA SIMD
(for the SwissTable group lookup) remain shared. It thereby demonstrates that the axis
framework also carries non-tree-like search methods without requiring a genus of its own.
...
(Appendix~\ref{app:blocks}). Rank-1 representatives are Hoard (A01, ASPLOS 2000), Slab
...
Rank-2/3 add CAMA (A12, RTAS 2011), StarMalloc (A13, OOPSLA 2024), Crystalline reclamation
