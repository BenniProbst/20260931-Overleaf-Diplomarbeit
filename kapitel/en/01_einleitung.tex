...
\end{nurlang}
\begin{nurkurz}
In parallel, a research corpus of its own on cache awareness, prefetching, and allocation strategies has
emerged: Cache-Sensitive Search Trees~\cite{rao1999css, rao2000csb}, cache-oblivious
layouts~\cite{frigo1999cache}, prefetching B\textsuperscript{+}-trees~\cite{chen2001prefetch}, and
hierarchical prefetching~\cite{zhang2025hierarchical} each address a separately considered aspect of the
search-algorithm architecture. Because so much of it rests on cache-line-optimized behaviour, a
segmentation of the search-algorithm constituents into \emph{axes} suggests itself --- a backward-directed
abstraction of known building blocks into reusable modules.
\end{nurkurz}
...
constituents, serves as the candidate under test --- the new search algorithm to be integrated on trial --- for state-of-the-art membership.
...
\begin{description}
  ...
  that is, on an identical compiler and flag basis) in three measurement series ---
  A (candidate under test vs.\ state of the
  ...
  new candidates under test and their design constituents integrate, how are they abstracted, and do they
  join existing concepts\footnote{In this thesis, concept denotes the topmost
  division of the algorithms by their externally visible interface (such as \texttt{Map} or
  \texttt{Container}); each concept carries its own families as tier interfaces, whose genera are
  the implementations.},
  found new concepts or a
  ...
  \item[RQ3 (Candidate comparison)] Does the candidate under test PRT-ART win against the eight Rank-1 designs
  ...
  measured under the three mandatory measurement series A (candidate under test vs.\ state of the
  ...
  by occupancy density and access type --- should switching occur adaptively?
  The candidate under test currently switches by fixed
  ...
  are the candidate's switching rules between the page representations; the 50~percent boundary named in the
...
candidate under test PRT-ART, compiles it via metaprogramming against the core constituents of the cache engine, delivers
...
--- the three interface levels (\emph{concept} as the outer measurement surface, \emph{family} as the tier
interface, in code the tier interface class\footnote{\emph{Tier} is the German word for \emph{animal}: in the
  metaphor canon of this thesis it names the fully composed module --- the living being with all its
  organs (see \emph{Tier binary} in the glossary) ---, not a level or layer.} \texttt{AnatomyGenus}, and \emph{genus}
as its implementation), the axis organs as a classification of their own, the four subsystems, and the ABI-stable delivery of the search-algorithm \emph{living beings}
--- is developed by
Chapter~\ref{ch:measurement} and Chapter~\ref{ch:impl}. Meant here are the four carrier subsystems
measurement driver, \texttt{CacheEngineBuilder}, \texttt{CacheEngine}, and candidate under test; to be distinguished from them are
...
\begin{itemize}
  ...
  topic namespace within the cache engine. The candidate's search core specializes its hybrid API variadically by
  ...
  provides the concept arities of its families.
  ...
  (\texttt{[[no-runtime-switch]]}) --- together with a compile-time fallback between the candidate stack and the
  state-of-the-art stack, which takes over sixteen of the eighteen axes from the cache-engine frame and
  replaces two by candidate slots.
  ...
  \item PRT-ART as a candidate under test with eight building-block layers of its own (4+1+2 pool allocator, optimistic
  ...
  signalling-bit serialization). In the measurement apparatus the candidate under test registers four candidate slots --- page type
  ...
  a system axis and not a composition slot; the candidate slot of the same name is a
  remaining holdover. The OLC concurrency is a building-block layer of the candidate under test, not a registered slot.
  Note that the slot numbers of the candidate under test do not follow the slot order T00--T17 of the
  cache engine. The deployment of the candidate under test demonstrates the extensibility of the system by example ---
...
measurement system itself, the candidate under test PRT-ART as demonstrator, and the heuristic evaluation
with its operating modes. Chapter~\ref{ch:impl} realizes this concept as a software architecture ---
repositories, the candidate under test in code, the axis architecture and compilation pipeline, layer
