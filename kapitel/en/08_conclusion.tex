...
% (1 occurrences); the active text uses eighteen organ axes (T0--T17).
...
migration, filter, and the two queuing axes) span a configuration space of around $10^{14}$
...
of the measurement diagrams, and the completion of the per-permutation module bodies.
...
results, is deliberately \emph{not yet} implemented and is reserved for a follow-up phase. It
presupposes three components still to be built: (i) a \emph{heuristic/ML classifier} that maps the
load-profile and axis features (operation mix, key characteristics, hardware identifier) to the
recommended axis configuration; (ii) an \emph{XML parser} for the heuristic policies stored as an
XML load-profile result and reusable per architecture focus; and (iii) a \emph{binary packager} that
delivers the thus-selected overall algorithm as a standalone binary behind the unified
search-algorithm interface. A prerequisite for all three is that the end-to-end measurement series
be fully available beforehand --- only the robust data basis across all permutations and load
profiles makes a measurement-driven, automatically controlled selection meaningful; until then, the
configuration recommendations of Section~\ref{sec:sensitivity} are derived manually.

\medskip
In the medium to long term: a GPU adapter (Grace Hopper cluster, ZIH), PIM-allocator integration
(A16), and formal verification of selected permutation classes (StarMalloc-style, A13). The
three-repository architecture lets future students add further probe algorithms analogously to
PRT-ART without modifying the cache-engine tool library --- a key design principle for the
scalability of the research infrastructure. Beyond the tree-like search structures treated here, the
anatomy principle extends through higher-level abstractions to the further structure classes of
