...
\begin{tikzpicture}[font=\footnotesize,>=Stealth,
  ...
  \node[hy] (s4) at (7.9,0){\textbf{Einschub: Hybrid}\\{\tiny optional, zwischen Stufe 2 und 3; Hybrid-\texttt{.so}}\\[2pt]{\tiny Mux-Schalter am CEB-Prüf-Dock; Dock-Slots 1 bis $N$ für Tier-Binaries; dieselben Pflicht-Symbole; \texttt{genus()} meldet den Ziel-Genus}};
  ...
  \node[fl] (f3) at (11.85,-2.95){Flächen 1--3: Genus-Interface, Stempel (drei Zeilen), Mess-Durchstich \texttt{IMessVisitor}};
...
Mess-Lebenszyklus sowie \texttt{engine\_name}, \texttt{engine\_kind} und \texttt{lifecycle\_state} als
Auskünfte). Unter ihr liegen die vier Gattungen und die sechs Genera (Abschnitt~\ref{sec:anatomy-levels});
die Gattung ist im Code die Ableitung des Genus (\texttt{gattung\_of}), kein zweites Etikett. Das Lebewesen
...
\texttt{comdare\_\allowbreak{}anatomy\_\allowbreak{}genus} sowie die Versions-Zeilen des Stempels. Die
Buchstabengleichheit ist Bedingung: Der gattungsunabhängige Loader lädt jede Binary über dieselbe
Ladestrecke (POSIX \texttt{dlopen}~\cite{posix_dlopen}) und ermittelt die Gattung aus dem Genus,
\emph{bevor} er eine Instanz baut. Die ABI führt die Major-Version~9, deren Naht das Genus-Interface ist.
...
Die Verträge zwischen den Träger-Stufen sind an den Binaries in Flächen zergliedert. Das
Drei-Flächen-Modell (Abschnitt~\ref{sec:anatomy-levels}) ist im Code namentlich vorhanden: Fläche~1 ist
das Genus-Interface der Tier-Binary, Fläche~2 der Stempel und Fläche~3 der Mess-Durchstich
\texttt{IMessVisitor} --- ein Visitor am \texttt{dlopen}-Übergang, der existiert, \emph{damit} Gattungs-
und Genus-Interfaces unverändert bleiben. Der Stempel-Vertrag ist als Matrix aus sieben Interfaces und
...
Kanal-Adressen Achse $\to$ Genus $\to$ Kategorie auf hunderte Mess-Kategorien (Umsetzungsstand: sechzehn,
...
Genus-Interface durch eine eigene Steuerungsfläche zurückgerollt, die CEB übernimmt den inneren Zustand
...
\begin{tikzpicture}[font=\footnotesize,>=Stealth,
  ...
  \node[font=\bfseries\scriptsize,anchor=north west] at (-4.5,-3.15){Hybrid-\texttt{.so} (Gattung HeuristikAdapter, Reroute-Genus) --- eingeschobene Mux-Stufe};
...
Pflicht-Symbole wie die plain Tiere, \texttt{comdare\_anatomy\_genus} meldet den \emph{Ziel}-Genus, nie
den Reroute-Genus, und die Genus-Bau-Zulassung führt als sechsten Eintrag das Reroute-Genus mit dem
