...
\end{nurlang}
\begin{nurkurz}
Dieses Kapitel steigt in das Detail: von der Repository-Architektur mit der Träger-Stufenkette
(Abschnitt~\ref{sec:anatomy-levels}) über die Schlüsselstellen von PRT-ART, die Achsen-Implementierung, die
Bau-Kette und die Contract-Tests bis zu den Realm-Wurzeln, der Hybrid-Stufe und dem Lager. Im Kern ist die
\emph{eine} Architektur eine selbst-optimierende Compiler-Compiler-Struktur: Der Planer erzeugt aus der
Anwender-XML die Bau-Aufträge, der Builder daraus die Kompilate, und die Auswertung der Messwerte speist die
nächste Planung (Abschnitt~\ref{sec:mess-chain}) --- die Linie der empirischen
Autotuning-Systeme~\cite{whaley2001atlas,frigo1998fftw,ansel2014opentuner,balaprakash2018autotuning}, nur
dass hier die Organ-Belegung eines Lebewesens permutiert und je Permutation eine eigene Binary erzeugt wird.
\end{nurkurz}
...
Planer $\to$ CEB $\to$ Tier mit dem optional zwischen CEB-Prüf-Dock und Tier eingeschobenen Hybrid
(Abschnitt~\ref{sec:anatomy-levels}): Der Stempel-Vertrag kennt genau vier Träger, und der Quellbaum
bildet sie unter \texttt{libs/traeger} als vier Unterprojekte in Stufenform ab --- \texttt{comdare\_planner},
\texttt{comdare\_ceb}, \texttt{comdare\_tier\_emission} und \texttt{comdare\_hybrid} ---, wobei in der
Bau-Reihenfolge Stufe $N+1$ nur an Stufe $N$ hängt (ohne Tier kein Hybrid), nie umgekehrt, und jede Stufe
mit einer strikten Konfigurationsprüfung auf ihre Vorstufe schließt.
...
\begin{tikzpicture}[font=\footnotesize,>=Stealth,
  ...
  \node[st] (s2) at (3.95,0){\textbf{Stufe 2: CEB}\\{\tiny\texttt{comdare-messung-driver}}\\[2pt]{\tiny führt die Mess-Wahl einkompiliert, permutiert System $\times$ Organ; ein Prüf-Dock als Anschluss für Tier oder Hybrid; Legende je Host \texttt{tier:build-batch:\allowbreak<host>}, Testate \texttt{zelle=\allowbreak[d,e,f]\allowbreak[g,h,i]}}};
  \node[st] (s3) at (11.85,0){\textbf{Stufe 3: Tier}\\{\tiny\texttt{comdare\_perm\_<fp>.so}}\\[2pt]{\tiny eine Organ-Permutation je Binary; sieben Pflicht-Symbole; Mess-Batch je Host \texttt{measure:\allowbreak[a,b,c]:\allowbreak batch:\allowbreak<host>}, Mess-Testate \texttt{zelle=\allowbreak[a,b,c]\allowbreak[d,e,f]\allowbreak[g,h,i]}}};
  \node[hy] (s4) at (7.9,0){\textbf{Einschub: Hybrid}\\{\tiny optional, zwischen Stufe 2 und 3; Hybrid-\texttt{.so}}\\[2pt]{\tiny Mux-Schalter am CEB-Prüf-Dock; Dock-Slots 1 bis $N$ für Tier-Binaries; dieselben Pflicht-Symbole; \texttt{genus()} meldet den Ziel-Gattung}};
  \draw[->,thick] (s1)--(s2); \draw[->,thick] (s2)--(s4); \draw[->,thick] (s4)--(s3);
  \draw[->,thick] (s2.north) -- ++(0,0.5) coordinate (bp) -| (s3.north);
  \node[font=\tiny,anchor=south,text width=11.5cm,align=center] at ([xshift=3.95cm,yshift=1pt]bp){Standardweg ohne Hybrid: CEB-Prüf-Dock $\to$ Tier-Binary direkt; mit Hybrid: Prüf-Dock $\to$ Mux $\to$ Dock-Slot $\to$ Tier-Binary};
  ...
  \node[fl] (f3) at (11.85,-2.95){Flächen 1--3: Gattungs-Interface, Stempel (drei Zeilen), Mess-Durchstich \texttt{IMessVisitor}};
  \node[fl] (f4) at (7.9,-2.95){Flächen 1--3 wie Tier, dazu die angeschlossenen Stempel-Zeilen (Laufzeit-Haken)};
  \draw[dashed,gray] (s1)--(f1); \draw[dashed,gray] (s2)--(f2); \draw[dashed,gray] (s3)--(f3); \draw[dashed,gray] (s4)--(f4);
\end{tikzpicture}
\caption[Die Träger-Stufenkette im Code]{Die Träger-Stufenkette im Code: vier Träger, davon der Hybrid als optional
zwischen CEB-Prüf-Dock und Tier-Binary eingeschobene Stufe (Stempel-Kette in fester Reihenfolge Planer $\to$ CEB
$\to$ [Hybrid] $\to$ Tier), je Stufe die Binary, ihre Legenden-Form in der emittierten Pipeline und die an ihr
ausgeprägten Vertrags-Flächen. In der Stempel-Kette hängt Stufe $N+1$ nur an Stufe $N$; der Standardweg führt vom
CEB-Prüf-Dock direkt zur Tier-Binary, der Hybrid ist ein Mux-Schalter am Prüf-Dock mit Dock-Slots für Tier-Binaries
(Bau-Abhängigkeit: ohne Tier kein Hybrid); nur die Mess-Testate enthalten alle drei Achsen-Klammern. Die Flächen
...
Instrumentierung. Alle vier bauen als CMake-Release; daneben gelten die Bau-Varianten Debug, Messung und Release fort, und die Debug-Variante wird weiter ausgebaut
--- ein messendes Debug-Regime existiert jedoch nicht mehr, weil
...
unter \texttt{--debug} fährt das Konformitäts-Gate zusätzlich den Standard-Testsatz des Tier-Interfaces, und dieser indirekte
Debug-Modus fällt mit dem Release-Bau zusammen und wird nicht separat geführt.
Umsetzungsstand: \texttt{compare} ist ein wählbares Etikett mit \texttt{release}-gleicher Bausemantik;
sein modus-spezifischer Ablauf (Replay-Vergleich statt Messen) ist für spätere Haupt-Wellen vorgesehen, aber
nicht implementiert (Abschnitt~\ref{sec:anatomy-levels}).
...
\begin{itemize}
  \item \texttt{comdare-cache-engine} --- \textbf{wie} gemessen wird: die Bausteine-Bibliothek und der
  Builder (Codegenerierung, Permutations-Schleife, Modul-Loader, der siebenphasige \texttt{ExperimentDriver}
  und der Planer \texttt{ExperimentPlanDirector}, Abschnitt~\ref{sec:impl-pipeline}), die ABI-Header unter
  \texttt{abi/} samt dem vom Bau isolierten Test-Cluster (Abschnitt~\ref{sec:axes-impl}), die
  33~SOTA-Profile als ausführbare XML (Abschnitt~\ref{sec:sota-instances}) und die Test-Suites; der
  Quellbaum folgt einem Pitchfork-Layout, hinzu treten \texttt{libs/traeger} mit den vier Träger-Stufen
  (unten) und \texttt{heuristik/} mit sechs Headern (Achsen-Spline, Break-Even, Messkurven-Lader,
  Offline-Clusterung, Merkmalsvektor, Optimierungs-Katalog).
  \item \texttt{comdare-prt-art} --- der \textbf{Prüfling}: die hybride \texttt{PrtArtSearchEngine} mit der
  1/2/N-Parameter-Regel (ein Parameter fordert die Funktionsinterfaces eines Vektors, zwei die einer Map,
  mehr als zwei die einer Map auf ein Tupel), ihre ABI-Adapter und Bausteine sowie die Re-Implementierungen
  der Rang-2/3-Arbeiten (\texttt{legacy\_\allowbreak{}reimpl}, vierzehn Verzeichnisse); Rang~1
  (Zentralitäts-Rang, Abschnitt~\ref{sec:rqs}) umfasst die acht eigenständigen Suchentwürfe mit
  Original-Adapter sowie die als Organe eingehenden Arbeiten P08 und P09. Pool-, Prefetch- und
  Layout-Komponenten sind Deskriptor-Skelette ohne eigene Speicheranbindung; welche Prüflings-Bausteine
  den Mess-Apparat erreichen, entscheidet die Slot-Belegung (Abschnitt~\ref{sec:prtart-impl}).
  \item \texttt{probst-Diplomarbeit-cache-engine} --- \textbf{was} durch den Anwender getestet wird: die
  Experiment-Konfigurationen (XML samt XSD-Schema), der Mess-Treiber \texttt{messung\_driver} (die
  CEB-Rolle) und die Auswertungs-Werkzeugkette aus neun numerierten Programmen von
  \texttt{01\_\allowbreak{}sample\_\allowbreak{}data\_\allowbreak{}generator} bis
  \texttt{09\_\allowbreak{}tex\_\allowbreak{}formatter}.
  \item das \textbf{Manuskript-Repository} --- die versionierte Experimentdurchführung im Dokument: Die
  Generator-Ausgabe der Werkzeugkette wird per Commit im Anhang abgelegt, das Repository besitzt eine
  eigene CI, und das Projekt-Repository stößt den Manuskript-Bau nach einem Messlauf an
  (\texttt{trigger:thesis}); ein Messwert im Dokument ist ein versionierter Artefakt-Stand mit Herkunft.
\end{itemize}
Die Vierer-Ordnung des Codes ist nicht die Repository-Grenze, sondern die \emph{Träger-Stufenkette}
Planer $\to$ CEB $\to$ Tier mit dem optional zwischen CEB-Prüf-Dock und Tier eingeschobenen Hybrid
(Abschnitt~\ref{sec:anatomy-levels}); der Quellbaum bildet sie unter \texttt{libs/traeger} als vier
Unterprojekte in Stufenform ab, wobei in der Bau-Reihenfolge Stufe $N+1$ nur an Stufe $N$
hängt (ohne Tier kein Hybrid). Umsetzungsstand: Die Unterprojekte sind ein Gerüst, die Implementierung liegt als Monolith unter
\texttt{libs/\allowbreak{}cache\_\allowbreak{}engine}. Der Prüfling ist keine Stufe, sondern der quer
liegende Lieferant von Organ-Bausteinen; die Stufenkette ist zugleich die Schicht, an deren Binaries die
Verträge in Flächen zergliedert sind (Abschnitt~\ref{sec:impl-pipeline}); Abbildung~\ref{fig:traeger-stufen}
zeigt sie mit ihren Binaries, Legenden-Formen und Flächen.
\begin{figure}[htbp]\centering
%% INTERN-272: KA-056
\begin{tikzpicture}[font=\footnotesize,>=Stealth,
  st/.style={draw,rounded corners,fill=blue!7,minimum width=3.75cm,minimum height=3.9cm,text width=3.55cm,align=center,inner sep=3pt},
...
sind ein Gerüst, die Implementierung liegt als Monolith unter \texttt{libs/cache\_engine}.}\label{fig:traeger-stufen}
\end{figure}
Quer dazu trennt die Terminologie zwei Engine-Begriffe: Die \texttt{CacheEngine} ist die \emph{Bibliothek
der Achsen-Bausteine} (je Achse die austauschbaren Entwurfsbestandteile als Concept-/CRTP-Komponenten, ohne
Mess-Logik), die \texttt{ExecutionEngine} die quer zu allen Achsen liegende \emph{Ausführungs- und
Mess-Infrastruktur} mit der messbaren Wurzel \texttt{IExecutionEngine}
(Abschnitt~\ref{sec:measurement-system}); die Mess-Kopplungen des Laufzeit-Adapters werden über
\texttt{COMDARE\_\allowbreak{}MEASUREMENT\_\allowbreak{}ON} geschaltet, und die CMake-Option
\texttt{COMDARE\_\allowbreak{}EXPERIMENT\_\allowbreak{}MODE} kodiert die Invariante, dass ein Experiment-Bau
stets ein Mess-Bau ist; die Trennung von Bausteinen und Werkzeugen hält den Produktiv-Pfad mess-frei.

Die vier \emph{Arbeitsmodi} \texttt{build}, \texttt{measure}, \texttt{compare} und \texttt{release} sind
als Unter-Achse des Mess-Realms registriert (Abschnitt~\ref{sec:anatomy-levels}): \texttt{build} baut ohne
zu messen und darf parallel laufen, \texttt{measure} ist der deterministische Ein-Thread-Messlauf und der
einzige Modus mit eingeschalteter Messung, \texttt{compare} liest das Messwertlager (Umsetzungsstand: ein
wählbares Etikett, der Replay-Vergleich ist nicht implementiert), \texttt{release} erzeugt die
Referenz-Binary ohne Instrumentierung; alle vier bauen als CMake-Release, und der Verdrahtungs-Check des
früheren Debug-Regimes ist als Schalter \texttt{--debug} der Planer-Shell realisiert --- ein Flag, kein
Zustand; unter \texttt{--debug} fährt das Konformitäts-Gate zusätzlich den Standard-Testsatz des Tier-Interfaces, ein indirekter
Debug-Modus, der mit dem Release-Bau zusammenfällt.
\end{nurkurz}
...
\begin{nurkurz}
\section{PRT-ART im Detail}\label{sec:prtart-impl}
Aufbauend auf dem Stand der Technik benennt dieser Abschnitt jede Schlüsselstelle von PRT-ART samt dem
verwandten Verfahren, aus dem sie hervorgeht: Pool-Allokation mit rollenreinen Pools (Hoard, jemalloc,
mimalloc~\cite{berger2000hoard,evans2006jemalloc,leijen2019mimalloc}), optimistisches Lock-Coupling
(ART~\cite{leis2016olc}), Software-Prefetch entlang der
Baumpfade~\cite{chen2001prefetch,zhang2025hierarchical}, cache-line-bewusste Knoten-Layouts (ART und
FAST~\cite{leis2013art,kim2010fast}) und kohärenz-bewusste Telemetrie gegen das
False-Sharing-Problem~\cite{torrellas1994falsesharing,kuehn2023bplustree}; der DensityTracker und die
Hypothesen-Metriken H1/H2/H3 sind eigene Beiträge ohne Fremdvorlage.

Der Prüfling steuert sechs eigene Schlüsselstellen bei, zwei davon auf der zweigeteilten Telemetrie-Achse
(Sweep-Unter-Achse des Mess-Toolings im Planer, Übersetzungszeit-Größe im Builder,
Abschnitt~\ref{sec:axis-triad}): ein \emph{4+1+2-Pool-Allokator} (vier Seiten-Pools mit den Rollen
Trie-Hülle, Dense-Pages, MultiLevel und Decision-Span, ein Rest-Pool, zwei Wert-Pools) als
cache-line-ausgerichtete, fragmentierungsarme Free-List; \emph{optimistisches Lock-Coupling (OLC) mit
reservierten Wert-Blöcken} über einen Concurrency-Manager (OLC, HTM, STM, Lock-Free, RCU, Hazard
Pointers~\cite{michael2004hazard}); ein \emph{pfadorientiertes Bundle-Prefetch} der heißen Suchpfade; ein
\emph{MultiLevel-Layout}; kohärenz-bewusste Telemetrie --- Per-Node-Zähler verursachen
Cache-Line-Ping-Pong und verfälschen die Messung, die sie erheben --- als \emph{Leaf-Only-Zähler}
(Hauptvariante), \emph{Sampling} und \emph{Offline-Recompute}, der Per-Node-Zähler bleibt nur als
gekennzeichnetes Anti-Pattern; und der DensityTracker samt den Metriken H1/H2/H3 als eigene
Telemetrie-Organe. Im Engine-Quellbaum liegen die Telemetrie-Bausteine (TM1--TM3) und die
ISA-Unter-Achsen (IS1--IS3) unter den Organ-Achsen, während Telemetrie-Regime und Ziel-ISA in der
System-Registry geführt werden (Abschnitt~\ref{sec:impl-pipeline}) --- eine doppelte Verortung im Code,
keine Zuordnungsfrage des Modells.

Diese Schlüsselstellen sind \emph{Bausteine des Prüfling-Repos}; welche davon den Mess-Apparat erreichen,
entscheidet die Slot-Belegung (Abschnitt~\ref{sec:prtart-demo}). Der Prüfling stellt vier Slot-Hüllen: den
Seitentyp der Build-Achse (\texttt{PrtArtBPlusPageType}), das Prefetch (\texttt{PrtArtRedirectPrefetch},
T7), das ChainRef-Wert-Handle (\texttt{PrtArtChainRefHandle}, T10) und einen Telemetrie-Wrapper mit dem
Per-Node-Zähler als Vergleichsbasis. Seine Slot-Komposition \texttt{PrtArtCompositionDemo} übernimmt
sechzehn der achtzehn Organe aus der ART-Referenz-Komposition der Cache-Engine und ersetzt Prefetch und
Wert-Handle; die Engine-seitige \texttt{PrtArtComposition} unterscheidet sich von ART nur im
Pfadkompressions-Redirect (T3, das \emph{R} in PRT-ART); Pool-Allokator (T6) und MultiLevel-Layout (T5)
laufen im Mess-Pfad über den Cache-Engine-Standard, ihre Anbindung ist vorgesehen, aber nicht
implementiert. Die verbleibenden zwei der acht Bausteinschichten aus der Beitragsliste
(Abschnitt~\ref{sec:contributions}), Wert-Handles und Signaling-Bit-Serialisierung, stellen den
Wert-Handle-Slot (T10) beziehungsweise sind als Baustein der Serialisierungs-Achse (T9) in
...
\begin{nurkurz}
\section{Achsen-Implementierung und Mess-System-Ebenen}\label{sec:axes-impl}
Jede Achse wird als generalisiertes Interface implementiert; daneben liegen die Ebenen des Mess-Systems bis
zur Sieben-Phasen-Pipeline, das Ebenen-Bild E4--E0 im Code (Abbildung~\ref{fig:impl-layers}) und die
Vertrags-Flächen; das Muster-Register folgt am Kapitelende (Abbildung~\ref{fig:muster}). Jede Achse ist ein
Topic-Verzeichnis mit einem zweischichtigen \emph{Concept}~\cite{sutton2012concepts} und einer
\texttt{requires}-gesicherten CRTP-Basisklasse. Der Live-Weg der Baustein-Auswahl ist monomorph: Aus der
Typliste aller Bausteine (\texttt{All*}) bildet ein Übersetzungszeit-Filter über das Enable-Flag
(\texttt{mp\_filter}, Boost.Mp11~\cite{dimov_mp11}) die \texttt{Enabled*}-Liste, und eine Composition
belegt je Slot genau einen Typ; der Hot-Path enthält weder \texttt{virtual}-Aufrufe noch Laufzeit-Switches
--- eine ungenutzte Abstraktion kostet nichts~\cite{stroustrup2012foundations}. SOTA-Entwürfe und
Allokatoren werden über dünne \emph{Adapter} integriert: Vendor-Shims schalten bei fehlendem Vendor den
Baustein ab, statt den Bau zu brechen, mit familienweise gewähltem Fallback
(\texttt{portable\_aligned\_alloc}; \texttt{StdMalloc} ist ein eigener Baustein, keine Notlösung), und ein
Stub, der zur Laufzeit einen \texttt{runtime\_error} wirft, kommt in den Organ-Achsen nicht vor. Die vom
Prüfling nicht belegten Organ-Achsen beziehen ihren Baustein aus der Cache-Engine
(Abschnitt~\ref{sec:prtart-impl}); das hierarchische Prefetching~\cite{zhang2025hierarchical} bringt den
Call-Graph-Analyzer \texttt{p27\_bundle\_finder} zur Übersetzungszeit mit.
\end{nurkurz}
...
\begin{nurkurz}
%% INTERN-272: KA-059
Eine Design-Entscheidung grenzt diesen Live-Weg von einer verworfenen Alternative ab: Eine variant-basierte
Baustein-Auflösung (\texttt{resolve\_baustein} über \texttt{baustein\_variants}) ist für statische
Organ-Achsen ausdrücklich verboten und existiert nur als vom Bau isolierter Test-Cluster. Der Grund ist die
Sparsamkeit des Kompilats: Eine Tier-Binary bildet genau die eine gefahrene Permutation ab;
\texttt{std::variant}, \texttt{virtual} und Laufzeit-Switches sind in Tier- und Hybrid-Binaries verboten
--- wer eine andere Permutation braucht, braucht ein anderes Kompilat. Die eine zugelassene variant-Stelle
des Hauses ist das Dock-Array der Hybrid-Stufe (\texttt{HybridDockVariant},
Abschnitt~\ref{sec:impl-hybrid-lager}); daneben duldet der Code \texttt{std::variant} in zwei achsenfremden
Bausteinen (\texttt{axis\_error}, \texttt{pressure\_state}) und in den drei Expected/Result-Nähten der
Hardware-Sonden (NUMA-Seiten, NUMA-Kern-Bindung, Betriebssystem). Das Verbot ist damit strenger als die
bloße Vermeidung virtueller Aufrufe~\cite{alexandrescu2001modern}: Es verbietet jede Form der
Laufzeit-Verzweigung zwischen Bausteinen.
\end{nurkurz}
...
Innerhalb jeder Achsen-Kategorie gibt es Haupt-Achsen, Unter-Achsen (sub-axes) und Meta-Meta-Achsen. Die
Kategorie ist im Code die Trichotomie \texttt{AxisCategory} mit der festen Ordnung mess $<$ system $<$
organ (ein \texttt{static\_assert} hält sie gegen stilles Umsortieren), die Familie der Diskriminator
\texttt{AxisKind} mit sechs Werten; eine Unter-Achse ist selbst eine Voll-Achse und kann zugleich
Permutations-Achse sein. Der Bestand je Kategorie: Der Organ-Realm umfasst achtzehn Haupt-Achsen, zwanzig
Unter-Achsen-Header und die Organ-Meta-Meta \texttt{disk\_io}; der System-Realm drei Haupt-Achsen, die
Komplex-Achse \texttt{build\_target\_complex} mit der Unter-Achsen-Gruppe \texttt{build\_toolchain}
(Compiler, Optimierungsstufe, Atomik), \texttt{target\_isa\_complex} (drei Glieder), das Scheduling mit fünf
Dimensionen und die System-Meta-Metas unter dem Hub der Erweiterungs-Hardware; der Mess-Realm die
Tooling-Haupt-Achse (wallclock/macro/micro), vier registrierte Unter-Achsen (sechzehn Kategorien,
Arbeitsmodus, Mess-Framework, Rückschreib-Methode) und die Mess-Meta-Meta \texttt{load\_framework}. Die
Benennung Meta-Meta folgt der vierstufigen Meta-Ebenen-Architektur der Object Management Group
(MOF)~\cite{omg2016mof}; die Lesart M0--M3 für diesen Apparat entwickelt
...
ein konkretes \emph{Strategie-Concept} mit mehreren austauschbaren Bausteinen, aus denen eine konkrete
Composition je Slot genau einen wählt; ihre Zahl ist als \texttt{kOrganAxisCount} $=$ 18 die Single-Source
der Organ-Zeile, und ihre Namen und Slot-Reihenfolge führt das Array \texttt{kCompositionAxisNames}; die
vollständige Bausteine-Matrix mit allen Herkunfts-Quellen führt Anhang~\ref{app:blocks}, die
SOTA-Beiträge je Achse Tabelle~\ref{tab:axes-overview}.
%% INTERN-272: KA-061
Neben den achtzehn kanonischen Haupt-Achsen steht die Organ-Meta-Meta \texttt{disk\_io}, die die
\texttt{binary\_id} nie berührt (Abschnitt~\ref{sec:axis-triad}); eine neunzehnte, optionale Organ-Achse
ist vorgesehen, aber nicht implementiert; erst ihre Implementierung führt sie als Slot T18. Dass keine
Composition ein Organ auslässt, erzwingt das \texttt{IsComposition}-Concept: Es verlangt je Algorithmus die
Belegung genau dieser achtzehn Organe, sodass keine Composition übersetzt, die eine Achse offenlässt.
\end{nurkurz}
...
Auskünfte. Unter ihr liegen auf Ebene~1 die vier Konzepte Map, Container, Graph und HeuristikAdapter
(Abschnitt~\ref{sec:anatomy-levels}) --- Graph als deklarierter Stub ohne Gattung --- und auf Ebene~2 die
sechs Gattungen SearchAlgorithm, Set, Sequence, Adapter, View und FunctionInterfaceReroute; die Gattung ist im
Code die Ableitung der Gattung (\texttt{gattung\_of}), kein zweites Etikett. Das Lebewesen
...
\begin{nurkurz}
\subsection{Die eine Architektur im Code}\label{sec:impl-architecture}
Die \emph{eine} Architektur aus Abschnitt~\ref{sec:anatomy-model} ist im Code eine einzige Hierarchie. Ihre
Wurzel ist \texttt{IExecutionEngine} mit drei Engine-Arten --- Anatomy, Virus, Hybrid --- und einem Vertrag
aus sieben Methoden (\texttt{warm\_up}, \texttt{run}, \texttt{reset} und \texttt{shutdown} als
Mess-Lebenszyklus sowie \texttt{engine\_name}, \texttt{engine\_kind} und \texttt{lifecycle\_state} als
Auskünfte). Unter ihr liegen die vier Konzepte und die sechs Gattungen (Abschnitt~\ref{sec:anatomy-levels});
die Gattung ist im Code die Ableitung der Gattung (\texttt{gattung\_of}), kein zweites Etikett. Das Lebewesen
\texttt{SearchAlgorithm} und seine ABI-Laufzeit-Sicht --- die \enquote{SearchEngine} --- sind zwei Sichten
desselben Gegenstands über die Modulgrenze: Der Körper ist \texttt{SearchAlgorithmAnatomy<C>}, die
Komposition und Verdrahtung der achtzehn Organe, und die Sicht ist \texttt{SearchAlgorithmAbiAdapter<A>}.
Die Schichtung \texttt{CacheEngine}\,$\to$\,\texttt{ExecutionEngine}\,$\to$\,\texttt{SearchEngine}
(Abschnitt~\ref{sec:repos}) trennt Rollen, nicht Klassenbäume; das gleichnamige Bibliotheks-Skelett
\texttt{libs/search\_engine} enthält keinen Code. Achsenlose \emph{Viren} stehen als Geschwister unter
derselben Wurzel, besitzen aber keine Anatomie (Abbildung~\ref{fig:one-architecture}). In dieser Hierarchie
ist der Compiler-Compiler-Charakter des Apparats verankert: Jede Composition ist ein Typ, aus dem der
Builder je Permutation ein eigenes Kompilat erzeugt.
\end{nurkurz}
...
\begin{figure}[!htbp]
  ...
  \begin{tikzpicture}[box/.style={draw, rounded corners, align=center, font=\footnotesize, inner sep=3pt, text width=3.0cm},
    ...
    \node[box] (ana) at (-4.4,-2.0) {\texttt{IAnatomyBase}\\{\scriptsize Lebewesen (vier Konzepte, Ebene~1)}};
    \node[box] (vir) at (3.4,-2.0) {\texttt{IVirus\-Execution\-Engine}\\{\scriptsize Virus (achsenlos, dennoch messbar; z.\,B.\ Graph-BFS)}};
    \node[box,text width=3.2cm] (g1) at (-7.5,-3.9) {Map-Konzept\\{\tiny Gattung \texttt{SearchAlgorithm} (Säugetier)}};
    \node[box,text width=3.2cm] (g2) at (-3.95,-3.9) {Container-Konzept\\{\tiny Gattungen Set, Sequence, Adapter, View}};
    \node[box,text width=3.2cm,dashed] (g3) at (-0.4,-3.9) {Graph-Konzept\\{\tiny deklarierter Stub, keine Gattung}};
    \node[box,text width=3.2cm] (g4) at (3.15,-3.9) {HeuristikAdapter-Konzept\\{\tiny Gattung \texttt{FunctionInterfaceReroute}}};
    ...
    \node[gen,text width=3.1cm] (n6) at (4.1,-5.7) {\texttt{FunctionInterfaceReroute}\\Reroute-Gattung};
    ...
    \node[box,fill=orange!8,text width=3.4cm] (comp) at (-6.6,-9.7) {konkrete Composition (z.\,B.\ \texttt{ArtComposition})\\{\scriptsize Ebene 3: Genus (z.\,B.\ ART) $=$ Belegung \emph{aller} 18 Organ-Achsen (Ebene 4)}};
    \node[box,fill=orange!16,text width=3.4cm] (bin) at (-6.6,-12.0) {Tier-Binary \texttt{comdare\_perm\_\-<fp>.so}\\{\scriptsize Ebene 5a: je Permutation kompiliert $+$ System-Achsen}};
    \draw[->] (root) -- (ana);
    \draw[->] (root) -- (vir);
  ...
  Anatomy, Virus und Hybrid; unter ihm verzweigen die anatomie-tragenden \emph{Konzepte} (Ebene~1: Map,
  Container, Graph, HeuristikAdapter) mit ihren \emph{Gattungen} (Ebene~2) und die achsenlosen \emph{Viren}.
  \texttt{SearchAlgorithm} \emph{ist} das fokale Lebewesen (Elefant); seine Anatomie ist sein
  \emph{Körper} --- die feste Komposition \emph{und Verdrahtung} der 18 Organe ---, und
  \enquote{SearchEngine} nur dessen ABI-Laufzeit-Sicht. Der Elefanten-Ast (Gattung \texttt{SearchAlgorithm}) ist bis zum Individuum
  ausgezogen: ein Genus (Ebene~3) mit seiner konkreten Achsen-Composition (Ebene~4) wird je Permutation
  als eigenständige Tier-Binary kompiliert (Ebene~5a, das Individuum). Container besitzt einen eigenen Organsatz mit den Gattungen Set, Sequence, Adapter und
  View, deren Aritäten die Gattungs-Bau-Zulassung führt; Graph ist deklariert und besitzt keine Gattung; das
  Konzept HeuristikAdapter führt die Reroute-Gattung der Hybrid-Stufe.}\label{fig:one-architecture}
\end{figure}
...
Buchstabengleichheit ist Bedingung, nicht Bequemlichkeit: Der konzeptunabhängige Loader lädt jede Binary
über dieselbe Ladestrecke (POSIX \texttt{dlopen}~\cite{posix_dlopen}) und kann das Konzept ermitteln,
\emph{bevor} er eine Instanz baut --- konzeptunabhängige Namen mit konzept-spezifischen Werten; die
Gattung ist dabei aus der Gattung abgeleitet. Die ABI führt die Major-Version~9, deren Naht das
Gattungs-Interface ist. Die variadische 1/2/N-Hybrid-API gehört zum Prüfling: Seine
...
\texttt{std::map} auf ein Tupel ---, und die Slot-Aritäten der Gattungen der Cache-Engine (achtzehn für das
Gattung SearchAlgorithm, je eigene für Set, Sequence, Adapter und View aus der Gattungs-Bau-Zulassung) sind
...
\end{nurlang}
\begin{nurkurz}
Konkret ist die Modulgrenze eine C-stabile Schnittstelle --- die ABI des Apparats ---, die je Permutation
genau ein Lebewesen exportiert: Jede Tier-Binary enthält sieben Pflicht-Symbole mit C-Bindung --- die vier
Ur-Symbole ABI-Version, Magic-Zahl, Factory \texttt{comdare\_\allowbreak{}create\_\allowbreak{}anatomy} und
Destroy, dazu die zwei über alle Gattungen buchstabengleichen Auskünfte
\texttt{comdare\_\allowbreak{}anatomy\_\allowbreak{}gattung} und
\texttt{comdare\_\allowbreak{}anatomy\_\allowbreak{}genus} sowie die Versions-Zeilen des Stempels. Die
Buchstabengleichheit ist Bedingung: Der konzeptunabhängige Loader lädt jede Binary über dieselbe
Ladestrecke (POSIX \texttt{dlopen}~\cite{posix_dlopen}) und ermittelt das Konzept aus der Gattung,
\emph{bevor} er eine Instanz baut. Die ABI führt die Major-Version~9, deren Naht das Gattungs-Interface ist.
Die variadische 1/2/N-Hybrid-API gehört zum Prüfling: \texttt{PrtArtSearchEngine<Ts\ldots>} spezialisiert
sich nach Parameterzahl (\texttt{std::vector}, \texttt{std::map}, \texttt{std::map} auf ein Tupel). Komplexe
Schlüssel reduziert eine Fingerprint-Komponente
(\texttt{fixed\_\allowbreak{}length\_\allowbreak{}fingerprint}, FNV-1a-Basis) auf 16-Byte-Binärstrings,
die der Prüfling als Schlüsseltyp verwendet (\texttt{BinaryKey}); dieser Schlüssel-Fingerprint ist von den
drei Identitäts-Fingerprints des Apparats --- \texttt{permutation\_\allowbreak{}fingerprint} im ABI-POD,
SHA-512-Tier-Fingerprint, SHA-256-Planer-Fingerprint --- zu unterscheiden (Abschnitt~\ref{sec:stamp-model}).
\end{nurkurz}
...
\end{lstlisting}
  ...
  und meldet über \texttt{genus} ihre Ziel-Gattung) und die variadische 1/2/N-API als Sicht des Prüflings.
...
\begin{tikzpicture}[font=\footnotesize,>=Stealth,align=center,
  ...
  \node[comp] (genus) at (-4.9,1.75){Gattung (6, Ebene 2)};
...
das Gattungs-Interface der Tier-Binary (Laufzeit, Abstract Factory), Fläche~2 der Stempel (Übersetzungszeit,
ABI-stabil) und Fläche~3 der Mess-Durchstich \texttt{IMessVisitor} --- ein Visitor am
\texttt{dlopen}-Übergang mit doppeltem Dispatch, der existiert, \emph{damit} Konzept- und Gattungs-Interfaces
...
\toprule
Interface & Inhalt & Planer & CEB & Hybrid & Tier \\
\midrule
...
\texttt{Angeschlossene} & angeschlossene Stempel-Zeilen (Laufzeit-Haken)    & V & V & P & V \\
\bottomrule
\end{tabular}
...
werden je Erbin per \texttt{static\_assert} geprüft. Die Spaltenordnung folgt wie
Abbildung~\ref{fig:traeger-flaechen} der Verarbeitungs-Topologie mit der Hybrid-Tier-Binary als
optionalem Einschub zwischen CEB und Tier-Binary.}\label{tab:stempel-matrix}
\end{table}
\end{nurlang}
\begin{nurkurz}
\subsection{Vom Profil zur Tier-Binary: Planer, Builder und Mess-Pipeline}\label{sec:impl-pipeline}
Die Kette von der Anwender-XML zur Tier-Binary (Abschnitt~\ref{sec:mess-chain}) hat im Code die Gestalt
zweier Binaries. Stufe~1, die Planer-Rolle, ist \texttt{comdare-experiment-planner} mit den Subkommandos
\texttt{validate}, \texttt{plan dump|ci|cmake}, \texttt{cache-key}, \texttt{fingerprint}, \texttt{status},
\texttt{check-size}, \texttt{simulate}, \texttt{version} und \texttt{help}: \texttt{validate} prüft rein
lesend gegen die einkompilierten Registry-Kataloge, \texttt{plan} fährt den deterministischen, byte-gleich
wiederholbaren Walk des \texttt{ExperimentPlanDirector} (\emph{Director} und
\emph{Builder}~\cite{gamma1994patterns}: ein Walk, je Ausgabe-Syntax ein Builder), \texttt{check-size} und
\texttt{simulate} rechnen die Mess-Menge vor dem Lauf aus (Abschnitt~\ref{sec:impl-contracts}). Stufe~2,
die CEB-Rolle, ist der Mess-Treiber \texttt{comdare-messung-driver} (\texttt{tier ci|cmake}, \texttt{run},
\texttt{version}, \texttt{help}): Er emittiert die Tier-Bau-Aufträge seines Bau-Raums und fährt den
Mess-Lauf; der Planer steuert die CEB-Aufträge, die CEB die Tier-Aufträge --- zwei Module, nicht nur
Oberfläche. Die Legenden der emittierten Testate besitzen drei Formen: \texttt{ceb:build:[a,b,c]} auf
Stufe~1, je Host ein Bau- und Prüf-Batch \texttt{tier:build-batch:<host>} mit den Testaten
\texttt{zelle=[d,e,f][g,h,i]} (die Meilenstein-Batches in Abschnitt~\ref{sec:fairness}) und je Host ein
Mess-Batch \texttt{measure:[a,b,c]:batch:<host>}, dessen Mess-Testate als einzige alle drei Klammern
enthalten (Abbildung~\ref{fig:traeger-stufen}).

%% INTERN-272: KA-065
Die Anlage hat zwei Steuerpfade, den Planer per Shell direkt und eine API-Kommandozeile mit headless
Dienst (Zelle auf Kubernetes oder bare metal), dazu einen kommandozeilengeführten Erstellungsmodus für die
Anwender-XML mit Hardware-Filter; implementiert ist der Shell-Pfad, Dienst und Erstellungsmodus sind
vorgesehen, aber nicht implementiert. Zwei Eigenschaften sichern den \emph{einen} Hauptkanal ab: Der
Text-Plan weist je Bau-Schritt den tatsächlich erzeugten Datei-Stamm aus (\texttt{built\_stem}), erzeugt
von derselben Funktion, die der Bau-Orchestrator aufruft; und es gibt \emph{eine} XML-Quelle und
\emph{einen} Kanal, der sie baut --- die Variantenbildung liegt in der C++23-Metaprogrammierung, CMake ist
auf das Bauen beschränkt.

Innerhalb der CEB überführt der Builder jedes Profil in ein vorkompiliertes C++23-Modul. Jede Permutation
ist durch eine bit-gepackte \emph{Permutations-Flag}-Struktur mit zehn Bänken zu je 64~Bit identifiziert,
ein Constraint-Filter (\texttt{is\_valid\_combination}) schließt inkohärente Kombinationen aus; die
achtzehn Organ-Achsen identifiziert der Mess-Pfad über den Achtzehn-Segment-Pfad (die \texttt{binary\_id},
Abschnitt~\ref{sec:stamp-model}) samt den drei Fingerprints, und die Permutations-Zählung folgt einer
Mixed-Radix-Kodierung --- der Produktraum der Variabilitätsmodelle der
Software-Produktlinien~\cite{pohl2005spl}. Darüber läuft die \emph{Sieben-Phasen-Mess-Pipeline}
(Abbildung~\ref{fig:pipeline}) des \texttt{ExperimentDriver}: (1)~\textsc{Enumerate},
(2)~\textsc{Generate}, (3)~\textsc{Compile}, (4)~\textsc{Link+Load}, (5)~\textsc{Execute},
(6)~\textsc{Measure} und (7)~\textsc{Persist}; zwei opt-in-Phasen kompilieren fehlende Module nach und
fahren je Modul einen Funktionstest mit ABI-Versionsprüfung, und die Messreihen-Modi \texttt{Defined},
\texttt{Full} und \texttt{FullSampled} (Abschnitt~\ref{sec:mess-chain}) wählen, welche Permutationen eine
Reihe umfasst. Die nachgelagerte Auswertungs-Werkzeugkette ist eine Folge von neun numerierten Programmen
von \texttt{sample\_data\_generator} bis \texttt{tex\_formatter}; die xlsx-Mappe ist die Ausgabe, die
CSV-Form ihr aus der Mappe erzeugter Baum-Abzug (Abschnitt~\ref{sec:eval-dataflow}); implementiert ist
eine flache Datei. Die Mess-Kopplungen sind über das
\texttt{COMDARE\_\allowbreak{}MEASUREMENT\_\allowbreak{}ON}-Gating vom Produktiv-Pfad getrennt
(Abschnitt~\ref{sec:repos}), und Continuous Integration~\cite{fowler2006ci} läuft über alle vier
Repositorien.
\begin{figure}[htbp]\centering
%% INTERN-272: KA-066
\begin{tikzpicture}[font=\footnotesize,>=Stealth,align=center,
...
Die Ebenen E4--E0 aus Abschnitt~\ref{sec:anatomy-layers} haben im Code je einen Ort, und die Grenzen
zwischen ihnen besitzen je einen Contract-Test (Abschnitt~\ref{sec:impl-contracts});
Abbildung~\ref{fig:impl-layers} zeigt diese Code-Gestalt: E4 ist das XML-Profil mit dem Planer und den
Auswertungs-Programmen 04 (\texttt{csv\_to\_latex}) und 08 (\texttt{appendix\_generator}), E3 der
Experiment-Baum (\texttt{experiment\_tree}) mit der
\texttt{binary\_id} als Pfad, E2 der ABI-Adapter mit der Modul-ABI-Deklaration, E1 das Interface
\texttt{IResourceControllableTier} mit den dynamischen Dimensionen der Mess-Registry-XML, E0 sind CI,
Runner und Lager, und der Querschnitt~M ist die System-Achsen-Wurzel \texttt{SystemAxis}
(Abschnitt~\ref{sec:impl-system-axes}).
\begin{figure}[htbp]\centering
%% INTERN-272: KA-068
\begin{tikzpicture}[font=\footnotesize,>=Stealth,
...
Die Verträge zwischen den Träger-Stufen sind an den Binaries in Flächen zergliedert. Das
Drei-Flächen-Modell (Abschnitt~\ref{sec:anatomy-levels}) ist im Code namentlich vorhanden: Fläche~1 ist
das Gattungs-Interface der Tier-Binary, Fläche~2 der Stempel und Fläche~3 der Mess-Durchstich
\texttt{IMessVisitor} --- ein Visitor am \texttt{dlopen}-Übergang, der existiert, \emph{damit} Gattungs-
und Gattungs-Interfaces unverändert bleiben. Der Stempel-Vertrag ist als Matrix aus sieben Interfaces und
vier Trägern ausgeführt --- 28~Zellen mit dreiwertiger Zulassung (Tabelle~\ref{tab:stempel-matrix}); die
Absenz der Organ-Zeile beim CEB ist die statische Sperre \enquote{der CEB führt keine Organ-Achsen}, und
ein \texttt{static\_assert(StempelVertrag<Erbin, Traeger>)} unter jeder Erbin macht eine fehlende Pflicht
oder ein verbotenes Interface zum Übersetzungsfehler --- eine Form des Consumer-Driven
...
das jede importierte Binary vor der Messung gegen den festgeschriebenen Vertrag prüft (unter
\texttt{--debug} zusätzlich gegen den Standard-Testsatz des Tier-Interfaces). Für die
...
mit je zwei Vertretern, das Persistenz-Ziel auf einen festgelegt; eine vereinfachende, überholte Annahme, denn
die Achsen tragen mehrere Auslegungen, der Katalog ist Regressions-Detektor, nicht Obergrenze --- und wird gegen eine festgeschriebene
...
\begin{nurkurz}
\section{Qualitätssicherung: Contract-Tests je Schicht}\label{sec:impl-contracts}
Das Schichtenmodell E4--E0 mit seinen Interface-Verträgen hat Abschnitt~\ref{sec:anatomy-layers} als
Konzept entwickelt; dieser Abschnitt beschreibt seine Umsetzung als Prüf-Infrastruktur. Änderungen über
mehrere Schichten zugleich erzeugen Fehler aus Wechselwirkungen, die weder lokale Tests noch Review
sichtbar machen (ADR-5, Anhang~\ref{app:adr}); weil der Produktraum jede vollständige Materialisierung
ausschließt (Abschnitt~\ref{sec:mess-chain}), wird jede Schicht einzeln fertiggestellt --- mit
festgeschriebenem Interface-Vertrag zur Nachbar-Ebene, eigenem Test-Harness und eigener
Abnahme-Definition.

Das tragende Prüfprinzip lautet: \emph{Jede Schicht wird gegen einen übersetzungszeitlichen Ersatz ihrer
Nachbar-Ebene getestet.} Die Nachbar-Ebene ersetzt je ein \emph{Fake} (eine Template- und Concept-basierte
Attrappe im Test-Scope, kein Laufzeit-Umschalter im Produktiv-Code), und die Prüffrage jeder Grenze, ob
die Schicht ihre Definition \emph{verlustfrei} weiterreicht, wird zum ausführbaren Abnahme-Test.

Je Schicht (die Harness-Namen zeigt Abbildung~\ref{fig:impl-layers}): Für E4 vergleichen Golden-Fixtures
die aus einem Profil emittierten Achsen-Ebenen gegen eine Referenz, ein Fake-E3 ersetzt den
Permutationsbaum; für E3 wird die Bijektion der Binary-Identität (mixed-radix hin und zurück) gegen ein
Fake-E2 gefahren, dazu prüft ein Konformitäts-Gate jede importierte Binary vor der Messung (unter
\texttt{--debug} auch gegen den Standard-Testsatz des Tier-Interfaces); für E2 ist der
Vertrag die ABI selbst: Die Wormhole-Referenz-DLL muss \texttt{IAnatomyBase}, \texttt{IObservableTier}
und \texttt{IResourceControllableTier} erfüllen, und die Antwort einer Stellgrößen-Anfrage
(\texttt{apply}) ist die \emph{Zahl der Achsen, die den Wert tatsächlich angenommen haben} --- nichts wird
still angenommen; für E1 prüfen Edge-Tests in-process die Randfälle des dynamischen Regimes (Reset eines
wiederverwendeten Tiers, Pool-Budget unterhalb des Datenbestands als benannter Zustand, jede Stellgröße
als benannte Spalte der Ergebnis-Datei, Erhalt der Zwei-Phasen-Operationsschleife). Geprüft wird die
Persistenz an der Ergebnis-Datei; Führung ist die Standard-xlsx, die CSV-Form ihr Baum-Abzug.
Umsetzungsstand: Die E1-Prüfung liest die CSV-Ausgabe, die die Naht als eine flache Datei schreibt; die
Prüfung an der xlsx und die Baumform sind vorgesehen, aber nicht implementiert.

Oberhalb der Schicht-Harnesse sichert ein \emph{Zwei-Gate-Protokoll} jedes Implementierungs-Paket ab
(Continuous Integration und Delivery als Sicherungsstufen~\cite{fowler2006ci,humble2010cd}).
\emph{Gate~1} ist das scharfe Gate des Engine-Repositories: Die volle Test-Suite läuft in vier Zellen, gcc
und clang jeweils als Release- und Debug-Bau, ohne Änderungs-Skip und ohne Ausnahmeliste; der Doppellauf
zwingt jede statische Prüfung durch zwei Übersetzer und macht die Run-to-Run-Stabilität der Zahl selbst
zum Prüfgegenstand. Bestandteil von Gate~1 ist die Golden-Verankerung des Permutationsraums
(Abschnitt~\ref{sec:mess-chain}): Der \emph{Referenz-Katalog} hält $2^{17} = 131072$ Binary-Identitäten (Annahme zweier Auslegungen
je Achse, überholt; Regressions-Detektor, nicht Obergrenze) und wird gegen eine festgeschriebene CRC-64-Prüfsumme bestätigt (\texttt{0x56F1B721C72DC10E}), ein % chktex 29
kleiner Katalog von $320$ Identitäten dient als byte-genaue Prüfung der Serialisierungsform --- ein
Charakterisierungs-Test im Sinne von Feathers~\cite{feathers2004legacy} ---, und die \emph{Mächtigkeit}
des in einer Messreihe geplanten Raums ist davon verschieden und nie statisch: Der Planer zählt sie im
Director-Walk aus der XML (\texttt{check-size}, \texttt{simulate}), nie als Konstante.
\emph{Gate~2} ist das Integrations-Gate des Projekt-Repositories: Seine Pipeline baut fehlerfrei ohne
Ausnahmemenge; die beiden deklarierten Fehlschlag-Duldungen (ein arm64-Smoke-Bau ohne Runner, ein
manueller Relock-Wartungsjob) sind eine Plattform-Grenze, keine Testmenge.
...
%% INTERN-272: KA-074
Die Fertigstellung folgt der Schichtung top-down --- E4, E3, E2, dann der Mess-Querschnitt~M vor E1, die
Infrastruktur~E0 als letzter Schritt (Abschnitt~\ref{sec:anatomy-layers}). Alle vier Grenzen besitzen
ihren Harness, und der Querschnitt~M besitzt einen eigenen ohne Tier-Binary: Eine synthetische Mess-Quelle
wird über die System-Achsen-Wurzel getrieben und bis in die Ergebnis-Spalten verfolgt. Umsetzungsstand:
Die Hardware-Erhebungskette läuft an der Freigabe-Naht des Builders (Abschnitt~\ref{sec:impl-system-axes});
der CEB-getriebene Freigabe-Pfad der Mess-Quelle ist vorgesehen, aber nicht implementiert. Die Code-Gestalt
des Querschnitts beschreibt der folgende Abschnitt.
\end{nurkurz}
...
%% INTERN-272: KA-075
Vier Abstraktionsebenen ordnen die Begriffe dieses Abschnitts (Abbildung~\ref{fig:m0-m3}), in Anlehnung an
die vierstufige Meta-Ebenen-Architektur der Object Management Group (MOF)~\cite{omg2016mof}: M3 sind die
\emph{Meta-Meta-Achsen} im Sinne der Indirektion einer Beschreibung --- Achsen, die beschreiben, wie andere
Achsen beschrieben werden (\texttt{load\_framework}, \texttt{disk\_io}, die System-Meta-Metas) ---, M2 die
Registry-Zeile mit Kategorie, Familie und Bausteinliste, M1 die CRTP-Achse mit ihrem Concept und M0 der
Binärcode der Tier-Binary mit genau einer einkompilierten Belegung. Im Stempel ist die Meta-Meta-Ebene
eines Eintrags mechanisch seine Klammer-Tiefe (Bits~3--5; eine Haupt-Achse ist klammerlos, eine Meta-Meta
hängt als Klammer-Anhang an ihrer Zeile, eine Tiefe über sieben bricht ab), und die Kurzform
\texttt{c\{p.e\}} eines Versions-Flags bezeichnet \texttt{c} mit den Untergliedern \texttt{p} und
\texttt{e}, nie ein flaches Kürzel. Je Realm gibt es einen eigenen Meta-Meta-Diskriminator; die
PMC-Erfassung (Performance Monitoring Counter, die Hardware-Zähler der CPU) ist eine Meta-Meta-Mess-Achse,
weil sie beschreibt, mit welchem Zähler-Instrument andere Messungen erhoben werden.
\begin{figure}[htbp]\centering
%% INTERN-272: KA-076
\begin{tikzpicture}[font=\footnotesize,>=Stealth,
  mb/.style={draw,rounded corners,minimum width=13.6cm,minimum height=1.05cm,text width=13.3cm,align=left,inner sep=3pt}]
...
\begin{nurkurz}
Die Dreiteilung der Achsen-Realms (Abschnitt~\ref{sec:axis-triad}) ist im Code als Typ-Trennung
ausgeführt. Alle Achsen teilen das Familiendach \texttt{topics::Axis<Derived>}, eine leere,
nicht-polymorphe CRTP-Basis, deren Concept-Prüfung jede eingeschleppte vtable schon beim Übersetzen
zurückweist, und deklarieren Kategorie und Familie über \texttt{AxisCategory} und \texttt{AxisKind}
(Abschnitt~\ref{sec:axes-impl}). Je Familie existiert eine eigene CRTP-Wurzel: \texttt{OrganAxis} und
\texttt{OrganMetaMetaAxis} für den Organ-Realm, \texttt{SystemAxis} für den host-seitigen Querschnitt,
\texttt{CebSystemAxis} für die Bau- und Steuer-Achsen der CEB, \texttt{SystemMetaMetaAxis} für die
System-Meta-Achsen und \texttt{MeasurementMetaMetaAxis} für die Mess-Realm-Achsen des Planers. Eine Achse
\emph{ist} ihre Familie durch ihre Basisklasse, nicht durch ein umhängbares Etikett: Der System-Realm
wurzelt an der CEB, der Mess-Realm am Planer (Abschnitt~\ref{sec:axis-triad}).
\end{nurkurz}
...
Auf der System-Seite ist die kanonische Ordnung selbst Code: Das konstante Drei-Element-Array
\texttt{kSystemAxisOrder} hält die System-Haupt-Achsen \texttt{target\_isa}, \texttt{operating\_system} und
\texttt{external\_utils} in ihrer verbindlichen Reihenfolge, und jeder Konsument, der eine Ordnung braucht,
liest sie dort, statt sie nachzubilden (Ordnungs-Single-Source mit Drift-Prüfungen). Drei
übersetzungszeitliche \emph{Rückkehr-Sperren} halten den Schnitt der Haupt-Achsen fest:
\texttt{static\_assert}-Prüfungen weisen Compiler, Scheduling und Last-Rahmenwerk als System-Haupt-Achsen
zurück --- der Compiler ist eine Unter-Achsen-Gruppe \texttt{build\_toolchain} des Komplex-Wrappers
\texttt{build\_target\_complex}, das Scheduling eine Unter-Achse an \texttt{target\_isa} mit fünf
Dimensionen, das Last-Rahmenwerk eine Meta-Meta-Haupt-Achse des Mess-Realms ---, und wer einen der drei
wieder einträgt, bricht den Bau und muss die Zuordnung ausdrücklich widerrufen. Auch die
Bau-Versionskennung der System-Zeile (Abschnitt~\ref{sec:stamp-model}) speist sich aus einer einzigen
deklarativen Quelle, der geordneten Acht-Segment-Folge \texttt{kSuffixSegmentOrder} (Compiler-Kennung,
Optimierung, SIMD-Erweiterung, CEB, Ziel-ISA, Telemetrie, Bau-Typ, Gate-Beitrag), in der ein leeres Glied
kein Segment erzeugt und das Gate-Segment am Ende steht (Präfix-Stabilität).
\end{nurkurz}
...
\end{nurlang}
\begin{nurkurz}
Auf der Mess-Seite besitzt der Planer seine eigene Meta-Meta-Wurzel \texttt{MeasurementMetaMetaAxis}; ihre
erste Trägerin ist das Last-Rahmenwerk \texttt{load\_framework} (Stempel-Token \texttt{ycsb@1.0.0.c}), und
wie alle Mess-Realm-Achsen ist sie strikt binary-id-neutral (\texttt{binary\_id} $=$ \texttt{never}). Die
Mess-Tooling-\emph{Haupt}-Achse hält genau drei versionierte Einträge in der einzig erlaubten Reihenfolge
wallclock/macro/micro (Alias w/ma/mi, Abschnitt~\ref{sec:measurement-system}); ein viertes Tooling ist ein
Übersetzungsfehler. Sie fächert vier Unter-Achsen-Registries auf: den Arbeitsmodus (\texttt{WorkMode}:
build, measure, compare, release, Abschnitt~\ref{sec:repos}), das Mess-Framework
(\texttt{MeasurementFramework}: genau ein Eintrag, YCSB), die Rückschreib-Methode
(\texttt{WritebackMethod}: CSV, LaTeX-Tabelle, Vergleichs-Metriken und xlsx als Default, ausdrücklich kein
PDF) und die sechzehn Mess-Kategorien (unten); jede Registry ist ein \texttt{constexpr}-Katalog mit der
Prüfung Index $=$ Enum-Wert, ein weiterer Eintrag bricht die Übersetzung. Zwei Gates spiegeln die
Tooling-Leiter: Das \emph{Statistik-Gate} schaltet die Observer-Statistik des Makro-Regimes, das
\emph{Segment-Timing-Gate} die achtzehn Per-Achsen-Segment-Timer; ein nicht gesetztes Segment-Timing-Gate
\emph{erbt} den Zustand des Statistik-Gates, ein gesetztes ohne Statistik-Gate bricht die Übersetzung. Der
wirksame Gate-Zustand jeder Übersetzungseinheit wandert als Mess-Gate-Glied mit neun \texttt{;}-getrennten
Feldern in das Fingerprint-Preimage des Stempels (Abschnitt~\ref{sec:stamp-model}) --- im Aus-Zustand
\texttt{mg=m0;s0;st0;x0;tw0;tm0;tmi0;hm0;hmi0}: Messung, Statistik, Segment-Timing, Experiment-Modus, dann
Wallclock, Makro und Mikro der Tier-Stufe und Makro und Mikro der Hybrid-Stufe ---; der Stempel behauptet
nie eine Ausstattung, die der Bau nicht fährt. Das Zielbild neben diesem Registry-Stand ist das
Achsen-Interface-Modell aus Abschnitt~\ref{sec:mess-chain}.
\end{nurkurz}
...
\begin{tikzpicture}[font=\footnotesize,>=Stealth,
  ...
  \node[ze,fill=orange!14] at (4.3,0){Zeit $+$ Parameter je Gattungs-Interface-Funktion};
...
Die Abschaltung der Messung ist ein Stufenmodell über den drei Mess-Ebenen
(Abbildung~\ref{fig:abschaltungsstufen}). Das Messgerät \texttt{checkpoint\_measure} erhebt die
Eingangsparameter stets zu einem \emph{Zeitpunkt}; die Wallclock ist das Minimum eines zeitmessenden,
sonst parameter-leeren Checkpoints und damit das minimale Messgerät der CEB\@. Die erste Abschaltungsstufe
schaltet in Makro und Mikro die Parameter-Erhebung ab, sodass die Wallclock für \texttt{measure} und
\texttt{compare} bestehen bleibt; die zweite, permutative Abschaltungsstufe lässt für \texttt{release}
auch die Wallclock samt den Flächen und Interfaces der Messung entfallen. Umsetzungsstand: Der Code
enthält die Gates G2 und G3 und den Release-Modus ohne Instrumentierung (\texttt{measurement\_on} aus);
die zweistufige Abschaltung als Permutations-Größe ist vorgesehen, aber nicht implementiert.
\begin{figure}[htbp]\centering
%% INTERN-272: KA-079
\begin{tikzpicture}[font=\footnotesize,>=Stealth,
  ze/.style={draw,rounded corners,minimum width=3.9cm,minimum height=1.35cm,text width=3.7cm,align=center,inner sep=2pt,font=\scriptsize},
...
%% INTERN-272: KA-080
Das PMC-Tooling ist mehrschichtig, weil die Hardware-Zähler je Kern-Sorte und Hersteller verschieden sind
(Zugriff über \texttt{perf\_event\_open}~\cite{linux_perf_event_open}). Eine generische Ereignis-Liste
(\texttt{pmc\_event\_set}) entscheidet, ob PMC eingebaut wird: Die Host-Sonde des Planers prüft genau die
Events, die die reale Quelle später öffnet. Ein modellgebundener RAW-Katalog füllt die Kanäle für
L2-Misses und Kohärenz je Mikroarchitektur: Für AMD Zen~5 (prod1) ist die Belegung gegen die benannten
\texttt{perf}-Events mit identischen Zählerständen (17\,314\,023 gegen 17\,314\,023) gegengeprüft ---
\texttt{cache\_misses\_l2} erfasst dort die Demand-Misses ohne L2-Prefetch,
\texttt{coherence\_invalidations} die Cache-zu-Cache-Fills aus einem fremden CCX (Core Complex, einem
Kern-Komplex mit eigenem L3) ---; für Intel liegt kein Eintrag vor, weil eine RAW-Belegung ohne
Gegenprüfung ein Rateversuch wäre~\cite{intel_sdm} --- eine ausgewiesene Lücke. Eine Vendor-Registry
führt die PMC als \emph{ein} Merkmal mit zwei Werten, AMD und
Intel, mit je eigener Versionskennung, weil die eingebaute Zählerform den Fingerprint der CEB mitbestimmt;
die CI fährt zwei Zähler-Pfade, \texttt{pmc:amd} auf prod1 und \texttt{pmc:intel} auf prod2 (Intel Alder
Lake mit P- und E-Kernen). Die getrennte Messung der P- und E-Kerne ist als Scheduling-Dimension angelegt
(Sechs-Modus-Modell, Abschnitt~\ref{sec:axis-triad}); das Laufzeit-Pinning je Kern-Sorte ist vorgesehen,
...
\end{nurlang}
\begin{nurkurz}
Der host-seitige Mess-Querschnitt des System-Realms --- der Querschnitt~M, der alle Schichten durchdringt,
ohne je die Binary-Identität zu berühren (Abschnitt~\ref{sec:axis-triad}) --- wurzelt in der CRTP-Basis
\texttt{SystemAxis} (Datei \texttt{system\_axis.hpp}), die nur lesend an die host-seitigen
Ergebnis-Strukturen andockt: eine leere, nicht-polymorphe zero-cost abstraction, deren Konformität ein
eigenes Concept erzwingt. Zwei \texttt{consteval}-Gates übersetzen die Konzept-Invarianten: Jede der
sechzehn Mess-Kategorien ist genau einem der beiden Regimes \texttt{TimeObserver} und \texttt{PmcCounter}
zugeordnet (neun zeit- und observer-basierte, sieben zählerbasierte PMC-Kategorien), und jede Achse, die
Kategorien beider Regimes mischt, wird zurückgewiesen --- das Regime-Misch-Verbot bricht so die
Übersetzung statt erst die Auswertung.
\end{nurkurz}
...
\begin{nurkurz}
Die sechzehn Mess-Kategorien führt eine einzige \texttt{constexpr}-Registry
(\texttt{measurement\_\allowbreak{}axis\_\allowbreak{}registry.hpp}) mit einem statischen Gate über Index,
Namen und Regime --- eine siebzehnte Kategorie oder eine stille Index-Drift wird damit zum
Übersetzungsfehler. Konzeptionell sind die Kategorien eine \emph{Unter}-Achse des Mess-Toolings und
manifestieren sich als Spalten der Ergebnis-Tabellen (Abschnitt~\ref{sec:axis-triad}); die Registry-Namen
sind zugleich das Berichts-Vokabular der Auswertungs-Schicht (die Spalten-Namen der E4-Berichte).
\end{nurkurz}
...
\begin{nurkurz}
Die Erhebung abstrahiert eine vendor-neutrale Mess-Quellen-Schnittstelle
(\texttt{i\_\allowbreak{}measurement\_\allowbreak{}source.hpp}): \texttt{IMeasurementSource} beschreibt
eine Quelle über ihre \emph{strukturellen} Fähigkeiten --- welche der zehn Ereignis-Semantiken (Zyklen,
Instruktionen, L1-/L2-/L3-Cache-, dTLB- und Branch-Misses, Speicher-Stalls, Kohärenz-Invalidierungen,
Energie) sie überhaupt ausdrücken kann --- und liefert je Messintervall ein \texttt{MeasuredDelta} mit
\emph{per-Ereignis-Gültigkeit}: Null mit gesetztem Gültigkeits-Flag ist eine tatsächliche Null-Messung,
ein nicht gesetztes Flag heißt \emph{nicht gemessen}. Als garantierter, ausdrücklich \emph{degradierter}
Fallback existiert die Wall-Clock-Quelle (\texttt{WallClockSource}); die Bestands-PMC-Familie bindet der
Adapter \texttt{PmcSourceAdapter} an, der nur die sieben strukturell ausdrückbaren Kanäle bewirbt, während
Zyklen, Instruktionen und Speicher-Stalls im Zähler-POD ungültig bleiben.
\end{nurkurz}
...
Kanal-Adressen Achse $\to$ Gattung $\to$ Kategorie (Hierarchie-Tiefe drei) auf hunderte Mess-Kategorien, je
...
Gattungs-Interface durch eine eigene Steuerungsfläche zurückgerollt, damit anschließend alle Achsen vom
...
%% INTERN-272: KA-083
Im Achsen-Interface-Modell wird die Mess-Quellen-Schnittstelle für das Mikro-Benchmarking je
Achsen-Interface speziell für dessen Parametersatz erhoben; die Kategorien-Menge wächst damit über die
Kanal-Adressen Achse $\to$ Gattung $\to$ Kategorie auf hunderte Mess-Kategorien (Umsetzungsstand: sechzehn,
durch die Registry statisch festgelegt). Dazu tritt die \emph{ungebremste Nachmessung}: Die Aktionen jeder
Achse werden über den Checkpoint der zuerst aufgerufenen Achsen-Interface-Funktion hinter dem
Gattungs-Interface durch eine eigene Steuerungsfläche zurückgerollt, die CEB übernimmt den inneren Zustand
einer Tier-Binary mit Messfühlern in die analoge Tier-Binary ohne Messfühler und misst vom selben Punkt
erneut, rekursiv je Makro-Benchmarking-Interface --- vorgesehen, aber nicht implementiert. Das
Prüfkriterium der Nachmessung ist die Plausibilitäts-Invariante, dass die Binary mit Messfühlern langsamer
ist als die ohne (Kapitel~\ref{ch:eval}).
\end{nurkurz}
...
\begin{nurkurz}
Am Auswertungs-Ende des Querschnitts liegt der erste Ansatz der Kurvenanpassung aus
Abschnitt~\ref{sec:heuristics-curves} als Gerüst vor (\texttt{curve\_fit.hpp}): Eine Mess-Achsen-Kurve
über die Working-Set-Größe wird per kleinster Quadrate an das Modell $y = a\log_2 x + b$ angepasst --- die
für Cache-Hierarchie-Kurven erwartete Form~\cite{hill1989associativity}. Kein Fit ohne gültige Daten: Der
Fit-Status unterscheidet \emph{keine Daten}, \emph{unterbestimmt} und \emph{ungültige Daten} vom
Erfolgsfall --- nie ein \emph{Ok} mit NaN-Koeffizienten ---, und der Ergebnis-Leser übernimmt nur streng
numerische Zellen. Je Messkurve steht ein kubischer Spline über $\log_2 x$, natürlich oder
monotonie-erhaltend nach Fritsch und Carlson~\cite{fritsch1980monotone}, der auf monotonen Daten keine
Schein-Extrema erzeugt. Die Heuristik-Familie (\texttt{heuristik/}) enthält den Achsen-Spline, den
Break-Even-Finder über den Schnittpunkten zweier Kurven derselben Achse, den Messkurven-Lader, die
Offline-Clusterung der Workloads und den Optimierungs-Katalog; Break-Even-Kurven werden nur über die
w/ma/mi-gemessenen Organ-Achsen-Parameter gebildet. Die Kurvenbildung selbst entwickelt
Abschnitt~\ref{sec:heuristics-curves}; hier steht nur ihre Code-Gestalt.
\end{nurkurz}
...
\begin{nurkurz}
Implementiert sind damit die Realm-Wurzeln samt Ordnung und Rückkehr-Sperren, die Registries, die
Quellen-Abstraktion mit degradiertem Fallback, die zweistufige Hardware-Erkennung
(Abschnitt~\ref{sec:hw-probe}: Provenienz-Vokabular, SPD-Parser, Fehler-Domäne, Erhebungs-Kette als Chain
of Responsibility, Übersetzungszeit-Factory je Plattform-Zelle und Laufzeit-Verdikt Deklaration gegen
Erhebung) und der erste Fit-Ansatz mit Spline; die Anbindung dieser Querschnitts-Bausteine an Runner und
Prüf-Dock (der Kollektor der Erfolgs-Parameter) und die Abbildung der Registry-Namen auf die
Bestands-Spalten der Ergebnis-Dateien sind nicht implementiert. Die drei Konzept-Invarianten der
System-Achsen lauten: Sie \emph{permutieren} den Bau- und Mess-Raum --- die Komplex-Achse
\texttt{build\_target\_complex} ist der Command-Wrapper der Rekombination der drei Haupt-Achsen mit der
Unter-Achsen-Gruppe \texttt{build\_toolchain} und dem Scheduling mit fünf Dimensionen, und dieser
System-Multiplikator vervielfacht den Organ-Raum je Plattform ---; sie stehen nicht im
Organ-Permutationsbaum der \texttt{binary\_id}; und sie berühren die Binary-Identität nie
(Registry-XML\@: \texttt{binary\_id} $=$ \texttt{never}; nur die achtzehn Organ-Haupt-Achsen gehen in den
Achtzehn-Segment-Pfad ein). Das wahrt die Bauform selbst: Die Wurzel dockt nur lesend an die host-seitigen
Ergebnis-Strukturen an und bleibt der ABI-Grenze gegenüber unsichtbar.
\end{nurkurz}
...
%% INTERN-272: KA-085
Die Hybrid-Stufe ist der vierte Träger, als optionale Mux-Stufe zwischen CEB-Prüf-Dock und Tier-Binary
eingeschoben (Abschnitt~\ref{sec:heuristics-modes}); ihr Code-Stand liegt
...
Klassifikations-Schicht (Klassifikation, Concept-Gate, Strategie je Gattung, Synthese-Matrix) und drei weitere
...
Tiere samt Konzept und Gattung, und \texttt{comdare\_anatomy\_genus} meldet den \emph{Ziel}-Gattung (das, was
das Modul für den Aufrufer ist), nie den Reroute-Gattung der Klassifikation --- derselbe
...
Identität, und die Gattungs-Bau-Zulassung führt sechs Einträge, den sechsten für die Reroute-Gattung mit dem
...
\begin{tikzpicture}[font=\footnotesize,>=Stealth,
  ...
  \node[font=\bfseries\scriptsize,anchor=north west] at (-4.5,-3.15){Hybrid-\texttt{.so} (Konzept HeuristikAdapter, Reroute-Gattung) --- eingeschobene Mux-Stufe};
  ...
  \node[hb,fill=blue!16] (ceb) at (-2.4,2.75){CEB-Prüf-Dock (Stufe 2)\\Anschluss (Mux) der optionalen Hybrid-Stufe};
  \node[hb,fill=gray!10] (tier) at (2.3,2.75){Tier-Binary (Stufe 3)\\Standardweg ohne Hybrid: direkt am CEB-Prüf-Dock};
  \draw[->,thick] (ceb)--node[right,font=\tiny]{Antrieb}(proxy);
  \draw[->,thick,dashed] (ceb)--node[above,font=\tiny]{Standardweg}(tier);
  ...
  \draw[->,dashed] (xml)--(d1);
  \node[ex] (sym) at (6.3,0.9){Exporte: sieben Pflicht-Symbole wie plain Tiere; \texttt{genus()} meldet den Ziel-Gattung (Weg C)};
  \draw[->,dashed] (frame.east)--(sym);
  \node[ex] (st) at (6.3,-1.4){Stempel-Kette der Hybrid-Stufe; Mess-Gate-Felder \texttt{hm}/\texttt{hmi}; Gattungs-Bau-Zulassung: 6.\ Eintrag};
\end{tikzpicture}
\caption[Die Hybrid-Stufe im Code]{Die Hybrid-Stufe im Code: die optional zwischen CEB-Prüf-Dock und Tier-Binary
eingeschobene Mux-Stufe, am CEB-Prüf-Dock angesetzt und mit Dock-Slots für Tier-Binaries (Standardweg ohne
Hybrid: Prüf-Dock direkt zur Tier-Binary). Der Binary-Proxy macht die Hybrid-.so nach außen zu einer
...
buchstabengleich zu den plain Tieren, \texttt{genus()} meldet den Ziel-Gattung. Nicht implementiert: \texttt{.so}-Export,
...
\begin{nurkurz}
\subsection{Muster-Register, Hybrid-Stufe und Lager}\label{sec:impl-hybrid-lager}
Die Entwurfsmuster des Apparats sind ein Register (Abbildung~\ref{fig:muster}); die Benennung folgt Gamma
et al.~\cite{gamma1994patterns} (\enquote{Gang of Four}, GoF), und jedes Muster ist übersetzungszeitlich
statt laufzeit-polymorph ausgeführt, weil der Hot-Path keine Laufzeit-Verzweigung enthalten darf
(Abschnitt~\ref{sec:axes-impl}): \emph{Strategy} je Achse (Bausteine hinter einem Concept, über
CRTP~\cite{coplien1995crtp} gebunden), \emph{Director} und \emph{Builder} für die Plan-Emission, eine
\emph{Abstract Factory} für die Hybrid-Docks und die Ergebnis-Mappen des Lagers (xlsx-Mappe als Standard,
CSV-Baum als wählbare Strategie derselben Factory; implementiert ist eine flache Datei), der
\emph{Visitor} \texttt{IMessVisitor} am \texttt{dlopen}-Übergang, eine \emph{Chain of Responsibility} mit
Typliste für die Hardware-Erhebungskette (Boot-Cache --- die gefilterte SMBIOS/DMI-Tabelle des
Systems~\cite{dmtf_smbios} --- $\to$ SPD $\to$ Deklaration) und die Auswahl-Filterkette der CEB\@; ein
\emph{Memento} für den Zustand statusbehafteter Achsen je Tier (\texttt{memento\_all} nach dem Warmlauf),
\emph{Command} als Command-Basis der Achsen und Command-Wrapper der Komplex-Achse
\texttt{build\_target\_complex} sowie eine \emph{Facade} für die Anhang-Erzeugung --- Programm~08 als
Fassade über die Programme 04 (\texttt{csv\_to\_latex}) und 05 (\texttt{diagram\_generator}) --- und die
lesende Plan-Emission des Planers.
\begin{figure}[htbp]\centering
%% INTERN-272: KA-084
\begin{tikzpicture}[font=\footnotesize,
  mu/.style={draw,rounded corners,fill=blue!7,minimum width=3.6cm,minimum height=2.55cm,text width=3.4cm,align=center,inner sep=2pt}]
...
Die Hybrid-Stufe ist der vierte Träger, als optionale Mux-Stufe zwischen CEB-Prüf-Dock und Tier-Binary
eingeschoben (Abschnitt~\ref{sec:heuristics-modes}); ihr Code-Stand liegt
unter \texttt{hybrid/}. Implementiert sind die Dock-Verträge (vier Vertragsformen \texttt{standard},
\texttt{rollback}, \texttt{scan} und \texttt{resource\_control} mit sechzehn Status-Konstanten), das
Prüf-Dock, die Dock-Factory als einziger Konstruktions-Ort, das Dock-Array mit \texttt{HybridDockVariant}
(die eine erlaubte variant-Stelle, Abschnitt~\ref{sec:axes-impl}) und dem Laufzeit-Deckel
\texttt{max\_docks}, der Parser der \texttt{<hybrid\_tier>}-Sektion und der Binary-Proxy, der eine
Hybrid-.so nach außen zu einer gewöhnlichen Tier-Binary macht; nicht implementiert sind \texttt{.so}-Export,
Eviction und Router --- sechs der neun Dateien der Schicht. Die Hybrid Module ABI exportiert dieselben
Pflicht-Symbole wie die plain Tiere, \texttt{comdare\_anatomy\_genus} meldet den \emph{Ziel}-Gattung, nie
den Reroute-Gattung, und die Gattungs-Bau-Zulassung führt als sechsten Eintrag die Reroute-Gattung mit dem
Dock-Deckel (Default 32) als Arität. Das Hybrid-Live-Ranking als vierte Auswertungs-Komponente der
compare-Stufe und der Zustandsabgleich per Memento mit zwei Release-Artefakten, single und hybrid, im
Lager sind vorgesehen (Abschnitt~\ref{sec:heuristics-modes}), aber nicht implementiert, ebenso nicht die
Behebung der Schema-Drift (das XSD führt die \texttt{<hybrid\_tier>}-Sektion nicht); der Hybrid-Modus
kompiliert nur mithilfe der CEB\@.

%% INTERN-272: KA-087
Das Lager (Abschnitt~\ref{sec:lager}) hat im Code ein übersetzungsstatisches Schlüssel-Schema
(\texttt{bestand\_\allowbreak{}schluessel\_\allowbreak{}schema}), das die vier Bestände deklariert:
Bestand~1, die Binaries, über den einen SHA-512-Schlüssel, der mit dem Tier-Fingerprint identisch ist;
Bestand~2, die Messwerte, über den 128-Hex-Fingerprint der vermessenen Binary und die Hardware-Identität
der messenden Maschine; Bestand~3, die Synthese-Batches, und Bestand~4, die Lösungen, als Arbeitsschritt
des Lager-Vollausbaus; Drift zwischen Schema und Mechanik bricht beim Bau. Der Versions-Lock
(\texttt{axis\_version.lock}, Format v3, 719 Datensätze) bindet jede Quell-Datei der Achsen --- die
Heuristik-Header und den Overlay-Schnitt (je Achse die Dateimenge ihrer eigenen Implementierung plus die
gemeinsame Anatomie-Substanz), über den das Overlay-Glied des Tier-Fingerprints hasht --- an
Inhalts-Digest und gestempelte Version, das CI-Gate \texttt{contract:axis-version-lock} erzwingt das. Aus
dem Schema folgen die Lager-Eigenschaften: Die Messung ist immer eingeschaltet, im Lager wird nur
nachgemessen, was fehlt; ein Versionswechsel einer Achse baut den betroffenen Lager-Stapel neu; und jeder
Maschinentyp hat seinen eigenen Messwert-Bestand. Umsetzungsstand: Schema, Lock und die Bestände~1 und~2
sind implementiert, der Vollausbau ist vorgesehen, aber nicht implementiert.
\end{nurkurz}
