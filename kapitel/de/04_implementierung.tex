...
Auswertungs-Programmen 04 und 08, E3 der Experiment-Baum (\texttt{experiment\_tree}) mit der
...
\texttt{coherence\_invalidations} die Cache-zu-Cache-Fills aus einem fremden CCX ---; für Intel liegt kein
Eintrag vor, weil eine RAW-Belegung ohne Gegenprüfung ein Rateversuch wäre~\cite{intel_sdm} --- eine
ausgewiesene Lücke. Eine Vendor-Registry führt die PMC als \emph{ein} Merkmal mit zwei Werten, AMD und
...
Typliste für die Hardware-Erhebungskette und die Auswahl-Filterkette der CEB\@; ein \emph{Memento} für den
Zustand statusbehafteter Achsen je Tier (\texttt{memento\_all} nach dem Warmlauf), \emph{Command} als
Command-Basis der Achsen und Command-Wrapper der Komplex-Achse \texttt{build\_target\_complex} sowie eine
\emph{Facade} für die Anhang-Erzeugung (Programm~08) und die lesende Plan-Emission des Planers.
\begin{figure}[htbp]\centering
%% INTERN-272: KA-084
\begin{tikzpicture}[font=\footnotesize,
  mu/.style={draw,rounded corners,fill=blue!7,minimum width=3.6cm,minimum height=2.55cm,text width=3.4cm,align=center,inner sep=2pt}]
...
(\texttt{axis\_version.lock}, Format v3, 719 Datensätze) bindet jede Quell-Datei der Achsen an
