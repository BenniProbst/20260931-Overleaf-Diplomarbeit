...
Boost.Mp11~\cite{boost_mp11}) die \texttt{Enabled*}-Liste bildet, und eine Composition belegt je Slot genau
...
Allokator-Adapter ersetzen den fehlenden Vendor durch \texttt{std::malloc}/\texttt{std::free},
Datenstruktur-Adapter durch einen Stub, der bei Benutzung kontrolliert eine
\texttt{std::runtime\_error}-Ausnahme wirft, sodass der Build eigenständig lauffähig bleibt. Die vom
...
quarantänierter Test-Cluster mit genau einem Test als Konsument und einer Striktheits-Wache, die jede
...
eine zugelassene variant-Stelle außerhalb der CEB ist das Dock-Array der Hybrid-Stufe
...
der CEB und in zwei achsenfremden Bausteinen (der Fehler-Domäne \texttt{axis\_error} und dem
Druck-Zustand \texttt{pressure\_state}), die keinem Organ angehören. Das Verbot ist damit strenger als
...
%% OWNER-VORLAGE: OV-K4-9 --- Gate 2 ohne Ausnahmemenge: die einzige deklarierte allow_failure-Stelle der
%% Flotte ist eine Plattform-Grenze (arm64-Smoke ohne Runner), keine Testmenge; Alternative: Owner nennt
%% eine Liste außerhalb der YAML. Widerspruch genügt.
\emph{Gate~2} ist das Integrations-Gate des Projekt-Repositories: Seine Pipeline baut grün ohne
Ausnahmemenge --- die Hausdoktrin kennt keine geduldeten roten Tests; die einzige deklarierte
Fehlschlag-Duldung der Flotte ist eine Plattform-Grenze im Engine-Repository (ein arm64-Smoke-Bau ohne
verfügbaren Runner), keine Testmenge.
...
Bau-Steuergrößen --- C++-Standard, Optimierung, SIMD-Erweiterung, CEB, Ziel-ISA, Telemetrie, Bau-Typ und
...
und über die Kern-Pinning-Bausteine angelegt; Durchführungs-Stand: Das Laufzeit-Pinning je Kern-Sorte ist
als Bau-Schritt benannt.
