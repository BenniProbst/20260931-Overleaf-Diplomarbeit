...
sich in Anhang~\ref{app:blocks}. Als nicht-baumartige Gegenprobe innerhalb derselben
Suchalgorithmus-Gattung tritt zusätzlich eine reine Hash-Tabelle
(\texttt{absl::flat\_hash\_map}/SwissTable) an: Sie realisiert dasselbe
\texttt{std::map}-Interface über ein \emph{limitiertes Achsen-Subset} --- die baum- und
trie-spezifischen Achsen (Pfadkompression, Knotentyp, Bereichs-Index) entfallen als
Durchreich-Variante, während Allokation, Prefetch, Nebenläufigkeit, Speicher-Layout und
ISA-SIMD (für das SwissTable-Gruppen-Lookup) geteilt bleiben. Sie belegt damit, dass das
Achsen-Framework auch nicht-baumartige Suchverfahren trägt, ohne eine eigene Gattung zu
erfordern.
