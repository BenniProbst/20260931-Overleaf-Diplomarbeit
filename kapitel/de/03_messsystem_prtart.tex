...
\end{tikzpicture}
\caption{Der Entwurfsraum: die Achsen spannen ein multidimensionales kartesisches Produkt auf; ein konkreter Algorithmus ist ein Punkt darin. Zur Veranschaulichung sind drei der achtzehn Organ-Achsen gewählt --- das reale Speicherorgan aus Knotentyp~(T4), Speicher-Layout~(T5) und Allokator~(T6); der volle Entwurfsraum hat achtzehn Organ-Dimensionen; dazu tritt eine optionale neunzehnte Organ-Achse für die Festplatten-Ein-/Ausgabe, deren Beschreibung selbst beschrieben wird (Abschnitt~\ref{sec:axis-triad}).}\label{fig:design-space}
\end{figure}
...
anderen Achsen verteilte Interfaces für deren Detail-Operationen --- genau diese \emph{Achsen-Interfaces} trägt das
...
Modellannahme gesetzt ---; Durchführungs-Stand: Die plattform-kalibrierten Schwellwerte werden mit den
realen Läufen festgelegt (Abschnitt~\ref{sec:answers}). Diese Konzepte bilden die
...
geht in der Kette nicht über den hybrid-compare nach dem single-release hinaus. Von diesen vier
...
originalgetreu durchzumessen; er ist nicht gebaut und bleibt Ausblick (Abschnitt~\ref{sec:outlook}).
