...
Kapitel~\ref{ch:impl} (Abschnitt~\ref{sec:axes-impl}); zuvor ordnet der folgende Unterabschnitt den
...
verstreuten Mess-Bestandteile unter dieser Wurzel ist Teil der Umsetzung (Kapitel~\ref{ch:impl}).
...
ausgeführt --- sind damit: \emph{Reihe~A} (PRT-ART vs.\ Stand der Technik) mit den Modi A\_defined (8
...
PRT-ART-OLC + tcmalloc + ART-Page). Jede Reihe wird in drei Granularitäten erhoben: \emph{Micro-Benchmarking}
...
Den so aufgespannten Raum reduziert das System gezielt. Mit der Erweiterung auf neunzehn Achsen plus rund
vierundsechzig Sub-Achsen wächst er auf über $10^{14}$ Permutationen; drei Mechanismen dämpfen die
Explosion: der \texttt{<expected\_workload>}-Profil-Filter routet nur kompatible Workloads, der
\texttt{Defined}-Modus führt nur die deklarierten Tupel aus (rund 100--1000 je Reihe, die ins Manuskript
