...
Identitäten zählt (unter der überholten Annahme genau zweier Auslegungen je Achse), während die Obergrenze der
Binary-Identitäten je System-Permutation das Produkt der je Achse im XML-Experiment gewählten Auslegungen ist,
das der Planer aus den Eigenschaften des XML-Experiments bestimmt und das erst zur Ausführung bekannt ist
(Abschnitt~\ref{sec:mess-chain}). Diese Rekombination wird in
...
Vorgesehen ist, alle verfügbaren Benchmark-Systeme für das Messsystem unter dem Dach dieser Achse zu
vereinen; im Code wie im Plan gelten die Benchmark-Systeme dabei über ihre Lastprofile als Unter-Achse des
Messsystems (der Laufzeit-Parameter \texttt{workload} des Last-Rahmenwerks), weil sie laufzeitspezifische
Änderungen der Kette von der XML zur Tier-Binary (Abschnitt~\ref{sec:mess-chain}) verwalten: Lastprofile und
Datensätze wechseln, ohne dass die Tier-Binary neu übersetzt wird.
\end{nurlang}
\begin{nurkurz}
Auch die \emph{Lasten} erscheinen als Instanzen. Die in der Literatur verwendeten Workloads entstammen
...
eine reine Hash-Tabelle als Gegenprobe innerhalb des Map-Konzepts --- als zwei wählbare
...
erfordert so kein eigenes Konzept.
\end{nurlang}
\begin{nurkurz}
...
belegt eine reine Hash-Tabelle als Gegenprobe innerhalb des Map-Konzepts: ein Open-Addressing-Hash nach Knuth
...
erfordert kein eigenes Konzept.
\end{nurkurz}
...
al.\ (P16/P17)~\cite{bender2000cobtree,bender2005coboblivious,bender2002treelayout,bender2002scanning,bender2002orderlist}
und Saikkonen/Soisalon-Soininen
...
Konformitäts-Gate gegen \texttt{std::map}),
...
bleiben dasselbe Konzept mit eingeschränktem Profil, denn ein eigenes Konzept entstünde erst, wenn sich
...
anderer. Deshalb gehört jedes Verfahren, das einen Schlüssel in einen Wert übersetzt, gleich wie, zum
Konzept Map und seiner Gattung \texttt{SearchAlgorithm} (Abschnitt~\ref{sec:anatomy-levels}). Zweitens
...
\begin{nurkurz}
Nicht-baumartige Suchverfahren --- etwa die Hash-Gegenprobe aus Abschnitt~\ref{sec:sota-instances} ---
bleiben dasselbe Konzept mit eingeschränktem Profil: Auch eine Hash-Tabelle ist ein Baum, der von der Wurzel
seines Interfaces über eine Auffächerung an Hashes sucht; jedes Verfahren, das einen Schlüssel in einen Wert
übersetzt, gehört zum Konzept Map und seiner Gattung \texttt{SearchAlgorithm} (Abschnitt~\ref{sec:anatomy-levels}).
...
Achsen-Matrix gesammelt werden. Software-technisch ist eine Achse ein Feature, das \emph{Konzept} die
Feature-\emph{Kern}-Komposition (der gemeinsame Interface-Kern aller Gattungen), die \emph{Gattung} die
Feature-\emph{Spezial}-Komposition (das reale Tier-Interface über den Kern hinaus) und das \emph{Genus} ein
konkretes Produkt dieser Spezial-Komposition (die Implementierung mit ihrer Achsen-Belegung,
...
eigene Container-Gattungen, und Teile des Achsen-Pools werden für die Container wiederverwendet
...
\begin{nurlang}
%% INTERN-272: KA-014
Eine Achse entspricht einem Feature, ein Lebewesen einer Feature-Komposition. Das \emph{Konzept} ist die
Feature-\emph{Kern}-Komposition: der gemeinsame Interface-Kern, den alle Gattungen eines Konzepts teilen und
der in der Metaprogrammierung über alle Variationen hinweg gleich verwendet wird --- so wie verschiedene
Map-Kategorien (Hash-Map, flache Map) einen gemeinsamen Kern an Interface-Funktionen teilen; für das
Container-Konzept besteht dieser Kern aus Identität, Achsen-Beobachtung und Größe. Die \emph{Gattung}
(Abschnitt~\ref{sec:anatomy-levels}) ist die Feature-\emph{Spezial}-Komposition:
...
eine statische Feature-Set-Definition; das \emph{Genus} ist ein konkretes Produkt dieses Feature-Sets, die
Implementierung des Gattungs-Interfaces mit ihrer Achsen-Belegung. Die Anatomie ist die Verdrahtung zwischen den Features. Welche
...
\end{figure}
%% INTERN-272: KA-016
\subsection{Das Drei-Ebenen-Modell und die vier Konzepte}\label{sec:anatomy-levels}
...
darunter lassen sich drei Interface-Ebenen unterscheiden: das \emph{Konzept} als nach außen sichtbare
Mess-Surface (die Tierklasse mit ihrem \emph{Prüf-Dock}), darunter die \emph{Gattung} als Tier-Interface,
darunter das \emph{Genus} als Implementierung dieses Interfaces; die \emph{Organe} bilden unterhalb des
Genus eine eigene Klassifikation (Abbildung~\ref{fig:three-levels})\@. Die
Ebenen-Trennung ist an der ABI-Grenze ablesbar: Jede Tier-Binary exportiert zwei buchstabengleiche
C-Symbole, die Konzept und Gattung ohne Instanz beantworten, wobei das Konzept nicht als zweites Datum
gepflegt, sondern zur Laufzeit aus der Gattung ermittelt wird --- der Lader ermittelt das Konzept so über ein
konzeptunabhängiges Symbol, ohne es vorab zu kennen\@. \enquote{Lebewesen} und \texttt{SearchAlgorithm} bezeichnen denselben
...
\begin{tikzpicture}[font=\footnotesize,>=Stealth,align=center,
  ...
  \node[lvl] at (-5.1,0){Ebene 0: Reich\\{\tiny Animalia}};
  \node[box] (root) at (1.1,0){\texttt{IExecutionEngine} (messbare Wurzel) $\to$ \texttt{IAnatomyBase} (anatomie-tragend)};
  \node[lvl] at (-5.1,-1.6){Ebene 1: Konzept\\{\tiny Tierklasse, Mess-Surface}};
  \node[box] (sa) at (-3.0,-1.6){Map\\{\scriptsize Säugetier}};
  \node[box] (co) at (0,-1.6){Container\\{\scriptsize Vogel}};
  \node[box] (gr) at (2.6,-1.6){Graph\\{\scriptsize Pflanze}};
  \node[box] (ha) at (5.3,-1.6){HeuristikAdapter\\{\scriptsize Insekt}};
  \node[lvl] at (-5.1,-3.3){Ebene 2: Gattung\\{\tiny Tier-Interface, Template}};
  \node[box] (tier) at (-3.0,-3.3){\texttt{SearchAlgorithm}\\{\scriptsize Elefant}};
  \node[box] (cont2) at (0,-3.3){\texttt{Set}, \texttt{Sequence},\\\texttt{Adapter}, \texttt{View}};
  \node[box,dashed,fill=gray!8] (gr2) at (2.6,-3.3){keine Gattung\\{\scriptsize implementiert}};
  \node[box] (ha2) at (5.3,-3.3){\texttt{FunctionInterface}-\\\texttt{Reroute} {\scriptsize(Ameise)}};
  \node[lvl] at (-5.1,-5.0){Ebene 3: Genus\\{\tiny Unterart, Implementierung}};
  \node[box,fill=blue!6] (gen) at (-3.0,-5.0){Mammut $=$ ART,\\{\scriptsize afrikanischer Elefant $=$ HOT, \dots}};
  \node[box,dashed,fill=gray!8] (gen2) at (0.3,-5.0){Genera je Gattung\\{\scriptsize (Entwurf, nicht im Code)}};
  \node[lvl] at (-5.1,-6.6){Ebene 4: Organe\\{\tiny eigene Klassifikation}};
  \node[box,fill=green!8] (org) at (-3.0,-6.6){Achsen T0--T17};
  \node[box,fill=green!8] (org2) at (0.3,-6.6){Achsen des Pools};
...
\end{tikzpicture}
\caption{Das Drei-Ebenen-Modell: die drei Interface-Ebenen Konzept (Ebene~1, Tierklasse, Mess-Surface: Prüf-Dock) $\to$ Gattung (Ebene~2, Tier-Interface, Template nach außen) $\to$ Genus (Ebene~3, Unterart: Implementierung des Gattungs-Interfaces, ggf.\ mit erweiterten Spezial-Interfaces); darunter die Organe/Achsen (Ebene~4, eigene Klassifikation), darüber das Reich (Ebene~0). Vier Konzepte: \texttt{Map} (Säugetier) besitzt als einzige Gattung \texttt{SearchAlgorithm} (Elefant), deren Genera die konkreten Implementierungen sind (Mammut $=$ ART, afrikanischer Elefant $=$ HOT, indischer Elefant $=$ START); \texttt{Container} (Vogel) die Gattungen Set/Sequence/Adapter/View (Specht/Storch/Kakadu/Falke), die aus demselben Achsen-Pool gebildet werden; \texttt{Graph} (Pflanze) ist als Interface angelegt, eine Gattung ist nicht implementiert; \texttt{HeuristikAdapter} (Insekt) besitzt die Reroute-Gattung \texttt{FunctionInterfaceReroute} (Ameise) des Hybrid-Tiers. Heute trägt der Code das Genus nur als Achsen-Belegung; die Genus-Implementierungsklasse ist ein erkannter Architekturfehler (Behebung nach der Verteidigung). Tier-Binary (Ebene~5a) und Hybrid-Tier (Ebene~5b) sind die Nachphase der Kompilation (Abbildung~\ref{fig:genera}).}\label{fig:three-levels}
\end{figure}
\end{nurlang}
\begin{nurkurz}
...
darunter liegen drei Interface-Ebenen: das \emph{Konzept} als nach außen sichtbare Mess-Surface (die
Tierklasse mit ihrem \emph{Prüf-Dock}), die \emph{Gattung} als Tier-Interface und das \emph{Genus} als
dessen Implementierung; die \emph{Organe} bilden unterhalb des Genus eine eigene Klassifikation
(Abbildung~\ref{fig:three-levels}). Die Ebenen-Trennung ist an der ABI-Grenze ablesbar:
Jede Tier-Binary exportiert zwei buchstabengleiche C-Symbole, die Konzept und Gattung ohne Instanz beantworten,
wobei das Konzept zur Laufzeit aus der Gattung ermittelt wird.\ \enquote{Lebewesen} und \texttt{SearchAlgorithm} bezeichnen
...
\begin{nurlang}
Auf der obersten Ebene stehen vier Konzepte. Das \texttt{Map}-Konzept --- das
schlüssel-wert-orientierte Prüf-Dock und der Fokus dieser Arbeit --- besitzt als einzige Gattung
\texttt{SearchAlgorithm} den vollen Achtzehn-Achsen-\enquote{Elefanten}; das \texttt{Container}-Konzept
(schlüsselloser Speicher) fächert in die Gattungen Set, Sequence, Adapter und View auf, die ebenfalls aus
Achsen des gemeinsamen Pools gebildet werden und deren Slot-Zahl je Gattung übersetzungsstatisch
festliegt; das \texttt{Graph}-Konzept ist als Interface angelegt, eine Gattung ist nicht implementiert; und
das \texttt{HeuristikAdapter}-Konzept besitzt die Reroute-Gattung \texttt{FunctionInterfaceReroute} --- die
Gattung des Hybrid-Tiers (Abschnitt~\ref{sec:heuristics-modes}), das per Metaprogrammierung die
Interfaces eines Konzepts samt Gattung erbt und Anfragen an die eigentlichen Tier-Binaries durchstellt;
Klassifikation (welcher Art das Reroute ist) und Durchstellung (die geerbte Ziel-Gattung, die die Binary
nach außen meldet) liegen dabei auf getrennten Ebenen (Abbildung~\ref{fig:genera}). Merkregel:
\enquote{Adapter} ohne Präfix meint immer die Container-Gattung; das Hybrid-Konzept heißt stets
...
Nicht-baumartige Verfahren erfordern so kein eigenes Konzept, sondern belegen ein eingeschränktes
...
\begin{tikzpicture}[font=\footnotesize,>=Stealth,align=center,
  ...
  \node[draw,rounded corners,fill=red!6,font=\footnotesize] (root) at (1.3,1.6){Ebene 0: Reich \enquote{Animalia} ($\equiv$ \texttt{IAnatomyBase}, generische Mess-Methoden)};
  %% INTERN-272: KA-018
  \node[gen] (g1) at (-4.0,0){Map\\{\scriptsize\mdseries Säugetier}};
  \node[gen] (g2) at (0.6,0){Container\\{\scriptsize\mdseries Vogel}};
  \node[gen] (g3) at (4.3,0){Graph\\{\scriptsize\mdseries Pflanze}};
  \node[gen,minimum width=2.9cm] (g4) at (7.4,0){HeuristikAdapter\\{\scriptsize\mdseries Insekt}};
  \draw[->] (root)--(g1); \draw[->] (root)--(g2); \draw[->] (root)--(g3); \draw[->] (root)--(g4);
  %% INTERN-272: KA-019
  \node[sub,fill=green!18] (s1) at (-4.0,-1.4){\textbf{Elefant} $=$ \texttt{SearchAlgorithm}\\{\scriptsize \texttt{map}-Hülle $\cdot$ 18 Organ-Achsen}};
  \node[sub,anchor=west] (c1) at (-1.2,-1.25){Specht $=$ Set {\scriptsize(\texttt{set})}};
  \node[sub,anchor=west] (c2) at (-1.2,-2.05){Storch $=$ Sequence {\scriptsize(\texttt{vector})}};
  \node[sub,anchor=west] (c3) at (-1.2,-2.85){Kakadu $=$ Adapter {\scriptsize(\texttt{stack})}};
  \node[sub,anchor=west] (c4) at (-1.2,-3.65){Falke $=$ View {\scriptsize(\texttt{span})}};
  \node[tbd,text width=2.4cm,anchor=north] (gr1) at (4.3,-0.8){Interface angelegt,\\{\scriptsize keine Gattung}\\{\scriptsize implementiert}};
  \node[sub,minimum width=2.9cm,text width=2.9cm,anchor=north] (h1) at (7.4,-0.8){\texttt{FunctionInterface}\\\texttt{Reroute} $=$ Ameise\\{\scriptsize Hybrid-Tier:}\\{\scriptsize geerbte Ziel-Gattung}};
  ...
  %% INTERN-272: KA-021
  \node[band,minimum height=1.5cm] (l3) at (1.3,-5.0){\textbf{Ebene 3 --- Genus (Unterart):} Implementierung des Gattungs-Interfaces, ggf.\ mit erweiterten Spezial-Interfaces --- Mammut $=$ ART, afrikanischer Elefant $=$ HOT, indischer Elefant $=$ START, \dots; unter dem Genus liegen eine oder mehrere Suchstrategien (einfach $=$ Belegung einer einzelnen Achse, komplex $=$ Komposition mehrerer Achsen). Heute trägt der Code das Genus nur als Achsen-Belegung; die Genus-Implementierungsklasse ist ein erkannter Architekturfehler (Behebung nach der Verteidigung).};
  \node[band,minimum height=1.5cm] (l4) at (1.3,-6.75){\textbf{Ebene 4 --- Organe:} die 18 Organ-Achsen (Knotentyp, Layout, Allokator, Prefetch, \dots) bilden eine eigene Klassifikation mit Achsen-Interfaces und \emph{permutieren}; die Genus-Implementierung bindet sie: ein Vollprofil belegt \emph{alle} Achsen (ART, HOT, Masstree, SuRF, START, Wormhole, \dots; 30 Vollprofile), ein abstraktes Profil (PRT-ART, OLC, LOUDS, VAMPIR) nur einige; die Container-Gattungen bilden sich aus demselben Achsen-Pool.};
  \node[band,fill=orange!20,very thick,minimum height=1.4cm] (l5) at (1.3,-8.45){\textbf{Ebene 5a --- Tier-Binary (Nachphase der Kompilation, das Individuum):} je Permutation kompiliert zu \texttt{comdare\_perm\_\-<fp>.dll}\,/\,\texttt{.so}, verfeinert um die Prüfling-Build-Achse Seitentyp; die Plattform stempeln die drei System-Achsen, die Mess-Wahl die Mess-Zeile; jede Tier-Binary bietet die drei Interface-Flächen (Abbildung~\ref{fig:traeger-flaechen}).};
  \node[band,fill=orange!14,minimum height=0.95cm] (l5b) at (1.3,-9.85){\textbf{Ebene 5b --- Hybrid-Tier:} zwischen dem CEB-Prüfdock und den Tier-Binaries eingeordnet als Multiplexer mit Slots; es erbt die Ziel-Gattung und stellt Anfragen an die Tier-Binaries in seinen Slots durch.};
...
\end{tikzpicture}
\caption[Die vier Konzepte und die Tier-Hierarchie bis zum kompilierten Binary]{Die Konzept-/Tier-Hierarchie über die Ebenen~0 bis~5. Ebene~0 ist das Reich \enquote{Animalia} (\texttt{IAnatomyBase}, die generischen Mess-Methoden). \texttt{Map}, \texttt{Container}, \texttt{Graph} und \texttt{HeuristikAdapter} sind die vier \emph{Konzepte} (Ebene~1, die Tierklassen Säugetier/Vogel/Pflanze/Insekt: Mess-Surface und Prüf-Dock). Jedes Konzept beherbergt seine \emph{eigenen} Gattungen (Ebene~2, die Tier-Interfaces): \texttt{Map} ausschließlich den \enquote{Elefanten} (Gattung \texttt{SearchAlgorithm}, volle Achtzehn-Achsen-Anatomie), \texttt{Container} die Vogel-Gattungen Specht/Storch/Kakadu/Falke $=$ Set/Sequence/Adapter/View, \texttt{Graph} ein angelegtes Interface ohne implementierte Gattung, \texttt{HeuristikAdapter} die Reroute-Gattung \enquote{Ameise} des Hybrid-Tiers. Der Elefanten-Ast ist bis zum Individuum ausgezogen: die Genera (Ebene~3, Unterarten $=$ Implementierungen des Gattungs-Interfaces, z.\,B.\ Mammut $=$ ART, afrikanischer Elefant $=$ HOT, indischer Elefant $=$ START) binden die achtzehn Achsen-Organe (Ebene~4, eine eigene Klassifikation), die zu einer konkreten Achsen-Komposition permutieren; jede Permutation wird als eigenständige Tier-Binary kompiliert (Ebene~5a, die Nachphase der Kompilation und das Individuum, zusätzlich verfeinert um die Prüfling-Build-Achse Seitentyp und plattform-gestempelt durch die drei System-Achsen, Abschnitt~\ref{sec:axis-triad}); das Hybrid-Tier (Ebene~5b) ist zwischen dem CEB-Prüfdock und den Tier-Binaries als Multiplexer mit Slots eingeordnet\@. \emph{Jede Tier-Binary ist damit ein eigenes Tier mit einem tatsächlichen Binary-Gattungs-Interface und drei Interface-Flächen}; die Hierarchie reicht vom Reich bis zur einzelnen kompilierten Permutation. Vgl.\ das verdichtete Modell in Abbildung~\ref{fig:three-levels}.}\label{fig:genera}
\end{figure}
\end{nurlang}
\begin{nurkurz}
Auf der obersten Ebene stehen vier Konzepte (Abbildung~\ref{fig:genera}): \texttt{Map} --- das
schlüssel-wert-orientierte Prüf-Dock und der Fokus dieser Arbeit --- besitzt als einzige Gattung
\texttt{SearchAlgorithm} den vollen Achtzehn-Achsen-\enquote{Elefanten}; \texttt{Container} fächert in die Gattungen
Set, Sequence, Adapter und View auf, die aus demselben Achsen-Pool gebildet werden; \texttt{Graph} ist als
Interface angelegt, eine Gattung ist nicht implementiert; \texttt{HeuristikAdapter} besitzt die Reroute-Gattung
\texttt{FunctionInterfaceReroute} --- die Gattung des Hybrid-Tiers (Abschnitt~\ref{sec:heuristics-modes}), das
per Metaprogrammierung die Interfaces eines Konzepts samt Gattung erbt und Anfragen an die eigentlichen
Tier-Binaries durchstellt. Merkregel: \enquote{Adapter} ohne Präfix meint immer die Container-Gattung, das
Hybrid-Konzept heißt stets HeuristikAdapter; das Reich \enquote{Animalia} ($\equiv$ \texttt{IAnatomyBase}) ist
...
erzeugt ---, der Tier-Binary und der optional zwischen CEB-Prüf-Dock und Tier-Binary eingeschobenen
Hybrid-Tier-Binary. Der Planer löst die Anwender-XML gegen die drei
...
Gattungs-Interface einer Tier-Binary (Laufzeit, Abstract Factory), Fläche~2 der Stempel (Übersetzungszeit,
ABI-stabil, Abschnitt~\ref{sec:stamp-model}), Fläche~3 der Mess-Durchstich (\texttt{IMessVisitor}), der
die Messung als eigene Naht quert, damit Konzept- und Gattungs-Interfaces unverändert bleiben. An Tier-
...
\begin{tikzpicture}[font=\scriptsize,>=Stealth,align=center,
  ...
  %% INTERN-272: KA-022
  \node[tr] (t0) at (0,0){Planer\\{\tiny\mdseries Stufe 1}};
  \node[tr] (t1) at (2.5,0){CEB\\{\tiny\mdseries Stufe 2 (Prüf-Dock)}};
  \node[tr] (t2) at (7.5,0){Tier-Binary\\{\tiny\mdseries Stufe 3}};
  \node[tr,dashed] (t3) at (5.0,0){Hybrid-Tier\\{\tiny\mdseries Einschub, optional}};
  \draw[->,thick] (t0)--(t1); \draw[->,thick,dashed] (t1)--(t3); \draw[->,thick,dashed] (t3)--(t2);
  \draw[->,thick] (t1.north) -- ++(0,0.32) coordinate (bp) -| (t2.north);
  \node[font=\tiny,anchor=south,text width=10cm] at ([xshift=1.25cm,yshift=1pt]bp){Konfiguration wandert: XML $\to$ Planer $\to$ CEB $\to$ (Hybrid) $\to$ Tier (keine Stufe durchläuft Schichten)\\gestrichelt: optionaler Einschub --- Hybrid als Mux-Schalter am CEB-Prüf-Dock mit Dock-Slots für Tier-Binaries; Bogen: Standardweg CEB $\to$ Tier ohne Hybrid};
  %% INTERN-272: KA-023
  ...
  \node[v] at (0,-4.17){verboten}; \node[v] at (2.5,-4.17){verboten}; \node[v] at (7.5,-4.17){verboten}; \node[p] at (5.0,-4.17){Pflicht (RT-Hook)};
  %% INTERN-272: KA-024
  ...
  \node[fl] at (7.5,-5.25){1 Gattungs-Interface\\2 Stempel\\3 Mess-Durchstich \texttt{IMessVisitor}};
  \node[fl] at (5.0,-5.25){1 geerbte Ziel-Gattung\\2 Stempel $+$ angeschlossene\\3 Mess-Durchstich};
...
\end{tikzpicture}
\caption{Träger-Stufen und Interface-Flächen: die dreiwertige Zulassungsmatrix der sieben Stempel-Interfaces über die vier Träger-Stufen Planer, CacheEngineBuilder (CEB), Hybrid-Tier-Binary (optionaler Einschub) und Tier-Binary (28 Zellen; grün $=$ Pflicht, grau $=$ verboten; die Zelle Organ-Zeile/CEB ist die statische Sperre \enquote{der CEB führt keine Organ-Achsen}) und darunter die Interface-Flächen je Stufe --- Fläche~1 Gattungs-Interface (Laufzeit), Fläche~2 Stempel (Übersetzungszeit, ABI-stabil), Fläche~3 Mess-Durchstich. Die Konfiguration wandert von der XML über Planer und CEB bis zur Tier-Binary; die Hybrid-Tier-Binary ist eine optional zwischen CEB-Prüf-Dock und Tier-Binary eingeschobene Stufe (gestrichelt: Mux-Schalter am Prüf-Dock mit Dock-Slots für Tier-Binaries; die Spaltenordnung der Matrix folgt der Verarbeitungs-Topologie mit dem Hybrid an seiner Einschub-Position, der Bogen ist der Standardweg ohne Hybrid); keine Stufe durchläuft Schichten, jede enthält ihre Schicht einkompiliert.}\label{fig:traeger-flaechen}
\end{figure}
\end{nurlang}
\begin{nurkurz}
...
optional zwischen CEB-Prüf-Dock und Tier-Binary eingeschobenen Hybrid-Tier-Binary. Der Planer löst die
Anwender-XML gegen die drei Angebots-Kataloge auf und wählt zu seiner Laufzeit die Mess-Achsen; der CEB enthält diese Mess-Wahl einkompiliert, kennt die Plattform und permutiert
...
Fläche~1 ist das Gattungs-Interface einer Tier-Binary (Laufzeit), Fläche~2 der Stempel (Übersetzungszeit,
...
(Abbildung~\ref{fig:modi-kette}). Einen Debug-Arbeitsmodus gibt es nicht; die Bau-Varianten Debug, Messung und Release gelten fort, und die Debug-Variante wird weiter ausgebaut: \texttt{--debug} ist ein Schalter der Planer-Shell für den Verdrahtungs-Check --- ein Flag und kein eigener
Arbeitsmodus der Kette build/measure/compare/release; es schaltet den indirekten Debug-Modus der Debug-Variante ein
(den nicht offiziellen fünften Wartungsmodus): unter \texttt{--debug} fährt das Konformitäts-Gate zusätzlich den
Standard-Testsatz des Tier-Interfaces (wie geplant: Tests nur mit den Debug-Binaries,
Debug- und Release-Kompilat parallel), und dieser indirekte Debug-Modus fällt mit dem Release-Bau
zusammen und wird nicht separat geführt; für Anwender bleibt er gesperrt (rc~8). Umsetzungsstand: Die
...
Messwerten) ist für spätere Haupt-Wellen vorgesehen, aber nicht implementiert --- compare und release sind
...
nach; auf der Hybrid-Ebene wiederholt sich dieselbe Kette (Abbildung~\ref{fig:modi-kette}). Einen Debug-Arbeitsmodus gibt es nicht; die Bau-Varianten Debug, Messung und Release gelten fort, und die Debug-Variante wird weiter ausgebaut: \texttt{--debug} ist ein Flag der Planer-Shell und kein eigener Arbeitsmodus der Kette build/measure/compare/release; es schaltet den indirekten Debug-Modus der Debug-Variante ein (den nicht offiziellen fünften Wartungsmodus): unter
\texttt{--debug} fährt das Konformitäts-Gate zusätzlich den Standard-Testsatz des Tier-Interfaces, und dieser indirekte
Debug-Modus fällt mit dem Release-Bau zusammen (für Anwender gesperrt, rc~8).
Umsetzungsstand: Die vier Modi sind wählbar, validierbar und bau-wirksam; der modus-spezifische Ablauf von
compare und release ist für spätere Haupt-Wellen vorgesehen, aber nicht implementiert, beide sind Etiketten
mit release-gleicher Bausemantik.
\begin{figure}[htbp]\centering
\begin{tikzpicture}[font=\footnotesize,>=Stealth,thick,align=center,
  mode/.style={draw,rounded corners,minimum width=2.7cm,minimum height=1.15cm,text width=2.55cm,font=\scriptsize},
  built/.style={mode,fill=green!12},
...
in Kommentaren zu ABI-Adaptern der Gattungen, Plattform-Sonde, Code-Erzeugung, Permutations-Befehlen,
...
eigenständige, über die ABI-stabile Schnittstelle geladene Binary (Ebene~5a der Konzept-Hierarchie,
...
\emph{Anatomie-Sicht} --- es ordnet \emph{ein} Lebewesen vom Konzept über Gattung und Genus zu den
...
Dasselbe Lebewesen, das anatomisch in Konzept, Gattung, Genus und Organe zerfällt, wird durch die Schichten
...
\emph{Anatomie-Sicht} und ordnet die Struktur des Gemessenen (Konzept, Gattung, Genus, Organe), diese sind die
...
Übersetzungszeit gegen Laufzeit; die Gattung, die nur eine Teilmenge der Achsen belegt; die zur
...
\begin{nurlang}
Die Container-Gattungen werden aus demselben Achsen-Pool gebildet wie die Suchalgorithmen und sind keine
...
C++-Bibliothek ginge diese Kontrolle verloren. Der gemeinsame Kern des Container-Konzepts besteht aus
Identität, Achsen-Beobachtung und Größe (Abschnitt~\ref{sec:anatomy-metaphor}); die Gattungen Set, Sequence,
...
bündelt eine eigene Bibliothek über die Iteratoren der Gattungs-Interfaces, die alle kartesischen Produkte
der Achsen zur Übersetzungszeit gleichermaßen abdeckt; diese Gattungs-Iterator-Bibliothek ist nicht implementiert.
\end{nurlang}
\begin{nurkurz}
Die Container-Gattungen werden aus demselben Achsen-Pool gebildet wie die Suchalgorithmen und sind keine
...
einheitliches Interface vergleichbar. Der gemeinsame Kern des Container-Konzepts besteht aus Identität,
Achsen-Beobachtung und Größe (Abschnitt~\ref{sec:anatomy-metaphor}); die Gattungen Set, Sequence, Adapter und View
sind implementiert; die Hüllen \texttt{std::map} und \texttt{std::vector} sind vollständig aufzuführen, und die
Gattungs-Iterator-Bibliothek für die ordnungsbasierten Operationen ist nicht implementiert.
\end{nurkurz}
...
zentral über eine ABI-stabile Per-Achsen-Beobachter-Schnittstelle ---; ein \emph{Konformitäts-Gate}
prüft vor jeder Messung den \texttt{std::map}-Vertrag, damit nur Vergleichbares verglichen wird, und fährt
unter \texttt{--debug} zusätzlich den Standard-Testsatz des jeweiligen Tier-Interfaces: den generischen
Gattungs-Teil (gültig für alle Genera der Gattung) und den Genus-speziellen Teil (die gegebenenfalls
erweiterten Spezial-Interfaces des Genus); und ein
...
Beobachter-Schnittstelle); ein \emph{Konformitäts-Gate} prüft vor jeder Messung den
\texttt{std::map}-Vertrag und fährt unter \texttt{--debug} zusätzlich den Standard-Testsatz des
Tier-Interfaces (generischer Gattungs-Teil für alle Genera, Genus-spezieller Teil für die erweiterten
Spezial-Interfaces); und ein Plausibilitäts-Kriterium verwirft jeden Lauf, in dem eine Messung mit
...
Gattungs-Interface-Funktionen der Tier-Binary; \emph{micro} (mi) --- die Zeit und die Erfolgs-Parameter je
...
$\to$ Gattung $\to$ Kategorie adressiert, nie über einen freien Namen. Drei Punkte sind an dieser Stelle nicht
...
build, measure, compare und release (Abschnitt~\ref{sec:anatomy-levels}), wobei \texttt{--debug} ein Flag der Planer-Shell und kein eigener Arbeitsmodus der Kette
build/measure/compare/release ist; es schaltet den indirekten Debug-Modus der Debug-Variante ein (den nicht offiziellen fünften Wartungsmodus), die weiter ausgebaut wird: unter \texttt{--debug} fährt das Konformitäts-Gate zusätzlich den
Standard-Testsatz des Tier-Interfaces, und dieser indirekte Debug-Modus fällt mit dem Release-Bau zusammen; die
Bau-Varianten Debug, Messung und Release gelten fort. Keine dieser
Unter-Achsen wird je Teil einer Binary-Identität:
...
Meta-Meta-Ebene, null die Haupt-Achse. Auf Tiefe~0 stehen die Hardware-Basis-Flags \texttt{c} $=$ CPU, \texttt{g} $=$ GPU, \texttt{f} $=$ FPGA
und \texttt{n} $=$ NPU, die die Ziel-Hardware-Klasse nennen, neben den SIMD-Breiten-Basen \texttt{x128},
\texttt{x256} und \texttt{x512}, der Medien-Basis \texttt{m64} und Companion-Flags wie \texttt{gfni}. Die
Basen \texttt{g}, \texttt{f} und \texttt{n} sind im Katalog reserviert und werden heute nicht produziert, weil
die Cache-Engine ausschließlich CPU-Code erzeugt; jede von ihr erzeugte Version trägt die CPU-Basis \texttt{c}:
Ein Eintrag mit Flags, aber ohne \texttt{c} (etwa \texttt{1.0.0.g}) ist nicht wohlgeformt, die flaglose Form
\texttt{1.0.0} ist wohlgeformt und fällt allein an der separaten CPU-Pflicht-Wache. Unter \texttt{c} stehen
als Unterglieder die Kern-Klassen \texttt{p} $=$ Performance-Core und \texttt{e} $=$ Efficiency-Core;
\texttt{c} ohne Klammer bedeutet semantisch \texttt{c\{p\}}, als Stempel-Zeichenketten werden \texttt{c}
und \texttt{c\{p\}} aber nicht zusammengeführt, weil der Stempel Identität ist und der Parser nicht
normalisiert; \texttt{c\{e\}} bezeichnet einen Efficiency-Core-Algorithmus, \texttt{c\{p.e\}} beide
Kern-Klassen. Die Identität eines Flags ist das Paar aus Token und Elternteil: \texttt{f} auf Tiefe~0 ist
FPGA, \texttt{f} in der Klammer von \texttt{x512} das AVX-512-Foundation-Subset.
...
Gattung und Realm gegliedert --- \texttt{gattung=} $\to$ \texttt{genus=} $\to$ \texttt{realm=}
(\texttt{binaries} oder \texttt{messdaten}) ---, und das Hybrid-Konzept wird darin wie jede andere
...
\texttt{c\{p.e\}} ist die Basis \texttt{c} mit den Untergliedern \texttt{p} und \texttt{e} und wird nie flach
als \texttt{cpe} gelesen; die Klammer-Tiefe eines Eintrags ist seine Meta-Meta-Ebene. Auf Tiefe~0 stehen
die Hardware-Basis-Flags \texttt{c} $=$ CPU, \texttt{g} $=$ GPU, \texttt{f} $=$ FPGA und \texttt{n} $=$ NPU
neben den SIMD-Breiten-Basen \texttt{x128}/\texttt{x256}/\texttt{x512}, der Medien-Basis \texttt{m64} und
Companion-Flags wie \texttt{gfni}; nur \texttt{c} wird produziert (\texttt{g}/\texttt{f}/\texttt{n} sind
reserviert), ein Eintrag mit Flags ohne \texttt{c} ist nicht wohlgeformt, die flaglose Form fällt an der
separaten CPU-Pflicht-Wache. Unter \texttt{c} stehen die Kern-Klassen \texttt{p} $=$ Performance-Core und
\texttt{e} $=$ Efficiency-Core (\texttt{c} bedeutet semantisch \texttt{c\{p\}}, als Stempel-Zeichenketten
bleiben \texttt{c} und \texttt{c\{p\}} getrennt); \texttt{c\{e\}} bezeichnet einen
Efficiency-Core-Algorithmus, und die Identität eines Flags ist das Paar aus Token und Elternteil (\texttt{f}
unter \texttt{x512} ist das AVX-512-Foundation-Subset, nicht FPGA). Eine vierte Realm-Zeile für die Misch-Strategie eines Prüflings
gibt es nicht: Der Prüfling stempelt unter eigenem Namensraum-Präfix in der Organ-Zeile (Muster
...
Stempel-Schlüsseln liegt (Abbildung~\ref{fig:lager-bestaende}); der Lager-Baum ist nach Konzept, Gattung und Realm
...
Abschnitt~\ref{sec:axis-triad}), jede mit vollem Achtzehn-Segment-Pfad (Abschnitt~\ref{sec:stamp-model}),
unter der vereinfachenden Annahme genau zweier Auslegungen je Achse (an/aus) über siebzehn Achsen; diese
Annahme ist überholt, die Achsen tragen mehrere Auslegungen, und der Katalog bleibt Regressions-Detektor des
Ganzsystems mit einer festgeschriebenen CRC-64-Prüfsumme über alle Identitäten, nicht Obergrenze (auch der
kleine 320er-Katalog ist als Mess-Kanon überholt; Kanon ist das XML-Profil über die Planer- und
Director-Kette). Die Obergrenze der Binary-Identitäten je System-Permutation ist keine Konstante, sondern das
Produkt über die Organ-Achsen der Zahl der im XML-Experiment gewählten Auslegungen (je Achse mehrere, nicht
an/aus); der Planer bestimmt sie aus den Eigenschaften des XML-Experiments, sie ist erst zur Ausführung
bekannt. Die Mächtigkeit des realen Permutationsraums über alle Achsen-Kataloge samt ihrer Unter-Achsen
rechnet der Planer dagegen dynamisch aus seinen Eingängen (Subkommando \texttt{simulate}) als Obergrenze,
die erst zur Ausführung bekannt ist --- sie liegt Größenordnungen darüber und ist nie eine feste Anzahl. Die
Planungs-Simulation rechnet heute das Produkt der XML-Freigaben je Organ-Achse mal System-Permutationen
arithmetisch; die Planer-Erweiterung, die diese Obergrenze und die Batch-Größe eines Experiments als
Grundlage der Last-Abschätzung bei der Ausführung korrekt erfasst, ist geplant und steht aus. Drei Mechanismen dämpfen die Explosion: Der \texttt{<expected\_workload>}-Profil-Filter routet
...
Achtzehn-Segment-Pfad (Abschnitt~\ref{sec:stamp-model}), unter der vereinfachenden Annahme genau zweier
Auslegungen je Achse (an/aus) über siebzehn Achsen; diese Annahme ist überholt, die Achsen tragen mehrere
Auslegungen, und der Katalog bleibt Regressions-Detektor mit festgeschriebener CRC-64-Prüfsumme, nicht
Obergrenze (der 320er-Katalog ist als Mess-Kanon überholt; Kanon ist das XML-Profil über die Planer-Kette).
Die Obergrenze der Binary-Identitäten je System-Permutation ist das Produkt über die Organ-Achsen der Zahl der
im XML-Experiment gewählten Auslegungen (mehrere je Achse); der Planer bestimmt sie aus dem XML-Experiment,
sie ist erst zur Ausführung bekannt. Die Mächtigkeit des realen Permutationsraums rechnet der Planer dagegen
dynamisch aus seinen Eingängen (Subkommando \texttt{simulate}) als Obergrenze, die erst zur Ausführung bekannt
ist --- sie liegt Größenordnungen darüber und ist nie eine feste Anzahl; die Planer-Erweiterung, die diese
Obergrenze und die Batch-Größe eines Experiments als Grundlage der Last-Abschätzung korrekt erfasst, ist
geplant und steht aus. Drei Mechanismen dämpfen die Explosion: der \texttt{<expected\_workload>}-Profil-Filter (nur
...
gleicher Zugriffsmuster auf die Konzept- und Gattungs-Interfaces der Tier-Binaries geclustert: Jedes
...
ähnlichen Abfolgen gleicher Zugriffsmuster auf die Konzept- und Gattungs-Interfaces (je Profil ein gerichteter
...
Kernstück ist, dass der Heuristik-Adapter eine eigene \emph{Gattung} mit eigenem Gattung ist --- die vierte
Gattung \texttt{HeuristikAdapter} mit der Reroute-Gattung \texttt{FunctionInterfaceReroute}
(Abschnitt~\ref{sec:anatomy-levels}): Er übernimmt per Metaprogrammierung die Gattungs- und
Gattungs-Interfaces seiner Tier-Binaries, wird selbst zu einer Tier-Binary kompiliert und dockt am
...
Pflicht-Symbole exportiert wie jede andere und als Gattung die geerbte Ziel-Gattung meldet --- Klassifikation
...
\begin{nurkurz}
Der \emph{Hybrid-Modus} schließlich --- das Ziel der Arbeit --- verbindet Arbeitsmodus und Messung. Sein
Kernstück ist, dass der Heuristik-Adapter eine eigene \emph{Gattung} mit eigenem Gattung ist, das vierte Konzept
\texttt{HeuristikAdapter} mit der Reroute-Gattung \texttt{FunctionInterfaceReroute}
(Abschnitt~\ref{sec:anatomy-levels}): Er übernimmt per Metaprogrammierung die Konzept- und Gattungs-Interfaces
...
dieselben Pflicht-Symbole exportiert und als Gattung die geerbte Ziel-Gattung meldet. Die Rekombination der
...
CacheEngineBuilder, die Gattungs-Interface-Zeit der Tier-Binary (macro) und die Achsen-Interface-Zeit
...
Die drei Mess-Einrichtungen --- Wall-Clock im CacheEngineBuilder, Gattungs-Interface-Zeit der Tier-Binary (macro)
