...
\begin{tikzpicture}[font=\scriptsize,>=Stealth,align=center,
  ...
  \node[tr] (t2) at (5.0,0){Tier-Binary\\{\tiny\mdseries Stufe 3}};
  \node[tr,dashed] (t3) at (7.5,0){Hybrid-Tier\\{\tiny\mdseries Einschub, optional}};
  \draw[->,thick] (t0)--(t1); \draw[->,thick] (t1)--(t2); \draw[->,thick] (t3)--(t2);
  \draw[->,thick,dashed] (t1.north) -- ++(0,0.32) coordinate (bp) -| (t3.north);
  \node[font=\tiny,anchor=south] at ([xshift=1.25cm,yshift=1pt]bp){Konfiguration wandert: XML $\to$ Planer $\to$ CEB $\to$ (Hybrid) $\to$ Tier (keine Stufe durchläuft Schichten)\\gestrichelt: optionaler Einschub --- Hybrid als Mux-Schalter am CEB-Prüf-Dock mit Dock-Slots für Tier-Binaries};
  %% INTERN-272: KA-023
