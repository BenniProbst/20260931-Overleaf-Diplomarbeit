...
\begin{tikzpicture}[font=\footnotesize,>=Stealth,align=center,
  ...
  %% INTERN-272: KA-021
  \node[band,minimum height=1.5cm] (l3) at (1.3,-5.0){\textbf{Ebene 3 --- Genus (Unterart):} Implementierung des Gattungs-Interfaces, ggf.\ mit erweiterten Teil-Interfaces --- Mammut $=$ ART, afrikanischer Elefant $=$ HOT, indischer Elefant $=$ START, \dots; unter dem Genus liegen eine oder mehrere Suchstrategien (einfach $=$ Belegung einer einzelnen Achse, komplex $=$ Komposition mehrerer Achsen). Heute trägt der Code das Genus nur als Achsen-Belegung; die Genus-Implementierungsklasse ist ein erkannter Architekturfehler (Behebung nach der Verteidigung).};
...
erweiterten Teil-Interfaces des Genus); und ein
...
Teil-Interfaces); und ein Plausibilitäts-Kriterium verwirft jeden Lauf, in dem eine Messung mit
...
Meta-Meta-Ebene, null die Haupt-Achse. Das Flag-Alphabet auf Tiefe~0 nennt die Ziel-Hardware-Klasse:
\texttt{c} $=$ CPU, \texttt{g} $=$ GPU, \texttt{f} $=$ FPGA, \texttt{n} $=$ NPU\@. Die Basen \texttt{g}, \texttt{f} und
\texttt{n} sind im Katalog reserviert und werden heute nicht produziert, weil die Cache-Engine ausschließlich
CPU-Code erzeugt --- jede von ihr erzeugte Version trägt die CPU-Basis \texttt{c}, ein Eintrag ohne \texttt{c}
fällt an der Katalog-Wache. Unter \texttt{c} stehen als Unterglieder die Kern-Klassen \texttt{p} $=$
Performance-Core und \texttt{e} $=$ Efficiency-Core; \texttt{c} ohne Klammer bedeutet \texttt{c\{p\}} (Default),
\texttt{c\{e\}} bezeichnet einen Efficiency-Core-Algorithmus, \texttt{c\{p.e\}} beide Kern-Klassen. Die Identität
eines Flags ist das Paar aus Token und Elternteil: \texttt{f} auf Tiefe~0 ist FPGA, \texttt{f} in der Klammer von
\texttt{x512} das AVX-512-Foundation-Subset.
...
als \texttt{cpe} gelesen; die Klammer-Tiefe eines Eintrags ist seine Meta-Meta-Ebene. Das Flag-Alphabet auf Tiefe~0
nennt die Ziel-Hardware-Klasse (\texttt{c} $=$ CPU, \texttt{g} $=$ GPU, \texttt{f} $=$ FPGA und \texttt{n} $=$ NPU als Klassen;
nur \texttt{c} wird produziert und ist Pflicht, \texttt{g}/\texttt{f}/\texttt{n} sind reserviert), unter \texttt{c}
die Kern-Klassen \texttt{p} $=$ Performance-Core (Default: \texttt{c} $=$ \texttt{c\{p\}}) und \texttt{e} $=$
Efficiency-Core; \texttt{c\{e\}} bezeichnet einen Efficiency-Core-Algorithmus, und die Identität eines Flags ist
das Paar aus Token und Elternteil (\texttt{f} unter \texttt{x512} ist das AVX-512-Foundation-Subset, nicht FPGA). Eine vierte Realm-Zeile für die Misch-Strategie eines Prüflings
