...
als Lösungskonzept und das darauf aufbauende Mess-System --- demonstriert am Prüfling PRT-ART\@.
...
Pool-Budgets) als Achsen-Parameter gegen eine bereits geladene Binary permutiert werden --- abgestuft
nach dem, was Hardware und Betriebssystem erlauben, von voll laufzeit-konfigurierbar (mutables Linux mit
MSR-Zugriff) über übersetzungsstatisch (immutable Betriebssysteme) bis fixiert (read-only-Cache-Register
...
folgende Abschnitt~\ref{sec:sota-instances} dialektisch (These--Antithese--Synthese); das Lösungskonzept
...
der Anhang. Die achsenweise Sezierung dieses Korpus folgt einer web-verifizierten Provenienz-Map über
...
\texttt{std::map}-Interface (\texttt{find}/\texttt{insert}/\texttt{erase}), belegt ein limitiertes
Achsen-Subset --- Allokation, Prefetch, Nebenläufigkeit, Speicher-Layout und ISA-SIMD bleiben geteilt,
die trie-spezifischen Achsen entfallen --- und erfordert so keine eigene Gattung.
...
\begin{tikzpicture}[font=\footnotesize,
  ...
  \node[so] (a2) at (4.3,0){\textbf{HOT} {\scriptsize(P02)}\\{\tiny Trie}\\[1pt]{\tiny höhenoptimierter dynamischer Span, kompakte SIMD-Knoten}\\{\tiny\itshape Achsen T4, T0}};
  \node[so] (a3) at (8.6,0){\textbf{Masstree} {\scriptsize(P03)}\\{\tiny Trie $+$ B\textsuperscript{+}}\\[1pt]{\tiny Trie aus B\textsuperscript{+}-Bäumen, 8-Byte-Slices, B\textsuperscript{+}-Blattknoten}\\{\tiny\itshape Achsen T4, T11}};
  \node[so] (a4) at (12.9,0){\textbf{CoCo-Trie} {\scriptsize(P04)}\\{\tiny Succinct}\\[1pt]{\tiny kompakt-kollabierter Trie, succinct kodierte Pfade}\\{\tiny\itshape Achsen T3, T4}};
  ...
  \node[so] (b2) at (4.3,-2.4){\textbf{B\textsuperscript{2}-Tree} {\scriptsize(P06)}\\{\tiny Hybrid}\\[1pt]{\tiny B\textsuperscript{+} mit binären In-Knoten, cache-bewusst}\\{\tiny\itshape Achsen T4, T5}};
  \node[so] (b3) at (8.6,-2.4){\textbf{Wormhole} {\scriptsize(P07)}\\{\tiny Hybrid}\\[1pt]{\tiny Trie $+$ Hash $+$ B\textsuperscript{+}-Leaf, $O(\log L)$ Punktsuche}\\{\tiny\itshape Achsen T0, T2}};
  \node[so] (b4) at (12.9,-2.4){\textbf{LOUDS} {\scriptsize(P09)}\\{\tiny Succinct}\\[1pt]{\tiny unäre Grad-Sequenz, Bitmap-kodierter Trie}\\{\tiny\itshape Achsen T9, T5}};
  ...
  \node[so] (c2) at (4.3,-4.8){\textbf{CSS-Tree} {\scriptsize(P11)}\\{\tiny cache-sensitiv}\\[1pt]{\tiny zeigerloser impliziter Baum, arithmetische Navigation}\\{\tiny\itshape Achsen T4, T5}};
  \node[so] (c3) at (8.6,-4.8){\textbf{CSB\textsuperscript{+}-Tree} {\scriptsize(P12)}\\{\tiny cache-sensitiv}\\[1pt]{\tiny B\textsuperscript{+} mit \emph{einem} Kind-Gruppen-Zeiger}\\{\tiny\itshape Achsen T4, T5}};
  \node[draw,rounded corners,fill=blue!8,minimum width=4.0cm,minimum height=2.0cm,text width=3.75cm,align=center,inner sep=2pt] (c4) at (12.9,-4.8){\textbf{SwissTable}\\{\tiny Hash (Gegenprobe)}\\[1pt]{\tiny SIMD-Control-Bytes; nicht-baumartig, im selben Achsen-System}\\{\tiny\itshape Achsen T0, T12}};
...
HdrHistogramm, Per-Achsen-Observer, die Statistik-Triade aus Median/Perzentilen, Signifikanztest und
...
Hardware-Zähler-Zugriff gebunden.
...
Auf diesen Mess-Mustern aufbauend trennt das Mess-System vier Subsysteme und prüft in drei Stufen, die die drei Pflicht-Messreihen speisen
(nicht eins-zu-eins, Tabelle~\ref{tab:stage-series});
...
Sieben-Phasen-Pipeline über seinen \texttt{ExperimentDriver} ausführt), die \texttt{CacheEngine} (die Achsen-Bausteine-Bibliothek mit den
...
die diese vier verbunden sind. Definierend ist die \emph{bidirektionale}
...
Permutations-Binaries erzeugen, die zur Laufzeit als ABI-stabile C++23-Module geladen werden; der
...
die Grundlage der Variations-Messreihe~B\@; und die Regressions-Messreihe~C ist an keine einzelne Stufe
gebunden, sondern läuft build-übergreifend über aufeinanderfolgende Stände derselben Konfiguration
...
\texttt{messreihen.xml} --- sind damit: \emph{Reihe~A} (PRT-ART vs.\ Stand der Technik) mit den Modi A\_defined (8
Rang-1-Profile) und A\_full (alle 30 SOTA-Profile); \emph{Reihe~B} (systematische Variation der
Achsen-Entscheidungen unter sonst gleichen Bedingungen, um den Beitrag jeder Achse zu isolieren); und
\emph{Reihe~C} (konkrete Merge-Punkte zwischen PRT-ART-Bausteinen und Cache-Engine-Permutationen, etwa
das PRT-ART-OLC mit tcmalloc und ART-Page, der 4+1+2-Pool mit der HOT-Page, das Path-Prefetch mit der
B\textsuperscript{2}-Baum-Page oder das MultiLevel-Layout mit der Wormhole-Page). Jede Reihe wird in drei Granularitäten erhoben: \emph{Micro-Benchmarking}
vergleicht jeden Entwurfsbestandteil isoliert über das gemeinsame Kategorie-Interface,
\emph{Makro-Benchmarking} misst die einzelnen \texttt{std::map}-Operationen, und \emph{Gesamt-Benchmarking}
fährt die YCSB-Lastprofile über jede Rekombination. Genau diese drei-granulare Methodik ist die Antwort auf
das Trennbarkeits-Problem: Indem jeder Bestandteil zunächst isoliert (Micro) vermessen wird, bevor er im
...
PRT-ART ist ein \emph{Prüfling}: ein abstraktes Lebewesen, das einige Achsen-Organe selbst stellt und für
...
Folgeschritt~E6); für die übrigen Achsen verwendet er bestehende SOTA-Bausteine über die
...
\end{figure}
\emph{Dies ist ein Ausblick-Mechanismus, keine im Rahmen dieser Arbeit umgesetzte Funktion.} Über die
...
Zunächst werden je Permutation Merkmale gewonnen --- die Lastprofil- und Datensatz-Merkmale (Workload-Typ,
Schlüssel-Verteilung, Alphabet- und Präfix-Charakteristik) sowie die Achsen-Belegung ---, mit der je
Lastmuster gemessenen Bestleistung als Zielgröße; darüber lernt ein \emph{ML-Klassifikator} die Abbildung
