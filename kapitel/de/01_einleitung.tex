...
Suchbäume und Indexstrukturen bilden das Fundament moderner datenintensiver Systeme --- von In-Memory-Datenbanken über
...
\end{nurlang}
\begin{nurkurz}
Parallel dazu ist ein eigener Forschungskorpus zu Cache-Bewusstheit, Prefetching und Allokations\-strategien
entstanden: Cache-Sensitive Search Trees~\cite{rao1999css, rao2000csb}, cache-oblivious
Layouts~\cite{frigo1999cache}, Prefetching-B\textsuperscript{+}-Bäume~\cite{chen2001prefetch} und hierarchisches
Prefetching~\cite{zhang2025hierarchical} adressieren jeweils einen separat betrachteten Aspekt der
Suchalgorithmus-Architektur. Weil so vieles davon auf cache-line-optimiertem Verhalten beruht, liegt eine Segmentierung
der Suchalgorithmus-Bestandteile in \emph{Achsen} nahe --- eine rückwärtsgerichtete Abstraktion bekannter
Bausteine in wiederverwendbare Module.
\end{nurkurz}
...
über demselben Suchalgorithmus-Interface, als bildeten sie die einzige Verzweigung unterhalb der Wurzel. Als
Prüfling für die Stand-der-Technik-Zugehörigkeit dient PRT-ART, ein Probst-Redirect-Tree-/ART-Hybrid und
eine abstrakte, unvollständige Sammlung neuartiger Suchalgorithmus-Bestandteile.
...
\begin{description}
  ...
  ordnen sie sich bestehenden Konzepten\footnote{Konzept heißt in dieser Arbeit die oberste
  Einteilung der Algorithmen nach ihrem nach außen sichtbaren Interface (etwa \texttt{Map} oder
  \texttt{Container}); jedes Konzept trägt seine eigenen Gattungen als Tier-Interfaces, deren Genera
  die Implementierungen sind.} zu,
  begründen sie neue Konzepte oder eine
...
Mess-Interface des Konzepts (Kapitel~\ref{ch:measurement}) --- zur Messung.
Das zugrunde liegende Architekturmodell
--- die drei Interface-Ebenen (\emph{Konzept} als äußere Mess-Surface, \emph{Gattung} als
Tier-Interface, im Code \texttt{AnatomyGenus}, und \emph{Genus} als dessen Implementierung), die
Achsen-Or\-ga\-ne als eigene Klassifikation, die vier Subsysteme und die ABI-stabile Auslieferung der Suchalgorithmus-\emph{Lebewesen}
--- entwickeln
...
Vorarbeit des Autors übernommen --- dem Achsen-Konzept dieser Vorläufer-Implementierung UltiHash (zur Herkunft siehe
...
\begin{itemize}
  ...
  (\texttt{IAnatomyBase} $\to$ \texttt{IExecutionEngine}) über den Achsen-Organen. Die Abgrenzung des
  Suchmaschinen-Moduls ist konzeptionell festgelegt, als eigenes Modul aber nicht implementiert; sein
  Provider ist ausdrücklich zurückgestellt, und das Suchmaschinen-Modul ist als
  Topic-Namensraum innerhalb der Cache-Engine realisiert. Der Prüfling-Suchkern spezialisiert seine Hybrid-API variadisch nach
  ...
  stellt dazu die Gattungs-Aritäten ihrer Gattungen bereit.
  \item Eine Achsen-Permutationsmatrix in dreigeteilter Ordnung: achtzehn Organ-Haupt-Achsen in fester
  Slot-Reihenfolge (T00--T17) bilden die Binary-Permutation; drei System-Haupt-Achsen --- Ziel-ISA,
  ...
  Organ-Achse für Festplatten-Ein-/Ausgabe, die nicht eine Belegung beschreibt, sondern beschreibt, wie andere
  Achsen beschrieben werden, tritt hinzu, sobald Festplatten-Ein-/Ausgabe in den Algorithmus-Entwurf eingebunden
  wird; sie stempelt im Klammer-Anhang der Organ-Zeile und bleibt außerhalb der
  Permutations-Identität. Umgesetzt ist die Matrix mit Boost.MP11-Compile-Time-Typlisten-Bausteinen ---
  Laufzeit-Tags und \texttt{std::variant} sind in den Tier-Binaries als Grundsatz dieser Arbeit ausgeschlossen
  ...
  Cache-Engine die einzelnen Algorithmus-Bestandteile zur Kompilierzeit tatsächlich ab und ist damit eine
  ...
  Patricia-Strategie enthält; im codegen-fähigen Mess-Pfad wirken davon zwei (Prefetch und
  ...
  verbleibender Bestand. Die OLC-Nebenläufigkeit ist eine Bausteinschicht des Prüflings, kein angemeldeter Slot.
  ...
  gestichprobte Full-Variante), profilbewusstem Workload-Routing --- im Standard die Voll-Permutation über alle registrierten Workloads, deren Umsetzungsstand
  ...
  ihre Erfassung durch die Probe ist jedoch nicht implementiert --- der Umsetzungsstand steht in
  Kapitel~\ref{ch:eval}. Der Messplan sieht für die beiden Produktionsmaschinen zwei Betriebssystem-Regimes
  vor (immutables Talos und root-Linux mit vollem Hardware-Zähler-Zugriff); gemessen ist das
  root-Linux-Regime, das Talos-Regime nicht.
