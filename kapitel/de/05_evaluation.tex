...
Tier-Binaries, deren Messung in der Mappe (xlsx) landet; die CSV-Form ist der aus der Mappe erzeugte,
...
\begin{tikzpicture}[font=\footnotesize,>=Stealth,
  ...
  \node[og] (cs) at (9.3,-2.6){\textbf{CSV-Baum} (Baum: Soll)\\Ordner je Blatt; WIDE 189 Spalten, Pipeline-Sicht 16 + 16};
...
unter konstantem Blattschlüssel; die Auffächerung in ein Blatt je Unter-Achsen-Permutation ist Soll), die
CSV-Form ihr aus ihr erzeugter, geordneter Baum aus CSV-Dateien (ein Ordner je Blatt; kein einzelnes
Kind-Dokument; im Durchführungs-Stand eine flache Datei), die Generatoren rendern je Sprache in das
...
je zwei Vertreter aus dem Angebot der Organ-Registry freischaltet und die achtzehnte
(\texttt{persistence\_target}) gepinnt ist
...
\begin{tikzpicture}[font=\footnotesize,>=Stealth,thick,
  ...
  \node[lg] (mp)   at (8.2,-2.6){\textbf{Mappe (xlsx)}\\CSV-Baum (Ordner je Blatt; Baum: Soll); Anhang-Generatoren};
...
Ordner je Blatt ist Soll; im Durchführungs-Stand eine flache Datei). Die Struktur des Lagers
...
Mappe (Durchführungs-Stand: eine flache Datei; Soll ist der CSV-Baum mit einem Ordner je Blatt, unten); die
...
Zyklen. F15-ALT trägt zwölf Spalten und, hinten angehängt, \texttt{degeneriert} (12~+~1).}\label{tab:eval-schema-ebenen}
\end{table}
...
der Lager-Ablage und schreibt über libxlsxwriter. Durchführungs-Stand: Die Naht schreibt die
...
(Abschnitt~\ref{sec:limitations}); die Projektion ist eine flache Datei, der CSV-Baum (ein Ordner je
Blatt) ist Soll; die Persistenz-Prüfung liest die CSV-Ausgabe, die Prüfung an der
