...
Vertrags-Test, der sie sichert) und die Messung (eine Zahl mit ihrem Lauf). Ein \enquote{strukturell
...
Quer zu allen fünf Fragen hatte die Einleitung eine Kategorien-Frage offen gehalten: ob sich die Achsen
...
Stand-der-Technik-Verfahren als Profil-Konfigurationen rekonstruierbar und permutierbar werden; FF1.a
...
Kriterien eine Plattform als verifiziert gilt; FF2.a fragt, ob die Mess-Instrumente selbst eigene Achsen
sein müssen, FF2.b nach der Nachmessung widersprüchlicher Literaturbefunde, FF2.c nach der
Erweiterbarkeit. Der \emph{Status}: FF2 ist strukturell beantwortet, FF2.a beantwortet. Die Messung ist in
...
gegenüber mehreren bis sechzehn Cache-Lines bei Hankins und Patel~\cite{hankins2003nodesize} und bei den
Prefetching-B\textsuperscript{+}-Bäumen von Chen et~al.~\cite{chen2001prefetch} --- werden nicht aus
...
(Abschnitt~\ref{sec:stamp-model}). Ob der Prüfling eine bestehende Kategorie trifft, eine neue
...
(Abschnitt~\ref{sec:outlook}). Zur Plattform-Verifikation: Eine Plattform gilt als verifiziert, wenn
das Konformitäts-Gatter die fünf Kern-Operationen \texttt{insert}, \texttt{lookup}, \texttt{erase},
\texttt{clear} und \texttt{size} je geladener Tier-Binary gegen \texttt{std::map} als Orakel bestätigt
--- der Vollausbau auf die in Anhang~\ref{app:interfaces} markierte Teilmenge und die volle
\texttt{std::map}-Schnittstelle ist Folgeschritt --- und die Plattform-Probe die gemeldete
Achsen-Teilmenge deckt; die plattform-kalibrierten Schwellwerte werden mit den realen Läufen
...
Abschnitt~\ref{sec:anatomy-levels} (Gattungs-Schnitt; Struktur) und Anhang~\ref{app:interfaces}
(Vertrags-Teilmenge; Test). Die \emph{Folge}: FF2 strukturell beantwortet, FF2.c konzeptionell beantwortet;
der Isolations-Modus und die Zahl zu FF2.b sind offen.
...
B\textsuperscript{2}-Baum, Wormhole, SuRF --- und der Rang-2/3-SOTA durch bessere
Mikroarchitektur-Eigenschaften gewinnt, gemessen in den Pflicht-Messreihen A, B und C\@; FF3.a fragt nach
...
\texttt{std::map}-Kernvertrag als host-seitiger Kompositions-Schicht über einem
Ordnungs-Such-Primitiv (Anhang~\ref{app:interfaces}). Das Konformitäts-Gatter prüft die fünf
Kern-Operationen; der Vollausbau auf die in Anhang~\ref{app:interfaces} markierte Teilmenge und die
volle Schnittstelle ist als Folgeschritt ausgewiesen. Der Benachrichtigungs-Haken der Mess-Schicht ist
...
--- je Verzweigungspunkt und die Umschaltregeln nach Füllstand und Suchtyp sind Teil des
...
übersetzungszeit-fixierte \emph{Haupt}-Entscheidung und laufzeit-variable \emph{Unter}-Entscheidungen
...
Abschnitt~\ref{sec:measurement-system} (Haupt- und Unter-Entscheidung; Struktur) und
...
Permutation zu erzeugen, und welche zur Laufzeit gewählt werden können, ohne die Vergleichbarkeit zu
verletzen; die Erweiterung FF4.E fragt, ob die Grenze entlang der Achsen-Kategorien verläuft.
Der \emph{Status}: FF4 ist strukturell beantwortet, FF4.E beantwortet. Die Grenze verläuft entlang der
...
nicht vollzogen (Abschnitt~\ref{sec:eval-platforms}). Als \emph{Beweis} tragen Abschnitt~\ref{sec:complex-axis}
(Komplex-Klammer; Struktur), Abschnitt~\ref{sec:hw-probe} (zweigeteilte Erkennung; Struktur),
Abschnitt~\ref{sec:stamp-model} (Stempel-Zeilen; Struktur) und Abschnitt~\ref{sec:impl-system-axes}
(Rückkehr-Sperren und Ordnungs-Wachen; Test). Die \emph{Folge}: FF4 strukturell beantwortet;
Heuristik-Automation und Passungs-Stempel offen.
...
beantwortet, offen), die tragenden Abschnitte mit ihrer Belegart (Struktur, Test, Messung) und der
offene Rest ohne Zahl.}\label{tab:ff-beweiskette}
\end{table}
...
(Abschnitt~\ref{sec:eval-xml}) --- ein Mess-Job der Smoke-Stufe mit 10000 Operationen und 1199,047~ns je
Operation als erster realer Messwert des Weges --- und der Kalibrierlauf der AMD-Lane, der 250 von 250
...
\begin{itemize}
  ...
  Speicher; xlsx ist der Standard, die CSV ihr wählbares Kind, beide zusammen sind wählbar
  ...
  Filterkette der Auswertung und Soll. Was der Anwender nicht kann: Ergebnisse je
  Unter-Achsen-Permutation als eigene Blätter erhalten.
...
der Auswertungs-Ausgabe rangiert und ausliefert --- die xlsx-Mappe ist die Ausgabe, die CSV ihr Kind,
und das Werkzeug liest die CSV-Projektion. Neben der Auswahl (Versprechen~1) trägt derselbe Strang die
