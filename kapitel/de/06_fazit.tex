...
Entwurfsraums sauber in Organ-, System- und Mess-Achsen trennen lassen. Sie ist beantwortet
...
aktiv (laufzeit-dynamische Cache-Line-Politik,
...
binary-id-neutral; die Trennung hält der doppelte Wurzel-Schnitt
...
als Lebewesen seiner Gattung ins Genus ein (Gattung = erste Ebene, Genus = zweite; die Genus-Bau-Zulassung
urteilt übersetzungszeitlich), dockt am Prüf-Dock an, durchläuft
...
Prüfling eine Gattung trifft oder eine neue begründet, entscheidet der Gattungs-Schnitt
(Abschnitt~\ref{sec:anatomy-levels}); der Paper-Isolations-Modus ist nicht implementiert
...
Abschnitt~\ref{sec:eval-workloads}); die Voll-Permutation ist nicht implementiert. Eine Plattform gilt als
...
Konformitäts-Gatter (siebzehn Operationen, zwölf Kern-Prüfblöcke, \texttt{std::map} als Orakel) ist
...
sichert der Tier-Fingerprint
(SHA-512, elf Glieder samt Overlay- und Toolchain-Glied, Format~6); der Planer-Fingerprint ist ein SHA-256,
und \texttt{get\_compiler()}
liefert den Original-Compiler der Veröffentlichung --- Herkunftsangabe, keine Bau-Kennung. Mess-Wahlen sowie
die Layout- und Seiten-Selektion über \texttt{binary\_id} und Cache-Line-Politik bleiben laufzeit-variabel.
...
entworfen, nicht implementiert (Abschnitt~\ref{sec:outlook}); die Betriebssystem-Dimension ist mit drei
...
durchgehender Lauf (43 Permutationen), headless Dienst, XML-Erstellungsmodus, \texttt{<publish>}-Sektion und
CSV-Baumform fehlen.
...
erbt ihre Kosten; die Modul-Bodies sind keine Grenze (Abschnitt~\ref{sec:impl-pipeline}).
\begin{itemize}
...
%% INTERN-272: KA-121
  ...
  nicht~\cite{graham2006planned,graham2009missing}); Last-Level-Ereignis, RAPL-Energiezähler und IPC sind
  nicht erhoben, in der Smoke-Reihe sind die Hardware-Spalten n/a (Tabelle~\ref{tab:le:limitierung},
  Zeile~1). Was der Anwender nicht kann: L2- und Kohärenz-Zähler auf anderen Modellen als Zen~5 erhalten
  (Abbildung~\ref{fig:measurement-triangle}).
  \item \textbf{Hardware-Passung ohne automatische Prüfung:} Die Hardware-Erkennung liegt in drei
  ...
  \item \textbf{Permutationsraum und Budget:} Ein vollständiger Bau des golden-Referenzkatalogs sprengt je
  System-Permutation Zeit- und Lager-Budget (Abschnitt~\ref{sec:answers}). Was der Anwender nicht kann: den
...
\begin{enumerate}[label=(\arabic*)]
  ...
  \item der Lager-Vollausbau: zwei Release-Artefakte, Kurven-Dateien, nur Fehlendes nachmessen
...
die Bausteine-Bibliothek zu ändern (Prüflings-Kontrakt). Über Baumstrukturen hinaus ist das Anatomie-Prinzip
