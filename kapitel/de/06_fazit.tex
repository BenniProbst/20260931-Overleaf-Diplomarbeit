...
Persistenz-Ziel auf einen Wert fest --- $2^{17} = 131072$ Binary-Identitäten je System-Permutation unter der
überholten Annahme zweier Auslegungen je Achse, als Regressions-Detektor (nicht Obergrenze) mit einer festgeschriebenen CRC-64-Prüfsumme\footnote{CRC-64: eine zyklische
...
ist, sondern die Rechnung des Planers je Profil und Plattform (Subkommando \texttt{simulate}) als Obergrenze, die
erst zur Ausführung bekannt ist; alle
...
Graph ordnet die Gattungen darunter (Abschnitt~\ref{sec:anatomy-levels}). FF1.a ist bejaht: Die Zerlegung
...
golden-Referenzkatalog mit $2^{17} = 131072$ Binary-Identitäten je System-Permutation (überholte Annahme zweier
Auslegungen je Achse), per CRC-64-Prüfsumme gegen Regression gesichert (Abschnitt~\ref{sec:impl-contracts}),
und die vom Planer gerechnete Mächtigkeit eines geplanten Raums (\texttt{simulate}) als Obergrenze, die erst
zur Ausführung bekannt ist. Die 33 Korpus-Arbeiten besitzen je ein Profil-XML (30
Vollprofile,
...
die Gattungen darunter (Abschnitt~\ref{sec:anatomy-levels}).
...
die Last-Rahmenwerke), der Makro-Ebene (\texttt{macro}; die Konzept- und Gattungs-Interface-Operationen
...
(Erweiterbarkeit): Ein neuer Prüfling gliedert sich als Lebewesen seiner Gattung ins Gattung ein ---
Gattung ist die erste Ebene (Map, Container, Graph), Gattung die zweite, und die Gattungs-Bau-Zulassung
...
(Abschnitt~\ref{sec:anatomy-levels}); ein eigenes Konzept entstünde erst bei grundlegend anderem
...
Plattform-Verifikation: Eine Plattform gilt als verifiziert, wenn das Konformitäts-Gate je
geladener Tier-Binary die siebzehn in Anhang~\ref{app:interfaces}
markierten \texttt{std::map}-Operationen (unter \texttt{--debug} zusätzlich den Standard-Testsatz des
Tier-Interfaces) --- die Operationsmenge seiner zwölf Kern-Prüfblöcke,
...
als Lebewesen seiner Gattung ins Gattung ein (die Gattungs-Bau-Zulassung urteilt übersetzungszeitlich), dockt am
...
verifiziert, wenn das Konformitäts-Gate je Tier-Binary die siebzehn in Anhang~\ref{app:interfaces}
markierten \texttt{std::map}-Operationen (zwölf Kern-Prüfblöcke, \texttt{std::map} als Orakel; unter
\texttt{--debug} zusätzlich den Standard-Testsatz des Tier-Interfaces) bestätigt
...
Scan-Fähigkeit (Anhang~\ref{app:interfaces}). Das Konformitäts-Gate prüft die siebzehn in
Anhang~\ref{app:interfaces} markierten Operationen in zwölf Kern-Prüfblöcken gegen \texttt{std::map}
als Orakel und unter \texttt{--debug} zusätzlich den Standard-Testsatz des Tier-Interfaces (generischer
Gattungs-Teil und Genus-spezieller Teil); der Vollausbau auf die volle Schnittstelle ist vorgesehen,
aber nicht implementiert. Der
...
\begin{nurlang}
%% INTERN-272: KA-119
\paragraph{Umsetzungsstand.} Der vollständige Bau ist nicht durchgeführt; sein Umfang ist kein deklarierter
fester Wert: Die Obergrenze der Binary-Identitäten je System-Permutation ist das Produkt der je Organ-Achse im
XML-Experiment gewählten Auslegungen (mehrere je Achse, nicht an/aus), vom Planer aus dem XML-Experiment
bestimmt und erst zur Ausführung bekannt; die Planungs-Simulation (Unterkommando \texttt{simulate}) rechnet
heute dieses Produkt mal System-Permutationen arithmetisch, die Planer-Erweiterung, die Obergrenze und
Batch-Größe eines Experiments als Grundlage der Last-Abschätzung korrekt erfasst, ist geplant und steht aus
(die 131072 Organ-Permutationen des golden-Referenzkatalogs mit dem Katalog-Kommentar von rund 224~GB und rund
24~Stunden je Bau sind eine historische Schätzung unter der überholten Annahme zweier Auslegungen je Achse,
kein Bau-Umfang); die End-to-End-Läufe des
...
Binary kalibriert hat; daraus folgt als Rechenbeispiel des historischen golden-Profils (vier
System-Permutationen mal $2^{17}$, zwei Optimierungsstufen mal zwei SIMD-Optionen) ein Lagerbedarf von rund
240~GB für 524288 Tier-Binaries --- kein Bau-Umfang des Systems. Die Smoke-Reihe --- 43 Permutationen in zwei Teilblöcken: 27 kartesische
...
\begin{nurkurz}
%% INTERN-272: KA-119
\paragraph{Umsetzungsstand.} Der vollständige Bau ist nicht durchgeführt; sein Umfang ist kein fester
Wert: Die Obergrenze je System-Permutation ist das Produkt der je Organ-Achse im XML-Experiment gewählten
Auslegungen, vom Planer erst zur Ausführung bestimmt; die Planungs-Simulation (\texttt{simulate}) rechnet
dieses Produkt heute arithmetisch, die Planer-Erweiterung für Obergrenze und Batch-Größe als Grundlage der
Last-Abschätzung steht aus (die 131072 Organ-Permutationen des golden-Referenzkatalogs mit rund 224~GB und
24~Stunden je Bau sind eine historische Schätzung unter der überholten Annahme zweier Auslegungen je Achse); die End-to-End-Läufe über die Zielplattformen (x86-64, aarch64)
...
0,521~s und rund 457~kB je Binary; daraus als Rechenbeispiel des historischen golden-Profils rund 240~GB für 524288 Tier-Binaries
über dessen vier System-Permutationen, kein Bau-Umfang des Systems). Die Smoke-Reihe (43 = 27 + 16 Permutationen) liegt in
...
\begin{enumerate}[label=(\arabic*)]
  \item der vollständige Bau nach dem vom Planer aus dem XML-Experiment bestimmten Umfang
  (Abschnitt~\ref{sec:eval-build-scope}) und die
