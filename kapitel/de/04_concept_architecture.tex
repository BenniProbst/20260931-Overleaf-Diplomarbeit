...
\end{verbatim}
  ...
  Über die reine Rangbildung hinaus ist als Zielsetzung ein durchgängiger Auswertungs-Schritt
  vorgesehen, der die Messergebnisse in eine ausgelieferte Konfiguration überführt. Die folgenden
  drei Stufen sind \emph{Ausblick und Zielsetzung} und im aktuellen Stand
  \emph{noch nicht implementiert}.

  \emph{Erstens} wird aus der Rangbildung der drei Messreihen~A/B/C je Lastprofil und Datentyp die
  beste Konfiguration ermittelt. Da jede Reihe pro Permutations-Binary einen vollständigen
  Achsen-Messvektor liefert, lässt sich für ein gegebenes Last-/Datentyp-Profil diejenige
  Achsen-Komposition bestimmen, die unter diesem Profil den vordersten Rang belegt.

  \emph{Zweitens} ist der angestrebte Auswahl-Mechanismus --- der von den Messergebnissen
  automatisch auf die beste Konfiguration schließt --- eine \emph{ML-Klassifikation}: Aus den
  Achsen-Merkmalen und den Merkmalen des Lastprofils (Zugriffsverteilung, Schreib-/Lese-Anteil,
  Schlüssel-/Wert-Datentyp) wird die geeignete Achsen-Konfiguration vorhergesagt. Dieser
  Heuristik-Auswahl-Mechanismus ist \emph{Future Work} und bislang nicht realisiert.

  \emph{Drittens} wird das Ergebnis dieser Auswertung als wiederverwendbares \emph{XML-Lastprofil}
  abgelegt, das die ermittelte Beste-Konfiguration an ein benanntes Last-/Datentyp-Profil bindet und
  so beim nächsten Lauf direkt als Eingabe verwendet werden kann --- symmetrisch zu den
  Messreihen-XML-Konfigurationen aus Abschnitt~\ref{sec:m-model}.
