...
Vertrags-Hüllen sind als getrennte Gattungs-Adapter implementiert: die Gattung SearchAlgorithm (Konzept
Map) über den funktionalen
...
\texttt{swap}, \texttt{get\_allocator}) besitzen in beiden Hüllen denselben Namen und dieselbe Rolle;
...
\emph{Zusammengesetzte Funktionen.} Einige Funktionen ergeben sich intern als Komposition
...
(\emph{function-handle-hops}\footnote{Projektinterner Begriff der Cache-Engine für die Folge der internen
Funktions-Sprünge (Handles) vom Eintritt an der Modulgrenze über die Achsen-Organe bis zum Ergebnis;
das Entwickler-Dokument verzeichnet sie je Standard-Funktion und markiert nicht angebundene
...
\emph{Konformitäts-Teilmenge.} Das Konformitäts-Gate (Abschnitt~\ref{sec:measurement-system})
prüft vor jeder Messung je Gattung eine Kern-Teilmenge der Hüllen-Operationen gegen ein Orakel der
Standardbibliothek --- \texttt{std::map} für die Gattung SearchAlgorithm (Konzept Map),
\texttt{std::deque} für die Sequenz-Gattung ---, damit nur Vergleichbares verglichen wird; die Teilmenge
ist in beiden Tafeln mit $\dagger$ gekennzeichnet. Unter \texttt{--debug} fährt das Gate zusätzlich den
Standard-Testsatz des Tier-Interfaces (generischer Gattungs-Teil und Genus-spezieller Teil).
...
Kern-Operationen, die das Konformitäts-Gate (Abschnitt~\ref{sec:measurement-system}) vor jeder
...
genau die Operationsmenge der zwölf Kern-Prüfblöcke des Gates: Keine markierte Operation bleibt
...
geordnet, und fehlt sie einer geladenen Binary, protokolliert das Gate diese Blöcke als
...
Kern-Operationen, die das Konformitäts-Gate der Sequenz-Gattung vor jeder Messung gegen ein
...
(\texttt{push\_front}), das das \texttt{deque}-Orakel zusätzlich böte, ist als Erweiterung der
Modulgrenze vorgesehen, aber nicht implementiert, und wird im Gatter als übersprungen protokolliert, nicht als bestanden.
...
Mehrere Schnittstellen-Funktionen sind intern Kompositionen primitiverer Operationen derselben Schnittstelle
...
Punkt-Suche und Default-Einfügen komponiert --- mit der ISO-Semantik (Einfügen bei Fehlen, kein
\texttt{out\_of\_range}), aber ohne Referenz-Rückgabe über die binäre Grenze; der Schreibpfad läuft über
...
geordneten Lesen mit linearem Aufwand. Genau diese Austauschbarkeit ist der Gegenstand der Messung
(Abschnitt~\ref{sec:measurement-system}). Das Konformitäts-Gate prüft je Gattung die in den
...
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{3.3cm} >{\raggedright\arraybackslash}p{5.6cm} >{\raggedright\arraybackslash}p{3.1cm}@{}}
  \caption{Zusammengesetzte Funktionen (interne Komposition primitiver Operationen)}\label{tab:if-decomp}\\
  \toprule
...
Einfüge-Antrieb der Gattung SearchAlgorithm (Konzept Map) trägt die Semantik von
\texttt{insert\_or\_assign} --- Einfügen oder
...
Standard-Funktionen, den das Gate gegen drei Orakel-Aufrufe stellt. Der ISO-Vertrag führt es
...
\texttt{insert}/\texttt{emplace}/\texttt{erase} ---, besitzen dieselbe Rolle, aber verschiedene
...
bereitstellt, welche der Host komponiert und welche nicht angebunden sind, verzeichnet das eingangs
