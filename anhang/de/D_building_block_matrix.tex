...
\texttt{Buffered\allowbreak{}Io} & Standard \texttt{read()}/\allowbreak{}\texttt{write()} via OS-Page-Cache mit automatischem read-ahead und write-back. Default für allzweck-DBMS\@. Trade-off: bequem, aber Double-Buffering (App-Cache + OS-Cache führt zu erhöhter Speichernutzung). Papier-Referenz: Stonebraker CACM 1981 (Konzept-/\allowbreak{}Position-Paper Operating System Support for Database Management, DOI 10.1145/\allowbreak{}358699.358703). Einordnung: Engineering-Pattern, kein Algorithmus-Code der Referenz. & P-Stonebraker-1981 \\
...
\texttt{Mmap\allowbreak{}Io} & Memory-Mapped File I/\allowbreak{}O (\texttt{mmap}) mit Persistent Memory oder Read-Heavy Workloads. OS pagiert automatisch, kein Copy zwischen User- und Kernel-Space. Trade-off: page-fault-Latenz-Spitzen vs. Reuse-Freundlichkeit. Papier-Referenz: Crotty et al. CIDR 2022 (Anti-Pattern-Baseline Are You Sure You Want to Use MMAP in Your DBMS?, db.cs.cmu.edu/\allowbreak{}\ldots/\allowbreak{}cidr2022-p13-crotty.pdf). Bewusst als Anti-Pattern-Referenz zitiert, um Grenzen und Overhead zu dokumentieren. OSS-Referenz MIT-lizenziert. & P-Crotty-2022 \\
\bottomrule
\end{longtable}
\end{scriptsize}
...
IS3 & ZIH-Cluster-Zugehörigkeit & Produktionsmaschinen des ZIH der TU Dresden~\cite{zih_hpc_compendium_hardware}: Barnard (Intel Xeon Platinum 8470, Sapphire Rapids, x86-64), Capella (AMD EPYC 9334, x86-64), das Grace-Hopper-Superchip-Testsystem (72 Arm-Neoverse-V2-Kerne, AArch64; Testsystem mit begrenzter Verfügbarkeit~\cite{zih_hpc_compendium_gh200}), N/\allowbreak{}A (RISC-V und Power: nicht unter den aktuellen HPC-Ressourcen des ZIH-Kompendiums). Ermöglicht Hardware-spezifische Experimente mit echten Produktionscluster-Profilen. \\
\bottomrule
\end{longtable}
\end{scriptsize}
...
Der Haus-Standard des Baus ist \texttt{O2}; \texttt{O3} bleibt auf XML-Wunsch wählbar, und die Wahl geht als Feld in den Bau-Versions-Suffix ein.
...
\emph{Vermerk zur Nicht-Erhebung:} Die Wiederholungszahl eines Laufs steht bewusst \emph{nicht} in dieser Liste: sie ist eine reine Etikett-Dimension ohne übersetzungszeitliche Entsprechung und wird erst nach ihrer Typisierung reflektierbar; ein handgeschriebener Eintrag wäre ein Phantom --- die Registry führt stattdessen eine explizit kodierte Nicht-Erhebung. Die Ablauf-Methoden dagegen sind typisiert: es sind \emph{vier} --- Bau-, Mess-, Vergleichs- und Release-Durchführung (\texttt{build}, \texttt{measure}, \texttt{compare}, \texttt{release}) ---, gestützt auf eine übersetzungszeitliche Registry ohne Laufzeit-Verzweigung; die Vergleichs-Durchführung besitzt die Wählbarkeit und liest das Messwertlager, ohne selbst Referenzwerte zu erheben. Sie stehen als Dimension \texttt{work\_\allowbreak{}mode} in der Liste.
...
Registries keine Achse, entsteht bewusst keine Datei; es wird keine leere Tabelle
erfunden.\IfFileExists{anhang/de/tabellen/axis_inventory.tex}{ Tabelle~\ref{tab:axis-inventory} zeigt den Stand des
...
\midrule
\endhead%
HW1 & CPU-Architektur-Familie (\texttt{cpu\_\allowbreak{}family}) & x86\_\allowbreak{}64 (Intel/\allowbreak{}AMD), aarch64 (ARM/\allowbreak{}Apple), riscv64 (RISC-V), generic (plattform-agnostisch). \\
