...
Eytzinger\allowbreak{}Search\allowbreak{}Algo (S12) & Eytzinger/\allowbreak{}BFS-Layout branch-free Suche (Khuong/\allowbreak{}Morin JEA 2017): Cache-Layout-Such-METHODE mit branch-free Kern. Keys in BFS-Ordnung des impliziten Suchbaums. Für große Arrays laut Paper das schnellste Allzweck-Layout. C++23-Re-Impl (Experiment-Harness ohne Standard-Lizenz). & Khuong/\allowbreak{}Morin JEA 2017~\cite{khuong2017eytzinger} \\
...
Die Tafel führt drei Gegenstände derselben Heimat; nur der erste Block ist die Wertemenge der Achse.
...
\emph{Registry-Ist:} Die generierte Organ-Registry führt für die Allokator-Achse drei freigeschaltete Bausteine --- \texttt{std\_\allowbreak{}malloc}, \texttt{pmr\_\allowbreak{}resource} und \texttt{pool\_\allowbreak{}resource}; der Katalog A01--A24 bleibt als Angebots-Katalog erhalten, die übrigen Wrapper sind je Vendor-Verfügbarkeit und Freigabe-Flag zuschaltbar.
...
\texttt{Non\allowbreak{}Clustered\allowbreak{}Index\allowbreak{}Organization} & Index-Reihenfolge $\neq$ Storage-Reihenfolge, mehrere Secondary-Indizes pro Tabelle. Lookup über Index + Pointer-Hop zur Daten-Row (Random-Stride, Cache-Misses). Standard: SQL Server NONCLUSTERED INDEX, Postgre\allowbreak{}SQL CREATE INDEX\@. Zugriffsmuster: Index-navigation gefolgt von zufälligen Storage-Hops (LCG-determiniert, REAL cache-Miss-Latenz durch TLB/\allowbreak{}L2-Invalidierung), anschließend ein Residual-Predicate-Vergleich pro geholtem Record (datenabhängiger Branch auf der geholten Daten-Row nach dem Pointer-Hop). & Comer, ACM Computing Surveys 1979 (The Ubiquitous B-Tree, 10.1145/\allowbreak{}356770.356776) \\
\bottomrule
\end{longtable}
\end{scriptsize}
...
Die drei Größen sind Entwurfs-Parameter der Achse; der Code trägt sie als Übersetzungszeit-Konstanten innerhalb der Entscheidungs-Scans (\texttt{w\_\allowbreak{}freq} = 3 und \texttt{w\_\allowbreak{}rec} = 1 in \texttt{axis\_\allowbreak{}migration\_\allowbreak{}adaptive.hpp}, \texttt{k\allowbreak{}Tiers} = 3 in \texttt{axis\_\allowbreak{}migration\_\allowbreak{}tier\_\allowbreak{}based.hpp}; der Hot/\allowbreak{}Cold-Scan entscheidet per Recency-Vergleich ohne Schwellwert), ein Laufzeit- oder Benutzer-Parameter ist für keine der drei gebaut, und die generierte Registry führt für T13 keine Laufzeit-Unter-Achse.
...
Die Achse macht das Ablage-Ziel der Struktur als eigenen Organ-Slot explizit, statt eine In-Memory-Annahme implizit mitzuführen. Im golden-Referenzkatalog ist sie auf ihren In-Memory-Vertreter gepinnt: siebzehn Achsen mit je zwei Ausprägungen und \texttt{persistence\_\allowbreak{}target} mit einer ergeben $2^{17} = 131072$ Referenz-Identitäten je System-Permutation; jede \texttt{binary\_\allowbreak{}id} trägt gleichwohl das achtzehnte Segment \texttt{persistence\_\allowbreak{}target=\allowbreak{}persistence\_\allowbreak{}memory\_\allowbreak{}only}. Die Mächtigkeit einer Kampagne ist davon verschieden: sie ist keine statische Zahl, sondern wird je Profil aus dem Director-Walk gezählt (Abschnitt~\ref{sec:mess-chain}).
...
\midrule
\endhead%
\texttt{Memory\allowbreak{}Only\allowbreak{}Target} & In-Memory-Ablage als Standard-Ziel; einziger freigeschalteter Vertreter des kanonischen Vermessungs-Katalogs. Kein Geräte-Schreibpfad (\texttt{has\_\allowbreak{}device\_\allowbreak{}writeback\_\allowbreak{}path()=false}). & Code/\allowbreak{}Baseline \\
...
\texttt{core\_\allowbreak{}class} & Kern-Klasse & \texttt{stage=runtime}, \texttt{value\_\allowbreak{}type=token}, \texttt{option\_\allowbreak{}source=machine\_\allowbreak{}resolved}. Die Klasse des ausführenden Kerns (P-/\allowbreak{}E-Kern auf hybriden Prozessoren) als Ausführungs-Lokalität der Messung --- Geschwister von \texttt{numa\_\allowbreak{}node} und \texttt{page}, die die Speicher-Lokalität tragen; abgegrenzt von \texttt{scheduling}, das die übersetzungszeitlich festgelegte Politik trägt: \texttt{core\_\allowbreak{}class} sagt, ob und welche Kern-Klassen die Maschine führt. Zulässige Werte sind maschinen-aufgelöst; die getrennte Messung auf P- und E-Kernen ist damit im Katalog als System-Seite des Kern-Pinnings benannt, die Messung selbst trägt das Mess-System (Abschnitt~\ref{sec:measurement-system}). \\
\bottomrule
\end{longtable}
\end{scriptsize}
...
\texttt{Risc\allowbreak{}VIsa} & RISC-V RV64\allowbreak{}GC Open-Source ISA (RV64 General + Compressed). Wachsende Bedeutung (Si\allowbreak{}Five, Alibaba Xuantie, Andes, ETH Zürich Cores). SIMD-Baseline: KEINE (RVV ist optional, nicht Teil von RV64\allowbreak{}GC baseline). Cache-Engine-Implementierung: bewusst plain skalarer Pfad (nicht entrollt) -- modelliert fehlende Vektoreinheit der Basis-ISA und erzeugt damit reale, langsamere Laufzeit gegenüber x86-64 und ARM\@. ZIH-Präsenz: im ZIH-Kompendium nicht gelistet. & Standards-Spec (RISC-V Intl.; UCB TR ab 2011) \\
\bottomrule
\end{longtable}
\end{scriptsize}

\emph{Registry-Ist:} Die bau-treibende System-Registry führt zwei freigeschaltete Bausteine --- \texttt{x86\_\allowbreak{}64} (nativ, ohne Triple- und March-Flags) und \texttt{aarch64} (Kreuz-Übersetzung mit \texttt{triple=\allowbreak{}-target aarch64-linux-gnu} und \texttt{march=\allowbreak{}-march=armv8-a}). Der oben aufgeführte Vier-ISA-Katalog bleibt als \emph{Angebots}-Katalog erhalten: Power-ISA und RISC-V sind als Bausteine beschrieben, im Bau-Angebot aber nicht freigeschaltet.

\emph{Beitragende Quellen:} ---

\section{System-Sphäre: Betriebssystem (\texttt{operating\_system})}

Diese Größe ist die zweite System-Haupt-Achse und damit Teil des \emph{Bluts} und keines Organs (vgl.\ Abschnitt~\ref{sec:axis-triad}); in der kanonischen, code-gewachten Ordnung steht sie an Platz zwei von drei.

Die Haupt-Achse trägt ausschließlich die \emph{Klassen}-Identität: genau drei Betriebssystem-Familien (\texttt{baustein\_\allowbreak{}count=3}), dazu \texttt{stage=ct} und \texttt{binary\_\allowbreak{}id=never}. Die \emph{Instanz} --- Distribution, Release, Kernel und Patch-Stand --- liegt dagegen vollständig in den drei Unter-Achsen; eine Distribution als vierte Ausprägung der Haupt-Achse einzutragen, verfehlte diesen Schnitt. Die reale Flotte (Windows 11 und Windows Server 2022, die Linux-Docker-Matrix, zwei macOS-Maschinen) fällt vollständig in die drei Familien.
...
\texttt{build} & Build- und Patch-Stand & Der Build- bzw.\ Patch-Stand der Instanz \emph{einschließlich} des Update-Zustands (Entwurfsentscheidung dieser Arbeit, wörtlich: \enquote{Wir mergen den Update Zustand in Build}). Ein separates \texttt{update\_\allowbreak{}zustand} ist damit ausdrücklich \emph{keine} vierte Unter-Achse, sondern Bestandteil dieser. \\
\bottomrule
\end{longtable}
\end{scriptsize}

\emph{Notiz:} Alle drei sind \texttt{stage=\allowbreak{}runtime} mit \texttt{value\_\allowbreak{}type=\allowbreak{}token} und \texttt{option\_\allowbreak{}source=\allowbreak{}machine\_\allowbreak{}resolved}: Sie haben keinen übersetzungszeitlichen Options-Katalog, weil ihre zulässigen Werte an der konkreten Maschine hängen und zur Bauzeit nicht erkennbar sind. Die Laufzeit-Erhebung ist ein eigener Bau-Schritt (\texttt{operating\_\allowbreak{}system\_\allowbreak{}probe\_\allowbreak{}\{linux,\allowbreak{}macos,\allowbreak{}windows\}.hpp}); die Einhängung selbst ist nur das Angebot. Die drei Namen sind Teil der generierten Registry und damit festgeschrieben.
...
\texttt{macos} & macOS-Familie (\texttt{Macos\allowbreak{}Operating\allowbreak{}System}, \texttt{posix=true}). Deckt die Apple-Silicon- und Intel-Maschinen der Flotte ab. & Code/\allowbreak{}Baseline \\
\bottomrule
\end{longtable}
\end{scriptsize}
...
\emph{Angebot und Bau-Minimum:} Die Treiber-Kennungen \texttt{g++-16} und \texttt{clang++-22} sind die Vorgabe-Treiber des Angebots (\texttt{driver\_\allowbreak{}default} der Übersetzer-Bausteine); der Bau selbst fordert \texttt{g++} $\geq$ 15.3 als Minimum und Flotten-Standard und pinnt bewusst keine Version (neuere \texttt{g++} zugelassen); die CI-Testzellen bevorzugen \texttt{g++-15}, wo vorhanden, und der clang-Vollbau läuft auf clang 22. Clang und MSVC tragen kein Versions-Minimum.
...
\emph{Versions-Flag-Grammatik:} Die Version eines jeden Achsen-Algorithmus ist einheitlich dreistellig; ein \texttt{v}-Präfix ist ausgeschlossen, Kurzformen sind verboten. Übersetzungszeitliche Wachen setzen die Form durch:
...
\emph{Vermerk zur Nicht-Erhebung:} Die Wiederholungszahl eines Laufs steht bewusst \emph{nicht} in dieser Liste: sie ist eine reine Etikett-Dimension ohne übersetzungszeitlichen Anker und wird erst nach ihrer Typisierung reflektierbar; ein handgeschriebener Eintrag wäre ein Phantom --- die Registry trägt stattdessen eine explizit kodierte Nicht-Erhebung. Die Ablauf-Methoden dagegen sind typisiert: es sind \emph{vier} --- Bau-, Mess-, Vergleichs- und Release-Durchführung (\texttt{build}, \texttt{measure}, \texttt{compare}, \texttt{release}) ---, getragen von einer übersetzungszeitlichen Registry ohne Laufzeit-Verzweigung; die Vergleichs-Durchführung trägt die Wählbarkeit und liest das Messwertlager, ohne selbst Referenzwerte zu erheben. Sie stehen als Dimension \texttt{work\_\allowbreak{}mode} in der Liste.
...
\midrule
\endhead%
\texttt{ycsb} & Das YCSB-Lastprofil-Rahmenwerk; freigeschaltet, mit dem Unter-Achsen-Etikett \texttt{workload}. Einziger freigeschalteter Baustein der Achse. & Code/\allowbreak{}Baseline \\
\bottomrule
\end{longtable}
\end{scriptsize}
