...
Diese Achse ist das \emph{Persistenz-Ziel-Organ} (T17) des Lebewesens \texttt{SearchAlgorithm} (vgl.\ Abschnitt~\ref{sec:sota-instances}); sie ist mit der Achsen-Neuordnung als achtzehnter Organ-Slot hinzugekommen.

Die Achse macht das Ablage-Ziel der Struktur als eigenen Organ-Slot explizit, statt eine In-Memory-Annahme implizit mitzuführen. Im kanonischen Vermessungs-Katalog ist sie auf ihren In-Memory-Vertreter gepinnt, damit der Permutationsraum bei $2^{17}$ Binary-Identitäten je System-Permutation bleibt (Abschnitt~\ref{sec:mess-chain}); jede \texttt{binary\_id} trägt gleichwohl das achtzehnte Segment \texttt{persistence\_target=persistence\_memory\_only}.
...
\texttt{Disk\allowbreak{}Writeback\allowbreak{}Target} & Platten-Rückschreibe-Ziel; als Baustein vollständig vorhanden (Typ, Registry, Sub-Achsen PT1--PT2), im kanonischen Katalog jedoch per Option abgeschaltet, sodass die Achse dort einelementig bleibt. & Code/\allowbreak{}Baseline \\
\bottomrule
\end{longtable}
\end{scriptsize}
...
Diese Größe war in der Neunzehner-Zählung das \emph{ISA-Organ} (damals T12); seit der Achsen-Neuordnung ist sie die System-Haupt-Achse \texttt{target\_isa} --- selbst eine Komplex-Achse (vgl.\ Abschnitte~\ref{sec:axis-triad} und~\ref{sec:complex-axis}).
...
Über diese drei Entwurfs-Dimensionen hinaus führt die bau-treibende System-Registry vier Unter-System-Achsen am Komplex-Wrapper: zwei übersetzungszeitliche (\texttt{target\_\allowbreak{}isa\_\allowbreak{}complex} und \texttt{scheduling}, je \texttt{stage=ct} mit \texttt{kind=fixed\_\allowbreak{}enum\_\allowbreak{}tuple}) sowie zwei dynamische (\texttt{numa\_\allowbreak{}node} und \texttt{page}). Die erste bündelt die benannten Rekombinationen der Zielmaschinen (\texttt{complex\_\allowbreak{}count=2}) mit je drei festen Gliedern (Owner OD-2).
...
\texttt{prod2\_\allowbreak{}raptor\_\allowbreak{}lake} & \texttt{cas\_\allowbreak{}latency\_\allowbreak{}cl} & \texttt{declared=false} --- keine deklarierte Zahl. \\
...
\emph{Notiz:} Die Werte stammen aus \texttt{k\allowbreak{}Declared\allowbreak{}Machines}, das seinerseits den \texttt{<machines>}-Block der Anwender-XML spiegelt (O-4b). \texttt{declaration\_\allowbreak{}source} konserviert den eingefrorenen Ursprung einer deklarierten Zahl (A2-Notiz, keine Provenienz-Stufe); Laufzeit-Stufen stehen niemals in dieser XML (P3).

Die zweite übersetzungszeitliche Unter-Achse \texttt{scheduling} trägt fünf feste Glieder. Sie war früher eine System-Haupt-Achse und ist mit Paket A3 als \texttt{sub\_\allowbreak{}axis} an den \texttt{target\_\allowbreak{}isa}-Komplex-Wrapper umgehängt worden --- ein Umzug, keine Löschung.
...
\emph{Namensfalle (compile-time gesichert):} Die Unter-Achse heißt \texttt{page} und ist nicht \texttt{page\_\allowbreak{}type}. \texttt{axis\_\allowbreak{}01\_\allowbreak{}page\_\allowbreak{}type} ist der Baum-Knoten-\texttt{Page\allowbreak{}Kind} der Organ-Welt und eine völlig andere Achse (Bauplan D2.6/\allowbreak{}IV-BLOCKER-5); der \texttt{static\_\allowbreak{}assert} in \texttt{target\_\allowbreak{}isa\_\allowbreak{}sub\_\allowbreak{}axes.hpp} hält das fest. Der Baustein-Katalog des Seitentyps steht unverändert in der konservierten Alt-Sektion \enquote{Build-PG}.
...
\texttt{build} & Build- und Patch-Stand & Der Build- bzw.\ Patch-Stand der Instanz \emph{einschließlich} des Update-Zustands (Owner-Entscheid E-12, wörtlich: \enquote{Wir mergen den Update Zustand in Build}). Ein separates \texttt{update\_\allowbreak{}zustand} ist damit ausdrücklich \emph{keine} vierte Unter-Achse, sondern Bestandteil dieser. \\
\bottomrule
\end{longtable}
\end{scriptsize}

\emph{Notiz:} Alle drei sind \texttt{stage=\allowbreak{}runtime} mit \texttt{value\_\allowbreak{}type=\allowbreak{}token} und \texttt{option\_\allowbreak{}source=\allowbreak{}machine\_\allowbreak{}resolved}: Sie haben keinen übersetzungszeitlichen Options-Katalog, weil ihre zulässigen Werte an der konkreten Maschine hängen und zur Bauzeit nicht erkennbar sind. Die Laufzeit-Erhebung ist ein eigener Bau-Schritt (Paket OS-U3, \texttt{operating\_\allowbreak{}system\_\allowbreak{}probe\_\allowbreak{}\{linux,\allowbreak{}macos,\allowbreak{}windows\}.hpp}); die Einhängung selbst ist nur das Angebot. Die Namen stehen seit dem Owner-Entscheid E3 vom 02.08.2026 fest (Paket A14/\allowbreak{}OS-U1).
...
\emph{Stempel-Neutralität (A-15):} Die drei Unter-Achsen sind Laufzeit-Unter-Achsen und stehen deshalb niemals im Binary-Stempel. Ihre Einhängung ändert weder die Stempel-Zeilen noch den Bau-Versions-Suffix, und sie darf den Achsen-Code-Stand von \texttt{operating\_\allowbreak{}system} nicht anheben (Bump-Verbots-Wache): Ein solcher Sprung verschöbe den SHA512 aller Neubauten gegen den Bestand und erzwänge genau den Voll-Neubau aller Alt-Binaries, den der Owner-Entscheid ausschließt.

\section{System-Sphäre: Erweiterungs-Hardware (\texttt{external\_utils})}

Diese Größe ist die dritte System-Haupt-Achse; sie ist ausschließlich der Sammel-Hub der Hardware-Meta-Achsen --- SIMD- und Vektor-Erweiterungen, GPU, FPGA und NPU sowie externe Beschleuniger (vgl.\ Abschnitt~\ref{sec:axis-triad}).

Der Hub trägt \emph{nur} System-Meta-Metas; das Last-Rahmenwerk gehört ausdrücklich nicht dazu. Weil die Meta-Meta-Achsen keine eigene Stempel-Zeile bekommen, sondern als Klammer-Anhang an das Ende der jeweiligen Realm-Zeile treten, schließt dieser Hub den System-Realm ab: Der Anhang wird typgetrieben aus der Glied-Typliste des Hubs gefaltet (\texttt{External\allowbreak{}Utils\allowbreak{}Hub} $\rightarrow$ \texttt{Simd\allowbreak{}External\allowbreak{}Utils\allowbreak{}Family}), die Ebene steckt in der Klammer-Tiefe und nicht im Namen (Owner-Entscheid E2 und Owner-Antwort Q1 vom 02.08.2026). Im bau-treibenden Angebot führt die Achse heute \texttt{baustein\_\allowbreak{}count=1}, \texttt{stage=ct} und \texttt{binary\_\allowbreak{}id=never}.
...
\emph{Notiz (Q2/\allowbreak{}K2):} \texttt{simd} materialisiert als Flag der Übersetzungs-Naht und erscheint damit im Bau-Versions-Suffix (vgl.\ Abschnitt~\ref{sec:stamp-model}) --- niemals als Laufzeit-Typ-Umschalter und niemals in der \texttt{binary\_\allowbreak{}id}.
...
\emph{Konservierter Angebots- und Historien-Katalog:} Die Alt-Achsen-Kennung \texttt{axis\_\allowbreak{}09b} ist durch die Haupt-Achse \texttt{external\_\allowbreak{}utils} mit ihrer Unter-Achse \texttt{simd} abgelöst. Der folgende Acht-Bausteine-Katalog bleibt als Angebots- und Historien-Katalog unter dieser Haupt-Achse vollständig erhalten, ebenso die Sub-Achsen-Tabelle SE1--SE3 als historischer Block.
...
Als Klammer ist er ausdrücklich \emph{keine} vierte Haupt-Achse: Die kanonische System-Achsen-Ordnung bleibt bei dreien. Drei übersetzungszeitliche Abgangs-Wachen (\texttt{static\_\allowbreak{}assert}) halten das fest --- \texttt{compiler}, \texttt{scheduling} und \texttt{load\_\allowbreak{}framework} dürfen nicht als Haupt-Achse zurückkehren; ihre Abgänge waren Umzüge, keine Löschungen. Das Last-Rahmenwerk ist auch kein \emph{Glied} dieser Klammer: Es lebt seit Paket A3 im Mess-Realm, während die System-Meta-Metas unter dem Hub \texttt{external\_\allowbreak{}utils} fahren. Die Achse führt \texttt{stage=ct} und \texttt{binary\_\allowbreak{}id=never}.
...
\midrule
\endhead%
\texttt{compiler} & \texttt{build\_\allowbreak{}target\_\allowbreak{}complex} & \texttt{stage=ct}, \texttt{baustein\_\allowbreak{}count=2}. Die Übersetzer-Wahl selbst; sie war früher eine System-Haupt-Achse und ist mit Paket A3 zur Unter-Achsen-Gruppe der äußeren Klammer geworden. \\
...
\emph{Notiz:} Die Gruppe hängt an \texttt{build\_\allowbreak{}target\_\allowbreak{}complex} und nicht an \texttt{target\_\allowbreak{}isa}, weil sie den \emph{Bau} aus der Rekombination aller drei Haupt-Achsen beschreibt und nicht eine Eigenschaft der Ziel-ISA allein (O-1r).
...
\emph{Versions-Flag-Grammatik v2 (Owner-KERN vom 07.08.2026, löst die Q3-Grammatik vollständig ab):} Die Version eines jeden Achsen-Algorithmus ist einheitlich dreistellig; ein \texttt{v}-Präfix gibt es seit der Flag-Grammatik v2 nicht mehr, Kurzformen bleiben verboten. Übersetzungszeitliche Wachen setzen die Form durch:
...
Vor jedem Flag steht ein Punkt, auch vor dem ersten (\texttt{1.0.0.c}, nie \texttt{1.0.0c}); die Basis hängt dabei direkt an ihrer öffnenden Klammer, ohne eigenen Punkt dazwischen (\texttt{c\{p.e\}}, nie \texttt{c.\{p.e\}}). Das Suffix \texttt{e} bedeutet seither \emph{Efficiency Core} und ist ein Sub-Flag wie jedes andere unter der Basis \texttt{c}; das frühere Konzept eines experimentell eingebrachten Prüflings-Algorithmus ist ersatzlos entfallen. Beispielform: \texttt{memory\_layout=SoaMemoryLayout@1.0.0.c\{p.e\}.x512\{f.vl.bw.dq\}.gfni}.
...
Diese Größe war in der Neunzehner-Zählung das \emph{Telemetrie-Organ} (damals T10); seit der Achsen-Neuordnung ist sie eine \emph{zweigeteilte} Haupt-Größe --- Sweep-Unter-Achse des Mess-Toolings im Planer \emph{und} Übersetzungszeit-Hauptgröße im Builder (vgl.\ Abschnitt~\ref{sec:axis-triad}). Ihre Baustein-Katalogisierung bleibt hier als konservierter Katalog erhalten.
...
Sie führt \texttt{category=measurement\_\allowbreak{}meta\_\allowbreak{}meta}, \texttt{stage=ct}, \texttt{binary\_\allowbreak{}id=never} und \texttt{baustein\_\allowbreak{}count=1}. Ihre Umzugs-Geschichte ist Teil ihrer Identität: Sie hat die System-Welt mit Paket A3 \emph{ersatzlos} Richtung Mess-Realm verlassen und ist dort die Meta-Meta-Haupt-Achse des Planers; unter den Hub der System-Meta-Metas gehört sie ausdrücklich nicht. Im Stempel erhält sie \emph{keine} eigene Meta-Meta-Zeile: Ihre Kennung tritt als dynamischer Klammer-Anhang an das \emph{Ende} der bestehenden Mess-Realm-Zeile (Owner-Entscheid E2 und Owner-Antwort Q1 vom 02.08.2026). Der Speiseweg ist hier eintragsgetrieben --- die gewählte Kennung samt Version kommt aus der Rahmenwerk-Registry des Mess-Realms und steht deshalb nicht im Typ, anders als beim typgetriebenen System-Hub.

\subsection*{Dynamische Sub-Achsen}

Die Registry führt sechs Sweep-Dimensionen (\texttt{resource\_\allowbreak{}control\_\allowbreak{}version=1}); alle sind \texttt{stage=runtime}.
...
\emph{Ehrlichkeits-Notiz:} Die Wiederholungszahl eines Laufs und die Mess-Modi sind bewusst \emph{nicht} in dieser Liste --- aus zwei \emph{verschiedenen} Gründen. Die Wiederholungszahl ist weiterhin eine reine Etikett-Dimension ohne übersetzungszeitlichen Anker und wird erst nach ihrer Typisierung reflektierbar. Die Modi dagegen sind inzwischen typisiert: es sind \emph{vier} (Debug-, Mess-, Release- und Vergleichs-Durchführung), getragen von einer übersetzungszeitlichen Registry ohne Laufzeit-Verzweigung, wobei die Vergleichs-Durchführung heute die Wählbarkeit trägt und noch nicht den eigenen Ablauf. Sie fehlen hier dennoch, weil die Auffächerung dieser Unter-Achse in die Reflexions-Matrix aussteht. Ein handgeschriebener Eintrag wäre in beiden Fällen ein Phantom --- die honest-0-Doktrin verbietet ihn (offener Punkt W2-B).
...
\midrule
\endhead%
\texttt{ycsb} & Das YCSB-Lastprofil-Rahmenwerk; freigeschaltet, mit dem Unter-Achsen-Etikett \texttt{workload}. Einziger heute freigeschalteter Baustein der Achse. & Code/\allowbreak{}Baseline \\
\bottomrule
\end{longtable}
\end{scriptsize}
...
Codes ab --- Organ-, System- und Mess-Realm --- und schreibt sie je Lauf nach
\texttt{anhang/de/tabellen/}. Sie ist damit eine Ableitung aus der übersetzungszeitlichen
Reflektion und kein zweiter, gepflegter Stand neben den Registries. Enthalten die Registries keine
Achse, entsteht bewusst keine Datei; es wird keine leere Tabelle erfunden.

\InputIfFileExists{anhang/de/tabellen/axis_inventory.tex}{}{%
  \emph{(Das Achsen-Inventar erscheint nach dem nächsten Lauf des Anhang-Generators.)}}
