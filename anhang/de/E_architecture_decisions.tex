...
%% INTERN-272: KA-165
  ...
  Arbeitsmodus ist eine Unter-Achse des Mess-Realms und geht nicht in die Binary-Identität ein, einen eigenen Debug-Arbeitsmodus gibt es nicht (\texttt{--debug} ist ein Flag der Planer-Shell und kein eigener
  Arbeitsmodus der Kette build/measure/compare/release; es schaltet den indirekten Debug-Modus der Debug-Variante
  ein, den nicht offiziellen fünften Wartungsmodus, die weiter ausgebaut wird: unter \texttt{--debug} fährt das
  Konformitäts-Gate zusätzlich den Standard-Testsatz des Tier-Interfaces, ein indirekter Debug-Modus, der mit dem
  Release-Bau zusammenfällt; die Bau-Varianten Debug, Messung und Release gelten fort), und den Mess-Aggregator führt der \texttt{ExperimentDriver} host-seitig.
  ...
  Messwerten erzeugte optimale Release-Binary sind für spätere Haupt-Wellen vorgesehen, aber nicht
  implementiert;
