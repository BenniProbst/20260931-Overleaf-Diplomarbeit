...
ADR-4 & Durchreich-Doktrin und erzwungene Vollbelegung & Abschn.~\ref{sec:axes-impl} & teilweise (Durchreich-Baustein des Knotentyps) \\
...
\begin{description}
  ...
  %% OWNER-VORLAGE: OV-02 --- ADR-4 Durchreich-Doktrin 'None je baum-/trie-spezifischer Achse' (Plan) gegen
  %% den Code: None-Baustein nur für die Pfadkompression (path_compression_none), der Knotentyp trägt keine
  %% None-Variante (Registry node_type = node4/16/48/256; Lens-r15 M-2). Als Plan halten (Knotentyp-None =
  %% Code-Posten, Vollzugsstand teilweise) oder als Ist-Stand-Präzisierung schließen; Default Plan-Kennzeichnung
  %% + 'teilweise', Widerspruch genügt.
  ...
  gebaut ist der Durchreich-Baustein für die Pfadkompression
  (\texttt{Path\allowbreak{}Compression\allowbreak{}None}), der Knotentyp trägt keine None-Variante und wird
  regulär mit einem der vier ART-Knotenformate belegt, die ein nicht-baumartiger Baustein nicht nutzt
  (Plan; ohne Code-Baustein);
  ...
  %% das Code-Vokabular honest-0/honest-100/honest-empty steht nur noch als Implementierungsname in der
  %% Fußnote. Default gesetzt;
  %% Widerspruch genügt.
  ...
  \texttt{honest-0}/\texttt{honest-100}/\texttt{honest-empty} ist der Implementierungsname dieser
  Doktrin (Anhang~\ref{app:glossary});
  ...
  Stufe $N+1$ nur an Stufe $N$ hängt; Planer und CEB sind zwei Binaries --- der Planer
