...
\section{T10 Value Handle Storage Strategy}
...
\texttt{Non\allowbreak{}Clustered\allowbreak{}Index\allowbreak{}Organization} & Index-order $\neq$ storage-order, multiple secondary indexes per table. Lookup via index + pointer-hop to row data (random-stride, cache-misses). Standard: SQL Server NONCLUSTERED INDEX, Postgre\allowbreak{}SQL CREATE INDEX\@. Access pattern: index navigation followed by random storage hops (LCG-deterministic, REAL cache-miss latency from TLB/\allowbreak{}L2 invalidation), then a residual-predicate comparison per fetched record (data-dependent branch on the fetched row data after the pointer-hop). & Comer, ACM Computing Surveys 1979 (The Ubiquitous B-Tree, 10.1145/\allowbreak{}356770.356776) \\
\bottomrule
\end{longtable}
\end{scriptsize}
...
The Filter Axis T14 provides selectable probabilistic and deterministic membership-query filters for point and range queries. It covers classic Bloom filters with k hash functions, the deletable Cuckoo Filter with bucket hashing and fingerprints, the succinct Range Filter Su\allowbreak{}RF using LOUDS-Trie navigation, and the space-efficient Xor Filter with constant 3-way-XOR latency. Purpose: Make space, query latency, mutability, and false-positive-rate trade-offs directly measurable in cache-engine experiments.
...
\section{T17 Persistence Target}

This axis is the \emph{persistence-target organ} (T17) of the living being \texttt{SearchAlgorithm} (cf.\ Section~\ref{sec:sota-instances}); it joined as the eighteenth organ slot with the axis reordering.

The axis makes the storage target of the structure explicit as an organ slot of its own instead of carrying an implicit in-memory assumption. In the canonical measurement catalogue it is pinned to its in-memory representative so that the permutation space stays at $2^{17}$ binary identities per system permutation (Section~\ref{sec:mess-chain}); every \texttt{binary\_id} nevertheless carries the eighteenth segment \texttt{persistence\_target=persistence\_memory\_only}.
...
\texttt{Disk\allowbreak{}Writeback\allowbreak{}Target} & Disk write-back target; fully present as a building block (type, registry, sub-axes PT1--PT2) but switched off per option in the canonical catalogue, so the axis remains single-element there. & code/\allowbreak{}baseline \\
\bottomrule
\end{longtable}
\end{scriptsize}
...
In the nineteen-axis numbering this quantity was the \emph{ISA organ} (then T12); since the axis reordering it is the system main axis \texttt{target\_isa} --- itself a complex axis (cf.\ Sections~\ref{sec:axis-triad} and~\ref{sec:complex-axis}).

The axis specifies the target CPU instruction set architecture (ISA) as a compile-time constant, enabling cross-platform optimizations across four dominant server architectures: x86-64 (Intel/\allowbreak{}AMD), AArch64 (ARM), Power-ISA (IBM), and RISC-V (open-source). Each variant defines a hardware-specific SIMD baseline (SSE2, NEON, VSX, scalar) and thus characterizes the vector acceleration potential for cache-engine operations such as \texttt{simd\_\allowbreak{}field\_\allowbreak{}sum} (accumulation of 32-bit word fields).
...
Beyond these three design dimensions, the build-driving system registry carries four system sub-axes on the complex wrapper: two compile-time ones (\texttt{target\_\allowbreak{}isa\_\allowbreak{}complex} and \texttt{scheduling}, each \texttt{stage=ct} with \texttt{kind=fixed\_\allowbreak{}enum\_\allowbreak{}tuple}) and two dynamic ones (\texttt{numa\_\allowbreak{}node} and \texttt{page}). The first bundles the named recombinations of the target machines (\texttt{complex\_\allowbreak{}count=2}) with three fixed members each (owner OD-2).
...
\texttt{prod2\_\allowbreak{}raptor\_\allowbreak{}lake} (\texttt{isa=x86\_\allowbreak{}64}) & \texttt{ram\_\allowbreak{}frequency\_\allowbreak{}mhz} & \texttt{declared=true}, \texttt{value=4800}, \texttt{declaration\_\allowbreak{}source=\allowbreak{}dmidecode 2026-07}. \\
\texttt{prod2\_\allowbreak{}raptor\_\allowbreak{}lake} & \texttt{cas\_\allowbreak{}latency\_\allowbreak{}cl} & \texttt{declared=false} --- no declared figure. \\
\texttt{prod2\_\allowbreak{}raptor\_\allowbreak{}lake} & \texttt{cpu\_\allowbreak{}fabrication} & \texttt{declared=true}, \texttt{value=Genuine\allowbreak{}Intel/\allowbreak{}6/\allowbreak{}151/\allowbreak{}2}. \\
\bottomrule
\end{longtable}
\end{scriptsize}

\emph{Note:} the values originate from \texttt{k\allowbreak{}Declared\allowbreak{}Machines}, which in turn mirrors the \texttt{<machines>} block of the user XML (O-4b). \texttt{declaration\_\allowbreak{}source} preserves the frozen origin of a declared figure (A2 note, not a provenance level); runtime levels never appear in this XML (P3).

The second compile-time sub-axis \texttt{scheduling} carries five fixed members. It used to be a system main axis and was re-attached as a \texttt{sub\_\allowbreak{}axis} of the \texttt{target\_\allowbreak{}isa} complex wrapper with package A3 --- a relocation, not a deletion.
...
\emph{Name trap (guarded at compile time):} the sub-axis is called \texttt{page} and is not \texttt{page\_\allowbreak{}type}. \texttt{axis\_\allowbreak{}01\_\allowbreak{}page\_\allowbreak{}type} is the tree-node \texttt{Page\allowbreak{}Kind} of the organ world and an entirely different axis (blueprint D2.6/\allowbreak{}IV-BLOCKER-5); the \texttt{static\_\allowbreak{}assert} in \texttt{target\_\allowbreak{}isa\_\allowbreak{}sub\_\allowbreak{}axes.hpp} records this. The building-block catalogue of the page type remains unchanged in the conserved legacy section \enquote{Build-PG}.
...
\emph{Registry state:} the build-driving system registry currently carries two enabled building blocks --- \texttt{x86\_\allowbreak{}64} (native, without triple and march flags) and \texttt{aarch64} (cross compilation with \texttt{triple=\allowbreak{}-target aarch64-linux-gnu} and \texttt{march=\allowbreak{}-march=armv8-a}). The four-ISA catalogue listed above is retained as an \emph{offering} catalogue: Power-ISA and RISC-V are described as building blocks but are not enabled in today's build offering.

\emph{Contributing sources:} ---

\section{System sphere: operating system (\texttt{operating\_system})}

This quantity is the second system main axis and thus part of the \emph{blood} and of no organ (cf.\ Section~\ref{sec:axis-triad}); in the canonical, code-guarded order it occupies position two of three.

The main axis carries the \emph{class} identity only: exactly three operating-system families (instance catalogue OP-10, \texttt{baustein\_\allowbreak{}count=3}), together with \texttt{stage=ct} and \texttt{binary\_\allowbreak{}id=never}. The \emph{instance} --- distribution, release, kernel and patch level --- resides entirely in the three sub-axes; entering a distribution as a fourth expression of the main axis would miss that cut. The real fleet (Windows 11 and Windows Server 2022, the Linux Docker matrix, two macOS machines) falls entirely into the three families.
...
\texttt{build} & Build and patch level & The build and patch level of the instance \emph{including} the update state (owner decision E-12, verbatim: \enquote{Wir mergen den Update Zustand in Build}). A separate \texttt{update\_\allowbreak{}zustand} is therefore explicitly \emph{not} a fourth sub-axis but a constituent of this one. \\
\bottomrule
\end{longtable}
\end{scriptsize}

\emph{Note:} all three are \texttt{stage=runtime} with \texttt{value\_\allowbreak{}type=token} and \texttt{option\_\allowbreak{}source=machine\_\allowbreak{}resolved}: they have no compile-time option catalogue because their admissible values depend on the concrete machine and are not knowable at build time. Runtime acquisition is a build step of its own (package OS-U3, \texttt{operating\_\allowbreak{}system\_\allowbreak{}probe\_\allowbreak{}\{linux,\allowbreak{}macos,\allowbreak{}windows\}.hpp}); the attachment itself is only the offering. The names have been fixed since owner decision E3 of 2 August 2026 (package A14/\allowbreak{}OS-U1).
...
\texttt{macos} & macOS family (\texttt{Macos\allowbreak{}Operating\allowbreak{}System}, \texttt{posix=true}). Covers the Apple Silicon and Intel machines of the fleet. & Code/\allowbreak{}Baseline \\
\bottomrule
\end{longtable}
\end{scriptsize}
...
\emph{Stamp neutrality (A-15):} the three sub-axes are runtime sub-axes and therefore never appear in the binary stamp. Their attachment changes neither the stamp lines nor the build-version suffix, and it must not raise the axis code level of \texttt{operating\_\allowbreak{}system} (bump prohibition guard): such a bump would shift the SHA512 of all new builds against the existing stock and would force exactly the full rebuild of all legacy binaries that the owner decision rules out.

\section{System sphere: extension hardware (\texttt{external\_utils})}

This quantity is the third system main axis; it is exclusively the collecting hub of the hardware meta axes --- SIMD and vector extensions, GPU, FPGA and NPU as well as external accelerators (cf.\ Section~\ref{sec:axis-triad}).

The hub carries \emph{only} system meta-metas; the load framework explicitly does not belong to it. Because meta-meta axes receive no stamp line of their own but appear as a bracketed suffix at the end of the respective realm line, this hub closes the system realm: the suffix is folded type-driven from the hub's member type list (\texttt{External\allowbreak{}Utils\allowbreak{}Hub} $\rightarrow$ \texttt{Simd\allowbreak{}External\allowbreak{}Utils\allowbreak{}Family}), and the level resides in the bracket depth rather than in the name (owner decision E2 and owner answer Q1 of 2 August 2026). In today's build-driving offering the axis carries \texttt{baustein\_\allowbreak{}count=1}, \texttt{stage=ct} and \texttt{binary\_\allowbreak{}id=never}.
...
\midrule
\endhead%
\texttt{simd} & Vector extension level & \texttt{stage=runtime}, \texttt{value\_\allowbreak{}type=token}, \texttt{option\_\allowbreak{}count=3}. The family identifier is at the same time the sub-axis label (notation \texttt{external\_\allowbreak{}utils.simd=\dots}); the per-dialect reflected compiler flags come from the option, not from the family. \\
\bottomrule
\end{longtable}
\end{scriptsize}
...
\emph{Note (Q2/\allowbreak{}K2):} \texttt{simd} materializes as a flag of the compilation seam and therefore appears in the build-version suffix (cf.\ Section~\ref{sec:stamp-model}) --- never as a runtime type switch and never in the \texttt{binary\_\allowbreak{}id}.
...
\emph{Machine signatures (\texttt{count=3}):} the registry records for each target machine the subset of features actually present --- \texttt{prod1\_\allowbreak{}zen5} (\texttt{host\_\allowbreak{}isa=x86\_\allowbreak{}64}) with 22 features, \texttt{prod2\_\allowbreak{}raptor\_\allowbreak{}lake} with 9 and \texttt{odroid\_\allowbreak{}gracemont} with 9. Only the Zen 5 machine carries the \texttt{avx512} tier; the other two end at \texttt{avx256} together with the companion and scalar features.
...
\midrule
\endhead%
\texttt{simd} & Vector extension family (\texttt{Simd\allowbreak{}External\allowbreak{}Utils\allowbreak{}Family}, \texttt{family\_\allowbreak{}id=simd}, enabled). The only member of the hub enabled today; the families for GPU, FPGA and NPU are foreseen in the design but are not part of the build-driving offering. & Code/\allowbreak{}Baseline \\
\bottomrule
\end{longtable}
\end{scriptsize}
...
\section[Build-SIMD: Vector/Accelerator Extension Axis]{Build-SIMD: Vector/\allowbreak{}Accelerator Extension Axis (Hardware Accelerators)} % chktex 13 (Titel-Doppelpunkt nach Grossbuchstabe, kein Satzende)

\emph{Conserved offering and history catalogue:} the legacy axis identifier \texttt{axis\_\allowbreak{}09b} has been superseded by the main axis \texttt{external\_\allowbreak{}utils} with its sub-axis \texttt{simd}. The eight-building-block catalogue below is retained in full as an offering and history catalogue under that main axis, as is the sub-axis table SE1--SE3 as a historical block.
...
\section{System sphere: outer complex bracket (\texttt{build\_target\_complex}) and build toolchain (\texttt{build\_toolchain})}

This quantity is the command-pattern bracket wrapper over the fixed recombination of the three system main axes (\texttt{member\_\allowbreak{}count=3}: \texttt{target\_\allowbreak{}isa}, \texttt{operating\_\allowbreak{}system}, \texttt{external\_\allowbreak{}utils}) and thus closes the system realm block (cf.\ Section~\ref{sec:complex-axis}).

As a bracket it is explicitly \emph{not} a fourth main axis: the canonical system axis order stays at three. Three compile-time departure guards (\texttt{static\_\allowbreak{}assert}) record this --- \texttt{compiler}, \texttt{scheduling} and \texttt{load\_\allowbreak{}framework} must not return as main axes; their departures were relocations, not deletions. The load framework is not a \emph{member} of this bracket either: since package A3 it lives in the measurement realm, whereas the system meta-metas travel under the hub \texttt{external\_\allowbreak{}utils}. The axis carries \texttt{stage=ct} and \texttt{binary\_\allowbreak{}id=never}.
...
\midrule
\endhead%
\texttt{compiler} & \texttt{build\_\allowbreak{}target\_\allowbreak{}complex} & \texttt{stage=ct}, \texttt{baustein\_\allowbreak{}count=2}. The compiler choice itself; it used to be a system main axis and became the sub-axis group of the outer bracket with package A3. \\
...
\emph{Note:} the group hangs on \texttt{build\_\allowbreak{}target\_\allowbreak{}complex} and not on \texttt{target\_\allowbreak{}isa} because it describes the \emph{build} arising from the recombination of all three main axes and not a property of the target ISA alone (O-1r).
...
The five options of the sub-axis \texttt{opt\_\allowbreak{}level} with their flags per dialect; the column \texttt{ieee754\_\allowbreak{}deterministic} states whether the level leaves floating-point semantics untouched:
...
\emph{Build version identifier:} \texttt{opt\_\allowbreak{}level}, \texttt{simd} and \texttt{atomic128} materialize as flags of the compilation seam and therefore appear in the build-version suffix of the binary --- an ordered eight-segment sequence in which an empty member simply produces no segment (cf.\ Section~\ref{sec:stamp-model}). They never appear in the \texttt{binary\_\allowbreak{}id}.

\emph{Version flag grammar (A13, owner decision E2 and owner answer Q3 of 2 August 2026):} the version of every axis algorithm is uniformly three-part and begins with \texttt{v}; short forms are forbidden. Compile-time guards enforce the form:

\begin{quote}
\verb+version := 'v' UINT '.' UINT '.' UINT [ HWFLAG [ 'e' ] ]+\\
\verb+HWFLAG  := 'c' (CPU) | 'g' (GPU) | 'f' (FPGA) | 'n' (NPU)+
\end{quote}

The hardware flag is exactly \emph{one} lowercase letter appended directly to the numeric triple; the order is fixed: first the hardware flag, then optionally the \texttt{e} for an experimental specimen algorithm. An \texttt{e} without a hardware flag is invalid. Because this work produces CPU code exclusively, in the target state all versions end in \texttt{c} or \texttt{ce}.

\section{Measurement sphere: measurement categories (\texttt{measurement\_category})}

This quantity belongs to the third realm --- the measurement axes, the \emph{blood of measuring} (cf.\ Section~\ref{sec:axis-triad}).
...
\section{Measurement sphere: measurement tooling/\allowbreak{}collector (\texttt{collector})}
...
It carries \texttt{stage=ct}, \texttt{binary\_\allowbreak{}id=never} and \texttt{baustein\_\allowbreak{}count=3}. Every building block carries its regime and the set of categories it serves; the union of the three sets covers ten of the sixteen categories, the remainder being described in the offering catalogue but not yet assigned to a tool.
...
\section{Measurement sphere: telemetry building blocks (conserved catalogue)}

In the nineteen-axis numbering this quantity was the \emph{telemetry organ} (then T10); since the axis reordering it is a \emph{two-way split} main quantity --- sweep sub-axis of the measurement tooling in the planner \emph{and} compile-time main quantity in the builder (cf.\ Section~\ref{sec:axis-triad}). Its building-block catalogue is retained here as a conserved catalogue.
...
\section{Measurement sphere: load framework (\texttt{load\_framework}) --- meta-meta axis}
...
It carries \texttt{category=measurement\_\allowbreak{}meta\_\allowbreak{}meta}, \texttt{stage=ct}, \texttt{binary\_\allowbreak{}id=never} and \texttt{baustein\_\allowbreak{}count=1}. Its relocation history is part of its identity: with package A3 it left the system world \emph{without replacement} towards the measurement realm, where it is the planner's meta-meta main axis; it explicitly does not belong under the hub of the system meta-metas. In the stamp it receives \emph{no} meta-meta line of its own: its identifier appears as a dynamic bracketed suffix at the \emph{end} of the existing measurement realm line (owner decision E2 and owner answer Q1 of 2 August 2026). The feeding path here is entry-driven --- the chosen identifier together with its version comes from the framework registry of the measurement realm and therefore does not reside in the type, unlike the type-driven system hub.

\subsection*{Dynamic sub-axes}

The registry carries six sweep dimensions (\texttt{resource\_\allowbreak{}control\_\allowbreak{}version=1}); all are \texttt{stage=runtime}.
...
\emph{Honesty note:} the repetition count of a run and the measurement modes are deliberately \emph{not} in this list --- for two \emph{different} reasons. The repetition count remains a pure label dimension without a compile-time anchor and becomes reflectable only once it is typed. The modes, by contrast, are typed by now: there are \emph{four} of them (debug, measurement, release and comparison execution), carried by a compile-time registry without runtime branching, where comparison execution today carries selectability and not yet its own procedure. They are nevertheless absent here because the fan-out of this sub-axis into the reflection matrix is still pending. A hand-written entry would be a phantom in either case --- the honest-0 doctrine forbids it (open item W2-B).
...
\midrule
\endhead%
\texttt{ycsb} & The YCSB load-profile framework; enabled, with the sub-axis label \texttt{workload}. The only building block of the axis enabled today. & Code/\allowbreak{}Baseline \\
\bottomrule
\end{longtable}
\end{scriptsize}
...
\emph{Current status (conserved catalogue):} this axis is excluded from the build-driving offering of the system registry. The registry records the exclusion verbatim: \texttt{extension\_\allowbreak{}hardware\_\allowbreak{}system\_\allowbreak{}axis.hpp} is a deprecated island, \texttt{hardware\_\allowbreak{}isa\_\allowbreak{}system\_\allowbreak{}axis.hpp} a pure host descriptor that does not drive the build --- neither belongs to the build-driving offering. The static platform profiles with their fixed constants for cache-line size, page size and SIMD bit width are conceptually superseded by the runtime hardware detection of the \texttt{Cache\allowbreak{}Engine\allowbreak{}Builder} --- a factory over ISA and operating system that acquires hardware values instead of setting them statically (cf.\ Section~\ref{sec:hw-probe}). The five sub-axis entries (\texttt{hw1} to \texttt{hw5}) and the three hardware profiles are retained in full as a historical catalogue.
...
(\texttt{Code/08\_appendix\_generator}) derives it from the three generated axis registries of the
code --- organ, system and measurement realm --- and writes it to \texttt{anhang/en/tabellen/} on
each run. It is therefore a projection of the compile-time reflection rather than a second,
hand-kept state beside the registries. If the registries contain no axis, no file is produced on
purpose; no empty table is invented.

\InputIfFileExists{anhang/en/tabellen/axis_inventory.tex}{}{%
  \emph{(The axis inventory appears after the next run of the appendix generator.)}}
