...
\begin{axis}[
    ...
    scale only axis,
    title={latency spread p50-p99 per search algorithm x operation},
    xlabel={search algorithm x operation},
    ylabel={latency (ns, log; point=p50, whisker to p99)},
    grid=both,
    enlargelimits=0.05,
    ymode=log,
    % F-4/B: EINE Kombination -> numerischer Index statt symbolic x coords, Fenster explizit
    xmin=-0.5, xmax=0.5,
    xtick={0},
    xticklabels={k\_ary/lookup},
...
coordinates {
    (0,750.0000) +- (0,420.0000)
};
\addlegendentry{lookup}
\end{axis}
\end{tikzpicture}
}%
\caption{latency spread p50-p99 per search algorithm x operation; point=p50, whisker=p99.}
\end{figure}
