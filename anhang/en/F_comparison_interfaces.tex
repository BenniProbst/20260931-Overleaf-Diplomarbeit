...
\emph{Variadic mapping.} The hybrid search engine of the probe PRT-ART ties the number of
...
hulls are implemented as separate genus adapters: the Map genus via the functional drive of the
module boundary with the five operations \texttt{tier\_insert}, \texttt{tier\_lookup},
\texttt{tier\_erase}, \texttt{tier\_clear}, and \texttt{tier\_size}, the Sequence genus via the
sequence drive with \texttt{tier\_push\_back}, \texttt{tier\_at}, \texttt{tier\_size}, and
\texttt{tier\_clear}; an adapter cutting across the genus is ruled out by the type system. The case
...
(Section~\ref{sec:measurement-system}) checks, per genus, a core subset of the hull operations
against an oracle of the standard library --- \texttt{std::map} for the Map genus,
\texttt{std::deque} for the Sequence genus ---, so that only comparable things are compared; the
subset is marked with $\dagger$ in both tables.
...
of the module boundary (\texttt{insert\_or\_assign} to the insert drive, \texttt{find} to the point
query, \texttt{erase} and \texttt{clear} to their drives); \texttt{at}, \texttt{operator[]},
...
point query and insertion; the three non-overwriting forms run over one path and are set against
...
conformance gate of the Sequence genus checks before each measurement against a
...
own at the module boundary but is composed host-side from point query and default insertion ---
...
and modifiers of the hull rest on a single query primitive of the module boundary --- the point
query, flanked by insertion and deletion ---; an axis implementation therefore only needs to
provide a correct point query together with node linking, and the rest follows as a fixed
...
(Section~\ref{sec:measurement-system}). The conformance gate checks, per genus, the core subset
...
insert drive of the Map genus carries the semantics of \texttt{insert\_or\_assign} --- insert or
...
\texttt{try\_emplace} are composed host-side as a point query followed by insertion only on a
...
genera have separate drive interfaces and separate oracles, and an adapter foreign to its genus is
