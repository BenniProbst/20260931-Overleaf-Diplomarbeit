...
hulls are implemented as separate concept adapters: the Map concept via the functional drive of the
...
(Section~\ref{sec:measurement-system}) checks, per concept, a core subset of the hull operations
against an oracle of the standard library --- \texttt{std::map} for the Map concept,
\texttt{std::deque} for the Sequence family ---, so that only comparable things are compared; the
...
(Section~\ref{sec:measurement-system}). The conformance gate checks, per concept, the core subset
...
insert drive of the Map concept carries the semantics of \texttt{insert\_or\_assign} --- insert or
