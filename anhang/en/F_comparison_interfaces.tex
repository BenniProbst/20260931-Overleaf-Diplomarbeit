...
This appendix presents --- complementing Section~\ref{sec:software-means} --- the complete
listing of all standard interface functions of the two comparison shells \texttt{std::map} (ordered
associative) and \texttt{std::vector} (sequence) as of C++23, with their officially expected
semantics and complexity guarantee per the ISO C++ standard~\cite{iso_cpp}. It also serves as a
central reference for the further development of the cache engine: because the search-algorithm
shell replaces its internals with dynamically interchangeable axis algorithms
(Section~\ref{sec:software-means}), it becomes explanation-worthy, per standard function, what
behaviour the external contract guarantees.

\emph{Variadic mapping.} One template parameter provides the \texttt{std::vector} API, two
parameters the \texttt{std::map} API, and $N>2$ a \texttt{std::map<K,\,std::tuple<\ldots>>}
(Section~\ref{sec:software-means}); the shared functions (iteration,
\texttt{size}/\texttt{empty}/\texttt{clear}, \texttt{swap}, \texttt{get\_allocator}) carry the same
semantics in both shells.

\emph{Composite functions.} Some functions arise ``under the hood'' as a composition of more
primitive operations --- e.g.\ \texttt{operator[]} as \texttt{lower\_bound} followed by default
insertion on a missing key, or \texttt{at} as \texttt{find} with a thrown
\texttt{std::out\_of\_range} exception on absence ---; such cases are marked in the ``Note'' column.
The ``Since'' column names the introducing standard version. The exact cache-engine-internal
function-handle hops are documented in a dedicated cache-engine developer document.

\emph{Conformance subset.} The conformance gate (Section~\ref{sec:software-means}) checks a
core subset of the \texttt{std::map} operations before each measurement, so that only comparable
things are compared; it is marked as such in the \texttt{std::map} table.
...
that the conformance gate (Section~\ref{sec:software-means}) checks before each measurement,
so that only comparable things are compared. Complexities refer to $n=\texttt{size()}$; for range
operations $N$ is the number of input elements.
...
\texttt{operator<=>} since C++20, and \texttt{!=} is rewritten from \texttt{operator==}. The
heterogeneous (transparent-key) overloads of \texttt{operator[]}, \texttt{at},
\texttt{try\_emplace}, and \texttt{insert\_or\_assign} belong to C++26 and are excluded here; the
C++23 additions are exactly the \texttt{from\_range} constructor, \texttt{insert\_range}, and the
transparent-key overloads of \texttt{erase}/\texttt{extract}.
...
C++23 ($n=\texttt{size()}$). All members are \texttt{constexpr} since C++20.
...
\texttt{swap(reference,\allowbreak\ reference)}, and a \texttt{std::hash} specialization. The
comparison operators behave as for \texttt{std::map}.
...
(Table~\ref{tab:if-decomp}); the table describes the ISO standard contract. The measurement wrapper of
this work deliberately deviates in two places, because the ABI conformance contract demands
\texttt{noexcept} and a throwing wrapper counts as non-conformant: there, \texttt{operator[]} is a
non-mutating \texttt{std::optional} read (the write path goes through
\texttt{set()}/\texttt{insert\_or\_assign}), and \texttt{at} does not throw (\texttt{optional}/status
return). For this thesis that is the central observation: all map accessors and
modifiers rest on a single ordered-search primitive (\texttt{lower\_bound}/\texttt{find}); an axis
implementation therefore only needs to provide a correct ordered search plus node linking, and the
rest follows as a fixed composition --- exactly the substitutability whose comparison is the central
measurement act (Section~\ref{sec:software-means}). The conformance gate checks the core subset
marked with $\dagger$ in Table~\ref{tab:if-map}.
...
\texttt{operator[]}, for instance, inserts in the map but is an unchecked index access in the
vector. The exact cache-engine-internal function-handle hops behind these contracts are documented
in a dedicated cache-engine developer document.
