...
ADR-4 & Pass-through rule and enforced full assignment & Sect.~\ref{sec:axes-impl} & partial (pass-through building block of the node type) \\
ADR-5 & Layer model E4--E0 with fixed contracts & Sect.~\ref{sec:anatomy-layers} & implemented \\
...
\begin{description}
  ...
  concept: the four primal symbols ABI version, magic, factory (\texttt{comdare\_create\_anatomy()})
  and destroy, the two identity symbols concept and family, whereby the concept is not maintained as a
  second datum but derived statically at compile time from the family, and the stamp-line symbol. The
  identity of names across all families is the stability argument: the loader can determine the
  concept via a concept-independent symbol without knowing it in advance. Behind it lie two engine
  ...
  \texttt{ICacheEngine}; the name \enquote{SearchEngine} denotes the ABI runtime view of the family
  ...
  identities per system permutation in the golden reference catalogue --- one of them, the persistence
  target, pinned to its in-memory representative, under the superseded assumption of two configurations per
  axis ---, the upper bound per system permutation is the product of the configurations selected per axis in
  the XML experiment (known only at execution time), and the raw variant product space over all axis
  catalogues including their sub-axes lies orders of magnitude above that; the hot
  ...
  \textbf{Decision:} There is exactly one hierarchy without parallel trees --- concept (level~1,
  measurement surface: the probe-dock concepts Map, Container and Graph as well as the fourth concept
  HeuristikAdapter, which has no dock of its own because its binaries report the target family via
  \texttt{genus()} and dock at the dock of the target) $\to$ family (level~2, the tier interface with
  a fixed axis set; the six families \texttt{SearchAlgorithm}, \texttt{Set},
  \texttt{Sequence}, \texttt{Adapter}, \texttt{View} and \texttt{FunctionInterfaceReroute}) $\to$
  genus (level~3, the implementation of the family interface, e.g.\ ART, HOT, START) $\to$
  axis organs (level~4, a classification of their own) --- under \texttt{IExecutionEngine} as the
  common measurable root; a concept
  of its own would arise only if the base axis set differed fundamentally.
  \textbf{Consequence:} The family \texttt{SearchAlgorithm} under the concept Map and its ABI runtime
  ...
  \item[ADR-4 --- Pass-through rule and enforced full assignment]\hfill\\
  ...
  \texttt{IsComposition} of the family \texttt{SearchAlgorithm} enforces at compile time the
  assignment of all eighteen organs (eighteen type requirements and an organ counter of eighteen
  with a check for ABI break), so that no composition of this family with an open axis compiles.
  \textbf{Consequence:} Non-tree-shaped methods require no concept of their own but a restricted
  axis subset within the same family.
  \textbf{Evidence:} Sections~\ref{sec:sota-instances},~\ref{sec:anatomy-levels}
  and~\ref{sec:axes-impl}.

  \item[ADR-5 --- Layer model E4--E0 with fixed contracts and contract tests]\hfill\\
  ...
  one code location each. Each layer is completed individually, with an explicitly fixed interface
  contract towards the neighbouring level (the fixing of the contract of layer E1 in the ABI
  adapter: signatures, data layout and capabilities fixed, semantic changes only additive), and
  ...
  \textbf{Context:} Measurement apparatus, candidate under test, experiment definition including evaluation, and
  ...
  Appendix~\ref{app:glossary}), \texttt{comdare-prt-art} (the candidate under test), the project repository
  ...
  and diagram files per commit ---; orthogonal to this division lie the four carrier stages in build order
  planner $\to$ CacheEngineBuilder $\to$ tier binary $\to$ hybrid tier binary, where stage $N+1$ hangs
  only on stage $N$ (processing chain: CEB probe dock $\to$ [optional hybrid mux] $\to$ tier binary);
  planner and CEB are two binaries --- the planner
  ...
  separated, and the candidate under test is not a stage but the transversal supplier of organ building blocks.
  ...
  shut-off levels of the measurement. The run methodology knows four work modes of the same
  ...
  sub-axis of the measurement realm and does not enter the binary identity, a debug work mode of its own does not exist (\texttt{--debug} is a flag of the planner shell and not a work mode
  of its own in the chain build/measure/compare/release; it enables the indirect debug mode of the debug variant,
  the unofficial fifth maintenance mode, which is being extended further: under \texttt{--debug} the conformance
  gate additionally runs the standard test set of the tier interface, an indirect debug mode that coincides with
  the release build; the build variants debug, measurement, and release remain in force),
  and the measurement aggregator is run host-side by the \texttt{ExperimentDriver}.
  ...
  release binary generated from measured values are planned for later main waves but not
  implemented;
  ...
  \texttt{std::map<Key,\,Value>}~\cite{iso_cpp}; the container families are held against one
  ...
  \texttt{std::vector} serves as the inner substrate of the family \texttt{Adapter}. A conformance
  gate checks this contract before every measurement in the binding order import $\to$ gate $\to$
  measure: the seventeen \texttt{std::map} operations marked in Appendix~\ref{app:interfaces} (twelve
  core check blocks, \texttt{std::map} as oracle), with conformance ratios as the result form; the five
  drive operations of the module boundary (\texttt{insert}, \texttt{lookup}, \texttt{erase},
  \texttt{clear}, \texttt{size}) are the ABI primitives over which the host-side adapter composes this
  contract, not the scope of the check. Under \texttt{--debug} the gate additionally runs the standard
  test set of the respective tier interface: the family-generic part (valid for all genera) and the
  genus-specific part (the possibly extended special interfaces); this indirect debug mode coincides with
  the release build and is not kept as a separate mode.
  ...
  interface and the family iterator library are not implemented; the requirement to run the
  container families through a complete \texttt{std::vector} hull is not met --- the implemented
  ...
  as well, ADR-13), \emph{Abstract Factory} (hybrid docks, composition factory, candidate registry),
  ...
  CacheEngineBuilder, tier binary, and the hybrid tier binary optionally inserted between the CEB probe
  dock and the tier binary); each stage needs an ABI-stable, compile-time identity into which no
  measured value enters.
  ...
  \emph{Concept and family interfaces remain unchanged.}\hfill\\
  ...
  choice of fields; measurement fields in the domain interfaces would alter the concept and family
  contracts.
  \textbf{Decision:} Each carrier stage offers three interface surfaces: surface~1 the family
  ...
  \textbf{Context:} A reference space of $2^{17}$ compositions per system permutation in the golden reference catalogue
  (according to the catalogue header about 224~GB and 24~h of complete build; superseded assumption of two
  configurations per axis, the upper bound is determined by the planner from the XML experiment) is
  manageable only with reuse of
  ...
  golden reference catalogue $2^{17}$ with CRC-64 checksum is the regression detector with fixed
  cardinality.
