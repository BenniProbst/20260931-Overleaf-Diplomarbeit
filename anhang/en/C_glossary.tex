...
\begin{description}
  ...
  which one organ uses the services of the others (for instance a container genus as the node
  ...
  switching rules of the probe are called \emph{density thresholds}.
  ...
  \item[Carrier stage] (\emph{Träger-Stufe}) One of the four stages that carry the anatomy, in the
  order of the chain: planner, CacheEngineBuilder (CEB), tier binary, hybrid tier binary. No stage
  ...
  \item[Density thresholds] (\emph{Dichte-Schwellen}) The three fixed switching rules of the probe
  ...
  (50 to 75~\%) $\to$ Vector$\langle$u16,u16$\rangle$ (above 75~\%); the probe switches by these
  ...
  \item[Family] (\emph{Gattung}; code: \texttt{AnatomyGattung}) An externally visible interface
  --- Map, Container, Graph, HeuristikAdapter ---, the topmost level (level~1) of the framework;
  the family is derived from the genus (one maintained source) and is determined via a
  family-independent C symbol (the symbol bears the same name for every family). In the
  product-line reading the family is the feature core composition, the genus the feature special
  composition. Reading note: \enquote{Adapter} without prefix always means the container genus, the
  hybrid family is always called HeuristikAdapter (see \emph{Three-level model}, \emph{Genus}).
  \item[Family subclass] (\emph{Gattungs-Unterklasse}) see \emph{Genus} (alias).
  ...
  construction kit --- the same type that the probe PRT-ART corresponds to. Of the 33 profiles,
  30 are full profiles and 3 abstract.
  \item[Genus] The second level beneath a family, with a fixed axis set (code:
  \texttt{AnatomyGenus}). Six genera: SearchAlgorithm under Map --- in the metaphor canon of this
  thesis the \enquote{mammal} with the full eighteen-axis anatomy ---, Set, Sequence, Adapter and
  View under Container (in the metaphor canon the animal classes \enquote{bird}, \enquote{reptile},
  \enquote{invertebrate} and \enquote{plant} as living beings, each with its own
  compile-time-statically fixed axis set; see \emph{Metaphor canon}) as well as
  FunctionInterfaceReroute under HeuristikAdapter. The latter classifies the hybrid artefact;
  \texttt{genus()} of a hybrid binary, by contrast, reports the inherited target genus, the
  rerouting remains transparent to the outside. The genus is the only maintained source of the
  classification; per docking genus there is exactly one probe dock (five docks for
  SearchAlgorithm, Set, Sequence, Adapter and View), whereas the classifying genus
  FunctionInterfaceReroute has none and its binaries dock at the dock of the reported target genus.
  Alias: family subclass, tier subclass.
  ...
  checksum instead of via the materialized list. It is to be kept apart from the cardinality of the
  real permutation space, which the planner computes dynamically per profile and platform from the
  activated catalogues (cf.\ Section~\ref{sec:mess-chain}).
  ...
  inclusion; no separate library artefact arises. The probe PRT-ART and the hybrid stage exist in
  this form.
  ...
  \item[Hybrid tier binary] The fourth carrier stage: a module of the family HeuristikAdapter
  (genus FunctionInterfaceReroute) that inherits at compile time the interfaces of one family and
  one genus and puts calls through to attached tier binaries according to heuristic criteria; it
  exports the same mandatory symbols as a tier, reports the target genus to the outside and is
  ...
  carrier stage offers: surface~1 the genus interface (run time, Abstract Factory), surface~2 the
  stamp (compile time, ABI-stable), surface~3 the measurement cut-through (\texttt{IMessVisitor}),
  which crosses the measurement as a seam of its own so that family and genus interfaces remain
  ...
  \item[Living being] (\emph{Lebewesen}) A completely composed tier module of a family --- all
  families are living beings; the SearchAlgorithm is the case with the full eighteen-axis anatomy,
  ...
  genus-interface functions of the tier binary; \emph{micro} (mi) --- time and success parameters
  ...
  their run times is the instrumentation artefact. The word \emph{probe} is not used for the
  sensor in this thesis, because it denotes the candidate under test (see \emph{Probe}).
  \item[Merge strategy] (\emph{Verbund-Strategie}) The manner, declared per axis, in which a probe
  slot is joined with the engine axis: \texttt{Verbund1\_CeOnly} (only the engine, no probe
  directive), \texttt{Verbund2\_Replace} (the probe axis replaces the engine axis, with fallback;
  the default), \texttt{Verbund2\_Hybrid} (engine and probe hybrid per probe) and
  ...
  uniformly in this thesis: organ $=$ axis; anatomy $=$ the body of a living being; animal
  (\emph{Tier}) $=$ a living being of a family (the animal classes as examples per genus:
  \enquote{mammal} $=$ SearchAlgorithm, \enquote{bird}, \enquote{reptile}, \enquote{invertebrate}
  and \enquote{plant} $=$ the four container genera Set, Sequence, Adapter and View); virus $=$
  axis-less Graph module; blood $=$ the liquid organ that connects all organ axes and is condensed
  in the planner into one compile-time configuration; habitat $=$ the system axes; laboratory $=$
  the measurement axes (cf.\ Section~\ref{sec:anatomy-metaphor}). Where the text means the
  ...
  \item[Page type] (\emph{Seitentyp}) The probe slot \texttt{page\_type} (prt-art
  \texttt{axis\_01}): the node form of the PRT-ART B$^+$ tree; in the canon of the engine it
  corresponds to the organ main axis \texttt{node\_type} (slot T04). The slot numbers of the probe
  ...
  resolves the user XML against the three offer catalogues, selects the measurement axes and steers
  ...
  meta-meta measurement axes.
  \item[Probe] (\emph{Prüfling}) The new search algorithm to be integrated on trial (here
  PRT-ART); it is compiled by the builder against the core constituents of the cache engine. In
  this thesis \emph{probe} always denotes this candidate under test, never the measurement
  instrumentation (see \emph{Measurement sensor}); the fixed names \emph{probe dock} and
  \emph{probe effect} keep their meaning (see \emph{Probe dock}, \emph{Instrumentation artefact}).
  \item[Probe dock] (\emph{Prüf-Dock}) The builder-side measurement orchestration per genus:
  exactly one dock per genus (SearchAlgorithm, Set, Sequence, Adapter, View) loads a living being,
  drives it through the interface operations and measures it; hybrid docks bind attached tiers.
  The dock is no ABI boundary --- the ABI boundary remains the genus-specific drive sub-interface
  ...
  assigning every file of a repository to a license via SPDX license identifier and
  \texttt{REUSE.toml}; SPDX is the standardized identifier catalogue for licenses. The assignment
  re-licenses nothing --- the license files remain authoritative~\cite{fsfe2024reuse}.
  ...
  family, genus and realm, a permutation is recognized via its fingerprint instead of being built
  ...
  conformance subset of the interface operations, which the comparison gate checks before every
  measurement (Appendix~\ref{app:interfaces}).
  \item[Subsystem] In this thesis the four carrier subsystems of the chain (measurement driver,
  CacheEngineBuilder, CacheEngine, probe); in the code, by contrast, the twelve
  ...
  \item[Three-level model] (\emph{Drei-Ebenen-Modell}) The layering of the one architecture:
  \emph{family} (level~1, the outer algorithm interface --- Map, Container, Graph,
  HeuristikAdapter), \emph{genus} (level~2, family subclass with a fixed axis set ---
  SearchAlgorithm, Set, Sequence, Adapter, View, FunctionInterfaceReroute) and
  \emph{organs}/\emph{axes} (level~3). See Section~\ref{sec:anatomy-levels}.
  \item[Tier binary] The third carrier stage: the dynamically loadable module of a tier, built by
  ...
  letter-identical C symbols (the four primal symbols, the family and genus query and the
  version lines) and lies in the store as stock~1. Alias: tier, permutation module.
  ...
  \item[Work mode] (\emph{Arbeitsmodus}) The four modes of the process chain, in this order:
  ...
  measurement store) and \texttt{release} (delivery without measurement sensors). The work mode is
  ...
  alias of the wallclock level in the measurement configuration. Alias: mode, process chain.
